...
class TrackingReader final : public axk::RandomAccessReader {
 public:
  ...
  [[nodiscard]] axk::Result<void> read_exact_at(
      std::uint64_t offset,
  ...
  [[nodiscard]] axk::Result<void> read_exact_at(
    ...
    if (!end || *end > size_) {
      return std::unexpected{axk::make_error(
          axk::ErrorCode::io_short_read, axk::ErrorCategory::io, "sparse fixture short read")};
    }
    ...
    for (const auto& [patch_offset, patch] : patches_) {
      ...
      std::copy(
          patch.begin() + static_cast<std::ptrdiff_t>(overlap_start - patch_offset),
...
std::shared_ptr<SparseReader> sparse_geometry_fixture(
  ...
  for (std::uint8_t index = 0; index < partition_count; ++index) {
    ...
    EXPECT_TRUE(superblock_writer.write_be32(0xa8U + index * 8U,
                                             static_cast<std::uint32_t>(start)));
    EXPECT_TRUE(superblock_writer.write_be32(0xacU + index * 8U,
...
TEST(SfsReader, EnumeratesEverySupportedPartitionCountAtMinimumAndTwoGiB) {
  constexpr std::uint64_t two_gib_sectors = (std::uint64_t{2} * 1024U * 1024U * 1024U) /
                                             sector_size;
  for (std::uint8_t count = 1; count <= 8; ++count) {
    for (const auto total_sectors : {
      ...
      ASSERT_TRUE(result) << "partitions=" << static_cast<unsigned int>(count)
...
TEST(SfsReader, ReportsContinuationCycleAndBitmapMismatchWithoutRepair) {
  ...
  EXPECT_TRUE(std::any_of(
      cycle_partition.diagnostics.begin(),
      cycle_partition.diagnostics.end(),
...
TEST(SfsReader, ReportsMissingDirectoryTargetsAndChildCycles) {
  ...
  EXPECT_TRUE(std::any_of(
      missing_diagnostics.begin(),
      missing_diagnostics.end(),
  ...
  EXPECT_TRUE(std::any_of(
      cycle_diagnostics.begin(),
      cycle_diagnostics.end(),
      [](const axk::Error& error) {
        return error.code == axk::ErrorCode::relationship_cycle &&
               error.context.object_name == "Samples";
      }));
}
