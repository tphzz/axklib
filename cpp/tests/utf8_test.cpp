...
TEST(Utf8, AcceptsScalarBoundariesNoncharactersAndEmbeddedNul) {
  for (const auto& value : {std::string{"ascii"}, std::string{"\0", 1U},
                           std::string{"\xc2\x80", 2U}, std::string{"\xe0\xa0\x80", 3U},
                           std::string{"\xef\xb7\x90", 3U},
                           std::string{"\xf0\x90\x80\x80", 4U},
...
TEST(Utf8, RejectsOverlongTruncatedSurrogateAndOutOfRangeSequences) {
  for (const auto& value : {std::string{"\x80", 1U}, std::string{"\xc0\x80", 2U},
...
TEST(Utf8, ConvertsPathsExplicitlyAndRejectsEmbeddedNul) {
  constexpr std::string_view utf8_path{"audio/Gr\xc3\xb6\xc3\x9f" "e.wav"};
...
TEST(Utf8, BuildsTemporarySiblingWithoutNarrowingTheNativeFilename) {
  constexpr std::string_view target_utf8{"audio/Gr\xc3\xb6\xc3\x9f" "e \xe9\x9f\xb3.wav"};
  ...
  EXPECT_EQ(axk::text::path_to_utf8(*temporary),
            "audio/.Gr\xc3\xb6\xc3\x9f" "e \xe9\x9f\xb3.wav.alter.1234.tmp");
