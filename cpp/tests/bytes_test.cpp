...
TEST(ByteReader, ReadsEndianAndSignedValues) {
  const std::array bytes{
      std::byte{0x80}, std::byte{0x12}, std::byte{0x34}, std::byte{0x56}, std::byte{0x78}};
...
TEST(Error, RendersSamplerContextBeforeOptionalTechnicalTrace) {
  ...
  const auto error = axk::make_error(
      axk::ErrorCode::object_malformed,
      axk::ErrorCategory::object,
      "sample bank is malformed",
      context);
...
TEST(ByteReader, CleansAsciiAndCanRejectInvalidBytes) {
  const std::array bytes{
      std::byte{'N'}, std::byte{'a'}, std::byte{'m'}, std::byte{'e'}, std::byte{' '},
      std::byte{' '}, std::byte{0x00}, std::byte{0xff}};
  const axk::ByteReader reader{bytes};
