...
template <typename T> const T &require(const axk::Result<T> &result, std::string_view label) {
  if (!result)
...
int main(int argc, char **argv) try {
  ...
  measurements.push_back(measure("cold_open", [&] { container = require(axk::open_image(source), "open"); }));
  ...
  measurements.push_back(
      measure("relationships", [&] { graph = axk::build_relationship_graph(catalog); }));
  ...
  measurements.push_back(measure("preview", [&] {
    ...
    const auto decoded = require(axk::decode_waveform(container, *waveform), "decode waveform");
    static_cast<void>(require(axk::build_preview_envelope(decoded, 512U), "preview"));
  }));
  measurements.push_back(measure("exact_export", [&] {
    const auto plan = require(axk::build_export_plan(container, catalog, graph), "export plan");
    static_cast<void>(require(axk::write_export_audio(plan, scratch / "export"), "export"));
  }));

  const auto build = require(axk::parse_hds_build_manifest(
  ...
  const auto alteration = require(axk::parse_alteration_manifest(
  ...
  measurements.push_back(measure("alteration_apply", [&] {
    static_cast<void>(require(axk::alter_hds(source, alteration, scratch / "altered.hds"),
                              "alteration apply"));
  }));
  ...
  for (const auto &measurement : measurements) {
    const auto bytes = measurement.name == "fresh_write" ? 1'048'576U : source_bytes;
    const auto throughput = measurement.wall_ms <= 0.0
                                ? 0.0
