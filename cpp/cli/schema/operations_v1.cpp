...
AlterationOutput project_alteration(const AlterationResult& altered) {
  AlterationOutput result{
      ...
      .output_path_utf8 = altered.output_path ? std::optional{text::path_to_utf8(*altered.output_path)}
                                               : std::nullopt,
  ...
  try {
    ...
    for (const auto& operation : output.operations) {
      operations.push_back({
          ...
          {"freed_clusters", operation.freed_clusters},
          {"allocated_clusters", operation.allocated_clusters},
          {"audio_import", operation.audio_import ? audio_json(*operation.audio_import)
                                                   : OrderedJson(nullptr)},
      });
    }
    return OrderedJson{
        {"source_path", output.source_path_utf8},
        {"output_path", output.output_path_utf8 ? OrderedJson(*output.output_path_utf8)
                                                 : OrderedJson(nullptr)},
