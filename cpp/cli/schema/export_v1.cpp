...
Result<std::string> serialize_volume_graph(const VolumeExport& volume,
  ...
  for (const auto &bank : volume.sample_banks) {
    ...
    OrderedJson resolved_range = {
        {"low_midi", range_low == 255U ? OrderedJson(range_bank.left.root_key)
                                       : OrderedJson(range_low)},
        {"high_midi", range_high == 128U ? OrderedJson(range_bank.left.root_key)
                                         : OrderedJson(range_high)},
        {"low_raw", range_low},
        {"high_raw", range_high},
        {"low_display",
         range_low == 255U ? OrderedJson("Orig") : OrderedJson(range_low)},
        {"high_display",
         range_high == 128U ? OrderedJson("Orig") : OrderedJson(range_high)},
        {"basis",
...
Result<std::string> serialize(const std::vector<VolumeSummaryOutput>& volumes, bool pretty) {
  try {
    ...
    return OrderedJson{{"schema_version", schema_version}, {"volumes", std::move(rows)}}
        .dump(pretty ? 2 : -1);
  } catch (const nlohmann::json::exception& error) {
    return std::unexpected{make_error(ErrorCode::invalid_argument, ErrorCategory::internal,
                                      std::string{"could not serialize export JSON: "} +
                                          error.what())};
  }
}
