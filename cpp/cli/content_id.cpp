...
Result<std::filesystem::path> PooledPathAllocator::allocate(
    const std::filesystem::path& selection_root, std::string_view kind,
  ...
  if (const auto existing = entries_.find(target); existing != entries_.end()) {
    if (existing->second.id.digest_hex != id.digest_hex ||
      ...
      return std::unexpected{content_id_error(
          "distinct WAV contents share pooled export path " + axk::text::path_to_utf8(target))};
    }
  } else {
