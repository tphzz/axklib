...
constexpr std::array object_ref_names{"object_key",      "source_key",       "target_key",
                                      "partition_index", "sfs_id",           "payload_offset",
                                      "raw_offset",      "object_offset",    "object_offset_hex",
                                      "source_path",     "source_container", "image"};

template <std::size_t Size>
bool contains(const std::array<const char *, Size> &values, std::string_view value) {
...
Result<void> write_atomic(const std::filesystem::path &path, std::string_view text,
                          bool overwrite) {
...
std::string semantic_notes(std::string_view name) {
  if (contains(quality_names, name))
    return "DataQuality marker; exact allowed values are part of the quality model, not a "
           "free-form status string.";
  if (name == "assignment_row_state")
    return "Program assignment row classification; decoded-row means a PROG row was decoded "
           "and reported separately from active assignment state.";
  if (name == "active_assignment_state")
    return "Conservative Program assignment classification. HDA/sampler-authored rows may be "
           "confirmed-active, confirmed-visible-off, or confirmed-duplicate-not-active; ISO "
           "source-load-assignment rows are matched source links whose loaded active state is "
           "reported separately from stored HDA active/off bytes.";
  if (name == "assignment_output1_byte_0x1d")
    return "Decoded PROG assignment row +0x1d byte retained as raw per-row output data; it is "
           "not the Rch Assign display selector by itself.";
  if (name == "assignment_rch_assign_gate_byte_0x28")
    return "Decoded PROG assignment row byte used for active/off classification where 0xff is "
           "active and 0x00 is visible/off.";
  if (name == "assignment_rch_assign_display")
    return "Conservative visible Rch Assign family: off, =SMP, 01 through 16, BasicRch, B01 "
