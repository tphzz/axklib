  ...
  for (const auto name : required) {
    if (!value.contains(name)) {
      return std::unexpected{manifest_error(
          std::string{context} + " is missing required field: " + std::string{name})};
    }
  }
...
Result<std::uint64_t> integer(
    const Json& value, std::string_view context, std::uint64_t minimum, std::uint64_t maximum) {
...
Result<WaveformSpec> waveform(
    const Json& value, std::string context, const std::filesystem::path& base) {
  if (auto valid = fields(value, context, {"id", "name", "path", "root_key"},
                          {"target_sample_rate"}); !valid) return std::unexpected{valid.error()};
...
Result<SampleBankSpec> sample_bank(
    const Json& value, std::string context, const std::filesystem::path& base) {
  if (auto valid = fields(
          value, context, {"name", "root_key", "key_low", "key_high"},
          {"level", "waveform_id", "right_waveform_id", "interleaved_audio_path",
           "left_waveform_name", "right_waveform_name", "target_sample_rate"}); !valid) {
  ...
  if (*high < *low) return std::unexpected{manifest_error(context + ".key_high precedes key_low")};
  ...
  const auto optional_text = [&](std::string_view field) -> Result<std::optional<std::string>> {
    if (!value.contains(field)) return std::optional<std::string>{};
  ...
  if (!left_id) return std::unexpected{left_id.error()};
  if (!right_id) return std::unexpected{right_id.error()};
  if (!left_name) return std::unexpected{left_name.error()};
  if (!right_name) return std::unexpected{right_name.error()};
...
Result<VolumeSpec> volume(
  ...
  for (std::size_t index = 0; index < value["waveforms"].size(); ++index) {
    ...
    if (!waveform_ids.insert(item->id).second) return std::unexpected{manifest_error(context + " has duplicate waveform ids")};
  ...
  for (std::size_t index = 0; index < value["sample_banks"].size(); ++index) {
    auto item = sample_bank(value["sample_banks"][index], context + ".sample_banks[" + std::to_string(index) + "]", base);
    if (!item) return std::unexpected{item.error()};
    if (!bank_names.insert(item->name).second) return std::unexpected{manifest_error(context + " has duplicate sample-bank names")};
    if (item->waveform_id && !waveform_ids.contains(*item->waveform_id)) return std::unexpected{manifest_error(context + " references an unknown waveform")};
    if (item->right_waveform_id && (!waveform_ids.contains(*item->right_waveform_id) || item->right_waveform_id == item->waveform_id)) return std::unexpected{manifest_error(context + " has an invalid right waveform reference")};
  ...
  for (std::size_t index = 0; index < groups.size(); ++index) {
    ...
    if (auto valid = fields(row, group_context, {"name"},
                            {"member_sample_bank", "member_sample_banks"}); !valid) {
      return std::unexpected{valid.error()};
    }
    ...
    if (!group_name) return std::unexpected{group_name.error()};
  ...
  for (std::size_t index = 0; index < programs.size(); ++index) {
    ...
    for (std::size_t assignment_index = 0; assignment_index < row["assignments"].size(); ++assignment_index) {
      ...
      if (auto valid = fields(assignment, assignment_context, {"receive_channel"},
                              {"sample_bank", "sample_bank_group"}); !valid) {
        return std::unexpected{valid.error()};
      }
      const bool bank = assignment.contains("sample_bank");
      const bool group = assignment.contains("sample_bank_group");
      if (bank == group) return std::unexpected{manifest_error(assignment_context + " must contain exactly one target")};
...
Result<HdsBuildManifest> parse(const Json& root, const std::filesystem::path& base) {
  if (auto valid = fields(root, "manifest", {"schema_version", "size_bytes", "partitions"}); !valid) return std::unexpected{valid.error()};
  auto version = text(root["schema_version"], "manifest.schema_version");
  if (!version) return std::unexpected{version.error()};
  if (*version != "1.0") return std::unexpected{manifest_error("manifest.schema_version must be '1.0'")};
  auto size = integer(root["size_bytes"], "manifest.size_bytes", minimum_hds_size, maximum_hds_size);
  if (!size) return std::unexpected{size.error()};
  if (*size % 512U != 0U) return std::unexpected{manifest_error("manifest.size_bytes must be a multiple of 512")};
  if (!root["partitions"].is_array() || root["partitions"].empty() || root["partitions"].size() > 8U) return std::unexpected{manifest_error("manifest.partitions must contain 1..8 partitions")};
  ...
  for (std::size_t index = 0; index < root["partitions"].size(); ++index) {
    ...
    if (auto valid = fields(row, context, {"name", "volumes"}); !valid) return std::unexpected{valid.error()};
    auto name = text(row["name"], context + ".name");
    if (!name) return std::unexpected{name.error()};
    if (!row["volumes"].is_array() || row["volumes"].empty()) return std::unexpected{manifest_error(context + ".volumes must be a non-empty array")};
    PartitionSpec partition{*name, {}};
    for (std::size_t volume_index = 0; volume_index < row["volumes"].size(); ++volume_index) {
      auto parsed = volume(row["volumes"][volume_index], context + ".volumes[" + std::to_string(volume_index) + "]", base);
      if (!parsed) return std::unexpected{parsed.error()};
...
Result<HdsBuildManifest> parse_hds_build_manifest(
    std::string_view json, const std::filesystem::path& base_directory) {
  try {
...
Result<HdsBuildManifest> load_hds_build_manifest(const std::filesystem::path& path) {
  ...
  if (!input) return std::unexpected{make_error(ErrorCode::io_open_failed, ErrorCategory::io, "could not open HDS manifest")};
  std::ostringstream text;
  text << input.rdbuf();
  if (!input && !input.eof()) return std::unexpected{make_error(ErrorCode::io_read_failed, ErrorCategory::io, "could not read HDS manifest")};
...
Result<std::vector<PartitionGeometry>> plan_hds_geometry(const HdsBuildManifest& manifest) {
  ...
  if (sectors < 2045U) return std::unexpected{manifest_error("partition slots are too small for SFS")};
