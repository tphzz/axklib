...
Result<std::vector<std::byte>> ascii(std::string_view value, std::size_t size,
  ...
  if (value.size() > size ||
      !std::ranges::all_of(value, [](unsigned char character) { return character < 0x80U; })) {
  ...
  std::ranges::transform(value, result.begin(),
...
std::uint16_t pitch_word(std::uint8_t root_key, std::uint32_t sample_rate) {
  ...
  const auto root =
      root_key == 0U ? 0x03ab : ((root_key - 1U) / 12U) * 1024U + fractions[(root_key - 1U) % 12U];
  const auto rate =
      static_cast<int>(std::log(static_cast<double>(sample_rate) / 44'100.0) * 1477.3197);
...
Result<std::vector<std::byte>> serialize_smpl(const WaveformSpec &spec, const ImportedAudio &audio,
  ...
  stored.insert(stored.end(), stored.begin(),
                stored.begin() +
                    static_cast<std::ptrdiff_t>(std::min<std::size_t>(stored.size(), 8U)));
  std::vector<std::byte> result(512U + stored.size());
  constexpr std::string_view magic{"FSFSDEV3SPLX"};
  std::ranges::transform(magic, result.begin(),
  ...
  constexpr std::array<std::byte, 8> identity{std::byte{0},    std::byte{0},    std::byte{0},
                                              std::byte{0x0a}, std::byte{0x87}, std::byte{0x7c},
...
Result<std::vector<std::byte>> serialize_sbnk(const SampleBankSpec &bank,
                                              const LoadedWaveform &left,
  ...
  constexpr std::array<std::byte, 16> header{
      ...
      std::byte{0}, std::byte{4}, std::byte{0}, std::byte{0},   std::byte{1}, std::byte{0x34},
  ...
  constexpr std::array<std::byte, 7> suffix{std::byte{0xb8}, std::byte{0},    std::byte{0x0a},
                                            std::byte{0xf6}, std::byte{0x7a}, std::byte{0x01},
                                            std::byte{0x54}};
  ...
  const std::array<std::pair<std::size_t, std::uint8_t>, 32> defaults{
      {{0x109, 0},   {0x10a, 127},        {0x10b, 4},   {0x10c, 0},   {0x10d, 127}, {0x10e, 0},
       {0x10f, 0},   {0x110, 0},          {0x111, 0},   {0x112, 0},   {0x113, 0},   {0x114, 63},
       {0x115, 0},   {0x116, bank.level}, {0x117, 0},   {0x118, 0},   {0x119, 0},   {0x11a, 127},
       {0x11b, 0},   {0x11c, 0},          {0x11d, 127}, {0x11e, 127}, {0x11f, 127}, {0x120, 0},
       {0x121, 0},   {0x122, 26},         {0x123, 64},  {0x124, 10},  {0x125, 0},   {0x126, 127},
       {0x127, 127}, {0x128, 127}}};
  ...
  constexpr std::array<std::byte, 5> playback{std::byte{0xc1}, std::byte{0xe0}, std::byte{0x1e},
                                              std::byte{0x3a}, std::byte{0x20}};
  constexpr std::array<std::byte, 4> tone{std::byte{0x3e}, std::byte{0x20}, std::byte{0xe1},
                                          std::byte{0xc6}};
...
Result<std::vector<std::byte>> serialize_sbac(const SampleBankGroupSpec &group,
  ...
  for (std::size_t index = 0; index < group.member_sample_banks.size(); ++index) {
    ...
    std::ranges::copy(*member,
                      result.begin() + static_cast<std::ptrdiff_t>(0x14cU + index * 0x14U));
  }
  return result;
}
...
Result<std::vector<std::byte>> serialize_prog(const ProgramSpec &program) {
  ...
  constexpr std::array<std::byte, 24> defaults{
      std::byte{0},    std::byte{5},    std::byte{0xff}, std::byte{0xff}, std::byte{0},
      std::byte{0},    std::byte{0},    std::byte{1},    std::byte{0x40}, std::byte{0},
      std::byte{0x40}, std::byte{0x7f}, std::byte{0},    std::byte{0},    std::byte{0},
      std::byte{0xfe}, std::byte{0},    std::byte{0x5a}, std::byte{0x5a}, std::byte{0x27},
...
Result<std::vector<std::byte>> detail::prepare_smpl_payload(const WaveformSpec &spec,
...
detail::prepare_sbnk_payload(const SampleBankSpec &spec, const PreparedWaveformMember &left,
  ...
  if (right) {
    ...
    right_loaded.emplace(
        LoadedWaveform{std::move(right_spec), std::move(right_audio), right->link_id});
  }
  return serialize_sbnk(spec, left_loaded, right_loaded ? &*right_loaded : nullptr, grouped,
                        linked_programs);
}

Result<std::vector<std::byte>>
detail::prepare_sbac_payload(const SampleBankGroupSpec &group,
                             const std::map<std::string, SampleBankSpec> &banks) {
...
detail::prepare_partition_records(const PartitionSpec &partition, const PartitionGeometry &geometry,
  ...
  std::uint32_t next = 3;
  const auto marker =
      static_cast<std::uint8_t>(partition_count >= 3U ? geometry.index * 0x78U : 0U);
  ...
  for (const auto &volume : partition.volumes) {
    ...
    if (!volume.programs.empty()) {
      ...
      for (const auto &program : volume.programs) {
        if (program.assignments.size() != 2U || program.assignments[0].target_kind != "SBAC" ||
          ...
          return std::unexpected{
              make_error(ErrorCode::unsupported_profile, ErrorCategory::unsupported,
                         "Program profile requires SBAC channel 1 then SBNK channel 2")};
        }
        ...
        if (group == group_specs.end() || direct == bank_specs.end() ||
          ...
          return std::unexpected{make_error(ErrorCode::unsupported_profile,
        ...
        if (group->second->member_sample_banks.size() == 1U) {
          ...
          if (member->waveform_id != direct->second->waveform_id ||
            ...
            return std::unexpected{
                make_error(ErrorCode::unsupported_profile, ErrorCategory::unsupported,
                           "one-member group and direct control parameters must match")};
          }
        }
      }
    ...
    for (std::size_t index = 0; index < categories.size(); ++index)
      volume_entries.emplace_back(std::string{categories[index]}, category_ids[index],
                                  std::nullopt);
    ...
    for (std::size_t waveform_index = 0; waveform_index < volume.waveforms.size();
         ++waveform_index) {
      if (const auto check = cancellation.check(); !check)
        return std::unexpected{check.error()};
    ...
    for (const auto &bank : volume.sample_banks) {
      ...
      if (right != loaded.end() &&
        ...
        return std::unexpected{
            make_error(ErrorCode::unsupported_profile, ErrorCategory::unsupported,
                       "stereo SBNK members require matching rate and frame count")};
      auto payload =
          serialize_sbnk(bank, left->second, right == loaded.end() ? nullptr : &right->second,
  ...
  records.push_back(
      {1, std::move(*root_data), RecordKind::directory, static_cast<std::uint16_t>(root.size())});
  ...
  for (auto &record : records) {
    ...
    record.clusters =
        static_cast<std::uint32_t>((record.payload.size() + cluster_size - 1U) / cluster_size);
...
Result<WrittenImageLayout> write_hds_image(const HdsBuildManifest &manifest,
  ...
  for (std::size_t index = 0; index < geometries->size(); ++index) {
    auto records = detail::prepare_partition_records(
        manifest.partitions[index], (*geometries)[index], geometries->size(), cancellation);
    if (!records)
  ...
  std::ranges::transform(magic, superblock.begin(),
                         [](char value) { return static_cast<std::byte>(value); });
  ...
  for (std::size_t partition_index = 0; partition_index < geometries->size(); ++partition_index) {
    ...
    std::ranges::transform(token_text, token.begin(),
                           [](char value) { return static_cast<std::byte>(value); });
    ...
    std::ranges::transform(magic, header.begin(),
                           [](char value) { return static_cast<std::byte>(value); });
    auto name = ascii(manifest.partitions[partition_index].name,
    ...
    for (const auto &[offset, value] : std::array<std::pair<std::size_t, std::uint32_t>, 17>{
             {{0x104, static_cast<std::uint32_t>(geometry.start_sector)},
              ...
              {0x14c, static_cast<std::uint32_t>(geometries->size() >= 3U
              ...
              {0x160,
               static_cast<std::uint32_t>(geometry.index * geometry.filesystem_sector_count)},
              {0x164, dynamic},
              {0x178, static_cast<std::uint32_t>(geometry.start_sector)},
              {0x17c, static_cast<std::uint32_t>(geometry.filesystem_sector_count)},
              {0x184, dynamic},
              {0x194,
               geometries->size() >= 3U ? 0U : static_cast<std::uint32_t>(geometries->size())},
    ...
    for (const auto &record : records) {
      ...
      for (std::uint32_t cluster = record.cluster; cluster < record.cluster + record.clusters;
           ++cluster)
    ...
    if (!write_at(output, start + 2048U, std::span{bitmap}.first(512)) ||
        ...
        !write_at(output, (geometry.start_sector + geometry.directory_index_cluster * 2U) * 512U,
                  index))
      return std::unexpected{make_error(ErrorCode::io_read_failed, ErrorCategory::io,
    ...
    layout.partitions.push_back({geometry, manifest.partitions[partition_index].name, allocated,
  ...
  layout.unused_tail_sectors =
      manifest.size_bytes / 512U - (last.start_sector + last.filesystem_sector_count);
  return layout;
}
