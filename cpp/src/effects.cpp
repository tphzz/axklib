...
const generated::EffectTypeData *type_data(std::uint16_t raw_type) {
  const auto found =
      std::ranges::find(generated::effect_types, raw_type, &generated::EffectTypeData::raw_type);
  return found == generated::effect_types.end() ? nullptr : &*found;
}

const generated::EffectParameterData *parameter_data(std::uint16_t raw_type,
...
std::optional<std::int32_t> table_index(const generated::EffectParameterData &parameter,
...
std::optional<std::string> numeric_display(std::string_view source, std::uint8_t raw_value,
                                           std::int32_t index) {
  ...
  if (source.starts_with("Delay_01_") || source.starts_with("DelayTime_01_") ||
      source == "Initial_Delay")
...
known_display(std::uint16_t raw_type, std::uint8_t parameter_number, std::uint8_t raw_value) {
...
std::optional<EffectTypeInfo> effect_type_info(std::uint16_t raw_type,
  ...
  return EffectTypeInfo{row->raw_type, row->printed_number, row->printed_label, row->ui_label,
                        row->validated};
}

std::optional<EffectParameterInfo> effect_parameter_info(std::uint16_t raw_type,
                                                         std::uint8_t parameter_number,
                                                         EffectProfile profile) noexcept {
  if (!effect_type_supported(raw_type, profile))
    return std::nullopt;
  ...
  if (row == nullptr)
    return std::nullopt;
  return EffectParameterInfo{row->raw_type,        row->parameter_number, row->effect_label,
...
EffectDisplayValue format_effect_parameter(std::optional<std::uint16_t> raw_type,
                                           std::uint8_t parameter_number,
  ...
  if (!effect_type_supported(*raw_type, profile))
    return {{},
            ...
            "Unknown",
            {},
            "Raw effect type is not supported by the selected A-series profile."};
  ...
  if (const auto *known = known_display(*raw_type, parameter_number, *raw_value))
    return {std::string{known->display}, index, "Known", "validated UI-load check",
            std::string{validated_note}};
  ...
  if (const auto label = enum_label(parameter->value_source, *index))
    return {std::string{*label}, index, "Likely",
            std::format("{}:{}", parameter->table_source, parameter->value_source),
            std::string{table_note}};
  const auto has_enum_table =
      std::ranges::any_of(generated::effect_enum_values,
