...
[[nodiscard]] Result<std::vector<std::byte>> project_package_node_names(const PortablePackage &package,
                                                                        const PackageNode &node,
                                                                        const PackageNodeRelocationContext &context);

[[nodiscard]] Result<std::vector<std::byte>> relocate_package_node(const PortablePackage &package,
                                                                   const PackageNode &node,
                                                                   const PackageNodeRelocationContext &context);
