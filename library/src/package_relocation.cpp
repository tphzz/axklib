...
Result<void> add_range(RelocationProfile &profile, const DecodedObject &object,
                       std::uint32_t offset, std::uint32_t width, std::string role,
...
Result<std::string_view> target_name(const PackageNode &node,
  ...
  if (found == context.edge_target_names.end()) {
    return std::unexpected{
        relocation_error(node, "package relocation is missing an edge target name")};
  }
  return found->second;
}

Result<std::uint32_t> target_link_id(const PackageNode &node,
                                     const PackageNodeRelocationContext &context,
                                     const PackageRelationship &edge) {
  const auto found = context.edge_target_link_ids.find(edge.edge_id);
  if (found == context.edge_target_link_ids.end()) {
    return std::unexpected{
        relocation_error(node, "package relocation is missing an SMPL target link ID")};
  }
  return found->second;
}
  ...
  for (const auto &edge : package.relationships) {
    ...
    if (edge.role == "SBNK_LEFT_MEMBER_TO_SMPL") {
      ...
      if (program == nullptr || edge.ordinal >= program->assignments.size()) {
        return std::unexpected{
            relocation_error(node, "Program relationship source is not decoded")};
      }
      source_name = program->assignments[edge.ordinal].name;
    } else {
      return std::unexpected{
          relocation_error(node, "package relocation encountered an unsupported edge role")};
    }
    if (source_name != *name) {
...
Result<RelocationProfile> build_relocation_profile(const DecodedObject &object,
  ...
  if (object.format != ObjectFormat::current) {
    return std::unexpected{
        profile_error(object, "portable packages require a current Yamaha object profile")};
  }
  ...
  case ObjectType::sbnk: {
    ...
    if (auto added = add_range(result, object, 0xd0U, 1U, "SBNK_GROUP_MEMBERSHIP", group_mask);
        !added) {
      return std::unexpected{added.error()};
    }
    break;
  ...
  case ObjectType::sequ:
    return std::unexpected{profile_error(
        object, "SEQU portability is not admitted without a proven dependency profile")};
  case ObjectType::prf3:
    return std::unexpected{
        profile_error(object, "PRF3 portability is not admitted for package version 1")};
  case ObjectType::unknown:
    return std::unexpected{
        profile_error(object, "unknown Yamaha object types are not portable package nodes")};
  }
  return result;
}
...
Result<std::vector<std::byte>> relocate_package_node(const PortablePackage &package,
  ...
  if (node.object_type == "SMPL") {
    if (!context.smpl_link_id || *context.smpl_link_id < 0xbaU || result.size() < 0x7cU) {
      return std::unexpected{
          relocation_error(node, "SMPL relocation requires a valid destination link ID")};
    }
    ...
    if (!left_link) {
      return std::unexpected{
          relocation_error(node, "SBNK relocation requires its left SMPL relationship")};
    }
    ...
    if (right_link) {
      ...
      if (source->inactive_right.smpl_link_id == 0U) {
        ...
        return std::unexpected{
            relocation_error(node, "inactive SBNK right lane has an unsupported nonzero link ID")};
      }
    }
    ...
    for (const auto number : context.linked_program_numbers) {
      if (number < 1U || number > 128U || !unique_programs.emplace(number).second) {
        return std::unexpected{
            relocation_error(node, "SBNK relocation contains an invalid Program number")};
      }
  ...
  for (std::size_t offset = 0; offset < result.size(); ++offset) {
    if (result[offset] != node.raw_payload[offset] && !contains(allowed, offset)) {
      return std::unexpected{
          relocation_error(node, "package relocation changed an undeclared payload byte")};
    }
  }
  ...
  if (decoded->header.name != context.destination_name) {
    return std::unexpected{
        relocation_error(node, "relocated object header name does not match its destination")};
  }
  if (const auto *sample = std::get_if<CurrentSmpl>(&decoded->payload)) {
    if (!context.smpl_link_id || sample->link_id.value != *context.smpl_link_id ||
      ...
      return std::unexpected{
          relocation_error(node, "relocated SMPL IDs did not decode to the planned values")};
    }
      ...
      if (edge.role == "SBNK_LEFT_MEMBER_TO_SMPL") {
        ...
        if (bank->left.sample_name != *name || bank->left.smpl_link_id != *link) {
          return std::unexpected{relocation_error(
              node, "relocated SBNK left member did not decode to the planned target")};
        }
      } else if (edge.role == "SBNK_RIGHT_MEMBER_TO_SMPL") {
        const auto link = target_link_id(node, context, edge);
        if (!link)
        ...
        if (!bank->right || bank->right->sample_name != *name ||
          ...
          return std::unexpected{relocation_error(
              node, "relocated SBNK right member did not decode to the planned target")};
        }
      }
    }
    ...
    for (const auto &edge : package.relationships) {
      ...
      if (edge.ordinal >= group->slots.size() || group->slots[edge.ordinal].name != *name ||
        ...
        return std::unexpected{
            relocation_error(node, "relocated SBAC slot did not decode to the planned target")};
      }
    }
    ...
    for (const auto &edge : package.relationships) {
      ...
      if (edge.ordinal >= program->assignments.size() ||
        ...
        return std::unexpected{relocation_error(
            node, "relocated Program assignment did not decode to the planned target")};
      }
    }
  }
