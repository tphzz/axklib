...
Error profile_error(const DecodedObject &object, std::string message) {
    ...
    return make_error(ErrorCode::unsupported_profile,
                      ErrorCategory::unsupported, std::move(message),
                      std::move(context));
}
...
Result<void> add_range(RelocationProfile &profile, const DecodedObject &object,
                       std::uint32_t offset, std::uint32_t width,
                       std::string role, std::span<const std::byte> mask = {}) {
    if (offset > profile.normalized_payload.size() ||
        width > profile.normalized_payload.size() - offset ||
        (!mask.empty() && mask.size() != width)) {
        return std::unexpected{
            profile_error(object, "object relocation range is out of bounds")};
    }
    const auto source =
        std::span<const std::byte>{profile.normalized_payload}.subspan(offset,
                                                                       width);
...
Result<void> put_name(std::span<std::byte> payload, std::size_t offset,
                      std::string_view name) {
    if (name.empty() || name.size() > 16U ||
        !std::ranges::all_of(
            name, [](unsigned char value) { return value < 0x80U; }) ||
        offset > payload.size() || 16U > payload.size() - offset) {
...
void put_be32(std::span<std::byte> payload, std::size_t offset,
              std::uint32_t value) {
...
target_name(const PackageNode &node,
    ...
    if (found == context.edge_target_names.end()) {
        return std::unexpected{relocation_error(
            node, "package relocation is missing an edge target name")};
    }
    return found->second;
}

Result<std::uint32_t>
target_link_id(const PackageNode &node,
               const PackageNodeRelocationContext &context,
               const PackageRelationship &edge) {
    const auto found = context.edge_target_link_ids.find(edge.edge_id);
    if (found == context.edge_target_link_ids.end()) {
        return std::unexpected{relocation_error(
            node, "package relocation is missing an SMPL target link ID")};
    }
    return found->second;
}
...
bool contains(std::span<const ByteRange> ranges, std::size_t offset) {
    return std::ranges::any_of(ranges, [=](const ByteRange &range) {
        return offset >= range.offset && offset - range.offset < range.size;
    });
}

Result<std::vector<std::byte>>
patch_names(const PortablePackage &package, const PackageNode &node,
            const PackageNodeRelocationContext &context,
    ...
    if (decoded->header.name != context.destination_name) {
        if (auto patched = put_name(result, 0x32U, context.destination_name);
            !patched)
    ...
    for (const auto &edge : package.relationships) {
        ...
        if (edge.role == "SBNK_LEFT_MEMBER_TO_SMPL") {
            ...
            if (bank == nullptr)
                return std::unexpected{relocation_error(
                    node, "SBNK relationship source is not decoded")};
            ...
            if (bank == nullptr || !bank->right)
                return std::unexpected{relocation_error(
                    node, "SBNK right relationship is not decoded")};
            ...
            if (group == nullptr || edge.ordinal >= group->slots.size())
                return std::unexpected{relocation_error(
                    node, "SBAC relationship source is not decoded")};
            ...
            if (program == nullptr ||
                edge.ordinal >= program->assignments.size()) {
                return std::unexpected{relocation_error(
                    node, "Program relationship source is not decoded")};
            }
            source_name = program->assignments[edge.ordinal].name;
        } else {
            return std::unexpected{relocation_error(
                node,
                "package relocation encountered an unsupported edge role")};
        }
        if (source_name != *name) {
...
Result<RelocationProfile>
build_relocation_profile(const DecodedObject &object,
                         std::span<const std::byte> raw_payload) {
    if (object.format != ObjectFormat::current) {
        return std::unexpected{profile_error(
            object,
            "portable packages require a current Yamaha object profile")};
    }
    ...
    case ObjectType::smpl:
        if (!std::holds_alternative<CurrentSmpl>(object.payload))
            return std::unexpected{
                profile_error(object, "current SMPL payload is not decoded")};
        if (auto added = add_range(result, object, 0x6cU, 4U, "SMPL_GROUP_ID");
            !added)
            return std::unexpected{added.error()};
        if (auto added = add_range(result, object, 0x78U, 4U, "SMPL_LINK_ID");
            !added)
            return std::unexpected{added.error()};
        break;
    case ObjectType::sbnk: {
        if (!std::holds_alternative<CurrentSbnk>(object.payload))
            return std::unexpected{
                profile_error(object, "current SBNK payload is not decoded")};
        if (auto added =
                add_range(result, object, 0xa0U, 4U, "SBNK_LEFT_MEMBER_LINK");
            !added)
            return std::unexpected{added.error()};
        if (auto added =
                add_range(result, object, 0xa4U, 4U, "SBNK_RIGHT_MEMBER_LINK");
            !added)
            return std::unexpected{added.error()};
        if (auto added =
                add_range(result, object, 0xc0U, 16U, "SBNK_PROGRAM_BITMAP");
            !added)
            return std::unexpected{added.error()};
        constexpr std::array group_mask{std::byte{0x01}};
        if (auto added = add_range(result, object, 0xd0U, 1U,
                                   "SBNK_GROUP_MEMBERSHIP", group_mask);
            !added) {
            return std::unexpected{added.error()};
        }
        break;
    ...
    case ObjectType::sbac: {
        ...
        if (group == nullptr)
            return std::unexpected{
                profile_error(object, "current SBAC payload is not decoded")};
        for (const auto &slot : group->slots) {
            if (auto added = add_range(result, object, slot.offset + 16U, 4U,
                                       "SBAC_SLOT_HANDLE");
                !added) {
                return std::unexpected{added.error()};
            }
    ...
    case ObjectType::sequ:
        return std::unexpected{
            profile_error(object, "SEQU portability is not admitted without a "
                                  "proven dependency profile")};
    case ObjectType::prf3:
        return std::unexpected{profile_error(
            object, "PRF3 portability is not admitted for package version 1")};
    case ObjectType::unknown:
        return std::unexpected{profile_error(
            object,
            "unknown Yamaha object types are not portable package nodes")};
    }
    return result;
}

Result<std::vector<std::byte>>
project_package_node_names(const PortablePackage &package,
...
relocate_package_node(const PortablePackage &package, const PackageNode &node,
    ...
    auto source_profile =
        build_relocation_profile(*source_decoded, node.raw_payload);
    if (!source_profile)
    ...
    if (node.object_type == "SMPL") {
        if (!context.smpl_link_id || *context.smpl_link_id < 0xbaU ||
            result.size() < 0x7cU) {
            return std::unexpected{relocation_error(
                node, "SMPL relocation requires a valid destination link ID")};
        }
        ...
        if (result.size() < 0xd1U) {
            return std::unexpected{
                relocation_error(node, "SBNK relocation payload is truncated")};
        }
        ...
        if (!left_link) {
            return std::unexpected{relocation_error(
                node, "SBNK relocation requires its left SMPL relationship")};
        }
        ...
        if (right_link) {
            ...
            const auto *source =
                std::get_if<CurrentSbnk>(&source_decoded->payload);
            if (source == nullptr)
                return std::unexpected{relocation_error(
                    node, "SBNK source payload is not decoded")};
            if (source->inactive_right.smpl_link_id == 0U) {
                put_be32(result, 0xa4U, 0U);
            } else if (source->inactive_right.smpl_link_id ==
                       source->left.smpl_link_id) {
                put_be32(result, 0xa4U, *left_link);
            } else {
                return std::unexpected{
                    relocation_error(node, "inactive SBNK right lane has an "
        ...
        for (const auto number : context.linked_program_numbers) {
            if (number < 1U || number > 128U ||
                !unique_programs.emplace(number).second) {
                return std::unexpected{relocation_error(
                    node,
                    "SBNK relocation contains an invalid Program number")};
            }
            const auto word_offset =
                0xc0U + static_cast<std::size_t>((number - 1U) / 32U) * 4U;
            const auto bit = std::uint32_t{1} << ((number - 1U) % 32U);
            const auto word =
                (std::to_integer<std::uint32_t>(result[word_offset]) << 24U) |
                (std::to_integer<std::uint32_t>(result[word_offset + 1U])
                 << 16U) |
                (std::to_integer<std::uint32_t>(result[word_offset + 2U])
                 << 8U) |
        ...
        if (group == nullptr)
            return std::unexpected{
                relocation_error(node, "SBAC source payload is not decoded")};
        for (const auto &slot : group->slots)
            put_be32(result, slot.offset + 16U, 0U);
    } else if (node.object_type == "PROG") {
        const auto *program =
            std::get_if<CurrentProg>(&source_decoded->payload);
        if (program == nullptr)
            return std::unexpected{relocation_error(
                node, "Program source payload is not decoded")};
        for (std::size_t index = 0; index < program->assignments.size();
             ++index) {
            const auto &assignment = program->assignments[index];
        ...
        return std::unexpected{relocation_error(
            node,
            "package node type has no admitted relocation implementation")};
    }

    for (std::size_t offset = 0; offset < result.size(); ++offset) {
        if (result[offset] != node.raw_payload[offset] &&
            !contains(allowed, offset)) {
            return std::unexpected{relocation_error(
                node, "package relocation changed an undeclared payload byte")};
        }
    }
    ...
    if (decoded->header.name != context.destination_name) {
        return std::unexpected{relocation_error(
            node,
            "relocated object header name does not match its destination")};
    }
    if (const auto *sample = std::get_if<CurrentSmpl>(&decoded->payload)) {
        if (!context.smpl_link_id ||
            sample->link_id.value != *context.smpl_link_id ||
            sample->group_id.value != *context.smpl_link_id - 0xbaU) {
            return std::unexpected{relocation_error(
                node,
                "relocated SMPL IDs did not decode to the planned values")};
        }
    } else if (const auto *bank = std::get_if<CurrentSbnk>(&decoded->payload)) {
        std::set<std::uint8_t> expected_programs(
            context.linked_program_numbers.begin(),
            context.linked_program_numbers.end());
        const std::set<std::uint8_t> actual_programs(
            bank->linked_program_numbers.begin(),
            bank->linked_program_numbers.end());
        if (expected_programs != actual_programs ||
            ((bank->sample_flags & 1U) != 0U) != context.grouped)
            return std::unexpected{relocation_error(
                node,
                "relocated SBNK metadata did not decode to the planned graph")};
        for (const auto &edge : package.relationships) {
            if (edge.source_node_id != node.node_id)
                continue;
            const auto name = target_name(node, context, edge);
            ...
            if (edge.role == "SBNK_LEFT_MEMBER_TO_SMPL") {
                ...
                if (!link)
                    return std::unexpected{link.error()};
                if (bank->left.sample_name != *name ||
                    bank->left.smpl_link_id != *link) {
                    return std::unexpected{relocation_error(
                        node, "relocated SBNK left member did not decode to "
                              "the planned target")};
                }
            } else if (edge.role == "SBNK_RIGHT_MEMBER_TO_SMPL") {
                const auto link = target_link_id(node, context, edge);
                if (!link)
                    return std::unexpected{link.error()};
                if (!bank->right || bank->right->sample_name != *name ||
                    bank->right->smpl_link_id != *link) {
                    return std::unexpected{relocation_error(
                        node, "relocated SBNK right member did not decode to "
                              "the planned target")};
                }
            }
        }
        ...
        for (const auto &edge : package.relationships) {
            ...
            if (!name)
                return std::unexpected{name.error()};
            if (edge.ordinal >= group->slots.size() ||
                group->slots[edge.ordinal].name != *name ||
                group->slots[edge.ordinal].raw_handle != 0U) {
                return std::unexpected{
                    relocation_error(node, "relocated SBAC slot did not decode "
        ...
        if (std::ranges::any_of(program->assignments,
                                [](const ProgAssignment &assignment) {
            ...
            return std::unexpected{relocation_error(
                node, "relocated Program assignment handle is not zero")};
        }
        for (const auto &edge : package.relationships) {
            if (edge.source_node_id != node.node_id ||
            ...
            const auto expected_kind =
                edge.role == "PROG_ASSIGNMENT_TO_SBAC" ? 0x11U : 0x10U;
            if (edge.ordinal >= program->assignments.size() ||
                program->assignments[edge.ordinal].name != *name ||
                program->assignments[edge.ordinal].kind != expected_kind ||
                program->assignments[edge.ordinal].raw_handle != 0U) {
                return std::unexpected{
                    relocation_error(node, "relocated Program assignment did "
    ...
    auto expected_profile =
        build_relocation_profile(*projected_decoded, *projected);
    ...
    if (expected_profile->normalized_payload !=
        actual_profile->normalized_payload) {
        return std::unexpected{relocation_error(
            node, "relocated payload changed normalized identity")};
    }
    return result;
}
