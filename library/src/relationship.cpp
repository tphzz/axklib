    ...
    for (const auto *item : scope) {
        ...
        if (const auto *wave_data = std::get_if<CurrentSmpl>(&item->object.payload)) {
            ...
            if (generic != nullptr && generic->raw_payload.size() >= 0x7cU) {
                ...
                const auto link = static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(raw[0x78U])) << 24U |
...
bool same_volume(const ObjectSnapshot &left, const ObjectSnapshot &right) {
    return left.placement && right.placement && left.placement->partition.value == right.placement->partition.value &&
...
std::string local_suffix(const ObjectSnapshot &source) {
    return source.placement && !source.placement->container_directory.empty() ? "same-folder" : "same-volume";
}
...
Match match_named_target(const ObjectSnapshot &source, std::string_view name, ObjectType type, const ScopeIndex &index,
    ...
    return {nullptr, candidates, candidates.empty() ? RelationshipQuality::unknown : RelationshipQuality::tentative,
            candidates.empty() ? unique_basis == "active-sbac-slot-name" ? "active-sbac-slot-unmatched"
                                                                         : std::format("{}-unmatched", unique_basis)
                               : std::string{ambiguous_basis},
            candidates.empty() ? "no same-scope target has this name" : "multiple same-scope targets have this name"};
}
...
std::optional<std::uint8_t> program_number(const ObjectSnapshot &item) {
    ...
    if (result.ec != std::errc{} || result.ptr != name.data() + name.size() || value < 1U || value > 128U) {
        return std::nullopt;
    }
...
RelationshipGraph build_relationship_graph(const ObjectCatalog &catalog) {
    ...
    for (const auto &[scope_key, scope] : scopes) {
        ...
        for (const auto *item : scope) {
            ...
            comparison.indirect_assignment_programs = sorted_unique(comparison.indirect_assignment_programs);
            ...
            if (comparison.bitmap_without_direct.empty() && comparison.direct_without_bitmap.empty()) {
                ...
                if (comparison.direct_without_bitmap.empty()) {
                    ...
                    const auto indirect = std::set<std::uint8_t>(comparison.indirect_assignment_programs.begin(),
            ...
            if (!comparison.bitmap_programs.empty()) {
                ...
                const auto quality =
                    comparison.status == "match" ? RelationshipQuality::known : RelationshipQuality::tentative;
