...
struct ScopeIndex {
    std::map<std::pair<ObjectType, std::string>,
             std::vector<const ObjectSnapshot *>>
        typed_names;
    std::map<std::string, std::vector<const ObjectSnapshot *>> names;
    std::unordered_map<std::uint32_t, std::vector<const ObjectSnapshot *>>
        sample_links;
...
ScopeIndex index_scope(const std::vector<const ObjectSnapshot *> &scope) {
    ...
    for (const auto *item : scope) {
        index.typed_names[{item->object.header.type, item->object.header.name}]
            .push_back(item);
        index.names[item->object.header.name].push_back(item);
        index.keys.emplace(item->key, item);
        if (const auto *sample =
                std::get_if<CurrentSmpl>(&item->object.payload)) {
            index.sample_links[sample->link_id.value].push_back(item);
        } else if (item->object.header.type == ObjectType::smpl) {
            const auto *generic =
                std::get_if<GenericObject>(&item->object.payload);
            if (generic != nullptr && generic->raw_payload.size() >= 0x7cU) {
                const auto &raw = generic->raw_payload;
                const auto link =
                    static_cast<std::uint32_t>(
                        std::to_integer<std::uint8_t>(raw[0x78U]))
                        << 24U |
                    static_cast<std::uint32_t>(
                        std::to_integer<std::uint8_t>(raw[0x79U]))
                        << 16U |
                    static_cast<std::uint32_t>(
                        std::to_integer<std::uint8_t>(raw[0x7aU]))
                        << 8U |
                    static_cast<std::uint32_t>(
                        std::to_integer<std::uint8_t>(raw[0x7bU]));
...
bool same_volume(const ObjectSnapshot &left, const ObjectSnapshot &right) {
    return left.placement && right.placement &&
           left.placement->partition.value ==
               right.placement->partition.value &&
           left.placement->volume_directory.value ==
               right.placement->volume_directory.value;
}

bool same_container_folder(const ObjectSnapshot &left,
                           const ObjectSnapshot &right) {
    return left.placement && right.placement &&
           !left.placement->container_directory.empty() &&
           left.placement->container_directory ==
               right.placement->container_directory;
}

std::vector<const ObjectSnapshot *>
named(const ScopeIndex &index, ObjectType type, std::string_view name) {
    if (type == ObjectType::unknown) {
        const auto found = index.names.find(std::string{name});
        return found == index.names.end()
                   ? std::vector<const ObjectSnapshot *>{}
                   : found->second;
    }
    const auto found = index.typed_names.find({type, std::string{name}});
    return found == index.typed_names.end()
               ? std::vector<const ObjectSnapshot *>{}
               : found->second;
}

const ObjectSnapshot *
unique_local(const ObjectSnapshot &source,
             const std::vector<const ObjectSnapshot *> &candidates) {
    const ObjectSnapshot *result{};
    for (const auto *candidate : candidates) {
        const bool local =
            source.placement && !source.placement->container_directory.empty()
...
std::string local_suffix(const ObjectSnapshot &source) {
    return source.placement && !source.placement->container_directory.empty()
               ? "same-folder"
               : "same-volume";
}

Match match_member(const ObjectSnapshot &source,
                   const CurrentSbnkMember &member, const ScopeIndex &index) {
    ...
    if (exact.size() == 1) {
        return {exact.front(), exact, RelationshipQuality::known,
    ...
    if (by_name.size() > 1) {
        if (const auto *local = unique_local(source, by_name);
            local != nullptr) {
            return {
                local, by_name, RelationshipQuality::likely,
                std::format("sbnk-member-name+{}", local_suffix(source)),
                "duplicate member names have one local container candidate"};
        }
    }
    if (!by_name.empty())
        return {nullptr, by_name, RelationshipQuality::tentative,
                "sbnk-member-name-ambiguous",
                "member name matches multiple SMPL objects"};
    if (!by_link.empty())
        return {nullptr, by_link, RelationshipQuality::tentative,
                "sbnk-member-link-ambiguous",
...
Match match_named_target(const ObjectSnapshot &source, std::string_view name,
                         ObjectType type, const ScopeIndex &index,
                         std::string_view unique_basis,
                         std::string_view ambiguous_basis,
                         bool use_same_volume) {
        ...
        if (unique_basis.starts_with("assignment-kind-0x")) {
            const auto category = type == ObjectType::sbac   ? "SBAC"
                                  : type == ObjectType::sbnk ? "SBNK"
                                                             : "object";
            const auto selector = unique_basis.substr(
                std::string_view{"assignment-kind-"}.size(), 4U);
            return {candidates.front(), candidates, RelationshipQuality::known,
                    std::string{unique_basis},
    ...
    if (candidates.size() > 1 && use_same_volume) {
        if (const auto *local = unique_local(source, candidates);
            local != nullptr) {
            const auto quality =
                source.placement &&
                        source.placement->container_directory.empty()
                    ? RelationshipQuality::known
                    : RelationshipQuality::likely;
            return {local, candidates, quality,
    ...
    return {nullptr, candidates,
            candidates.empty() ? RelationshipQuality::unknown
...
Relationship edge(const ObjectSnapshot &source, std::string type,
                  const Match &match,
    ...
    const auto target = match.target
                            ? std::optional<std::string>{match.target->key}
                            : std::nullopt;
    const auto discriminator =
        target.value_or(match.candidates.empty() ? "missing" : "candidates");
    return {
        std::format("{}|{}|{}|{}", source.key, type, discriminator,
                    match.basis),
        source.key,
        target,
        keys(match),
...
RelationshipGraph::children(std::string_view key) const {
...
std::vector<const Relationship *>
RelationshipGraph::parents(std::string_view key) const {
    std::vector<const Relationship *> result;
    for (const auto &row : relationships) {
        if ((row.target_key && *row.target_key == key) ||
            std::ranges::find(row.candidate_keys, key) !=
                row.candidate_keys.end()) {
            result.push_back(&row);
        }
    }
    return result;
...
RelationshipGraph build_relationship_graph(const ObjectCatalog &catalog) {
    ...
    for (const auto &[scope_key, scope] : scopes) {
        ...
        for (const auto *item : scope) {
            if (const auto *bank =
                ...
                for (const auto &slot : group->slots) {
                    if (slot.name.empty())
                        continue;
                    result.relationships.push_back(edge(
                        *item, "SBAC_SLOT_TO_SBNK",
                        match_named_target(*item, slot.name, ObjectType::sbnk,
                                           scope_index, "active-sbac-slot-name",
                                           "active-sbac-slot-name-ambiguous",
                                           true)));
                }
                ...
                for (std::size_t index = 0; index < program->assignments.size();
                    ...
                    auto match = match_named_target(
                        *item, row.name, type, scope_index,
                        type == ObjectType::unknown
                            ...
                            : std::format("assignment-kind-0x{:02x}+name",
                                          row.kind),
                        type == ObjectType::unknown
                            ? "assignment-name-ambiguous"
                            : std::format(
                                  "assignment-kind-0x{:02x}+name-ambiguous",
                                  row.kind),
                        assignment_state(row) == AssignmentState::active ||
                            item->scope_key.starts_with("iso:"));
                    if (match.target != nullptr &&
                        match.basis.ends_with("+same-folder") &&
                        type != ObjectType::unknown) {
                        match.quality = RelationshipQuality::known;
                    }
                    if (match.quality == RelationshipQuality::unknown &&
                        type == ObjectType::sbnk &&
                        item->scope_key.starts_with("iso:")) {
                        auto source_load_candidates =
                            named(scope_index, ObjectType::unknown, row.name);
                        const auto *local =
                            unique_local(*item, source_load_candidates);
                        if (local != nullptr) {
                            match = {local, std::move(source_load_candidates),
                                     RelationshipQuality::likely,
                                     "assignment-name-unique",
                                     "source-load assignment uniquely matches "
                                     "an object in the same folder"};
                        } else if (source_load_candidates.size() > 1U) {
                            match = {nullptr, std::move(source_load_candidates),
                                     RelationshipQuality::tentative,
                                     "assignment-name-ambiguous",
                                     "multiple same-scope non-SBNK objects "
                    ...
                    if (match.quality == RelationshipQuality::unknown &&
                        (row.raw_handle == 0U ||
                         decoded_state == AssignmentState::unknown))
                        continue;
                    const auto rel_type = type == ObjectType::sbac
                    ...
                    result.relationships.push_back(edge(*item, rel_type, match,
                                                        index, row.name, state,
                                                        std::move(channel)));
                }
                const auto last_program_edge = result.relationships.size();
                for (std::size_t edge_index = first_program_edge;
                     edge_index < last_program_edge; ++edge_index) {
                    auto &relationship = result.relationships[edge_index];
                    if (relationship.assignment_state !=
                            AssignmentState::visible_off ||
                    ...
                    for (const auto &key : relationship.candidate_keys) {
                        ...
                        if (candidate != scope_index.keys.end() &&
                            same_volume(*item, *candidate->second))
                            ++same_volume_candidates;
                    }
                    if (relationship.basis ==
                        "assignment-kind-0x11+name-ambiguous") {
                        relationship.basis =
                            same_volume_candidates == 1U
                                ? "assignment-visible-off-same-volume-sbac-"
                                  "diagnostic"
                                : "assignment-visible-off-name-ambiguous-sbac";
                    } else if (relationship.basis ==
                               "assignment-kind-0x10+name-ambiguous") {
                        relationship.basis =
                            same_volume_candidates == 1U
                        ...
                        for (const auto &key : relationship.candidate_keys) {
                            ...
                            only_waveforms =
                                only_waveforms &&
                                candidate != scope_index.keys.end() &&
                                candidate->second->object.header.type ==
                                    ObjectType::smpl;
                        }
                        relationship.basis =
                            only_waveforms ? "assignment-visible-off-name-"
                ...
                for (const auto rel_type :
                    ...
                    for (std::size_t edge_index = first_program_edge;
                        ...
                        const auto kind =
                            rel_type.ends_with("SBAC") ? 0x11U : 0x10U;
                        relationship.basis =
                            std::format("assignment-kind-0x{:02x}+program-"
                ...
                for (std::size_t edge_index = first_program_edge;
                    ...
                    if (relationship.assignment_state ==
                        ...
                        relationship.basis =
                            "assignment-active-missing-local-target";
                    } else if (relationship.assignment_state ==
                                   AssignmentState::visible_off &&
                        ...
                        relationship.basis =
                            item->scope_key.starts_with("iso:")
                               ...
                               relationship.type == "PROG_ASSIGNMENT_TO_SBNK") {
                        relationship.basis =
                            "assignment-visible-off-missing-local-sbnk";
                    } else {
                        continue;
                    }
                    relationship.key =
                        std::format("{}|{}|missing|{}", relationship.source_key,
                                    relationship.type, relationship.basis);
                }
                ...
                for (std::size_t edge_index = first_program_edge;
                    ...
                    if (!relationship.target_key)
                        continue;
                    const auto identity =
                        std::tuple{relationship.type, *relationship.target_key,
                                   relationship.assignment_name};
                    if (relationship.assignment_state ==
                        AssignmentState::active) {
                        active_targets.insert(identity);
                    } else if ((relationship.assignment_state ==
                                    AssignmentState::source_load ||
                                relationship.assignment_state ==
                                    AssignmentState::visible_off) &&
                               !active_targets.insert(identity).second) {
                        relationship.assignment_state =
                            AssignmentState::duplicate_not_active;
                    }
                }
            }
        }
        ...
        for (const auto &row : result.relationships) {
            if (row.scope_key == scope_key && row.type == "SBAC_SLOT_TO_SBNK" &&
                row.target_key && row.quality == RelationshipQuality::known) {
        ...
        std::unordered_map<std::string, std::vector<std::uint8_t>>
            ambiguous_programs;
        std::unordered_map<std::string, std::map<std::uint8_t, bool>>
            direct_nondefault_flags;
        for (const auto &row : result.relationships) {
            ...
            if (row.type == "PROG_ASSIGNMENT_TO_SBNK" && row.target_key) {
                ...
                if (row.assignment_index) {
                    if (const auto *program = std::get_if<CurrentProg>(
                            &source->second->object.payload);
                        program != nullptr &&
                        *row.assignment_index < program->assignments.size()) {
                        nondefault =
                            program->assignments[*row.assignment_index].flags !=
                            0xffU;
                    }
                }
                auto &[program_number, all_nondefault] =
                    *direct_nondefault_flags[*row.target_key]
                         .try_emplace(*number, true)
                         .first;
        ...
        for (const auto *item : scope) {
            ...
            comparison.indirect_assignment_programs =
                indirect_programs[item->key];
            comparison.direct_assignment_programs =
                sorted_unique(comparison.direct_assignment_programs);
            comparison.indirect_assignment_programs =
                sorted_unique(comparison.indirect_assignment_programs);
            const auto ambiguous = sorted_unique(ambiguous_programs[item->key]);
            std::ranges::set_difference(
                comparison.bitmap_programs,
                comparison.direct_assignment_programs,
            ...
            if (comparison.bitmap_without_direct.empty() &&
                comparison.direct_without_bitmap.empty()) {
                comparison.status = "match";
                comparison.mismatch_class = "match";
            } else {
                comparison.status = "mismatch";
                if (comparison.direct_without_bitmap.empty()) {
                    const auto covered = std::ranges::includes(
                        ambiguous, comparison.bitmap_without_direct);
                    comparison.mismatch_class =
                        covered
                            ? "bitmap_disambiguates_ambiguous_direct_assignment"
                            : "bitmap_without_decoded_direct_assignment";
                } else {
                    std::set<std::string> classes;
                    const auto indirect = std::set<std::uint8_t>(
                        comparison.indirect_assignment_programs.begin(),
                    ...
                    for (const auto number : comparison.direct_without_bitmap) {
                        if (indirect.contains(number)) {
                            classes.insert(
                                "direct_assignment_also_reached_through_sbac");
                        } else if (direct_nondefault_flags[item->key][number]) {
                            classes.insert("nondefault_flag_direct_assignment_"
                                           "without_bitmap");
                        } else {
                            classes.insert(
                                "known_direct_assignment_missing_bitmap");
                        }
                    }
                    if (!comparison.bitmap_without_direct.empty())
                        classes.insert(
            ...
            if (!comparison.bitmap_programs.empty()) {
                ...
                const auto quality = comparison.status == "match"
                                         ? RelationshipQuality::known
                                         : RelationshipQuality::tentative;
                ...
                const auto basis =
                    comparison.status == "match"
                        ...
                        : std::format("sbnk-program-link-bitmap-{}-diagnostic",
                                      mismatch_basis);
                result.relationships.push_back({
                    std::format("{}|SBNK_PROGRAM_BITMAP_TO_PROG|{}|{}",
                                item->key, targets, basis),
                    item->key,
                    targets,
                    {},
