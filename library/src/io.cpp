...
Result<void> CancellationToken::check() const {
    if (is_cancelled()) {
        return std::unexpected{make_error(ErrorCode::operation_cancelled,
...
void CancellationSource::reset() const noexcept {
    state_->store(false, std::memory_order_release);
}

MemoryReader::MemoryReader(std::vector<std::byte> bytes)
...
Result<void>
MemoryReader::read_exact_at(std::uint64_t offset,
    ...
    std::copy_n(bytes_.begin() + static_cast<std::ptrdiff_t>(begin_index),
...
Result<std::shared_ptr<FileReader>>
FileReader::open(const std::filesystem::path &path) {
    ...
    return std::shared_ptr<FileReader>{
        new FileReader{path, static_cast<std::uint64_t>(file_size)}};
}
...
Result<void> FileReader::read_exact_at(std::uint64_t offset,
    ...
    if (*end > size_) {
        ...
        context.raw_offset = offset;
        return std::unexpected{
            make_error(ErrorCode::io_short_read, ErrorCategory::io,
...
Result<void>
FileReader::read_exact_at(std::uint64_t offset,
