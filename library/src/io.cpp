...
CancellationToken::CancellationToken(std::shared_ptr<std::atomic_bool> state) noexcept : state_{std::move(state)} {}

bool CancellationToken::is_cancelled() const noexcept { return state_->load(std::memory_order_acquire); }

Result<void> CancellationToken::check() const {
    if (is_cancelled()) {
        return std::unexpected{
            make_error(ErrorCode::operation_cancelled, ErrorCategory::cancelled, "operation cancelled")};
    }
    return {};
}
...
std::uint64_t MemoryReader::size() const noexcept { return static_cast<std::uint64_t>(bytes_.size()); }

Result<void> MemoryReader::read_exact_at(std::uint64_t offset, std::span<std::byte> destination) const {
    ...
    std::copy_n(bytes_.begin() + static_cast<std::ptrdiff_t>(begin_index), destination.size(), destination.begin());
    return {};
}
...
Result<std::shared_ptr<FileReader>> FileReader::open(const std::filesystem::path &path) {
    ...
    if (error) {
        ...
        return std::unexpected{make_error(ErrorCode::io_open_failed, ErrorCategory::io,
                                          "cannot open input file: " + error.message(), std::move(context))};
    }
...
Result<void> FileReader::read_exact_at(std::uint64_t offset, std::span<std::byte> destination) const {
    const auto end = checked_add(offset, destination.size());
    if (!end) {
        return std::unexpected{end.error()};
    }
