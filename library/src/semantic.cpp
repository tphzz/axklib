...
const ObjectSnapshot *find_object(const ObjectCatalog &catalog,
                                  std::string_view key) {
    const auto found =
...
std::uint64_t
mismatch_cluster_count(std::span<const AllocationMismatchRange> ranges) {
    std::uint64_t result{};
...
bool navigable(const Relationship &row) {
    ...
    return row.quality == RelationshipQuality::known &&
           row.target_key.has_value();
}
...
std::optional<std::string> assignment_detail(const Relationship &row) {
    ...
    if (row.assignment_state != AssignmentState::active ||
        row.receive_channel_display.empty() ||
        row.receive_channel_display == "off" ||
        row.receive_channel_display == "unknown")
        return std::nullopt;
    return std::format("Rch Assign: {}", row.receive_channel_display);
}

ContentNode bank_node(const ObjectSnapshot &item,
                      const Relationship *row = nullptr) {
    ContentNode result{
...
ContentNode group_node(const ObjectSnapshot &item, const ObjectCatalog &catalog,
                       ...
                       const Relationship *parent = nullptr,
    ...
    result.basis =
        parent == nullptr ? "current SBAC slot relationships" : parent->basis;
    if (parent != nullptr)
    ...
    if (with_children) {
        for (const auto *row : graph.children(item.key)) {
            ...
            if (const auto *child = find_object(catalog, *row->target_key);
                child != nullptr &&
                child->object.header.type == ObjectType::sbnk) {
...
bool partition_has_unknown_records(const Container &container,
                                   PartitionIndex index) {
    const auto partition =
        std::ranges::find(container.partitions(), index.value,
                          [](const Partition &p) { return p.index.value; });
    if (partition == container.partitions().end())
        return true;
    return std::ranges::any_of(
        partition->records, [](const IndexRecord &record) {
            return record.sfs_id.value != 0U &&
...
std::vector<VolumeSeed> sfs_volume_seeds(const Container &container) {
    ...
    for (const auto &partition : container.partitions()) {
        ...
        for (const auto &entry : root->directory_entries) {
            ...
            result.emplace_back(partition.index.value, volume->sfs_id.value,
...
static ContentTree build_content_tree_impl(
    ...
    for (const auto &[key, items] : volumes) {
        ...
        const auto &[partition_index, volume_id, partition_name, volume_name] =
            key;
    ...
    for (const auto &[key, items] : volumes) {
        ...
        if (duplicate_volume_names[{partition_name, volume_name}] > 1U &&
            !items.empty() && items.front()->placement &&
            !items.front()->placement->container_directory.empty()) {
            const auto &raw_path =
                items.front()->placement->container_directory;
            const auto separator = raw_path.find_last_of("/\\");
            const auto raw_volume = raw_path.substr(
                separator == std::string::npos ? 0U : separator + 1U);
            if (!raw_volume.empty())
                display_volume_name =
                    std::format("{} ({})", display_volume_name, raw_volume);
        }
        ContentNode volume{
            std::format("volume:{}:{}", partition_index, volume_id), "volume",
        ...
        for (const auto *item : items) {
            if (item->object.header.type == ObjectType::prog) {
                ...
                for (const auto *row : graph.children(item->key)) {
                    ...
                    if (target->object.header.type == ObjectType::sbac) {
                        node.children.push_back(
                            group_node(*target, catalog, graph, row, false));
                ...
                const auto slot = program_slot(*item);
                const bool quiet_default =
                    slot && node.children.empty() && node.notes.empty() &&
                ...
                const bool has_group = std::ranges::any_of(
                    graph.parents(item->key), [](const Relationship *row) {
        ...
        std::ranges::sort(programs, {}, [](const ContentNode &node) {
            return std::tuple{lowercase(node.display_name), node.object_type,
                              node.object_key};
        });
        for (auto &program : programs) {
            ...
            if (end == 3U && parsed.ec == std::errc{} && slot >= 1U &&
                slot <= 128U)
        ...
        const auto sort_nodes = [](std::vector<ContentNode> &nodes) {
            std::ranges::sort(nodes, {}, [](const ContentNode &node) {
                return std::tuple{lowercase(node.display_name),
        ...
        partition_volumes[{partition_index, partition_name}].push_back(
            std::move(volume));
    }
    ...
    for (const auto &[partition_index, partition_name] : partitions) {
        const auto duplicate_index =
            partition_index_counts[partition_index.value] > 1U;
        ContentNode root{
            duplicate_index
        ...
        root.children = std::move(
            partition_volumes[{partition_index.value, partition_name}]);
...
ContentTree build_content_tree(const Container &container,
    ...
    return build_content_tree_impl(
        text::path_to_utf8(container.source_path()), partitions, catalog, graph,
        include_default_programs, true, sfs_volume_seeds(container));
}

ContentTree build_content_tree(const MediaContainer &container,
                               const ObjectCatalog &catalog,
                               const RelationshipGraph &graph,
                               bool include_default_programs) {
    if (const auto *sfs = std::get_if<Container>(&container.storage()))
        return build_content_tree(*sfs, catalog, graph,
                                  include_default_programs);

    auto result =
        build_content_tree(text::path_to_utf8(container.source_path()), catalog,
                           graph, include_default_programs);
    ...
    for (const auto &row : graph.relationships) {
        ...
        if (row.type == "PROG_ASSIGNMENT_TO_SBNK") {
            ...
            if (const auto members = group_banks.find(*row.target_key);
                members != group_banks.end())
                reachable_banks.insert(members->second.begin(),
                                       members->second.end());
        }
    }
    ...
    for (const auto &bank_key : unresolved_banks) {
        ...
        result.issues.push_back(
            {"REL_SBNK_MEMBER_TARGET_MISSING", "warning",
        ...
        if (bank != nullptr) {
            if (bank->placement) {
                affected_volumes.emplace(
                    display_text(bank->placement->partition_name),
                    display_text(bank->placement->volume_name));
            }
            result.issues.push_back(
                {"REL_ACTIVE_PROGRAM_SBNK_MEMBER_TARGET_MISSING", "error",
                 "active Program reaches a sample-bank member without one "
                 "waveform",
    ...
    for (auto &root : result.roots) {
        for (auto &volume : root.children) {
            if (affected_volumes.contains(
                    {root.display_name, volume.display_name}))
    ...
    const auto name = container.kind() == MediaKind::fat12_floppy
                          ? "FAT root"
                          : "Standalone object";
...
ContentTree build_content_tree(std::string source_path,
                               const ObjectCatalog &catalog,
                               const RelationshipGraph &graph,
                               bool include_default_programs) {
    ...
    auto result =
        build_content_tree_impl(std::move(source_path), partitions, catalog,
                                graph, include_default_programs, false);
...
WaveformOrphanReport analyze_waveform_orphans(const Container &container,
    ...
    for (const auto &item : catalog.objects) {
        ...
        for (const auto *relation : graph.parents(item.key)) {
            if ((relation->type == "SBNK_LEFT_MEMBER_TO_SMPL" ||
                ...
                if (const auto *bank =
                        find_object(catalog, relation->source_key);
                    bank != nullptr) {
                    row.referencing_sample_banks.push_back(
                        bank->placement
                            ? std::format("{}/{}", bank->placement->volume_name,
        ...
        if (!row.referencing_sample_banks.empty()) {
            ...
            row.referencing_sample_banks.erase(
                std::unique(row.referencing_sample_banks.begin(),
            ...
            if (!item.placement)
                blockers.emplace_back(
                    "waveform has no exact SMPL directory placement");
            if (partition_has_unknown_records(container, item.partition)) {
                blockers.emplace_back(
                    "partition contains an unresolved allocated record");
            }
            for (const auto &issue : catalog.issues) {
            ...
            for (const auto &relation : graph.relationships) {
                if (relation.scope_key == item.scope_key &&
                    ...
                    relation.quality != RelationshipQuality::known) {
                    blockers.push_back(
                        std::format("sample-bank member is unresolved: {}",
                                    relation.source_key));
                }
            }
            if (blockers.empty()) {
    ...
    std::ranges::sort(result.rows, {}, [](const WaveformOrphanRow &row) {
        return std::tuple{row.partition.value, row.volume_name,
                          row.waveform_name, row.sfs_id.value};
    });
    return result;
}
...
ValidationReport validate_semantics(const Container &container,
                                    const ObjectCatalog &catalog,
                                    const RelationshipGraph &graph) {
    ...
    for (const auto &item : catalog.objects) {
        if (item.placement) {
            ...
            if (item.placement->category_name !=
                object_type_name(item.object.header.type)) {
                result.issues.push_back({
                    "VOL_OBJECT_CATEGORY_MISMATCH",
                    ValidationSeverity::error,
                    std::format("{} object '{}' is stored in the {} category",
                                object_type_name(item.object.header.type),
                                item.object.header.name,
    ...
    for (const auto &relation : graph.relationships) {
        ...
        if ((relation.type == "SBNK_LEFT_MEMBER_TO_SMPL" ||
            ...
            result.issues.push_back({
                ...
                source == nullptr
                    ? "sample-bank member does not resolve to one waveform"
                    : std::format("sample bank '{}' has a member that does not "
                                  "resolve to one waveform",
    ...
    for (const auto &comparison : graph.bitmap_comparisons) {
        ...
        result.issues.push_back({
            ...
            "sample-bank program bitmap differs from decoded direct Program "
            "assignments",
            source == nullptr ? "" : sampler_path(*source),
            comparison.sbnk_key,
        });
    ...
    for (const auto &partition : container.partitions()) {
        const auto stored_without_record = mismatch_cluster_count(
            partition.allocation.stored_not_reconstructed);
        const auto record_marked_free = mismatch_cluster_count(
        ...
        if (partition.allocation.invalid_extent_record_count != 0U ||
            ...
            stored_without_record != 0U || record_marked_free != 0U) {
            auto message =
