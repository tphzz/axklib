...
Result<std::uint64_t> align_up(std::uint64_t value, std::uint64_t alignment) {
  if (alignment == 0) {
    return std::unexpected{make_error(ErrorCode::invalid_argument, ErrorCategory::container,
...
Result<std::string> ByteReader::ascii_field(std::size_t offset, std::size_t count,
...
Result<void> ByteWriter::write_ascii_field(std::size_t offset, std::size_t count,
                                           std::string_view value, std::byte pad) {
  if (value.size() > count) {
