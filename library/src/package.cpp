...
Error root_error(const PackageRootSelector &selector, std::string message) {
  ...
  return make_error(ErrorCode::object_missing, ErrorCategory::manifest, std::move(message),
                    std::move(context));
}
...
Result<std::vector<SelectedRoot>> select_roots(const ObjectCatalog &catalog,
  ...
  if (selectors.empty())
    return std::unexpected{package_error("at least one package root selector is required",
    ...
    if (selector.kind == PackageRootKind::volume) {
      ...
      for (const auto *object : root.seeds) {
        if (object->placement->partition.value != placement.partition.value ||
          ...
          return std::unexpected{
              root_error(selector, "package volume selector matches more than one volume")};
        }
      }
    } else {
      ...
      for (const auto &object : catalog.objects) {
        if (object.object.header.type != expected ||
            object.object.header.name != selector.object_name || !matches_scope(object, selector) ||
            (selector.object_key && object.key != *selector.object_key)) {
          continue;
        }
        root.seeds.push_back(&object);
      }
      ...
      if (root.seeds.size() != 1U)
        return std::unexpected{
            root_error(selector, "package object selector is ambiguous; provide an object key")};
...
bool closure_relationship(std::string_view role) {
  return role == "SBNK_LEFT_MEMBER_TO_SMPL" || role == "SBNK_RIGHT_MEMBER_TO_SMPL" ||
         role == "SBAC_SLOT_TO_SBNK" || role == "PROG_ASSIGNMENT_TO_SBAC" ||
...
Result<std::vector<const Relationship *>> required_relationships(const ObjectSnapshot &object,
  ...
  const auto require_one = [&](std::string_view role,
    ...
    if (matches.size() != 1U || matches.front()->quality != RelationshipQuality::known ||
      ...
      return std::unexpected{
          make_error(matches.size() > 1U ? ErrorCode::relationship_ambiguous
                                         : ErrorCode::relationship_unresolved,
                     ErrorCategory::relationship,
                     std::format("package closure requires one known {} relationship", role),
                     std::move(context))};
    }
  ...
  if (const auto *bank = std::get_if<CurrentSbnk>(&object.object.payload)) {
    ...
    if (candidates.size() != static_cast<std::size_t>(active_slots)) {
      return std::unexpected{make_error(
          ...
          "package closure does not contain one relationship for every active SBAC slot")};
    }
    for (const auto *row : candidates) {
      if (row->type != "SBAC_SLOT_TO_SBNK" || row->quality != RelationshipQuality::known ||
          !row->target_key) {
        return std::unexpected{
            make_error(ErrorCode::relationship_unresolved, ErrorCategory::relationship,
                       "package closure requires known SBAC member relationships")};
      }
      result.push_back(row);
    }
      ...
      if (role.empty())
        return std::unexpected{
            make_error(ErrorCode::unsupported_profile, ErrorCategory::unsupported,
                       "active Program assignment has an unsupported target kind")};
...
auto canonical_node_order(const ProvisionalNode &node) {
  ...
  return std::tuple{node.identity_digest,    placement.group_name, placement.volume_name,
                    placement.category_name, placement.entry_name, node.payload_digest,
                    node.snapshot->key};
}
...
std::string edge_id(std::string_view source, std::string_view target, std::string_view role,
...
std::vector<const PackageRelationship *> package_children(const PortablePackage &package,
...
Result<void> validate_package_closure(const PortablePackage &package) {
  ...
  for (const auto &node : package.nodes) {
    ...
    if (!decoded)
      return std::unexpected{
          package_error("package object cannot be decoded for closure validation")};
    const auto require_edge =
        [&](std::string_view role, std::string_view target_name, ObjectType target_type,
            std::optional<std::uint32_t> ordinal = std::nullopt) -> Result<void> {
      auto edges = package_children(package, node.node_id, role);
      if (ordinal)
        std::erase_if(edges,
                      [&](const PackageRelationship *edge) { return edge->ordinal != *ordinal; });
      if (edges.size() != 1U)
        return std::unexpected{package_error(
            std::format("package closure requires one {} edge from {}", role, node.name))};
      ...
      if (target == nullptr || target->object_type != object_type_name(target_type) ||
        ...
        return std::unexpected{
            package_error("package relationship target does not match the raw object reference")};
      }
      return {};
    };
    ...
    if (const auto *bank = std::get_if<CurrentSbnk>(&decoded->payload)) {
      if (!bank->left.sample_name.empty()) {
        if (auto valid =
                require_edge("SBNK_LEFT_MEMBER_TO_SMPL", bank->left.sample_name, ObjectType::smpl);
            !valid) {
          return valid;
        }
      }
      if (bank->right && !bank->right->sample_name.empty()) {
        if (auto valid = require_edge("SBNK_RIGHT_MEMBER_TO_SMPL", bank->right->sample_name,
                                      ObjectType::smpl);
            !valid) {
          return valid;
        }
      ...
      for (std::size_t index = 0; index < program->assignments.size(); ++index) {
        ...
        if (role.empty())
          return std::unexpected{
              package_error("package Program contains an unsupported active assignment")};
        if (auto valid =
                require_edge(role, assignment.name, type, static_cast<std::uint32_t>(index));
            !valid) {
          return valid;
        }
      }
...
Result<void> validate_relocation_bindings(const PortablePackage &package) {
  ...
  for (std::size_t node_index = 0; node_index < package.nodes.size(); ++node_index) {
    for (std::size_t relocation_index = 0;
         relocation_index < package.nodes[node_index].relocations.size(); ++relocation_index) {
...
parse_manifest(const Json &manifest,
  ...
  if (!exact_keys(manifest, {"objects", "package_id", "package_kind", "payloads", "provenance",
                             "relationships", "roots", "schema_version"})) {
    return std::unexpected{
        package_error("package manifest has unknown or missing top-level fields")};
  }
  ...
  for (const auto &payload : manifest.at("payloads")) {
    ...
    if (!lowercase_sha256(digest) || !declared_payloads.emplace(path, digest).second ||
        ...
        (verify_payloads && (found->second.bytes == nullptr ||
                             package_internal::hex_digest(
                                 package_internal::sha256(*found->second.bytes)) != digest))) {
      return std::unexpected{
          package_error("package payload declaration does not match the archive")};
    }
  }
  ...
  for (const auto &root : manifest.at("roots")) {
    ...
    result.roots.push_back({*kind, root.at("display_name").get<std::string>(),
  ...
  for (const auto &object : manifest.at("objects")) {
    if (!exact_keys(object, {"audio_sha256", "name", "node_id", "normalized_sha256",
                             "object_format", "object_type", "payload_path", "payload_sha256",
                             "placement_hint", "relocations", "semantic_sha256"}) ||
        !exact_keys(object.at("placement_hint"),
    ...
    node.placement_hint = {
        hint.at("group_name").get<std::string>(), hint.at("volume_name").get<std::string>(),
    ...
    if (!node_ids.emplace(node.node_id).second || !type || found == archive_entries.end() ||
      ...
      return std::unexpected{
          package_error("package object identity or payload reference is invalid")};
    }
    if (verify_payloads) {
      ...
      if (package_internal::hex_digest(package_internal::sha256(node.raw_payload)) !=
          node.payload_sha256) {
      ...
      if (!decoded || decoded->header.type != *type || decoded->header.name != node.name ||
        ...
        return std::unexpected{
            package_error("package object payload does not match its declaration")};
      }
      const auto profile = package_internal::build_relocation_profile(*decoded, node.raw_payload);
      if (!profile || package_internal::hex_digest(package_internal::sha256(
  ...
  for (const auto &relationship : manifest.at("relationships")) {
    if (!exact_keys(relationship,
                    {"edge_id", "ordinal", "role", "source_node_id", "target_node_id"})) {
      return std::unexpected{package_error("package relationship has unknown or missing fields")};
    }
    PackageRelationship edge{relationship.at("edge_id").get<std::string>(),
    ...
    if (!closure_relationship(edge.role) || !node_ids.contains(edge.source_node_id) ||
        ...
        edge.edge_id !=
            edge_id(edge.source_node_id, edge.target_node_id, edge.role, edge.ordinal)) {
  ...
  for (const auto &root : result.roots) {
    if (root.node_ids.empty() || std::ranges::any_of(root.node_ids, [&](const std::string &id) {
...
bool recognized_extension(std::string_view extension) {
  constexpr std::array extensions{".axkvol",  ".axkprg", ".axksbac", ".axksbnk",
                                  ".axksmpl", ".axkseq", ".axkpkg"};
...
Result<std::filesystem::path> resolve_output_path(const std::filesystem::path &requested,
                                                  PackageKind kind) {
    ...
    if (extension != required) {
      ...
      return std::unexpected{
          make_error(ErrorCode::invalid_argument, ErrorCategory::io,
                     std::format("package output has {} extension {}; {} is required", qualifier,
                                 extension, required))};
    }
    return requested;
  }
...
Result<std::filesystem::path> reserve_unique_temporary(const std::filesystem::path &output) {
  ...
  for (std::size_t attempt = 0; attempt < 64U; ++attempt) {
    ...
    const auto descriptor =
        ::open(temporary->c_str(), O_CREAT | O_EXCL | O_WRONLY | O_CLOEXEC, 0600);
    if (descriptor >= 0) {
      ::close(descriptor);
      return *temporary;
    }
...
Result<PortablePackage>
parse_package_manifest(std::span<const std::byte> manifest_bytes,
    ...
    if (!extension.empty() && recognized_extension(extension) &&
      ...
      package->issues.push_back(
          {"PACKAGE_EXTENSION_MISMATCH",
           std::format("filename extension {} disagrees with manifest package kind {}", extension,
                       package_kind_name(package->kind)),
           false});
    }
    return package;
...
Result<PackageBuild> build_portable_package(const MediaContainer &source,
  ...
  for (const auto &key : included_keys) {
    ...
    auto profile =
        package_internal::build_relocation_profile(snapshot->object, snapshot->raw_payload);
    if (!profile)
      return std::unexpected{profile.error()};
    const auto payload_digest =
        package_internal::hex_digest(package_internal::sha256(snapshot->raw_payload));
...
Result<PortablePackage> open_portable_package(std::span<const std::byte> archive,
...
Result<PackagePublication> publish_portable_package(const PackageBuild &build,
  ...
  if (const auto available = preflight_output(*resolved, overwrite); !available)
    return std::unexpected{available.error()};
  if (const auto published = publish_temporary_package(*temporary, *resolved, overwrite);
      !published) {
