...
bool package_internal::portable_inactive_program_relationship(
...
Error package_error(std::string message,
                    ErrorCode code = ErrorCode::manifest_invalid) {
...
Error root_error(const PackageRootSelector &selector, std::string message) {
    ...
    return make_error(ErrorCode::object_missing, ErrorCategory::manifest,
...
std::string digest_text(std::string_view value) {
    return package_internal::hex_digest(
        package_internal::sha256(string_bytes(value)));
}
...
bool matches_scope(const ObjectSnapshot &object,
                   const PackageRootSelector &selector) {
    if (!object.placement)
        return false;
    if (selector.partition_index &&
        object.placement->partition.value != *selector.partition_index)
        return false;
    if (!selector.group_name.empty() &&
        object.placement->partition_name != selector.group_name)
        return false;
    return selector.volume_name.empty() ||
           object.placement->volume_name == selector.volume_name;
}

Result<std::vector<SelectedRoot>>
select_roots(const ObjectCatalog &catalog,
             std::span<const PackageRootSelector> selectors) {
    if (selectors.empty())
        return std::unexpected{
            package_error("at least one package root selector is required",
    ...
    for (const auto &selector : selectors) {
        ...
        if (selector.kind == PackageRootKind::volume) {
            ...
            if (root.seeds.empty())
                return std::unexpected{root_error(
                    selector, "package volume selector matches no objects")};
            const auto &placement = *root.seeds.front()->placement;
            root.display_name = placement.volume_name;
            for (const auto *object : root.seeds) {
                if (object->placement->partition.value !=
                        placement.partition.value ||
                    object->placement->volume_directory.value !=
                        placement.volume_directory.value ||
                    object->placement->container_directory !=
                        placement.container_directory) {
                    return std::unexpected{
                        root_error(selector, "package volume selector matches "
            ...
            for (const auto &object : catalog.objects) {
                if (object.object.header.type != expected ||
                    object.object.header.name != selector.object_name ||
                    !matches_scope(object, selector) ||
                    (selector.object_key &&
                     object.key != *selector.object_key)) {
                    continue;
                }
                root.seeds.push_back(&object);
            }
            if (root.seeds.empty())
                return std::unexpected{root_error(
                    selector, "package object selector matches no object")};
            if (root.seeds.size() != 1U)
                return std::unexpected{
                    root_error(selector, "package object selector is "
...
bool closure_relationship(std::string_view role) {
    return role == "SBNK_LEFT_MEMBER_TO_SMPL" ||
           role == "SBNK_RIGHT_MEMBER_TO_SMPL" || role == "SBAC_SLOT_TO_SBNK" ||
           role == "PROG_ASSIGNMENT_TO_SBAC" ||
           role == "PROG_ASSIGNMENT_TO_SBNK";
}

bool portable_program_assignment(AssignmentState state) {
    return state == AssignmentState::active ||
           state == AssignmentState::source_load ||
           state == AssignmentState::visible_off;
}

Result<std::vector<const Relationship *>>
required_relationships(const ObjectSnapshot &object,
    ...
    const auto require_one =
        ...
        if (matches.size() != 1U ||
            matches.front()->quality != RelationshipQuality::known ||
            ...
            return std::unexpected{make_error(
                matches.size() > 1U ? ErrorCode::relationship_ambiguous
                                    : ErrorCode::relationship_unresolved,
                ErrorCategory::relationship,
                std::format(
                    "package closure requires one known {} relationship", role),
                std::move(context))};
        }
    ...
    if (const auto *bank = std::get_if<CurrentSbnk>(&object.object.payload)) {
        ...
        if (bank->right && !bank->right->sample_name.empty()) {
            ...
            result.push_back(*row);
                   ...
                   std::get_if<CurrentSbac>(&object.object.payload)) {
        const auto active_slots =
            std::ranges::count_if(group->slots, [](const SbacSlot &slot) {
                return !slot.name.empty();
            });
        if (candidates.size() != static_cast<std::size_t>(active_slots)) {
            return std::unexpected{make_error(
                ErrorCode::relationship_unresolved, ErrorCategory::relationship,
        ...
        for (const auto *row : candidates) {
            if (row->type != "SBAC_SLOT_TO_SBNK" ||
                ...
                !row->target_key) {
                return std::unexpected{
                    make_error(ErrorCode::relationship_unresolved,
                               ErrorCategory::relationship,
    ...
    } else if (const auto *program =
                   std::get_if<CurrentProg>(&object.object.payload)) {
        for (std::size_t index = 0; index < program->assignments.size();
            ...
            if (assignment.name.empty() ||
                std::to_integer<std::uint8_t>(assignment.raw_row[0x28U]) !=
                    0xffU)
                continue;
            const auto role =
                assignment.kind == 0x11U   ? "PROG_ASSIGNMENT_TO_SBAC"
...
PackagePlacementHint placement_hint(const ObjectSnapshot &object) {
    if (!object.placement)
        return {};
    return {object.placement->partition_name, object.placement->volume_name,
            object.placement->category_name, object.placement->entry_name};
}

std::string identity_digest(const ObjectSnapshot &object,
...
Result<std::optional<WaveformDigests>>
waveform_digests(const DecodedObject &decoded,
    ...
    const auto audio =
        package_internal::hex_digest(package_internal::sha256(waveform->pcm));
    const auto optional_u64 =
        [](const std::optional<std::uint64_t> &value) -> Json {
    ...
    const Json semantic{
        ...
        {"loop_end_frame_exclusive",
         optional_u64(smpl->loop_end_frame_exclusive)},
        {"loop_end_frame_inclusive",
         optional_u64(smpl->loop_end_frame_inclusive)},
    ...
    return std::optional{
        WaveformDigests{digest_text(semantic.dump() + '\n'), audio}};
}

auto canonical_node_order(const ProvisionalNode &node) {
    const auto placement = placement_hint(*node.snapshot);
    return std::tuple{node.identity_digest,  placement.group_name,
                      placement.volume_name, placement.category_name,
                      placement.entry_name,  node.payload_digest,
                      node.snapshot->key};
}

std::map<std::string, std::string, std::less<>>
assign_node_ids(std::vector<ProvisionalNode> &nodes) {
    ...
    while (begin < nodes.size()) {
        ...
        while (end < nodes.size() &&
               nodes[end].identity_digest == nodes[begin].identity_digest)
            ++end;
        for (auto index = begin; index < end; ++index) {
            auto digest = nodes[index].identity_digest;
            if (end - begin > 1U)
                digest = digest_text(
                    std::format("{}#{}", digest, index - begin + 1U));
...
std::string edge_id(std::string_view source, std::string_view target,
                    std::string_view role, std::uint32_t ordinal) {
    std::string identity;
...
Json relocation_json(const PackageRelocation &relocation) {
    return Json{{"edge_ids", relocation.edge_ids},
                {"expected_hex", relocation.expected_hex},
                {"mask_hex", relocation.mask_hex},
                {"offset", relocation.offset},
                {"role", relocation.role},
...
Json manifest_json(const PortablePackage &package, bool include_id) {
    ...
    std::map<std::string, std::pair<std::string, std::uint64_t>, std::less<>>
        payload_map;
    for (const auto &node : package.nodes)
        payload_map.emplace(
            node.payload_path,
            std::pair{node.payload_sha256,
                      static_cast<std::uint64_t>(node.raw_payload.size())});
    Json payloads = Json::array();
    for (const auto &[path, digest_and_size] : payload_map) {
        payloads.push_back(
            {{"media_type", "application/vnd.axklib.yamaha-object"},
             {"path", path},
...
Result<PackageRelocation> parse_relocation(const Json &value) {
    if (!exact_keys(value, {"edge_ids", "expected_hex", "mask_hex", "offset",
                            "role", "width"}))
        return std::unexpected{
            package_error("package relocation has unknown or missing fields")};
        ...
        if (actual_ids.size() == 1U) {
            ...
            for (std::size_t index = 0; index < actual_ids.size(); ++index)
                expected_ids.push_back(
                    "n-" +
                    digest_text(std::format("{}#{}", identity, index + 1U)));
        }
        std::ranges::sort(actual_ids);
        std::ranges::sort(expected_ids);
        if (actual_ids != expected_ids)
            return std::unexpected{
                package_error("package object node ID is not deterministic")};
    }
    return {};
}
...
std::vector<const PackageRelationship *>
package_children(const PortablePackage &package, std::string_view source,
                 std::string_view role = {}) {
    std::vector<const PackageRelationship *> result;
    for (const auto &edge : package.relationships) {
        if (edge.source_node_id == source &&
            (role.empty() || edge.role == role))
...
Result<void> validate_manifest_graph(const PortablePackage &package) {
    for (const auto &root : package.roots) {
        if (root.kind != PackageRootKind::volume && root.node_ids.size() != 1U)
            return std::unexpected{package_error(
                "single-object package root must contain one node")};
        if (root.kind == PackageRootKind::volume)
            continue;
        const auto *node = node_by_id(package, root.node_ids.front());
        if (node == nullptr ||
            node->object_type != object_type_name(root_object_type(root.kind)))
            return std::unexpected{package_error(
                "package root kind does not match its object node")};
    }
    ...
    if (reachable.size() != package.nodes.size())
        return std::unexpected{package_error(
            "package contains an object unreachable from every root")};
    return {};
}
...
Result<void> validate_package_closure(const PortablePackage &package) {
    ...
    for (const auto &node : package.nodes) {
        ...
        if (!decoded)
            return std::unexpected{package_error(
                "package object cannot be decoded for closure validation")};
        const auto require_edge = [&](std::string_view role,
                                      std::string_view target_name,
                                      ObjectType target_type,
                                      std::optional<std::uint32_t> ordinal =
                                          std::nullopt) -> Result<void> {
            auto edges = package_children(package, node.node_id, role);
            if (ordinal)
                std::erase_if(edges, [&](const PackageRelationship *edge) {
                    return edge->ordinal != *ordinal;
                });
            if (edges.size() != 1U)
                return std::unexpected{package_error(
                    std::format("package closure requires one {} edge from {}",
                                role, node.name))};
            const auto *target =
                node_by_id(package, edges.front()->target_node_id);
            if (target == nullptr ||
                target->object_type != object_type_name(target_type) ||
                target->name != target_name) {
                return std::unexpected{
                    package_error("package relationship target does not match "
        ...
        if (const auto *bank = std::get_if<CurrentSbnk>(&decoded->payload)) {
            if (!bank->left.sample_name.empty()) {
                if (auto valid =
                        require_edge("SBNK_LEFT_MEMBER_TO_SMPL",
                                     bank->left.sample_name, ObjectType::smpl);
                    !valid) {
                    return valid;
                }
            }
            if (bank->right && !bank->right->sample_name.empty()) {
                if (auto valid = require_edge("SBNK_RIGHT_MEMBER_TO_SMPL",
                                              bank->right->sample_name,
                                              ObjectType::smpl);
                    !valid) {
                    return valid;
                }
                       ...
                       std::get_if<CurrentSbac>(&decoded->payload)) {
            auto edges =
                package_children(package, node.node_id, "SBAC_SLOT_TO_SBNK");
            ...
            if (edges.size() != static_cast<std::size_t>(active_slots))
                return std::unexpected{
                    package_error("package SBAC closure does not match its "
                                  "active member slots")};
            ...
            for (const auto *edge : edges) {
                ...
                const auto expected_name =
                    valid_ordinal
                        ? std::string_view{group->slots[edge->ordinal].name}
                        : std::string_view{};
                if (!ordinals.emplace(edge->ordinal).second ||
                    expected_name.empty() || target == nullptr ||
                    target->object_type != "SBNK" ||
                    target->name != expected_name) {
                    return std::unexpected{package_error(std::format(
                        "package SBAC slot {} requires SBNK '{}', but the edge "
                        "targets '{}'",
                        edge->ordinal, expected_name,
                        target == nullptr ? "<missing>" : target->name))};
                }
            }
        } else if (const auto *program =
                       std::get_if<CurrentProg>(&decoded->payload)) {
            for (std::size_t index = 0; index < program->assignments.size();
                 ++index) {
                const auto &assignment = program->assignments[index];
                if (assignment.name.empty() ||
                ...
                if (role.empty())
                    return std::unexpected{
                        package_error("package Program contains an unsupported "
                                      "active assignment")};
                if (auto valid =
                        require_edge(role, assignment.name, type,
...
Result<void> validate_relocation_bindings(const PortablePackage &package) {
    ...
    for (std::size_t node_index = 0; node_index < package.nodes.size();
         ++node_index) {
        for (std::size_t relocation_index = 0;
             relocation_index < package.nodes[node_index].relocations.size();
             ++relocation_index) {
            if (package.nodes[node_index]
                    .relocations[relocation_index]
                    .edge_ids != expected.nodes[node_index]
                                     .relocations[relocation_index]
                                     .edge_ids) {
                return std::unexpected{package_error(
                    "package relocation edge binding is invalid")};
            }
        }
    }
    return {};
...
parse_manifest(const Json &manifest,
               const std::map<std::string, ManifestArchiveEntry, std::less<>>
                   &archive_entries,
               bool verify_payloads) {
    if (!exact_keys(manifest, {"objects", "package_id", "package_kind",
                               "payloads", "provenance", "relationships",
                               "roots", "schema_version"})) {
        return std::unexpected{package_error(
            "package manifest has unknown or missing top-level fields")};
    }
    ...
    const auto parsed_kind =
        parse_package_kind(manifest.at("package_kind").get<std::string>());
    if (!parsed_kind)
    ...
    if (!exact_keys(manifest.at("provenance"), {"source_media_kind"}))
        return std::unexpected{
            package_error("package provenance has unknown or missing fields")};
    result.source_media_kind =
        manifest.at("provenance").at("source_media_kind").get<std::string>();

    std::map<std::string, std::string, std::less<>> declared_payloads;
    for (const auto &payload : manifest.at("payloads")) {
        if (!exact_keys(payload,
                        {"media_type", "path", "sha256", "size_bytes"}) ||
            payload.at("media_type") !=
                "application/vnd.axklib.yamaha-object") {
            return std::unexpected{
                package_error("package payload declaration is invalid")};
        }
        ...
        if (!lowercase_sha256(digest) ||
            !declared_payloads.emplace(path, digest).second ||
            found == archive_entries.end() || found->second.size != size ||
            (verify_payloads &&
             (found->second.bytes == nullptr ||
              package_internal::hex_digest(
                  package_internal::sha256(*found->second.bytes)) != digest))) {
            return std::unexpected{package_error(
                "package payload declaration does not match the archive")};
        }
    }
    if (declared_payloads.size() + 1U != archive_entries.size())
        return std::unexpected{
            package_error("package archive contains an undeclared entry")};

    for (const auto &root : manifest.at("roots")) {
        if (!exact_keys(root, {"display_name", "kind", "node_ids"}))
            return std::unexpected{
                package_error("package root has unknown or missing fields")};
        const auto kind = parse_root_kind(root.at("kind").get<std::string>());
        if (!kind)
            return std::unexpected{
                package_error("package root kind is invalid")};
        result.roots.push_back(
            {*kind, root.at("display_name").get<std::string>(),
             root.at("node_ids").get<std::vector<std::string>>()});
    }
    if (result.roots.empty() || derive_kind(result.roots) != result.kind)
        return std::unexpected{
            package_error("declared package kind does not match its roots")};

    std::set<std::string, std::less<>> node_ids;
    for (const auto &object : manifest.at("objects")) {
        if (!exact_keys(object,
                        {"audio_sha256", "name", "node_id", "normalized_sha256",
                         "object_format", "object_type", "payload_path",
                         "payload_sha256", "placement_hint", "relocations",
                         "semantic_sha256"}) ||
            !exact_keys(
                object.at("placement_hint"),
                {"category_name", "entry_name", "group_name", "volume_name"})) {
            return std::unexpected{
                package_error("package object has unknown or missing fields")};
        }
        PackageNode node;
        node.name = object.at("name").get<std::string>();
        node.node_id = object.at("node_id").get<std::string>();
        node.normalized_sha256 =
            object.at("normalized_sha256").get<std::string>();
        if (!object.at("semantic_sha256").is_null())
            node.semantic_sha256 =
                object.at("semantic_sha256").get<std::string>();
        ...
        node.placement_hint = {hint.at("group_name").get<std::string>(),
                               hint.at("volume_name").get<std::string>(),
                               hint.at("category_name").get<std::string>(),
                               hint.at("entry_name").get<std::string>()};
        ...
        if (!node_ids.emplace(node.node_id).second || !type ||
            found == archive_entries.end() ||
            declared == declared_payloads.end() ||
            node.payload_sha256 != declared->second ||
            !lowercase_sha256(node.normalized_sha256) ||
            (node.semantic_sha256 &&
             !lowercase_sha256(*node.semantic_sha256)) ||
            (node.audio_sha256 && !lowercase_sha256(*node.audio_sha256))) {
            return std::unexpected{package_error(
                "package object identity or payload reference is invalid")};
        }
        if (verify_payloads) {
            if (found->second.bytes == nullptr)
                return std::unexpected{
                    package_error("package payload bytes are unavailable")};
            node.raw_payload = *found->second.bytes;
            if (package_internal::hex_digest(package_internal::sha256(
                    node.raw_payload)) != node.payload_sha256) {
                return std::unexpected{
                    package_error("package object payload digest mismatch")};
            }
            const auto decoded = decode_object(node.raw_payload);
            if (!decoded || decoded->header.type != *type ||
                decoded->header.name != node.name ||
                object_format_name(decoded->format) != node.object_format) {
                return std::unexpected{package_error(
                    "package object payload does not match its declaration")};
            }
            const auto profile = package_internal::build_relocation_profile(
                *decoded, node.raw_payload);
            if (!profile ||
                package_internal::hex_digest(package_internal::sha256(
                    profile->normalized_payload)) != node.normalized_sha256) {
                return std::unexpected{package_error(
                    "package object normalized identity is invalid")};
            }
            auto digests = waveform_digests(*decoded, node.raw_payload,
                                            node.normalized_sha256);
            if (!digests)
                return std::unexpected{digests.error()};
            const auto expected_semantic =
                *digests ? std::optional{(*digests)->semantic} : std::nullopt;
            const auto expected_audio =
                *digests ? std::optional{(*digests)->audio} : std::nullopt;
            if (node.semantic_sha256 != expected_semantic ||
                node.audio_sha256 != expected_audio) {
                return std::unexpected{package_error(
                    "package object semantic identity is invalid")};
            }
            if (profile->relocations.size() != node.relocations.size())
                return std::unexpected{package_error(
                    "package object relocation registry is incomplete")};
            for (std::size_t index = 0; index < node.relocations.size();
                 ++index) {
                const auto &expected = profile->relocations[index];
                const auto &actual = node.relocations[index];
                if (expected.offset != actual.offset ||
                    expected.width != actual.width ||
                    expected.mask_hex != actual.mask_hex ||
                    expected.role != actual.role ||
                    expected.expected_hex != actual.expected_hex) {
                    return std::unexpected{package_error(
                        "package object relocation descriptor is invalid")};
                }
            }
        }
    ...
    if (result.nodes.empty())
        return std::unexpected{
            package_error("package contains no object nodes")};

    std::set<std::string, std::less<>> edge_ids;
    for (const auto &relationship : manifest.at("relationships")) {
        if (!exact_keys(relationship, {"edge_id", "ordinal", "role",
                                       "source_node_id", "target_node_id"})) {
            return std::unexpected{package_error(
                "package relationship has unknown or missing fields")};
        }
        PackageRelationship edge{
            relationship.at("edge_id").get<std::string>(),
            relationship.at("source_node_id").get<std::string>(),
            relationship.at("target_node_id").get<std::string>(),
            relationship.at("role").get<std::string>(),
            relationship.at("ordinal").get<std::uint32_t>()};
        if (!closure_relationship(edge.role) ||
            !node_ids.contains(edge.source_node_id) ||
            !node_ids.contains(edge.target_node_id) ||
            !edge_ids.emplace(edge.edge_id).second ||
            edge.edge_id != edge_id(edge.source_node_id, edge.target_node_id,
                                    edge.role, edge.ordinal)) {
            return std::unexpected{
                package_error("package relationship identity is invalid")};
        }
        result.relationships.push_back(std::move(edge));
    }
    for (const auto &root : result.roots) {
        if (root.node_ids.empty() ||
            std::ranges::any_of(root.node_ids, [&](const std::string &id) {
                return !node_ids.contains(id);
            })) {
            return std::unexpected{package_error(
                "package root references an undeclared object node")};
        }
    }
    for (const auto &node : result.nodes) {
        for (const auto &relocation : node.relocations) {
            if (std::ranges::any_of(relocation.edge_ids,
                                    [&](const std::string &id) {
                                        return !edge_ids.contains(id);
                                    })) {
                return std::unexpected{package_error(
                    "package relocation references an undeclared edge")};
            }
        }
    }
    if (!std::ranges::is_sorted(result.nodes, {}, &PackageNode::node_id) ||
        !std::ranges::is_sorted(result.relationships, {},
                                &PackageRelationship::edge_id)) {
        return std::unexpected{package_error(
            "package object or relationship order is not canonical")};
    }
...
bool recognized_extension(std::string_view extension) {
    constexpr std::array extensions{".axkvol",  ".axkprg",  ".axksbac",
                                    ".axksbnk", ".axksmpl", ".axkseq",
                                    ".axkpkg"};
...
resolve_output_path(const std::filesystem::path &requested, PackageKind kind) {
    const auto path_text = text::path_to_utf8(requested);
    if (!text::is_valid_utf8(path_text)) {
        return std::unexpected{
            make_error(ErrorCode::invalid_argument, ErrorCategory::io,
                       "package output path is not valid UTF-8")};
    }
    if (requested.empty() || requested.filename().empty() ||
        requested.filename() == "." || requested.filename() == "..") {
        return std::unexpected{
            make_error(ErrorCode::invalid_argument, ErrorCategory::io,
                       "package output path must name a file")};
    }

    const auto required = required_package_extension(kind);
    const auto extension =
        lower_extension(text::path_to_utf8(requested.filename()));
    if (!extension.empty()) {
        if (extension != required) {
            const auto qualifier = recognized_extension(extension)
                                       ? "recognized package"
                                       : "unrelated";
            return std::unexpected{make_error(
                ErrorCode::invalid_argument, ErrorCategory::io,
                std::format(
                    "package output has {} extension {}; {} is required",
                    qualifier, extension, required))};
        }
        return requested;
    }
...
Result<void> preflight_output(const std::filesystem::path &path,
                              bool overwrite) {
    ...
    if (error) {
        return std::unexpected{
            make_error(ErrorCode::io_open_failed, ErrorCategory::io,
                       "could not inspect package output path")};
    }
    if (!overwrite && exists) {
        ErrorContext context;
...
class TemporaryPackageCleanup {
  public:
    explicit TemporaryPackageCleanup(std::filesystem::path path)
        : path_{std::move(path)} {}
    ...
    TemporaryPackageCleanup &
    operator=(const TemporaryPackageCleanup &) = delete;
...
Result<std::filesystem::path>
reserve_unique_temporary(const std::filesystem::path &output) {
    std::random_device random;
    for (std::size_t attempt = 0; attempt < 64U; ++attempt) {
        auto temporary = text::temporary_sibling(
            output, std::format(".package.{:08x}.tmp", random()));
        if (!temporary)
        ...
        const auto descriptor = ::open(
            temporary->c_str(), O_CREAT | O_EXCL | O_WRONLY | O_CLOEXEC, 0600);
        if (descriptor >= 0) {
            ::close(descriptor);
            return *temporary;
        }
    ...
    if (!flushed) {
        return std::unexpected{
            make_error(ErrorCode::io_read_failed, ErrorCategory::io,
                       "could not durably flush temporary package")};
    }
    return {};
}

Result<void> publish_temporary_package(const std::filesystem::path &temporary,
...
read_package_file(const std::filesystem::path &path,
    ...
    if ((*reader)->size() > maximum_package_file_bytes ||
        (*reader)->size() > static_cast<std::uint64_t>(
                                std::numeric_limits<std::size_t>::max())) {
        return std::unexpected{
            make_error(ErrorCode::io_unsupported_size, ErrorCategory::io,
                       "package file exceeds the configured archive limit")};
    }
    std::vector<std::byte> bytes(static_cast<std::size_t>((*reader)->size()));
    if (const auto read = (*reader)->read_exact_at(0U, bytes, cancellation);
        !read)
        return std::unexpected{read.error()};
    return bytes;
}

Result<PortablePackage> parse_package_manifest(
    ...
    if (!text::is_valid_utf8(manifest_text) || !manifest_text.ends_with('\n') ||
        ...
        return std::unexpected{
            package_error("package manifest is not canonical UTF-8 JSON")};
    }
    try {
        const auto manifest = Json::parse(manifest_text);
        if (!json_has_only_integer_numbers(manifest) ||
            canonical_json(manifest) != manifest_text)
            return std::unexpected{
                package_error("package manifest JSON is not canonical")};
        ...
        if (!lowercase_sha256(declared_id) ||
            digest_text(canonical_json(identity_manifest)) != declared_id) {
            return std::unexpected{package_error(
                "package ID does not match the canonical manifest")};
        }
        ...
        return std::unexpected{
            package_error("package manifest JSON is malformed")};
    } catch (const std::exception &) {
        return std::unexpected{
            package_error("package manifest contains an invalid typed value")};
    }
}
...
Result<void> verify_portable_package(const PortablePackage &package) {
    try {
        if (!package.payloads_verified)
            return std::unexpected{
                package_error("package payloads have not been fully verified")};
        if (std::ranges::any_of(package.issues, &PackageIssue::fatal))
            return std::unexpected{
                package_error("package contains a fatal verification issue")};
        auto manifest = manifest_json(package, true);
        auto identity_manifest = manifest;
        identity_manifest.erase("package_id");
        if (digest_text(canonical_json(identity_manifest)) !=
            package.package_id)
            return std::unexpected{
                package_error("package identity does not match its manifest")};

        std::map<std::string, std::vector<std::byte>, std::less<>> entry_bytes;
        entry_bytes.emplace("manifest.json",
                            string_bytes(canonical_json(manifest)));
        for (const auto &node : package.nodes) {
            const auto [found, inserted] =
                entry_bytes.emplace(node.payload_path, node.raw_payload);
            if (!inserted && found->second != node.raw_payload)
                return std::unexpected{package_error(
                    "package payload path has conflicting bytes")};
        }
        ...
        if (manifest_json(*reparsed, true) != manifest)
            return std::unexpected{
                package_error("package verification changed its manifest")};
        return {};
    } catch (const Json::exception &) {
        return std::unexpected{
            package_error("package contains an invalid structured value")};
    }
}
...
build_portable_package(const MediaContainer &source,
    ...
    if (const auto checked = cancellation.check(); !checked)
        return std::unexpected{checked.error()};
    auto catalog =
        build_object_catalog(source, 64U * 1024U * 1024U, cancellation);
    if (!catalog)
    ...
    std::vector<std::pair<const Relationship *, std::uint32_t>>
    ...
    for (std::size_t cursor = 0; cursor < queue.size(); ++cursor) {
        ...
        if (object->raw_payload.empty())
            return std::unexpected{
                package_error("package object has no retained raw payload")};
        const auto profile = package_internal::build_relocation_profile(
        ...
        if (const auto *group =
            ...
            for (std::size_t index = 0; index < group->slots.size(); ++index) {
                if (!group->slots[index].name.empty())
                    sbac_slot_ordinals[group->slots[index].name].push_back(
                        static_cast<std::uint32_t>(index));
            }
        }
        for (const auto *relationship : *required) {
            ...
            if (found == objects.end())
                return std::unexpected{package_error(
                    "package relationship target is absent from catalog")};
            ...
            if (relationship->assignment_index) {
                ...
                if (positions == sbac_slot_ordinals.end() ||
                    next >= positions->second.size()) {
                    return std::unexpected{
                        package_error("package SBAC relationship has no "
    ...
    for (const auto &key : included_keys) {
        ...
        auto profile = package_internal::build_relocation_profile(
            snapshot->object, snapshot->raw_payload);
        if (!profile)
            return std::unexpected{profile.error()};
        const auto payload_digest = package_internal::hex_digest(
            package_internal::sha256(snapshot->raw_payload));
        const auto normalized_digest = package_internal::hex_digest(
            package_internal::sha256(profile->normalized_payload));
        auto digests = waveform_digests(snapshot->object, snapshot->raw_payload,
                                        normalized_digest);
        if (!digests)
            return std::unexpected{digests.error()};
        provisional.push_back(
            {snapshot, std::move(*profile), payload_digest, normalized_digest,
    ...
    for (const auto &node : provisional) {
        ...
        packaged.object_type =
            object_type_name(node.snapshot->object.header.type);
        packaged.object_format =
            object_format_name(node.snapshot->object.format);
        packaged.name = node.snapshot->object.header.name;
        packaged.payload_sha256 = node.payload_digest;
        packaged.payload_path =
            std::format("payloads/sha256/{}.bin", node.payload_digest);
    ...
    for (const auto &[relationship, ordinal] : included_relationships) {
        ...
        package.relationships.push_back(
            {edge_id(source_id, target_id, relationship->type, ordinal),
    ...
    package.package_id =
        digest_text(canonical_json(manifest_json(package, false)));
    ...
    for (const auto &node : package.nodes) {
        const auto [found, inserted] =
            payloads.emplace(node.payload_path, node.raw_payload);
        if (!inserted && found->second != node.raw_payload)
            return std::unexpected{
                package_error("package payload digest collision")};
    }
...
open_portable_package(std::span<const std::byte> archive,
    ...
    return parse_package_manifest(manifest_entry->second, descriptors, true,
                                  filename);
}

Result<PackagePublication> publish_portable_package(
    const PackageBuild &build, const std::filesystem::path &output_path,
    bool overwrite, const CancellationToken &cancellation) {
    if (const auto checked = cancellation.check(); !checked)
        return std::unexpected{checked.error()};
    const auto required =
        std::string{required_package_extension(build.package.kind)};
    if (build.required_extension != required) {
        return std::unexpected{
            package_error("package build required extension is inconsistent")};
    }
    auto resolved = resolve_output_path(output_path, build.package.kind);
    if (!resolved)
        return std::unexpected{resolved.error()};
    if (const auto available = preflight_output(*resolved, overwrite);
        !available)
        return std::unexpected{available.error()};
    auto verified = open_portable_package(
        build.archive, text::path_to_utf8(resolved->filename()));
    if (!verified)
        return std::unexpected{verified.error()};
    if (verified->package_id != build.package.package_id ||
        verified->kind != build.package.kind) {
        return std::unexpected{
            package_error("package build metadata disagrees with its archive")};
    }
    ...
    TemporaryPackageCleanup cleanup{*temporary};
    {
        ...
        if (!output) {
            return std::unexpected{
                make_error(ErrorCode::io_open_failed, ErrorCategory::io,
                           "could not open temporary package output")};
        }
        if (build.archive.size() >
            static_cast<std::size_t>(
                std::numeric_limits<std::streamsize>::max())) {
            return std::unexpected{make_error(
                ErrorCode::io_unsupported_size, ErrorCategory::io,
        ...
        if (!output) {
            return std::unexpected{
                make_error(ErrorCode::io_read_failed, ErrorCategory::io,
                           "could not write temporary package output")};
        }
    }
    if (const auto checked = cancellation.check(); !checked)
        return std::unexpected{checked.error()};
    ...
    if (reopened->package_id != build.package.package_id) {
        return std::unexpected{
            package_error("temporary package failed identity verification")};
    }
    if (const auto checked = cancellation.check(); !checked)
        return std::unexpected{checked.error()};
    if (const auto available = preflight_output(*resolved, overwrite);
    ...
    if (const auto published =
            publish_temporary_package(*temporary, *resolved, overwrite);
        !published) {
        return std::unexpected{published.error()};
    }
    cleanup.release();
    return PackagePublication{*resolved, build.package.package_id,
                              build.package.kind,
                              static_cast<std::uint64_t>(build.archive.size())};
}

Result<PackagePublication>
export_portable_package(const MediaContainer &source,
    ...
    if (build->package.kind != expected_kind) {
        return std::unexpected{package_error(
            "resolved package roots changed the requested root kind")};
    }
...
open_portable_package(const std::filesystem::path &path,
    ...
    if (!text::is_valid_utf8(filename)) {
        return std::unexpected{
            make_error(ErrorCode::invalid_argument, ErrorCategory::io,
                       "package input path is not valid UTF-8")};
    }
...
inspect_portable_package(const std::filesystem::path &path,
    ...
    for (const auto &entry : inspected->entries)
        descriptors.emplace(entry.path,
                            ManifestArchiveEntry{entry.size, nullptr});
    return parse_package_manifest(inspected->manifest.bytes, descriptors, false,
                                  filename);
}
