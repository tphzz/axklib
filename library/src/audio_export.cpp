...
std::string safe_component(std::string value, std::string_view fallback) {
    ...
    for (const auto character : value) {
        ...
        const bool invalid =
            byte < 0x20U || std::string_view{":\"/\\|?*"}.contains(character);
...
std::optional<std::int64_t> numeric(const CurrentSbnk &bank,
                                    std::string_view name) {
    const auto *field = bank.find_numeric_field(name);
    return field == nullptr ? std::nullopt : field->value;
}

const ObjectSnapshot *object(const ObjectCatalog &catalog,
                             std::string_view key) {
    const auto found =
        std::ranges::find(catalog.objects, key, &ObjectSnapshot::key);
    return found == catalog.objects.end() ? nullptr : &*found;
}

std::string sfz_region(const SampleBankExport &bank,
                       const PhysicalWaveformExport &waveform,
                       std::string_view role, std::string sample_path,
    ...
    const auto key_high =
        bank.key_high == 128U ? member->root_key : bank.key_high;
    ...
    std::ranges::transform(
        loop_label, loop_label.begin(), [](unsigned char character) {
    ...
    if (loop_label.contains("one") && loop_label.contains("shot")) {
        ...
        const auto loop_end =
            static_cast<std::uint64_t>(waveform.waveform.loop_start) +
            waveform.waveform.loop_length - 1U;
        line +=
            std::format(" loop_mode=loop_continuous loop_start={} loop_end={}",
...
Result<Waveform> materialize_audio(const AudioSource &source) {
    ...
    if (source.left == nullptr || source.right == nullptr) {
        return std::unexpected{make_error(ErrorCode::invalid_argument,
                                          ErrorCategory::audio,
                                          "audio export source is incomplete")};
    }
...
Result<bool> same_audio(const AudioSource &first, const AudioSource &second) {
    ...
    return first_waveform->format.channels ==
               second_waveform->format.channels &&
           first_waveform->format.sample_width_bytes ==
               second_waveform->format.sample_width_bytes &&
           first_waveform->format.sample_rate ==
               second_waveform->format.sample_rate &&
...
void populate_logical_exports(
    ...
    const std::unordered_map<std::string, PhysicalWaveformExport *>
        &physical_by_key,
    ...
    for (const auto &item : catalog.objects) {
        ...
        base.coarse_tune = static_cast<std::int8_t>(
            numeric(*bank, "coarse_tune_0x0d5").value_or(0));
        base.decoded = *bank;
        ...
        const auto add_member = [&](std::string_view type, std::string role) {
            for (const auto *relation : graph.children(item.key)) {
                if (relation->type != type ||
                    ...
                    !relation->target_key ||
                    !physical_by_key.contains(*relation->target_key)) {
                    continue;
                }
                const auto *physical =
                    physical_by_key.at(*relation->target_key);
                base.members.push_back({role, *relation->target_key,
                                        physical->relative_wav_path,
                                        relation->quality});
                bank_volumes[item.key].insert(
                    waveform_volume_keys.at(*relation->target_key));
        ...
        for (const auto &destination : bank_volumes[item.key]) {
            ...
            std::erase_if(output.members, [&](const auto &member) {
                return waveform_volume_keys.at(member.waveform_key) !=
                       destination;
            });
            if (output.members.size() == 2U &&
                ...
                const auto *left =
                    physical_by_key.at(output.members[0].waveform_key);
                const auto *right =
                    physical_by_key.at(output.members[1].waveform_key);
                output.stereo_decision =
                    stereo_render_decision(left->waveform, right->waveform);
                if (output.stereo_decision->renderable) {
                    output.rendered_wav_path =
                        std::filesystem::path{"RENDERED"} /
                        unique_wav_name(output.display_name,
                                        rendered_names[destination]);
                }
            }
            ...
            for (const auto &member : output.members) {
                for (const auto *relation :
                     graph.parents(member.waveform_key)) {
                    if (!relation->target_key ||
                        *relation->target_key != member.waveform_key ||
                        relation->quality != RelationshipQuality::known ||
                    ...
                    const auto *parameters =
                        std::get_if<CurrentSbnk>(&source->object.payload);
                    if (parameters != nullptr) {
                        output.parameter_contexts.push_back(
                            {source->key, source->object.header.name,
    ...
    for (const auto &item : catalog.objects) {
        if (!item.placement ||
            !std::holds_alternative<CurrentSbac>(item.object.payload))
            continue;
        ...
        for (const auto *relation : graph.children(item.key)) {
            ...
            for (const auto &destination :
                ...
                relationships_by_volume[destination].push_back(
                    *relation->target_key);
        ...
        for (auto &[destination, members] : members_by_volume) {
            ...
            volumes.at(destination)
                .sample_bank_groups.push_back(
                    {item.key, item.object.header.name, std::move(members),
    ...
    for (const auto &item : catalog.objects) {
        if (!item.placement ||
            !std::holds_alternative<CurrentProg>(item.object.payload))
            continue;
        ...
        for (const auto *relation : graph.children(item.key)) {
            ...
            const auto &destinations =
                relation->type == "PROG_ASSIGNMENT_TO_SBAC"
        ...
        for (auto &[destination, targets] : targets_by_volume) {
            volumes.at(destination)
                .programs.push_back(
                    {item.key, item.object.header.name, std::move(targets)});
        }
    }
}
...
Result<ExportPlan> build_export_plan(const Container &container,
    ...
    for (const auto &item : catalog.objects) {
        if (!item.placement)
            continue;
        const Key key{item.partition.value,
    ...
    for (auto &[key, volume] : volumes) {
        const auto waveform_count =
            std::ranges::count_if(catalog.objects, [&](const auto &item) {
                return item.placement &&
                       item.object.header.type == ObjectType::smpl &&
        ...
        for (const auto &item : catalog.objects) {
            ...
            const auto filename =
                unique_wav_name(item.object.header.name, used);
            volume.waveforms.push_back(
                {item.key, item.object.header.name,
                 std::filesystem::path{"SMPL"} / filename,
                 std::move(*waveform)});
            physical_by_key[item.key] = &volume.waveforms.back();
            waveform_volume_keys[item.key] = key;
        }
    }
    populate_logical_exports(catalog, graph, volumes, physical_by_key,
                             waveform_volume_keys, rendered_names);
    for (auto &[key, volume] : volumes) {
...
Result<ExportPlan> build_export_plan(const MediaContainer &container,
                                     const ObjectCatalog &catalog,
                                     const RelationshipGraph &graph,
                                     const CancellationToken &cancellation) {
    if (container.kind() == MediaKind::sfs) {
        return build_export_plan(std::get<Container>(container.storage()),
                                 catalog, graph, cancellation);
    }
    ...
    for (std::size_t index = 0; index < media_objects->size(); ++index) {
        ...
        roots_by_key.emplace(
            item.key, paths[index].relative_path.parent_path().parent_path());
    }
        ...
        for (const auto &item : catalog.objects) {
            ...
            if (source == media_by_key.end()) {
                return std::unexpected{make_error(
                    ErrorCode::object_missing, ErrorCategory::object,
            ...
            if (!waveform) {
                result.decode_errors.push_back(
                    std::format("{} ({}): {}", item.object.header.name,
                                item.key, waveform.error().message));
                continue;
            }
            volume.waveforms.push_back(
                {item.key, item.object.header.name,
                 std::filesystem::path{"SMPL"} /
                     unique_wav_name(item.object.header.name, used),
                 std::move(*waveform)});
            physical_by_key[item.key] = &volume.waveforms.back();
            waveform_volume_keys[item.key] = key;
...
Result<ExportResult>
write_export_audio(const ExportPlan &plan,
    ...
    const auto register_target = [&](const std::filesystem::path &path,
                                     AudioSource source) -> Result<void> {
        ...
        if (!*equal) {
            return std::unexpected{
                make_error(ErrorCode::invalid_argument, ErrorCategory::audio,
                           "distinct audio exports share output path: " +
    ...
    for (const auto &volume : plan.volumes) {
        for (const auto &waveform : volume.waveforms) {
            const auto path = (output_directory / volume.relative_root /
                               waveform.relative_wav_path)
                                  .lexically_normal();
            if (auto registered =
                    register_target(path, {.physical = &waveform.waveform});
                !registered)
        ...
        for (const auto &bank : volume.sample_banks) {
            ...
            const auto left = std::ranges::find(
                volume.waveforms, bank.members[0].waveform_key,
                &PhysicalWaveformExport::object_key);
            const auto right = std::ranges::find(
                volume.waveforms, bank.members[1].waveform_key,
                &PhysicalWaveformExport::object_key);
            if (left == volume.waveforms.end() ||
                right == volume.waveforms.end())
                continue;
            const auto path = (output_directory / volume.relative_root /
                               *bank.rendered_wav_path)
                                  .lexically_normal();
            if (auto registered = register_target(
                    path, {.left = &left->waveform, .right = &right->waveform});
                !registered)
                return std::unexpected{registered.error()};
        }
    }
    if (!overwrite) {
        const auto existing =
            std::ranges::find_if(targets, [](const auto &entry) {
                return std::filesystem::exists(entry.first);
            });
        if (existing != targets.end()) {
            return std::unexpected{
                make_error(ErrorCode::io_open_failed, ErrorCategory::io,
                           "refusing to replace an existing audio export: " +
                               text::path_to_utf8(existing->first))};
        }
    }
    for (const auto &[path, source] : targets) {
        ...
        if (source.physical != nullptr) {
            if (const auto written =
                    write_wav_atomic(path, *source.physical, overwrite);
                !written)
            ...
            if (!rendered)
                return std::unexpected{rendered.error()};
            if (const auto written =
                    write_wav_atomic(path, *rendered, overwrite);
                !written)
                return std::unexpected{written.error()};
        }
...
Result<SfzExportResult> write_sfz(const ExportPlan &plan,
    ...
    const auto reserve_path = [&](const VolumeExport &volume,
                                  std::string name) {
    ...
    for (const auto &volume : plan.volumes) {
        ...
        for (const auto &group : volume.sample_bank_groups) {
            reserve_path(volume,
                         "B " + display_text(group.display_name, "instrument"));
            grouped.insert(group.member_bank_keys.begin(),
                           group.member_bank_keys.end());
        }
        for (const auto &bank : volume.sample_banks) {
    ...
    if (!overwrite) {
        const auto existing =
            std::ranges::find_if(planned_paths, [](const auto &path) {
    ...
    for (const auto &volume : plan.volumes) {
        ...
        const auto write_instrument =
            [&](std::string name,
                const std::vector<const SampleBankExport *> &banks)
            -> Result<void> {
            ...
            for (const auto *bank : banks) {
                if (bank->rendered_wav_path && !bank->members.empty()) {
                    const auto waveform = std::ranges::find(
                        volume.waveforms, bank->members.front().waveform_key,
                        &PhysicalWaveformExport::object_key);
                    if (waveform != volume.waveforms.end()) {
                        text += "// " +
                                display_text(bank->display_name, "sample") +
                                '\n';
                        text +=
                            sfz_region(
                                *bank, *waveform, bank->members.front().role,
                    ...
                    for (const auto &member : bank->members) {
                        if (member.quality != RelationshipQuality::known)
                            continue;
                        const auto waveform = std::ranges::find(
                            volume.waveforms, member.waveform_key,
                            &PhysicalWaveformExport::object_key);
                        if (waveform == volume.waveforms.end())
                            continue;
                        const bool physical_pair =
                            ...
                            std::ranges::any_of(bank->members,
                                                [](const auto &item) {
                                                    return item.role == "left";
                                                }) &&
                            std::ranges::any_of(bank->members,
                                                [](const auto &item) {
                                                    return item.role == "right";
                                                });
                        std::optional<int> pan;
            ...
            if (region_count == 0U)
                return {};
            if (const auto written = write_text_atomic(path, text, overwrite);
                !written) {
                return std::unexpected{written.error()};
            }
            result.written_files.push_back(path);
        ...
        for (const auto &group : volume.sample_bank_groups) {
            ...
            for (const auto &key : group.member_bank_keys) {
                const auto found = std::ranges::find(
                    volume.sample_banks, key, &SampleBankExport::object_key);
                if (found != volume.sample_banks.end()) {
                    banks.push_back(&*found);
                    grouped.insert(key);
                }
            }
            if (const auto written = write_instrument(
                    "B " + display_text(group.display_name, "instrument"),
                    banks);
                !written) {
                return std::unexpected{written.error()};
        ...
        for (const auto &bank : volume.sample_banks) {
            if (grouped.contains(bank.object_key))
                continue;
            if (const auto written =
                    write_instrument(bank.display_name, {&bank});
                !written) {
                return std::unexpected{written.error()};
            }
        }
