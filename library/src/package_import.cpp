...
Result<std::uint8_t> planned_program_number(const PlannedPackageObject &object) {
  ...
  const auto [end, error] =
      std::from_chars(object.destination_name.data(),
                      object.destination_name.data() + object.destination_name.size(), number);
  if (error != std::errc{} ||
      end != object.destination_name.data() + object.destination_name.size() || number < 1U ||
      number > 128U) {
    return std::unexpected{
        planner_error("destination Program names must be decimal slots 001 through 128")};
  }
...
relocation_context(const PortablePackage &package, const PackageImportPlan &plan,
  ...
  for (const auto &edge : package.relationships) {
    ...
    if (target == nullptr) {
      return std::unexpected{
          planner_error("package relationship target is absent from the planned destination")};
    }
...
std::vector<ExistingObject> existing_objects(const ObjectCatalog &catalog) {
  ...
  for (const auto &object : catalog.objects) {
    ...
    const auto profile =
        package_internal::build_relocation_profile(object.object, object.raw_payload);
    if (profile) {
...
void add_conflict(PackageImportPlan &plan, std::string code, std::string message,
                  ...
                  const PortablePackage *package = nullptr, const PackageNode *node = nullptr) {
...
Result<std::string>
projected_normalized_sha256(const PortablePackage &package, const PackageNode &node,
  ...
  for (const auto &edge : package.relationships) {
    ...
    if (target == names.end()) {
      return std::unexpected{
          planner_error("package relationship target is absent from its destination closure")};
    }
...
reserve_clusters(PartitionCapacity &capacity, std::uint32_t payload_cluster_count) {
  ...
  for (std::uint32_t cluster = first;
       cluster < capacity.partition->cluster_count && selected.size() < payload_cluster_count;
       ++cluster) {
  ...
  const auto list_count = extent_count <= 4U ? 0U
                                             : (extent_count + extents_per_list_cluster - 1U) /
                                                   extents_per_list_cluster;
  std::vector<std::uint32_t> selected_lists;
  for (std::uint32_t cluster = first;
       cluster < capacity.partition->cluster_count && selected_lists.size() < list_count;
       ++cluster) {
...
Result<PackageImportPlan> plan_fat12_import(const std::filesystem::path &target_path,
  ...
  for (const auto &file : fat.files()) {
    if (file.path.find('/') != std::string::npos) {
      add_conflict(plan, "FAT12_PROFILE_UNSUPPORTED",
                   "package import supports only the Yamaha root-level FAT12 profile");
      break;
    }
  }
  ...
  for (const auto &destination : request.root_destinations) {
    if (destination.package_index >= packages.size() ||
      ...
      add_conflict(plan, "DESTINATION_ROOT_INVALID",
                   "root destination references a missing package or root", &destination);
      continue;
    }
    if (!destination.partition_index || *destination.partition_index != 0U ||
        !destination.group_name.empty() || destination.volume_name != "FAT root" ||
        !destination.raw_group.empty() || !destination.raw_volume.empty() ||
        destination.create_destination) {
      add_conflict(plan, "FAT12_DESTINATION_INVALID",
                   "Yamaha FAT12 imports require partition 0 and the existing FAT root",
    ...
    if (!destinations.emplace(key, &destination).second) {
      add_conflict(plan, "DESTINATION_ROOT_DUPLICATE",
                   "package root has more than one destination mapping", &destination,
                   &packages[destination.package_index]);
    }
  }
  for (std::size_t package_index = 0; package_index < packages.size(); ++package_index) {
    for (std::size_t root_index = 0; root_index < packages[package_index].roots.size();
         ++root_index) {
      if (!destinations.contains({package_index, root_index})) {
        ...
        add_conflict(plan, "DESTINATION_ROOT_MISSING",
                     "every package root requires one explicit destination", &missing,
  ...
  for (const auto &rename : request.policy.renames) {
    if (rename.package_index >= packages.size() ||
        ...
        !renames.emplace(std::pair{rename.package_index, rename.node_id}, rename.destination_name)
             .second) {
      add_conflict(plan, "PACKAGE_RENAME_INVALID",
                   "package rename must identify one node and contain 1 to 16 ASCII bytes");
    }
  }
  ...
  for (const auto &[key, destination] : destinations) {
    ...
    for (const auto *node : closure) {
      auto name = node->name;
      if (const auto renamed = renames.find({package_index, node->node_id});
          renamed != renames.end()) {
        name = renamed->second;
      }
    ...
    for (const auto *node : closure) {
      ...
      if (!valid_sfs_name(name)) {
        add_conflict(plan, "FAT12_OBJECT_NAME_INVALID",
                     "Yamaha object names must contain 1 to 16 ASCII bytes", destination, &package,
                     node);
        continue;
      }
      if (node->object_type == "PROG") {
        ...
        if (const auto number = planned_program_number(program); !number) {
          add_conflict(plan, "FAT12_PROGRAM_SLOT_INVALID", number.error().message, destination,
                       &package, node);
          continue;
        }
      }
  ...
  for (const auto &candidate : candidates) {
    ...
    if (matches.size() > 1U) {
      ...
      add_conflict(plan, "FAT12_TARGET_NAME_AMBIGUOUS",
                   "FAT root contains multiple objects with the same type and name",
      ...
      if (matches.front()->normalized_sha256 == object.normalized_sha256) {
        ...
        add_conflict(plan, "FAT12_NAME_CONFLICT",
                     "FAT root already contains the same object name with different content",
                     candidate.destination, candidate.package, candidate.node);
      }
    } else {
      ...
      if (const auto found = planned_names.find(name_key); found != planned_names.end()) {
        ...
        if (canonical.normalized_sha256 == object.normalized_sha256 &&
          ...
          add_conflict(plan, "FAT12_NAME_CONFLICT",
                       "incoming roots assign different content to the same FAT object name",
                       candidate.destination, candidate.package, candidate.node);
        }
      } else {
  ...
  if (plan.conflicts.empty()) {
    for (auto &object : plan.objects) {
      ...
      auto relocated =
          package_internal::relocate_package_node(packages[object.package_index], *node, *context);
      if (!relocated)
      ...
      if (*relocated != found->snapshot->raw_payload) {
        if (object.object_type != "SBNK") {
          return std::unexpected{
              planner_error("existing FAT12 object relocation fields differ from the target")};
        }
        object.actions.push_back(PackageImportObjectAction::relocate);
      }
    }
  ...
  delta.remaining_object_ids = final_entries > fat.geometry().root_entry_count
                                   ? 0U
                                   : fat.geometry().root_entry_count - final_entries;
  delta.remaining_clusters = used_clusters > fat.geometry().data_cluster_count
                                 ? 0U
  ...
  std::ranges::sort(plan.conflicts, [](const auto &left, const auto &right) {
    return std::tie(left.code, left.package_index, left.root_index, left.package_id, left.node_id,
                    left.partition_index, left.group_name, left.volume_name, left.raw_group,
                    left.raw_volume, left.message) <
           std::tie(right.code, right.package_index, right.root_index, right.package_id,
                    right.node_id, right.partition_index, right.group_name, right.volume_name,
                    right.raw_group, right.raw_volume, right.message);
  });
...
Result<PackageImportPlan> plan_iso9660_import(const std::filesystem::path &target_path,
  ...
  for (const auto &[raw_path, label] : iso.volume_labels()) {
    ...
    if (separator != std::string::npos)
      volume_labels.emplace(
          IsoScopeKey{raw_path.substr(0, separator), raw_path.substr(separator + 1U)}, label);
  }
  ...
  for (const auto &file : iso.files()) {
    ...
    if (group == group_labels.end() || volume == volume_labels.end()) {
      add_conflict(plan, "ISO9660_LABEL_METADATA_MISSING",
                   "an existing Yamaha raw volume lacks confirmed group or volume catalog labels");
      continue;
    }
  ...
  for (auto &[group, volumes] : existing_group_volumes) {
    ...
    for (std::size_t index = 0; index < volumes.size(); ++index) {
      if (volumes[index] != std::format("F{:03}", index + 1U)) {
        add_conflict(
            plan, "ISO9660_RAW_VOLUME_SEQUENCE_INVALID",
            std::format("raw group '{}' does not use contiguous volumes F001..Fnnn", group));
        break;
      }
    }
  }
  ...
  for (const auto &requested : request.root_destinations) {
    ...
    if (!destination.partition_index || *destination.partition_index != 0U ||
      ...
      add_conflict(plan, "ISO9660_DESTINATION_INVALID",
                   "ISO destinations require partition 0, a raw group, and bounded sampler "
    ...
    if (destination.create_destination) {
      ...
      if (!valid_iso_raw_volume(destination.raw_volume) || group_volume_count >= 999U) {
        add_conflict(plan, "ISO9660_RAW_VOLUME_ALLOCATION_INVALID",
                     "new ISO destinations must use the next contiguous Fnnn raw volume",
                     &destination, &packages[destination.package_index]);
        continue;
      }
      if (const auto group = group_labels.find(destination.raw_group);
        ...
        add_conflict(plan, "ISO9660_GROUP_LABEL_CONFLICT",
                     "new ISO volume uses a different label for an existing raw group",
                     &destination, &packages[destination.package_index]);
        continue;
      }
      ...
      if (const auto planned = planned_scopes.find(scope); planned != planned_scopes.end()) {
        if (planned->second.group_name != destination.group_name ||
            ...
            !destination_creation.contains(scope) || !destination_creation.at(scope)) {
          add_conflict(plan, "ISO9660_DESTINATION_POLICY_CONFLICT",
                       "package roots disagree about one newly created raw volume", &destination,
                       &packages[destination.package_index]);
          continue;
        }
      } else {
      ...
      if (!valid_iso_raw_volume(destination.raw_volume)) {
        add_conflict(plan, "ISO9660_DESTINATION_INVALID",
                     "existing ISO destinations require an explicit F001..F999 raw volume",
                     &destination, &packages[destination.package_index]);
        continue;
      }
      ...
      if (found == scopes.end()) {
        add_conflict(plan, "ISO9660_DESTINATION_MISSING",
                     "ISO destination raw volume does not exist", &destination,
                     &packages[destination.package_index]);
        continue;
      }
      if (found->second.group_name != destination.group_name ||
        ...
        add_conflict(plan, "ISO9660_DESTINATION_LABEL_MISMATCH",
                     "ISO destination labels do not match the existing Yamaha catalogs",
                     &destination, &packages[destination.package_index]);
        continue;
      }
    }
  ...
  for (const auto &[key, destination_index] : destination_indices) {
    ...
    for (const auto *node : closure) {
      ...
      if (node->object_type == "PROG") {
        ...
        program.destination_name = name;
        if (const auto number = planned_program_number(program); !number) {
          add_conflict(plan, "ISO9660_PROGRAM_SLOT_INVALID", number.error().message, destination,
                       &package, node);
          continue;
        }
      }
  ...
  std::ranges::sort(candidates, [](const Candidate &left, const Candidate &right) {
    return std::tuple{left.destination->raw_group,
                      ...
                      left.destination->package_index,
                      left.destination->root_index,
                      left.node->node_id} < std::tuple{right.destination->raw_group,
  ...
  std::map<std::tuple<std::string, std::string, std::string, std::string>, std::size_t>
      planned_names;
  std::map<std::tuple<std::string, std::string, std::string>, std::set<IsoScopeKey>>
  ...
  for (const auto &candidate : candidates) {
    ...
    if (matches.size() > 1U) {
      ...
      add_conflict(plan, "ISO9660_TARGET_NAME_AMBIGUOUS",
                   "raw volume contains multiple objects with the same type and name",
                   candidate.destination, candidate.package, candidate.node);
    } else if (matches.size() == 1U) {
      if (matches.front()->normalized_sha256 == object.normalized_sha256) {
        object.actions.push_back(PackageImportObjectAction::reuse);
        ...
        add_conflict(plan, "ISO9660_NAME_CONFLICT",
                     "raw volume already contains the same object name with different content",
                     candidate.destination, candidate.package, candidate.node);
      }
    } else {
      const auto name_key = std::tuple{object.raw_group, object.raw_volume, object.object_type,
      ...
      if (const auto found = planned_names.find(name_key); found != planned_names.end()) {
        ...
        if (canonical.normalized_sha256 == object.normalized_sha256 &&
          ...
          mark_conflict(plan.objects[found->second]);
          add_conflict(plan, "ISO9660_NAME_CONFLICT",
                       "incoming roots assign different content to the same raw-volume object name",
                       candidate.destination, candidate.package, candidate.node);
        }
      } else {
        if (!candidate.node->relocations.empty())
        ...
        if ((equal_elsewhere || equal_planned_elsewhere) &&
            duplicate_warnings.emplace(object.raw_group, object.raw_volume, object.node_id)
                .second) {
          plan.warnings.push_back(
              {"ISO9660_CROSS_VOLUME_DUPLICATE",
               std::format("{} '{}' is duplicated in raw volume {}/{} because ISO reuse is "
                           "volume-local",
                           object.object_type, object.destination_name, object.raw_group,
                           object.raw_volume),
               false});
        }
  ...
  if (plan.conflicts.empty()) {
    for (auto &object : plan.objects) {
      ...
      if (*relocated != found->snapshot->raw_payload) {
        if (object.object_type != "SBNK") {
          return std::unexpected{
              planner_error("existing ISO9660 object relocation fields differ from the target")};
        }
        object.actions.push_back(PackageImportObjectAction::relocate);
      }
    }
  ...
  for (auto &object : plan.objects) {
    ...
    if (std::ranges::contains(object.actions, PackageImportObjectAction::insert) &&
      ...
      object.payload_sectors =
          (node_by_id(packages[object.package_index], object.node_id)->raw_payload.size() + 2047U) /
          2048U;
  ...
  for (const auto &[key, count] : category_counts) {
    if (count > 50U) {
      add_conflict(plan, "ISO9660_DIRECTORY_CAPACITY_EXHAUSTED",
                   std::format("ISO category {}/{}/{} exceeds the narrow one-sector directory "
                               "profile",
  ...
  for (const auto &[group, count] : group_volume_counts) {
    if (count > 50U)
      add_conflict(
          plan, "ISO9660_DIRECTORY_CAPACITY_EXHAUSTED",
          std::format("ISO group '{}' exceeds the narrow one-sector directory profile", group));
  }
...
Result<void> verify_package_import_plan(const PackageImportPlan &plan) {
  if (plan.schema_version != "1.0" || !valid_digest(plan.target_snapshot_id) ||
  ...
  std::set<std::tuple<std::uint8_t, std::string, std::string, std::string, std::string>>
      destination_keys;
  for (const auto &destination : plan.destinations) {
    ...
    if (destination.volume_name.empty() ||
      ...
      return std::unexpected{
          planner_error("package import plan contains an invalid destination action")};
    }
  }
  ...
  for (const auto &object : plan.objects) {
    ...
    const auto conflicts =
        std::ranges::contains(object.actions, PackageImportObjectAction::conflict);
    const std::set<PackageImportObjectAction> unique_actions(object.actions.begin(),
                                                             object.actions.end());
    ...
    if (unique_actions.size() != object.actions.size() || (inserts && reuses) ||
        ...
        (object.object_type != "SBNK" &&
         (!object.target_program_numbers.empty() || object.target_grouped)) ||
      ...
      return std::unexpected{
          planner_error("package import plan action decision is incomplete or contradictory")};
    }
  ...
  for (const auto &object : plan.objects) {
    ...
    if (canonical == actions.end() ||
      ...
      return std::unexpected{
          planner_error("package import plan canonical reuse binding is invalid")};
    }
  }
  for (const auto &allocation : plan.allocation) {
    const auto iso = plan.target_kind == MediaKind::iso9660;
    if ((iso &&
         (allocation.payload_clusters != 0U || allocation.continuation_clusters != 0U ||
      ...
      return std::unexpected{
          planner_error("package import plan allocation units do not match the target media")};
    }
  }
  if (plan.plan_id != plan_identity(plan))
    return std::unexpected{
        planner_error("package import plan identity does not match its actions")};
  return {};
}
...
Result<PackageImportPlan> plan_package_import(const std::filesystem::path &target_path,
  ...
  if (target->kind() == MediaKind::fat12_floppy) {
    return plan_fat12_import(target_path, packages, request, *target, std::move(plan), *before,
                             cancellation);
  }
  if (target->kind() == MediaKind::iso9660) {
    return plan_iso9660_import(target_path, packages, request, *target, std::move(plan), *before,
                               cancellation);
  }
  if (target->kind() != MediaKind::sfs) {
    add_conflict(plan, "TARGET_ADAPTER_UNSUPPORTED",
                 "portable package import is not yet implemented for this target profile");
    plan.plan_id = plan_identity(plan);
    return plan;
  }
  if (request.policy.sfs_waveform_reuse_scope != PackageWaveformReuseScope::volume) {
    add_conflict(plan, "SFS_REUSE_SCOPE_NOT_PROMOTED",
                 "SFS package import currently requires volume-local waveform reuse");
  }
  ...
  for (const auto &rename : request.policy.renames) {
    if (rename.package_index >= packages.size() ||
        node_by_id(packages[std::min(rename.package_index, packages.size() - 1U)],
    ...
    if (!valid_sfs_name(rename.destination_name)) {
      add_conflict(plan, "RENAME_NAME_INVALID",
  ...
  for (const auto &[key, destination] : destinations) {
    ...
    if (!destination->partition_index || destination->volume_name.empty() ||
        !destination->group_name.empty() || !destination->raw_group.empty() ||
        !destination->raw_volume.empty()) {
      add_conflict(plan, "SFS_DESTINATION_INVALID",
                   "SFS destinations require a partition and volume but no group or raw ISO "
                   "identifiers",
                   destination, &package);
      continue;
    }
    if (!capacities.contains(*destination->partition_index)) {
      add_conflict(plan, "SFS_DESTINATION_PARTITION_MISSING",
                   "SFS destination partition does not exist", destination, &package);
      continue;
    }
    ...
    if ((destination_exists && destination->create_destination) ||
      ...
      add_conflict(
          plan, destination_exists ? "SFS_DESTINATION_ALREADY_EXISTS" : "SFS_DESTINATION_MISSING",
    ...
    if (!inserted && creation->second != destination->create_destination) {
      add_conflict(plan, "SFS_DESTINATION_POLICY_CONFLICT",
                   "package roots disagree about destination volume creation", destination,
                   &package);
      continue;
    }
    const auto closure = root_closure(package, root_index);
    ...
    for (const auto *node : closure) {
      ...
      if (!valid_sfs_name(name)) {
        add_conflict(plan, "SFS_OBJECT_NAME_INVALID",
                     "SFS object names must contain 1 to 16 ASCII bytes", destination, &package,
                     node);
        continue;
      }
      if (node->object_type == "PROG") {
        ...
        program.destination_name = name;
        if (const auto number = planned_program_number(program); !number) {
          add_conflict(plan, "SFS_PROGRAM_SLOT_INVALID", number.error().message, destination,
                       &package, node);
          continue;
        }
      }
    ...
    if (create) {
      ...
      if (capacity == capacities.end()) {
        ...
        add_conflict(plan, "SFS_OBJECT_ID_EXHAUSTED",
                     "partition lacks six SFS records for destination volume scaffolding");
      } else {
        ...
        if (cluster_failure) {
          add_conflict(plan, "SFS_CLUSTER_EXHAUSTED",
                       "partition lacks clusters for destination volume scaffolding");
        }
  ...
  for (const auto &[partition_index, count] : new_volume_counts) {
    const auto partition = std::ranges::find(container.partitions(),
                                             PartitionIndex{partition_index}, &Partition::index);
  ...
  std::map<std::tuple<std::uint8_t, std::string, std::string, std::string>, std::size_t>
      planned_names;
  for (const auto &candidate : candidates) {
    PlannedPackageObject object;
    object.action_id = action_identity(candidate);
    ...
    if (matches.size() > 1U) {
      ...
      add_conflict(plan, "SFS_TARGET_NAME_AMBIGUOUS",
                   "destination contains multiple objects with the same type and name",
                   candidate.destination, candidate.package, candidate.node);
    } else if (matches.size() == 1U) {
      if (matches.front()->normalized_sha256 == candidate.projected_normalized_sha256) {
        ...
        add_conflict(plan, "SFS_NAME_CONFLICT",
                     "destination already contains the same object name with different content",
                     candidate.destination, candidate.package, candidate.node);
      }
    } else {
      const auto name_key = std::tuple{object.partition_index, object.volume_name,
      ...
      if (const auto found = planned_names.find(name_key); found != planned_names.end()) {
        ...
        if (canonical.normalized_sha256 == object.normalized_sha256 &&
          ...
          mark_conflict(object);
          add_conflict(plan, "SFS_NAME_CONFLICT",
                       "incoming package roots assign different content to the same object name",
                       candidate.destination, candidate.package, candidate.node);
          mark_conflict(plan.objects[found->second]);
          const auto &canonical = plan.objects[found->second];
  ...
  std::map<std::tuple<std::uint8_t, std::string, std::string>, std::vector<std::size_t>>
      category_insertions;
  for (std::size_t index = 0; index < plan.objects.size(); ++index) {
    ...
    if (capacity.next_id >= capacity.free_ids.size()) {
      mark_conflict(object);
      add_conflict(plan, "SFS_OBJECT_ID_EXHAUSTED",
                   "partition has no free SFS object record for the planned import");
      auto &conflict = plan.conflicts.back();
    ...
    const auto clusters = std::max<std::uint32_t>(
        2U, static_cast<std::uint32_t>((package_node->raw_payload.size() + 1023U) / 1024U));
    ...
    category_insertions[{object.partition_index, object.volume_name, object.object_type}].push_back(
        index);
  }
  ...
  for (const auto &[key, indices] : category_insertions) {
    ...
    if (planned_destination != destination_creation.end() && planned_destination->second) {
      ...
      if (64U + growth > 2048U) {
        ...
        add_conflict(plan, "SFS_DIRECTORY_CAPACITY_EXHAUSTED",
                     "new object category directory cannot contain all planned objects");
        auto &conflict = plan.conflicts.back();
    ...
    if (category->second->data_size + growth > capacity_bytes) {
      for (const auto index : indices)
        mark_conflict(plan.objects[index]);
      add_conflict(plan, "SFS_DIRECTORY_CAPACITY_EXHAUSTED",
                   "object category directory has insufficient retained extent capacity");
      auto &conflict = plan.conflicts.back();
      conflict.partition_index = partition_index;
      conflict.volume_name = volume_name;
    }
  ...
  for (const auto &object : plan.objects) {
    ...
    if (found == existing.end())
      continue;
    if (const auto *bank = std::get_if<CurrentSbnk>(&found->snapshot->object.payload)) {
      metadata.program_numbers.insert(bank->linked_program_numbers.begin(),
                                      bank->linked_program_numbers.end());
      metadata.grouped = (bank->sample_flags & 1U) != 0U;
    }
  }
  for (const auto &owner : plan.objects) {
    ...
    for (const auto &edge : package.relationships) {
      ...
      auto &metadata =
          sbnk_metadata[{target_action->partition_index, *target_action->target_sfs_id}];
      if (edge.role == "SBAC_SLOT_TO_SBNK") {
        metadata.grouped = true;
      } else {
  ...
  if (plan.conflicts.empty()) {
    for (auto &object : plan.objects) {
      ...
      if (normalized != object.normalized_sha256) {
        return std::unexpected{
            planner_error("planned package relocation changed normalized object identity")};
      }
      if (std::ranges::contains(object.actions, PackageImportObjectAction::insert)) {
        if (*relocated != node->raw_payload &&
            !std::ranges::contains(object.actions, PackageImportObjectAction::relocate)) {
      ...
      if (object.object_type != "SBNK") {
        return std::unexpected{planner_error(
            "existing package object relocation fields do not match the projected target")};
      }
  ...
  std::ranges::sort(plan.conflicts, [](const auto &left, const auto &right) {
    return std::tie(left.code, left.package_index, left.root_index, left.package_id, left.node_id,
                    left.partition_index, left.group_name, left.volume_name,
                    left.message) < std::tie(right.code, right.package_index, right.root_index,
