...
Result<OperationReport> insert_waveform(TransactionState &state, const OperationView &operation,
  ...
  report.audio_import = AudioImportSummary{
      ...
      audio->source_channels == 2U, audio->dither_algorithm,   audio->clipped_samples};
  return report;
}
...
Result<std::string> serialize_alteration_manifest_template() {
  try {
    ...
    return std::unexpected{transaction_error(
        std::string{"could not serialize alteration manifest template: "} + error.what())};
  }
}

Result<void> write_alteration_manifest_template(const std::filesystem::path &output_path,
                                                bool overwrite) {
...
Result<AlterationResult> alter_hds(const std::filesystem::path &source_path,
  ...
  for (std::size_t operation_index = 0; operation_index < manifest.operations.size();
    ...
    if (progress)
      progress->report({ProgressPhase::writing, operation_index + 1U, manifest.operations.size(),
                        operation.type,
