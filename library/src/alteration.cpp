...
Error transaction_error(std::string message) {
    return make_error(ErrorCode::transaction_rejected,
                      ErrorCategory::transaction, std::move(message));
}
...
AlterationOperation typed_operation(OperationView value) {
    ...
    if (value.type == "delete_volume") {
        data = DeleteVolumeOperation{std::move(value.partition),
                                     std::move(value.volume_name)};
    } else if (value.type == "insert_volume") {
        data = InsertVolumeOperation{std::move(value.partition),
                                     std::move(*value.volume)};
    } else if (value.type == "delete_sbnk") {
        data = DeleteSampleBankOperation{std::move(value.partition),
                                         std::move(value.volume_name),
                                         std::move(value.sample_bank_name)};
    } else if (value.type == "insert_sbnk") {
        data = InsertSampleBankOperation{std::move(value.partition),
                                         std::move(value.volume_name),
                                         std::move(*value.sample_bank)};
    } else if (value.type == "insert_waveform") {
        data = InsertWaveformOperation{std::move(value.partition),
                                       std::move(value.volume_name),
                                       std::move(*value.waveform)};
    } else if (value.type == "delete_waveform") {
        data = DeleteWaveformOperation{std::move(value.partition),
                                       std::move(value.volume_name),
                                       std::move(value.waveform_name)};
    } else if (value.type == "rename_waveform") {
        data = RenameWaveformOperation{
            std::move(value.partition), std::move(value.volume_name),
            std::move(value.waveform_name), std::move(value.new_waveform_name)};
    } else if (value.type == "rename_sbnk") {
        data = RenameSampleBankOperation{std::move(value.partition),
                                         std::move(value.volume_name),
                                         std::move(value.sample_bank_name),
                                         std::move(value.new_sample_bank_name)};
    } else if (value.type == "delete_sbac") {
        data = DeleteSampleBankGroupOperation{
            std::move(value.partition), std::move(value.volume_name),
            std::move(value.sample_bank_group_name)};
    } else if (value.type == "insert_sbac") {
        data = InsertSampleBankGroupOperation{
            std::move(value.partition), std::move(value.volume_name),
            std::move(*value.sample_bank_group)};
    } else if (value.type == "rename_sbac") {
        data = RenameSampleBankGroupOperation{
            std::move(value.partition), std::move(value.volume_name),
            std::move(value.sample_bank_group_name),
            std::move(value.new_sample_bank_group_name)};
    } else if (value.type == "delete_program") {
        data = DeleteProgramOperation{std::move(value.partition),
                                      std::move(value.volume_name),
                                      *value.program_number};
    } else {
        data = InsertProgramOperation{std::move(value.partition),
                                      std::move(value.volume_name),
...
Result<std::string> required_text(const Json &row, std::string_view field,
    ...
    if (!row.contains(field) || !row[field].is_string() ||
        ...
        return std::unexpected{
            transaction_error(std::string{context} + "." + std::string{field} +
                              " must be a non-empty string")};
    }
    return row[field].get<std::string>();
}

Result<void> exact_fields(const Json &row,
                          std::initializer_list<std::string_view> expected,
                          std::string_view context) {
    if (!row.is_object() || row.size() != expected.size()) {
        return std::unexpected{
            transaction_error(std::string{context} + " has invalid fields")};
    }
    for (const auto field : expected) {
        if (!row.contains(field)) {
            return std::unexpected{transaction_error(std::string{context} +
                                                     " is missing field " +
                                                     std::string{field})};
        }
    }
...
Result<std::uint8_t> midi_value(const Json &row, std::string_view field,
                                std::string_view context,
...
Result<std::string> object_name(const Json &row, std::string_view field,
                                std::string_view context) {
    ...
    if (result->size() > 16U ||
        !std::ranges::all_of(
            *result, [](unsigned char value) { return value < 0x80U; })) {
        return std::unexpected{transaction_error(std::string{context} + "." +
                                                 std::string{field} +
...
const IndexRecord *record(const Partition &partition, SfsId id) {
    const auto found =
        std::ranges::find(partition.records, id, &IndexRecord::sfs_id);
    return found == partition.records.end() ? nullptr : &*found;
}

const MutablePartition::InsertedRecord *
current_record(const MutablePartition &partition, SfsId id) {
    if (partition.deleted.contains(id))
        return nullptr;
    if (const auto found = partition.inserted.find(id);
        found != partition.inserted.end()) {
        return &found->second;
    }
    if (const auto found = partition.changed.find(id);
        found != partition.changed.end()) {
        return &found->second;
    }
    return nullptr;
}
...
bool record_exists(const MutablePartition &partition, SfsId id) {
    return current_record(partition, id) != nullptr ||
           (!partition.deleted.contains(id) &&
            record(*partition.source, id) != nullptr);
}

Result<std::vector<std::byte>>
current_payload(TransactionState &state, MutablePartition &partition, SfsId id,
    ...
    if (partition.deleted.contains(id) ||
        record(*partition.source, id) == nullptr) {
        return std::unexpected{transaction_error(
            "SFS record does not exist in transaction state")};
    }
    return state.container.read_record_data(partition.source->index, id,
...
std::string directory_name(std::span<const std::byte> bytes) {
    ...
    const auto declared = static_cast<std::size_t>(
        (std::to_integer<std::uint16_t>(bytes[2]) << 8U) |
...
Result<std::vector<ParsedDirectoryEntry>>
parse_directory(std::span<const std::byte> payload, SfsId id) {
    ...
    for (std::size_t offset = 0; offset + 32U <= payload.size();
        ...
        if (std::ranges::all_of(row.first(8U), [](std::byte value) {
    ...
    if (result.empty() || result.front().name != ".") {
        return std::unexpected{
            transaction_error("SFS ID " + std::to_string(id.value) +
...
void set_bitmap(std::vector<std::byte> &bitmap, std::uint32_t cluster,
                bool used) {
...
void put_be16(std::span<std::byte> bytes, std::size_t offset,
              std::uint16_t value) {
    bytes[offset] = static_cast<std::byte>(value >> 8U);
    bytes[offset + 1U] = static_cast<std::byte>(value);
}

void put_be32(std::span<std::byte> bytes, std::size_t offset,
              std::uint32_t value) {
...
Result<std::vector<std::byte>>
current_root_payload(TransactionState &state, MutablePartition &partition,
                     const CancellationToken &cancellation) {
    if (partition.root_payload)
        return *partition.root_payload;
    return state.container.read_record_data(partition.source->index, SfsId{1},
                                            64U * 1024U, cancellation);
}

Result<void> set_root_payload(TransactionState &state,
                              MutablePartition &partition,
                              std::vector<std::byte> payload,
                              const CancellationToken &cancellation) {
    const auto *root = record(*partition.source, SfsId{1});
    if (root == nullptr || root->extents.size() != 1U ||
        !root->continuation_clusters.empty() ||
        payload.size() >
            static_cast<std::size_t>(root->extents[0].cluster_count) * 1024U) {
        return std::unexpected{transaction_error(
            "partition root relocation is not enabled for this transaction")};
    }
    Result<std::vector<std::byte>> raw =
        partition.root_index
            ? Result<std::vector<std::byte>>{*partition.root_index}
            : read_raw(state.container.source_path(), root->record_offset.value,
                       72);
    if (!raw)
...
Result<void> replace_record_payload(TransactionState &state,
                                    MutablePartition &partition, SfsId id,
                                    std::vector<std::byte> payload,
                                    const CancellationToken &cancellation) {
    if (partition.deleted.contains(id)) {
        return std::unexpected{
            transaction_error("cannot change a deleted SFS record")};
    }
    MutablePartition::InsertedRecord *target{};
    if (const auto found = partition.inserted.find(id);
        found != partition.inserted.end()) {
        target = &found->second;
    } else if (const auto changed = partition.changed.find(id);
               changed != partition.changed.end()) {
        target = &changed->second;
    } else {
        const auto *source = record(*partition.source, id);
        if (source == nullptr) {
            return std::unexpected{
                transaction_error("cannot change a missing SFS record")};
        }
        auto raw = read_raw(state.container.source_path(),
                            source->record_offset.value, 72U);
        if (!raw)
            return std::unexpected{raw.error()};
        ...
        auto [inserted, unused] = partition.changed.emplace(
            id, MutablePartition::InsertedRecord{
                    id, std::move(*raw), std::move(*original), source->extents,
    ...
    if (payload.size() > capacity) {
        return std::unexpected{transaction_error(
            "record payload growth exceeds its current extent capacity")};
    }
    ...
    if (target->extents.size() <= 4U) {
        ...
        for (std::size_t index = 0; index < target->extents.size(); ++index) {
            const auto capacity_for_extent =
                target->extents[index].cluster_count * 1024U;
            const auto byte_count =
                remaining == 0U ? capacity_for_extent
                                : std::min(remaining, capacity_for_extent);
    ...
    if (target->extents.size() == 1U) {
        put_be32(target->raw_index, 0x12U,
    ...
    if (id.value == 1U) {
        ...
        put_be16(target->raw_index, 0x46U,
                 static_cast<std::uint16_t>(entries->size() - 2U));
    }
    target->payload = std::move(payload);
    return {};
}

Result<SfsId> unique_directory_child(TransactionState &state,
                                     MutablePartition &partition,
                                     SfsId directory, std::string_view name,
                                     const CancellationToken &cancellation) {
    if (current_payload_kind(partition, directory) != PayloadKind::directory) {
        return std::unexpected{transaction_error(
            "directory path resolves to a non-directory record")};
    }
    ...
    if (matches.size() != 1U) {
        return std::unexpected{transaction_error(
            "directory requires exactly one entry named " + std::string{name})};
    }
...
Result<SfsId> volume_category(TransactionState &state,
                              MutablePartition &partition,
                              std::string_view volume_name,
                              std::string_view category_name,
                              const CancellationToken &cancellation) {
    auto volume = unique_directory_child(state, partition, SfsId{1},
                                         volume_name, cancellation);
    if (!volume)
        return std::unexpected{volume.error()};
    return unique_directory_child(state, partition, *volume, category_name,
                                  cancellation);
}

Result<std::pair<SfsId, SfsId>>
category_object(TransactionState &state, MutablePartition &partition,
                std::string_view volume_name, std::string_view category_name,
                std::string_view object_name, std::string_view expected_type,
                const CancellationToken &cancellation) {
    auto category = volume_category(state, partition, volume_name,
                                    category_name, cancellation);
    if (!category)
        return std::unexpected{category.error()};
    auto object = unique_directory_child(state, partition, *category,
                                         object_name, cancellation);
    if (!object)
...
category_objects(TransactionState &state, MutablePartition &partition,
                 std::string_view volume_name, std::string_view category_name,
                 ObjectType expected_type,
                 const CancellationToken &cancellation) {
    auto directory = volume_category(state, partition, volume_name,
                                     category_name, cancellation);
    if (!directory)
    ...
    for (const auto &entry : *entries) {
        if (entry.name == "." || entry.name == "..")
            continue;
        auto object_payload =
            current_payload(state, partition, entry.id, cancellation);
        if (!object_payload)
        ...
        if (!decoded || decoded->header.type != expected_type) {
            return std::unexpected{transaction_error(
                "category contains an unresolved or incorrectly typed object")};
        }
        result.push_back(CategoryObject{entry.name, entry.id,
...
Result<void> replace_fixed_object_payload(
    TransactionState &state, MutablePartition &partition, SfsId id,
    ...
    if (payload.size() != current->size()) {
        return std::unexpected{transaction_error(
            "fixed-size object metadata update changed payload size")};
    }
    return replace_record_payload(state, partition, id, std::move(payload),
                                  cancellation);
}

Result<bool> sbnk_program_bit(std::span<const std::byte> payload,
                              std::uint8_t program) {
    const auto offset =
        0xc0U + static_cast<std::size_t>((program - 1U) / 32U) * 4U;
    if (payload.size() < offset + 4U) {
        return std::unexpected{transaction_error(
            "SBNK payload is too short for its Program-link bitmap")};
    }
    const auto word =
        (std::to_integer<std::uint32_t>(payload[offset]) << 24U) |
...
Result<void> set_sbnk_program_bit(TransactionState &state,
                                  MutablePartition &partition, SfsId id,
    ...
    return replace_fixed_object_payload(state, partition, id,
                                        std::move(*payload), cancellation);
}

Result<void> set_sbnk_group_flag(TransactionState &state,
                                 MutablePartition &partition, SfsId id,
                                 bool enabled,
                                 const CancellationToken &cancellation) {
    auto payload = current_payload(state, partition, id, cancellation);
    ...
    if (payload->size() <= 0xd0U) {
        return std::unexpected{transaction_error(
            "SBNK payload is too short for its sample-bank-group flag")};
    }
    auto value = std::to_integer<std::uint8_t>((*payload)[0xd0U]);
    value = enabled ? static_cast<std::uint8_t>(value | 1U)
                    : static_cast<std::uint8_t>(value & 0xfeU);
    (*payload)[0xd0U] = static_cast<std::byte>(value);
    return replace_fixed_object_payload(state, partition, id,
                                        std::move(*payload), cancellation);
}

Result<void> remove_directory_entry(TransactionState &state,
                                    MutablePartition &partition,
                                    SfsId directory, SfsId child,
    ...
    if (!entries)
        return std::unexpected{entries.error()};
    const auto found =
        std::ranges::find_if(*entries, [&](const ParsedDirectoryEntry &entry) {
            return entry.id == child && entry.name == name;
        });
    if (found == entries->end()) {
        return std::unexpected{transaction_error(
            "directory entry is absent from transaction state")};
    }
    payload->erase(
        payload->begin() + static_cast<std::ptrdiff_t>(found->offset),
        payload->begin() + static_cast<std::ptrdiff_t>(found->offset + 32U));
    return replace_record_payload(state, partition, directory,
                                  std::move(*payload), cancellation);
}
...
Result<PartitionIndex> resolve_partition(const TransactionState &state,
    ...
    const auto found = std::ranges::find(state.reports, reference.operation_id,
                                         &OperationReport::id);
    if (found == state.reports.end()) {
        return std::unexpected{
            transaction_error("operation reference has no earlier result")};
    }
    return found->partition;
}

Result<std::set<SfsId>> volume_closure(const Partition &partition,
                                       const DirectoryEntry &volume) {
    ...
    while (!queue.empty()) {
        ...
        if (item == nullptr)
            return std::unexpected{transaction_error(
                "volume closure references a missing SFS record")};
        result.insert(id);
    ...
    for (const auto &item : partition.records) {
        ...
        for (const auto &child : item.directory_entries) {
            if (child.name != "." && child.name != ".." &&
                result.contains(SfsId{child.link_id.value})) {
                return std::unexpected{transaction_error(
                    "a directory outside the volume references its closure")};
            }
        }
    }
    return result;
...
Result<OperationReport> delete_volume(TransactionState &state,
    ...
    if (partition_state == state.partitions.end())
        return std::unexpected{
            transaction_error("partition index does not exist")};
    ...
    if (root == nullptr || root->payload_kind != PayloadKind::directory ||
        ...
        return std::unexpected{transaction_error(
            "partition root must use one readable direct extent")};
    }
    ...
    if (matches.size() != 1U)
        return std::unexpected{transaction_error(
            "volume name is not unique in the selected partition")};
    ...
    for (const auto &relation : state.graph.relationships) {
        ...
        if (source == ids.end() || target == ids.end())
            continue;
        if (closure->contains(source->second) !=
            closure->contains(target->second)) {
            return std::unexpected{transaction_error(
    ...
    if (offset > payload->size() || payload->size() - offset < 32U) {
        return std::unexpected{transaction_error(
            "volume directory entry lies outside the root payload")};
    }
    payload->erase(payload->begin() + static_cast<std::ptrdiff_t>(offset),
                   payload->begin() +
                       static_cast<std::ptrdiff_t>(offset + 32U));
    if (auto replaced = set_root_payload(state, mutable_partition,
                                         std::move(*payload), cancellation);
        !replaced)
...
std::vector<SfsId> free_ids(const MutablePartition &partition,
                            std::size_t count) {
    const auto capacity =
        (static_cast<std::uint64_t>(
             partition.source->directory_index_span_clusters) *
         partition.source->sectors_per_cluster * 512U / 1024U) *
        14U;
    std::vector<SfsId> result;
    for (std::uint32_t value = 3; value < capacity && result.size() < count;
         ++value) {
        const SfsId id{value};
        if (!partition.inserted.contains(id) &&
            !partition.changed.contains(id) &&
            (partition.deleted.contains(id) ||
             record(*partition.source, id) == nullptr)) {
            result.push_back(id);
        }
...
Result<std::vector<Extent>> allocate_extents(MutablePartition &partition,
                                             std::uint32_t count) {
    std::vector<std::uint32_t> selected;
    const auto first = partition.source->directory_index_cluster +
                       partition.source->directory_index_span_clusters;
    for (std::uint32_t cluster = first;
         cluster < partition.source->cluster_count && selected.size() < count;
         ++cluster) {
    ...
    if (selected.size() != count)
        return std::unexpected{
            transaction_error("partition has insufficient free clusters")};
    std::vector<Extent> result;
    for (const auto cluster : selected) {
        set_bitmap(partition.bitmap, cluster, true);
        if (!result.empty() &&
            result.back().cluster_offset + result.back().cluster_count ==
                cluster) {
...
allocate_list_clusters(MutablePartition &partition, std::size_t count) {
    std::vector<std::uint32_t> result;
    const auto first = partition.source->directory_index_cluster +
                       partition.source->directory_index_span_clusters;
    for (std::uint32_t cluster = first;
         cluster < partition.source->cluster_count && result.size() < count;
         ++cluster) {
        if (!bitmap_used(partition.bitmap, cluster))
    ...
    if (result.size() != count) {
        return std::unexpected{transaction_error(
            "partition has insufficient continuation-list clusters")};
    }
...
std::vector<std::byte>
index_for(const detail::PreparedRecord &source,
    ...
    if (!list_clusters.empty()) {
        ...
        for (std::size_t index = 0; index < extents.size(); ++index) {
            ...
            const auto byte_count =
                remaining == 0U ? capacity : std::min(remaining, capacity);
            remaining = remaining > byte_count ? remaining - byte_count : 0U;
...
Result<std::pair<SfsId, std::uint64_t>>
allocate_record(MutablePartition &partition, std::vector<std::byte> payload,
                PayloadKind payload_kind,
                std::optional<SfsId> requested_id = {},
                std::uint16_t directory_tail = 0U) {
    const auto ids =
        requested_id ? std::vector{*requested_id} : free_ids(partition, 1U);
    if (ids.empty() ||
        (requested_id && (record_exists(partition, *requested_id) ||
                          partition.inserted.contains(*requested_id)))) {
        return std::unexpected{
            transaction_error("partition has no free SFS record")};
    }
    const auto clusters = std::max<std::uint32_t>(
        2U, static_cast<std::uint32_t>((payload.size() + 1023U) / 1024U));
    ...
    if (extents->size() > 4U) {
        auto allocated_lists = allocate_list_clusters(
            partition, (extents->size() + extents_per_list_cluster - 1U) /
    ...
    prepared.kind = payload_kind == PayloadKind::directory
                        ? detail::RecordKind::directory
                        : detail::RecordKind::object;
    prepared.tail = directory_tail;
    auto raw =
        index_for(prepared, *extents,
                  static_cast<std::uint32_t>(payload.size()), list_clusters);
    const auto id = ids.front();
    partition.deleted.erase(id);
    partition.inserted.emplace(id, MutablePartition::InsertedRecord{
                                       id, std::move(raw), std::move(payload),
                                       std::move(*extents),
                                       std::move(list_clusters), payload_kind});
    return std::pair{
        id, clusters + partition.inserted.at(id).continuation_clusters.size()};
}

Result<void> append_directory_entry(TransactionState &state,
                                    MutablePartition &partition,
                                    SfsId directory, SfsId child,
                                    std::string_view name,
                                    const CancellationToken &cancellation) {
    if (name.empty() || name.size() > 16U ||
        !std::ranges::all_of(
            name, [](unsigned char value) { return value < 0x80U; })) {
        return std::unexpected{
            transaction_error("SFS object name must fit 16 ASCII bytes")};
    }
    auto payload = current_payload(state, partition, directory, cancellation);
    if (!payload)
        return std::unexpected{payload.error()};
    ...
    if (std::ranges::any_of(*entries, [&](const ParsedDirectoryEntry &entry) {
        ...
        return std::unexpected{transaction_error(
            "directory already contains entry " + std::string{name})};
    }
    ...
    std::ranges::transform(name, entry.begin() + 8U, [](char value) {
...
Result<void> rename_directory_entry(TransactionState &state,
                                    MutablePartition &partition,
                                    SfsId directory, SfsId child,
    ...
    if (std::ranges::any_of(*entries, [&](const auto &entry) {
        ...
        return std::unexpected{
            transaction_error("rename destination already exists")};
    }
    const auto found = std::ranges::find_if(*entries, [&](const auto &entry) {
        return entry.id == child && entry.name == old_name;
    });
    if (found == entries->end()) {
        return std::unexpected{
            transaction_error("rename source directory entry is absent")};
    }
    put_be16(*payload, found->offset + 2U, 17U);
    std::fill(
        payload->begin() + static_cast<std::ptrdiff_t>(found->offset + 8U),
        payload->begin() + static_cast<std::ptrdiff_t>(found->offset + 24U),
        std::byte{' '});
...
Result<void> rename_object_payload(TransactionState &state,
                                   MutablePartition &partition, SfsId id,
                                   std::string_view old_name,
                                   std::string_view new_name,
                                   const CancellationToken &cancellation) {
    ...
    if (payload->size() < 0x42U) {
        return std::unexpected{
            transaction_error("object payload name is truncated")};
    }
    std::string actual;
    ...
    if (actual != old_name) {
        return std::unexpected{transaction_error(
            "object payload name disagrees with directory identity")};
    }
    std::fill(payload->begin() + 0x32, payload->begin() + 0x42, std::byte{' '});
    std::ranges::transform(new_name, payload->begin() + 0x32, [](char value) {
        return static_cast<std::byte>(value);
    });
    return replace_fixed_object_payload(state, partition, id,
                                        std::move(*payload), cancellation);
...
Result<std::uint64_t> release_record(MutablePartition &partition, SfsId id) {
    ...
    if (const auto found = partition.inserted.find(id);
        ...
        return std::unexpected{
            transaction_error("cannot release a missing SFS record")};
    }
...
std::vector<std::byte>
remap_directory(std::vector<std::byte> payload,
                const std::map<std::uint32_t, SfsId> &ids) {
    const auto directory_id =
        payload.size() >= 8U
    ...
    for (std::size_t offset = 0; offset + 32U <= payload.size();
         offset += 32U) {
        const auto old =
            (std::to_integer<std::uint32_t>(payload[offset + 4U]) << 24U) |
        ...
        if (old == 1U && payload[offset + 8U] == std::byte{'.'} &&
            ...
            payload[offset + 11U] = static_cast<std::byte>(
                mapped == ids.end() ? 0U : mapped->second.value & 0xffU);
        }
    }
    return payload;
}

Result<OperationReport> insert_volume(TransactionState &state,
                                      const OperationView &operation,
                                      const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    if (found == state.partitions.end() || !operation.volume)
        return std::unexpected{
            transaction_error("insert-volume target is invalid")};
    ...
    for (std::size_t offset = 64U; offset + 32U <= root_payload->size();
         offset += 32U) {
        const auto length = static_cast<std::size_t>(
            std::to_integer<std::uint16_t>((*root_payload)[offset + 2U]) << 8U |
            std::to_integer<std::uint16_t>((*root_payload)[offset + 3U]));
        std::string name;
        for (std::size_t index = 0; index + 1U < length && index < 24U;
             ++index) {
            const auto character =
                std::to_integer<char>((*root_payload)[offset + 8U + index]);
            if (character != '\0')
                name += character;
        }
        ...
        if (name == operation.volume->name)
            return std::unexpected{transaction_error(
                "partition already contains the requested volume")};
    }
    HdsBuildManifest template_manifest{
        "1.0", minimum_hds_size, {{"AXK ALTER", {*operation.volume}}}};
    auto geometry = plan_hds_geometry(template_manifest);
    if (!geometry)
        return std::unexpected{geometry.error()};
    auto prepared = detail::prepare_partition_records(
        template_manifest.partitions[0], (*geometry)[0], 1, cancellation);
    if (!prepared)
    ...
    if (ids.size() != templates.size())
        return std::unexpected{
            transaction_error("partition has insufficient free SFS records")};
    ...
    for (std::size_t index = 0; index < 16U; ++index)
        entry[8U + index] = index < encoded.size()
                                ? static_cast<std::byte>(encoded[index])
                                : std::byte{' '};
    root_payload->insert(root_payload->end(), entry.begin(), entry.end());
    if (auto replaced = set_root_payload(
            state, partition, std::move(*root_payload), cancellation);
        !replaced)
        return std::unexpected{replaced.error()};
    OperationReport report;
    report.id = operation.id;
...
Result<OperationReport> delete_sbnk(TransactionState &state,
                                    const OperationView &operation,
                                    const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    auto &partition = found->second;
    auto located =
        category_object(state, partition, operation.volume_name, "SBNK",
    ...
    if (bank == nullptr) {
        return std::unexpected{
            transaction_error("sample bank is not a current SBNK object")};
    }
    if (!bank->linked_program_numbers.empty()) {
        return std::unexpected{transaction_error(
            "sample bank is referenced by its Program link bitmap")};
    }
    for (const auto &[edge_partition, source, target] : state.known_edges) {
        if (edge_partition == *partition_index && target == bank_id &&
            ...
            return std::unexpected{transaction_error(
                "sample bank is referenced by a Program or sample-bank group")};
        }
    }
    if (auto removed =
            remove_directory_entry(state, partition, directory_id, bank_id,
...
waveform_member(TransactionState &state, MutablePartition &partition,
                std::string_view volume_name, std::string_view waveform_name,
                const CancellationToken &cancellation) {
    auto located = category_object(state, partition, volume_name, "SMPL",
                                   waveform_name, "SMPL", cancellation);
    if (!located)
        return std::unexpected{located.error()};
    auto payload =
        current_payload(state, partition, located->second, cancellation);
    if (!payload)
        return std::unexpected{payload.error()};
    auto decoded = decode_object(*payload);
    if (!decoded)
    ...
    if (sample == nullptr || sample->link_id.value == 0U) {
        return std::unexpected{
            transaction_error("waveform has no usable current SMPL link ID")};
    }
    return detail::PreparedWaveformMember{
        std::string{waveform_name}, sample->link_id.value,
        sample->duplicate_sample_rate.value, sample->wave_length_frames.value};
}

Result<OperationReport> insert_sbnk(TransactionState &state,
                                    const OperationView &operation,
                                    const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    if (found == state.partitions.end() || !operation.sample_bank) {
        return std::unexpected{
            transaction_error("insert-sbnk target is invalid")};
    }
    auto &partition = found->second;
    const auto &spec = *operation.sample_bank;
    auto directory = volume_category(state, partition, operation.volume_name,
                                     "SBNK", cancellation);
    if (!directory)
        return std::unexpected{directory.error()};
    auto directory_payload =
        current_payload(state, partition, *directory, cancellation);
    ...
    if (std::ranges::any_of(*entries, [&](const ParsedDirectoryEntry &entry) {
        ...
        return std::unexpected{transaction_error(
            "volume already contains the requested sample bank")};
    }
    if (!spec.waveform_id) {
        return std::unexpected{
            transaction_error("sample bank requires waveform_name")};
    }
    auto left = waveform_member(state, partition, operation.volume_name,
                                *spec.waveform_id, cancellation);
    if (!left)
        return std::unexpected{left.error()};
    std::optional<detail::PreparedWaveformMember> right;
    if (spec.right_waveform_id) {
        auto member = waveform_member(state, partition, operation.volume_name,
                                      *spec.right_waveform_id, cancellation);
        if (!member)
            return std::unexpected{member.error()};
        if (member->sample_rate != left->sample_rate ||
            member->frame_count != left->frame_count) {
            return std::unexpected{
                transaction_error("stereo sample bank requires matching sample "
    ...
    auto allocated =
        allocate_record(partition, std::move(*payload), PayloadKind::object);
    if (!allocated)
        return std::unexpected{allocated.error()};
    const auto [bank_id, cluster_count] = *allocated;
    if (auto appended = append_directory_entry(
            state, partition, *directory, bank_id, spec.name, cancellation);
        !appended) {
        return std::unexpected{appended.error()};
    }
    const auto left_object =
        category_object(state, partition, operation.volume_name, "SMPL",
                        *spec.waveform_id, "SMPL", cancellation);
    if (!left_object)
        return std::unexpected{left_object.error()};
    state.known_edges.emplace_back(*partition_index, bank_id,
                                   left_object->second);
    if (spec.right_waveform_id) {
...
Result<OperationReport> insert_waveform(TransactionState &state,
                                        const OperationView &operation,
                                        const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    if (found == state.partitions.end() || !operation.waveform) {
        return std::unexpected{
            transaction_error("insert-waveform target is invalid")};
    }
    auto &partition = found->second;
    ...
    for (const auto &entry : *entries) {
        ...
        if (std::ranges::contains(spec.waveform_names, entry.name)) {
            return std::unexpected{transaction_error(
                "volume already contains a requested waveform name")};
        }
        auto payload =
            current_payload(state, partition, entry.id, cancellation);
        if (!payload)
            return std::unexpected{
                transaction_error("existing SMPL record is unresolved; link-ID "
        ...
        if (sample == nullptr || sample->link_id.value == 0U) {
            return std::unexpected{transaction_error(
                "existing waveform has no current SMPL link ID")};
        }
    ...
    options.expected_channels =
        static_cast<std::uint8_t>(spec.waveform_names.size());
    ...
    for (std::size_t channel = 0; channel < spec.waveform_names.size();
         ++channel) {
        ...
        if (!stored)
            return std::unexpected{stored.error()};
        if (auto appended = append_directory_entry(
                state, partition, *directory, stored->first,
    ...
    report.audio_import = AudioImportSummary{
        audio->source_path,        audio->source_format,
        audio->source_subtype,     audio->source_channels,
        audio->source_sample_rate, audio->output_sample_rate,
...
Result<OperationReport> delete_waveform(TransactionState &state,
                                        const OperationView &operation,
                                        const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    auto waveform_payload =
        current_payload(state, partition, waveform_id, cancellation);
    if (!waveform_payload)
    ...
    if (waveform == nullptr || waveform->link_id.value == 0U) {
        return std::unexpected{transaction_error(
            "waveform cannot be classified as known_unreferenced")};
    }

    auto bank_directory = volume_category(
        state, partition, operation.volume_name, "SBNK", cancellation);
    if (!bank_directory)
        return std::unexpected{bank_directory.error()};
    auto bank_directory_payload =
        current_payload(state, partition, *bank_directory, cancellation);
    if (!bank_directory_payload)
        return std::unexpected{bank_directory_payload.error()};
    auto bank_entries =
        parse_directory(*bank_directory_payload, *bank_directory);
    if (!bank_entries)
    ...
    for (const auto &entry : *bank_entries) {
        ...
        if (!payload) {
            return std::unexpected{
                transaction_error("waveform ownership is ambiguous because an "
                                  "SBNK is unreadable")};
        }
        ...
        if (references(bank->left) ||
            (bank->right && references(*bank->right))) {
            return std::unexpected{transaction_error(
                "waveform is referenced, not known_unreferenced")};
        }
    }
    for (const auto &[edge_partition, source, target] : state.known_edges) {
        ...
        if (edge_partition == *partition_index && target == waveform_id) {
            return std::unexpected{
                transaction_error("waveform has a known incoming reference, "
                                  "not known_unreferenced")};
        }
    }
    if (auto removed =
            remove_directory_entry(state, partition, directory_id, waveform_id,
    ...
    std::erase_if(state.known_edges, [&](const auto &edge) {
        const auto &[edge_partition, source, target] = edge;
        return edge_partition == *partition_index &&
               (source == waveform_id || target == waveform_id);
    });
    OperationReport report;
    report.id = operation.id;
    report.type = operation.type;
...
Result<OperationReport> delete_program(TransactionState &state,
                                       const OperationView &operation,
                                       const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    if (found == state.partitions.end() || !operation.program_number) {
        return std::unexpected{
            transaction_error("delete-program target is invalid")};
    }
    auto &partition = found->second;
    const auto name = std::format("{:03}", *operation.program_number);
    auto located = category_object(state, partition, operation.volume_name,
                                   "PROG", name, "PROG", cancellation);
    if (!located)
        return std::unexpected{located.error()};
    auto payload =
        current_payload(state, partition, located->second, cancellation);
    ...
    for (const auto &assignment : program->assignments) {
        ...
        auto bank =
            category_object(state, partition, operation.volume_name, "SBNK",
                            assignment.name, "SBNK", cancellation);
        if (!bank)
            return std::unexpected{bank.error()};
        assigned_banks.insert(bank->second);
    }
    auto banks = category_objects(state, partition, operation.volume_name,
                                  "SBNK", ObjectType::sbnk, cancellation);
    if (!banks)
    ...
    if (assigned_banks != bitmap_banks) {
        return std::unexpected{
            transaction_error("Program direct assignments do not match SBNK "
                              "Program-link bitmaps")};
    }
    for (const auto id : assigned_banks) {
        if (auto updated = set_sbnk_program_bit(state, partition, id,
                                                *operation.program_number,
                                                false, cancellation);
            !updated)
            return std::unexpected{updated.error()};
    }
    if (auto removed =
            remove_directory_entry(state, partition, located->first,
                                   located->second, name, cancellation);
        !removed)
        return std::unexpected{removed.error()};
...
Result<OperationReport> insert_program(TransactionState &state,
                                       const OperationView &operation,
                                       const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    if (found == state.partitions.end() || !operation.program) {
        return std::unexpected{
            transaction_error("insert-program target is invalid")};
    }
    auto &partition = found->second;
    ...
    if (std::ranges::any_of(
            *entries, [&](const auto &entry) { return entry.name == name; })) {
        return std::unexpected{
            transaction_error("volume already contains Program " + name)};
    }
    if (spec.assignments.size() != 2U) {
        return std::unexpected{
            transaction_error("Program requires exactly two assignments")};
    }
    const auto &group_assignment = spec.assignments[0];
    const auto &bank_assignment = spec.assignments[1];
    auto group =
        category_object(state, partition, operation.volume_name, "SBAC",
    ...
    auto existing_programs =
        category_objects(state, partition, operation.volume_name, "PROG",
    ...
    for (const auto &existing : *existing_programs) {
        const auto *decoded_program =
            std::get_if<CurrentProg>(&existing.decoded.payload);
        for (const auto &assignment : decoded_program->assignments) {
            if ((assignment.kind == 0x11U &&
                 assignment.name == group_assignment.target_name) ||
                (assignment.kind == 0x10U &&
                 assignment.name == bank_assignment.target_name)) {
                return std::unexpected{transaction_error(
                    "Program target is already assigned by another Program")};
            }
        }
    }
    auto bank_payload =
        current_payload(state, partition, bank->second, cancellation);
    if (!bank_payload)
    ...
    if (*bit)
        return std::unexpected{
            transaction_error("SBNK already links this Program")};
    ...
    if (!allocated)
        return std::unexpected{allocated.error()};
    if (auto appended = append_directory_entry(
            state, partition, *directory, allocated->first, name, cancellation);
        !appended)
        return std::unexpected{appended.error()};
    if (auto updated = set_sbnk_program_bit(state, partition, bank->second,
                                            spec.number, true, cancellation);
        !updated)
        return std::unexpected{updated.error()};
    state.known_edges.emplace_back(*partition_index, allocated->first,
                                   group->second);
    state.known_edges.emplace_back(*partition_index, allocated->first,
                                   bank->second);
    OperationReport report;
...
Result<OperationReport> delete_sbac(TransactionState &state,
                                    const OperationView &operation,
                                    const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    if (group == nullptr ||
        group->active_slot_count > group->maximum_slot_count) {
        return std::unexpected{
            transaction_error("sample-bank group slots are unreadable")};
    }
    auto programs = category_objects(state, partition, operation.volume_name,
                                     "PROG", ObjectType::prog, cancellation);
    if (!programs)
        return std::unexpected{programs.error()};
    for (const auto &program_row : *programs) {
        const auto *program =
            std::get_if<CurrentProg>(&program_row.decoded.payload);
        if (std::ranges::any_of(
                program->assignments, [&](const ProgAssignment &assignment) {
                    return assignment.kind == 0x11U &&
                           assignment.name == operation.sample_bank_group_name;
                })) {
            return std::unexpected{transaction_error(
                "sample-bank group is referenced by a Program")};
        }
    }
    auto groups = category_objects(state, partition, operation.volume_name,
                                   "SBAC", ObjectType::sbac, cancellation);
    if (!groups)
    ...
    for (const auto &other : *groups) {
        ...
        const auto *other_group =
            std::get_if<CurrentSbac>(&other.decoded.payload);
        for (const auto &slot : other_group->slots) {
            if (members.contains(slot.name)) {
                return std::unexpected{transaction_error(
                    "another sample-bank group shares a member")};
            }
        }
    }
    for (const auto &slot : group->slots) {
        auto bank = category_object(state, partition, operation.volume_name,
                                    "SBNK", slot.name, "SBNK", cancellation);
        if (!bank)
            return std::unexpected{bank.error()};
        auto bank_payload =
            current_payload(state, partition, bank->second, cancellation);
        ...
        if (bank_payload->size() <= 0xd0U ||
            (std::to_integer<std::uint8_t>((*bank_payload)[0xd0U]) & 1U) ==
                0U) {
            return std::unexpected{
                transaction_error("member SBNK is missing its grouped flag")};
        }
        if (auto updated = set_sbnk_group_flag(state, partition, bank->second,
                                               false, cancellation);
            !updated)
            return std::unexpected{updated.error()};
    }
    if (auto removed = remove_directory_entry(
            state, partition, located->first, located->second,
...
Result<OperationReport> insert_sbac(TransactionState &state,
                                    const OperationView &operation,
                                    const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    if (found == state.partitions.end() || !operation.sample_bank_group) {
        return std::unexpected{
            transaction_error("insert-sbac target is invalid")};
    }
    auto &partition = found->second;
    ...
    if (!directory)
        return std::unexpected{directory.error()};
    auto existing_groups =
        category_objects(state, partition, operation.volume_name, "SBAC",
    ...
    for (const auto &existing : *existing_groups) {
        if (existing.name == spec.name) {
            return std::unexpected{
                transaction_error("sample-bank group already exists")};
        }
    ...
    for (const auto &name : spec.member_sample_banks) {
        if (existing_members.contains(name)) {
            return std::unexpected{
                transaction_error("SBNK already belongs to another group")};
        }
        auto bank = category_object(state, partition, operation.volume_name,
                                    "SBNK", name, "SBNK", cancellation);
        if (!bank)
            return std::unexpected{bank.error()};
        auto bank_payload =
            current_payload(state, partition, bank->second, cancellation);
        ...
        if (current_bank == nullptr || current_bank->right_slot_present ||
            (std::to_integer<std::uint8_t>((*bank_payload)[0xd0U]) & 1U) !=
                0U) {
            return std::unexpected{transaction_error(
                "SBAC profile requires ungrouped mono SBNK members")};
        }
    ...
    for (const auto id : member_ids) {
        if (auto updated =
                set_sbnk_group_flag(state, partition, id, true, cancellation);
            !updated)
            return std::unexpected{updated.error()};
        state.known_edges.emplace_back(*partition_index, allocated->first, id);
    }
    if (auto appended =
            append_directory_entry(state, partition, *directory,
                                   allocated->first, spec.name, cancellation);
        !appended)
        return std::unexpected{appended.error()};
    OperationReport report;
    report.id = operation.id;
...
void put_padded_name(std::span<std::byte> payload, std::size_t offset,
                     std::string_view name) {
    std::fill(payload.begin() + static_cast<std::ptrdiff_t>(offset),
              payload.begin() + static_cast<std::ptrdiff_t>(offset + 16U),
              std::byte{' '});
    std::ranges::transform(
        name, payload.begin() + static_cast<std::ptrdiff_t>(offset),
        [](char value) { return static_cast<std::byte>(value); });
}

Result<OperationReport> rename_waveform(TransactionState &state,
                                        const OperationView &operation,
                                        const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    if (waveform == nullptr || waveform->link_id.value == 0U)
        return std::unexpected{
            transaction_error("waveform has no current link identity")};
    auto waveforms = category_objects(state, partition, operation.volume_name,
                                      "SMPL", ObjectType::smpl, cancellation);
    if (!waveforms)
    ...
    for (const auto &other : *waveforms) {
        ...
        if (other.name == operation.new_waveform_name ||
            ...
            return std::unexpected{
                transaction_error("waveform rename identity is not unique")};
        }
    }
    auto banks = category_objects(state, partition, operation.volume_name,
                                  "SBNK", ObjectType::sbnk, cancellation);
    ...
    for (const auto &bank_row : *banks) {
        ...
        const auto inspect = [&](const CurrentSbnkMember &member,
                                 std::size_t offset) -> Result<void> {
            const auto name_matches =
                member.sample_name == operation.waveform_name;
            const auto link_matches =
                member.smpl_link_id == waveform->link_id.value;
            if (name_matches != link_matches)
                return std::unexpected{transaction_error(
                    "SBNK waveform name and link identity disagree")};
            if (name_matches)
        ...
        if (!offsets.empty()) {
            ...
            if (auto replaced = replace_fixed_object_payload(
                    state, partition, bank_row.id, std::move(payload),
                    cancellation);
                !replaced)
                return std::unexpected{replaced.error()};
    ...
    for (const auto &[edge_partition, source, target] : state.known_edges) {
        if (edge_partition == *partition_index && target == located->second &&
            ...
            return std::unexpected{transaction_error(
                "known waveform references exceed exact rename set")};
    }
    if (auto renamed = rename_object_payload(
            state, partition, located->second, operation.waveform_name,
            operation.new_waveform_name, cancellation);
        !renamed)
        return std::unexpected{renamed.error()};
    if (auto renamed = rename_directory_entry(
            state, partition, located->first, located->second,
...
Result<OperationReport> rename_sbac(TransactionState &state,
                                    const OperationView &operation,
                                    const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    if (std::ranges::any_of(*groups, [&](const auto &group) {
            return group.id != located->second &&
                   group.name == operation.new_sample_bank_group_name;
        }))
        return std::unexpected{
            transaction_error("sample-bank group rename destination exists")};
    auto group_payload =
        current_payload(state, partition, located->second, cancellation);
    if (!group_payload)
    ...
    if (group == nullptr || group->slots.empty() || group->slots.size() > 3U ||
        ...
        return std::unexpected{
            transaction_error("SBAC rename requires 1..3 readable slots")};
    std::set<SfsId> member_ids;
    for (const auto &slot : group->slots) {
        if (slot.raw_handle != 0U)
            return std::unexpected{transaction_error(
                "SBAC member has unsupported nonzero handle")};
        auto member = category_object(state, partition, operation.volume_name,
                                      "SBNK", slot.name, "SBNK", cancellation);
        if (!member)
            return std::unexpected{member.error()};
        auto member_payload =
            current_payload(state, partition, member->second, cancellation);
        if (!member_payload)
            return std::unexpected{member_payload.error()};
        if (member_payload->size() <= 0xd0U ||
            (std::to_integer<std::uint8_t>((*member_payload)[0xd0U]) & 1U) ==
                0U) {
            return std::unexpected{
                transaction_error("SBAC member is missing its grouped flag")};
        }
    ...
    for (const auto &other : *groups) {
        ...
        if (std::ranges::any_of(other_group->slots, [&](const SbacSlot &slot) {
            ...
            return std::unexpected{
                transaction_error("another SBAC shares a rename member")};
        }
    }
    ...
    if (known_members != member_ids) {
        return std::unexpected{
            transaction_error("SBAC raw members disagree with known edges")};
    }
    auto programs = category_objects(state, partition, operation.volume_name,
                                     "PROG", ObjectType::prog, cancellation);
    if (!programs)
    ...
    for (const auto &program_row : *programs) {
        ...
        bool changed{};
        for (std::size_t index = 0; index < program->assignments.size();
             ++index) {
            ...
            if (assignment.name == operation.new_sample_bank_group_name)
                return std::unexpected{transaction_error(
                    "Program already assigns rename destination")};
            if (assignment.name != operation.sample_bank_group_name)
                continue;
            if (assignment.raw_handle != 0U)
                return std::unexpected{transaction_error(
                    "Program assignment has unsupported nonzero handle")};
            put_padded_name(payload, 0x120U + index * 0x38U,
                            operation.new_sample_bank_group_name);
            changed = true;
        }
        if (changed) {
            if (auto replaced = replace_fixed_object_payload(
                    state, partition, program_row.id, std::move(payload),
                    cancellation);
                !replaced)
                return std::unexpected{replaced.error()};
    ...
    if (known_programs != updated_programs) {
        return std::unexpected{transaction_error(
            "SBAC raw Program references disagree with known edges")};
    }
    if (auto renamed = rename_object_payload(
            state, partition, located->second, operation.sample_bank_group_name,
...
Result<OperationReport> rename_sbnk(TransactionState &state,
                                    const OperationView &operation,
                                    const CancellationToken &cancellation) {
    auto partition_index = resolve_partition(state, operation.partition);
    if (!partition_index)
    ...
    if (std::ranges::any_of(*banks, [&](const auto &bank) {
            return bank.id != located->second &&
                   bank.name == operation.new_sample_bank_name;
        }))
        return std::unexpected{
            transaction_error("SBNK rename destination exists")};
    auto bank_payload =
        current_payload(state, partition, located->second, cancellation);
    if (!bank_payload)
        return std::unexpected{bank_payload.error()};
    ...
    for (const auto &group_row : *groups) {
        const auto *group =
            std::get_if<CurrentSbac>(&group_row.decoded.payload);
        auto payload = group_row.payload;
        bool changed{};
        for (const auto &slot : group->slots) {
            if (slot.name == operation.new_sample_bank_name)
                return std::unexpected{transaction_error(
                    "SBAC already references SBNK rename destination")};
            if (slot.name != operation.sample_bank_name)
                continue;
            if (slot.raw_handle != 0U)
                return std::unexpected{transaction_error(
                    "SBAC member has unsupported nonzero handle")};
            put_padded_name(payload, slot.offset,
                            operation.new_sample_bank_name);
            changed = true;
        }
        if (changed) {
            if (auto replaced = replace_fixed_object_payload(
                    state, partition, group_row.id, std::move(payload),
                    cancellation);
                !replaced)
                return std::unexpected{replaced.error()};
    ...
    if (group_references.size() != (grouped ? 1U : 0U))
        return std::unexpected{transaction_error(
            "SBNK grouped flag disagrees with exact SBAC membership")};
    ...
    for (const auto &program_row : *programs) {
        ...
        const auto parsed_number = std::from_chars(
            program_row.name.data(),
            program_row.name.data() + program_row.name.size(), number);
        if (parsed_number.ec != std::errc{} ||
            parsed_number.ptr !=
                program_row.name.data() + program_row.name.size()) {
            return std::unexpected{
                transaction_error("Program slot name is unsupported")};
        }
        if (number < 1 || number > 128 ||
            std::format("{:03}", number) != program_row.name)
            return std::unexpected{
                transaction_error("Program slot name is unsupported")};
        const auto *program =
            std::get_if<CurrentProg>(&program_row.decoded.payload);
        ...
        for (std::size_t index = 0; index < program->assignments.size();
            ...
            if (assignment.name == operation.new_sample_bank_name)
                return std::unexpected{transaction_error(
                    "Program already assigns SBNK rename destination")};
    ...
    if (bitmap_numbers != direct_numbers)
        return std::unexpected{transaction_error(
            "SBNK Program bitmap disagrees with exact Program assignments")};
    ...
    if (known_incoming != expected) {
        return std::unexpected{
            transaction_error("SBNK raw references disagree with known edges")};
    }
    if (auto renamed = rename_object_payload(
            state, partition, located->second, operation.sample_bank_name,
            operation.new_sample_bank_name, cancellation);
        !renamed)
        return std::unexpected{renamed.error()};
    if (auto renamed = rename_directory_entry(
            state, partition, located->first, located->second,
            operation.sample_bank_name, operation.new_sample_bank_name,
            cancellation);
        !renamed)
        return std::unexpected{renamed.error()};
    OperationReport report;
...
Result<void>
write_continuation_lists(std::fstream &output, const Partition &partition,
    ...
    for (std::size_t list_index = 0;
         list_index < record.continuation_clusters.size(); ++list_index) {
        const auto extent_begin = list_index * extents_per_cluster;
        const auto extent_count =
            std::min(extents_per_cluster, record.extents.size() - extent_begin);
        ...
        const auto absolute =
            (static_cast<std::uint64_t>(partition.start_sector) +
             static_cast<std::uint64_t>(
                 record.continuation_clusters[list_index]) *
...
copy_to_unique_temporary(const std::filesystem::path &source,
                         const std::filesystem::path &output) {
    std::random_device random;
    for (std::size_t attempt = 0; attempt < 32U; ++attempt) {
        const auto temporary = text::temporary_sibling(
            output, std::format(".alter.{:08x}.tmp", random()));
        if (!temporary)
...
Result<void> validate_temporary(const std::filesystem::path &temporary,
    ...
    for (const auto &[index, expected] : state.partitions) {
        const auto partition = std::ranges::find(
            actual->partitions(), PartitionIndex{index}, &Partition::index);
        if (partition == actual->partitions().end())
            return std::unexpected{transaction_error(
                "post-write validation lost a planned partition")};
        ...
        if (actual_ids != expected_ids)
            return std::unexpected{transaction_error(
                "post-write SFS record set differs from the transaction plan")};

        const auto compare_payload =
            [&](SfsId id, std::span<const std::byte> payload) -> Result<void> {
            auto written = actual->read_record_data(
                partition->index, id, 64U * 1024U * 1024U, cancellation);
            if (!written)
                return std::unexpected{written.error()};
            if (!std::ranges::equal(*written, payload))
                return std::unexpected{
                    transaction_error("post-write object payload differs from "
                                      "the transaction plan")};
            return {};
        };
        if (expected.root_payload) {
            if (auto compared =
                    compare_payload(SfsId{1}, *expected.root_payload);
                !compared)
                return compared;
        }
        ...
        for (const auto &[id, inserted] : expected.inserted) {
            if (auto compared = compare_payload(id, inserted.payload);
                !compared)
                return compared;
        }
        ...
        if (*bitmap != expected.bitmap)
            return std::unexpected{
                transaction_error("post-write allocation bitmap differs from "
                                  "the transaction plan")};
    }
    return {};
}
...
open_transaction_state(const std::filesystem::path &source_path,
                       const CancellationToken &cancellation,
    ...
    for (const auto &object : state.catalog.objects)
        object_ids.emplace(object.key,
                           std::pair{object.partition, object.sfs_id});
    for (const auto &relationship : state.graph.relationships) {
        if (relationship.quality != RelationshipQuality::known ||
        ...
        if (source != object_ids.end() && target != object_ids.end() &&
            ...
            state.known_edges.emplace_back(source->second.first,
    ...
    for (const auto &partition : state.container.partitions()) {
        if (!partition.allocation.reconstructed_not_stored.empty() ||
            ...
            return std::unexpected{transaction_error(
                "source allocation cannot safely support alteration")};
        }
        const auto bitmap_size = (partition.cluster_count + 7U) / 8U;
        const auto bitmap_offset =
            (static_cast<std::uint64_t>(partition.start_sector) +
        ...
        state.partitions.emplace(partition.index.value,
                                 std::move(mutable_partition));
    }
    return state;
}

Result<void>
publish(const TransactionState &state, const std::filesystem::path &output_path,
        const CancellationToken &cancellation, bool overwrite = false,
        const std::function<Result<void>(const std::filesystem::path &)>
            &temporary_validator = {}) {
    ...
    auto temporary_result =
        copy_to_unique_temporary(state.container.source_path(), output_path);
    if (!temporary_result)
    ...
    for (const auto &[index, item] : state.partitions) {
        ...
        for (const auto id : item.deleted) {
            ...
            if (auto written = write_bytes(
                    output, source_record->record_offset.value, zero);
                !written) {
                output.close();
                cleanup();
                return written;
        ...
        if (item.root_index && item.root_payload) {
            const auto *root = record(partition, SfsId{1});
            if (auto written = write_bytes(output, root->record_offset.value,
                                           *item.root_index);
                !written) {
                output.close();
                cleanup();
            ...
            const auto payload_offset =
                ...
                512U;
            if (auto written =
                    write_bytes(output, payload_offset, *item.root_payload);
                !written) {
                output.close();
                cleanup();
                return written;
        ...
        for (const auto &[id, changed] : item.changed) {
            ...
            if (source_record == nullptr) {
                ...
                cleanup();
                return std::unexpected{transaction_error(
                    "changed record has no source index location")};
            }
            if (auto written =
                    write_bytes(output, source_record->record_offset.value,
                                changed.raw_index);
                !written) {
                output.close();
                cleanup();
            ...
            for (const auto &extent : changed.extents) {
                const auto capacity =
                    static_cast<std::size_t>(extent.cluster_count) * 1024U;
                const auto count =
                    std::min(capacity, changed.payload.size() - payload_offset);
                const auto absolute =
                    (static_cast<std::uint64_t>(partition.start_sector) +
                    ...
                    512U;
                if (auto written =
                        write_bytes(output, absolute,
                                    std::span{changed.payload}.subspan(
            ...
            if (auto written =
                    write_continuation_lists(output, partition, changed);
                !written) {
                output.close();
                cleanup();
                return written;
        ...
        for (const auto &[id, inserted] : item.inserted) {
            const auto index_offset =
                index_base + (id.value / 14U) * 1024U + (id.value % 14U) * 72U;
            if (auto written =
                    write_bytes(output, index_offset, inserted.raw_index);
                !written) {
                output.close();
                cleanup();
                return written;
            ...
            for (const auto &extent : inserted.extents) {
                const auto capacity =
                    static_cast<std::size_t>(extent.cluster_count) * 1024U;
                const auto count = std::min(capacity, inserted.payload.size() -
                                                          payload_offset);
                const auto absolute =
                    (static_cast<std::uint64_t>(partition.start_sector) +
                     static_cast<std::uint64_t>(extent.cluster_offset) *
                ...
                if (auto written =
                        write_bytes(output, absolute,
                                    std::span{inserted.payload}.subspan(
                                        payload_offset, count));
                    !written) {
                    output.close();
                    cleanup();
            ...
            if (auto written =
                    write_continuation_lists(output, partition, inserted);
                !written) {
                output.close();
                cleanup();
                return written;
        ...
        const auto bitmap_offset =
            (static_cast<std::uint64_t>(partition.start_sector) +
             static_cast<std::uint64_t>(partition.bitmap_cluster) *
                 partition.sectors_per_cluster) *
            512U;
        if (auto written = write_bytes(output, bitmap_offset, item.bitmap);
            !written) {
            output.close();
            cleanup();
            return written;
        }
        const auto mirror_offset =
            static_cast<std::uint64_t>(partition.start_sector) * 512U + 2048U;
        if (auto written =
                write_bytes(output, mirror_offset,
                            std::span{item.bitmap}.first(std::min<std::size_t>(
                                512, item.bitmap.size())));
            !written) {
            output.close();
            cleanup();
            return written;
    ...
    if (auto published = publish_temporary(temporary, output_path, overwrite);
        !published) {
        cleanup();
...
bool has_action(const PlannedPackageObject &object,
                PackageImportObjectAction action) {
    return std::ranges::contains(object.actions, action);
}

const PackageNode *package_node(const PortablePackage &package,
                                std::string_view node_id) {
    const auto found =
        std::ranges::find(package.nodes, node_id, &PackageNode::node_id);
...
const PlannedPackageObject *planned_node(const PackageImportPlan &plan,
                                         ...
                                         std::string_view node_id) {
    const auto found =
        std::ranges::find_if(plan.objects, [&](const auto &candidate) {
            return candidate.package_index == owner.package_index &&
                   candidate.root_index == owner.root_index &&
                   candidate.node_id == node_id &&
                   candidate.partition_index == owner.partition_index &&
                   candidate.group_name == owner.group_name &&
                   candidate.volume_name == owner.volume_name &&
                   candidate.raw_group == owner.raw_group &&
                   candidate.raw_volume == owner.raw_volume;
        });
    return found == plan.objects.end() ? nullptr : &*found;
}

Result<package_internal::PackageNodeRelocationContext>
relocation_context(const PortablePackage &package,
    ...
    for (const auto &edge : package.relationships) {
        ...
        if (target == nullptr) {
            return std::unexpected{transaction_error(
                "package import plan omits a relationship target action")};
        }
        context.edge_target_names.emplace(edge.edge_id,
                                          target->destination_name);
        if (target->target_link_id)
            context.edge_target_link_ids.emplace(edge.edge_id,
...
Result<std::string>
normalized_payload_digest(std::span<const std::byte> payload) {
    auto decoded = decode_object(payload);
    if (!decoded)
        return std::unexpected{decoded.error()};
    auto profile =
        package_internal::build_relocation_profile(*decoded, payload);
    if (!profile)
        return std::unexpected{profile.error()};
    return package_internal::hex_digest(
        package_internal::sha256(profile->normalized_payload));
}
...
package_iso_raw_scope(const ObjectSnapshot &snapshot) {
    ...
    if (separator == std::string::npos ||
        path.find('/', separator + 1U) != std::string::npos)
        return std::nullopt;
    return std::pair{path.substr(0, separator), path.substr(separator + 1U)};
}

Result<void>
validate_flat_package_result(const std::filesystem::path &temporary,
    ...
    if (media->kind() != plan.target_kind)
        return std::unexpected{transaction_error(
            "package result reopened as the wrong media kind")};
    auto catalog =
        build_object_catalog(*media, 64U * 1024U * 1024U, cancellation);
    if (!catalog)
        return std::unexpected{catalog.error()};
    ...
    for (const auto &object : plan.objects) {
        ...
        for (const auto &snapshot : catalog->objects) {
            ...
            const auto scope_matches =
                ...
                (scope && scope->first == object.raw_group &&
                 scope->second == object.raw_volume);
        ...
        if (matches.size() != 1U) {
            return std::unexpected{transaction_error(
                "post-write package object is not unique in its media scope")};
        }
        auto normalized =
            normalized_payload_digest(matches.front()->raw_payload);
        if (!normalized)
            return std::unexpected{normalized.error()};
        if (*normalized != object.normalized_sha256)
            return std::unexpected{transaction_error(
                "post-write package object changed identity")};
        if (object.target_link_id) {
            const auto *sample =
                std::get_if<CurrentSmpl>(&matches.front()->object.payload);
            if (sample == nullptr ||
                sample->link_id.value != *object.target_link_id) {
                return std::unexpected{transaction_error(
                    "post-write SMPL link ID differs from the import plan")};
            }
        }
        if (object.object_type == "SBNK") {
            const auto *bank =
                std::get_if<CurrentSbnk>(&matches.front()->object.payload);
            if (bank == nullptr ||
                bank->linked_program_numbers != object.target_program_numbers ||
                (((bank->sample_flags & 1U) != 0U) != object.target_grouped)) {
                return std::unexpected{
                    transaction_error("post-write SBNK graph metadata differs "
    ...
    for (const auto &owner : plan.objects) {
        ...
        for (const auto &edge : package.relationships) {
            ...
            const auto *target_plan =
                planned_node(plan, owner, edge.target_node_id);
            if (target_plan == nullptr)
                return std::unexpected{
                    transaction_error("post-write edge has no target action")};
            const auto source = actual_by_action.find(owner.action_id);
            const auto target = actual_by_action.find(target_plan->action_id);
            if (source == actual_by_action.end() ||
                target == actual_by_action.end())
                return std::unexpected{
                    transaction_error("post-write edge endpoint is missing")};
            const auto relationships = graph.children(source->second->key);
            if (std::ranges::find_if(
                    relationships, [&](const Relationship *actual) {
                        return actual->type == edge.role &&
                               actual->quality == RelationshipQuality::known &&
                               actual->target_key &&
                               *actual->target_key == target->second->key;
                    }) == relationships.end()) {
                return std::unexpected{transaction_error(
                    std::format("post-write {} relationship from {} to {} "
                                "differs from the package plan",
                                edge.role, owner.destination_name,
                                target_plan->destination_name))};
            }
        }
    }
    return {};
...
Result<PackageImportReport> apply_fat12_package_import(
    ...
    if (media->kind() != MediaKind::fat12_floppy)
        return std::unexpected{
            transaction_error("FAT12 package target changed media kind")};
    ...
    for (const auto &object : *media_objects) {
        ...
        prepared.objects.push_back({object.decoded.header.type,
    ...
    for (const auto &object : plan.objects) {
        ...
        if (!object.canonical_action_id) {
            ...
            if (updated_physical_objects.insert(physical_key).second &&
                ...
                if (object.existing_object_key) {
                    const auto existing =
                        object_indices.find(*object.existing_object_key);
                    if (existing == object_indices.end())
                        return std::unexpected{transaction_error(
                            "planned FAT12 reused object is absent")};
                    prepared.objects[existing->second] = {
                        decoded->header.type, object.destination_name,
                        std::move(*payload)};
                } else {
                    prepared.objects.push_back({decoded->header.type,
        ...
        ++completed;
        if (progress) {
            progress->report({ProgressPhase::writing, completed,
                              plan.objects.size(),
                              "rebuilding FAT12 portable package graph",
                              output_path.string()});
        }
    }
    std::ranges::sort(
        prepared.objects, [](const auto &left, const auto &right) {
            return std::tuple{left.type, left.name, left.payload.size()} <
                   std::tuple{right.type, right.name, right.payload.size()};
        });
    std::ranges::sort(prepared.retained_files, {},
                      &detail::PreparedMediaFile::path);
    const auto validator = [&](const std::filesystem::path &temporary) {
        return validate_flat_package_result(temporary, packages, plan,
                                            cancellation);
    };
    auto written = detail::write_prepared_media_image(
        prepared, output_path, overwrite, cancellation, validator);
    if (!written)
        return std::unexpected{written.error()};
...
Result<PackageImportReport> apply_iso9660_package_import(
    ...
    if (media->kind() != MediaKind::iso9660)
        return std::unexpected{
            transaction_error("ISO9660 package target changed media kind")};
    auto media_objects = media->objects(64U * 1024U * 1024U, cancellation);
    if (!media_objects)
        return std::unexpected{media_objects.error()};
    ...
    for (const auto &[raw_path, label] : iso.volume_labels()) {
        ...
        if (separator != std::string::npos)
            volume_labels.emplace(std::pair{raw_path.substr(0, separator),
                                            raw_path.substr(separator + 1U)},
                                  label);
    }
    std::map<std::pair<std::string, std::string>, std::size_t> volume_indices;
    std::map<std::string, std::size_t, std::less<>>
        existing_group_volume_counts;
    for (const auto &file : iso.files()) {
        if (!file.is_directory)
            continue;
        const auto separator = file.path.find('/');
        if (separator == std::string::npos ||
            file.path.find('/', separator + 1U) != std::string::npos)
            continue;
        const auto scope = std::pair{file.path.substr(0, separator),
                                     file.path.substr(separator + 1U)};
        ...
        if (group == group_labels.end() || volume == volume_labels.end())
            return std::unexpected{transaction_error(
                "cataloged ISO9660 volume labels changed after planning")};
        ...
        prepared.iso_volumes.push_back(
            {scope.first, group->second, scope.second, volume->second, {}});
    }
    for (const auto &destination : plan.destinations) {
        const auto scope =
            std::pair{destination.raw_group, destination.raw_volume};
        if (volume_indices.contains(scope))
            continue;
        if (!destination.create)
            return std::unexpected{
                transaction_error("planned ISO9660 destination is absent")};
        volume_indices.emplace(scope, prepared.iso_volumes.size());
    ...
    for (const auto &object : *media_objects) {
        ...
        if (volume == volume_indices.end())
            return std::unexpected{transaction_error(
                "existing ISO9660 object has no cataloged raw volume")};
        auto &objects = prepared.iso_volumes[volume->second].objects;
        object_indices.emplace(object.key,
                               std::pair{volume->second, objects.size()});
        objects.push_back({object.decoded.header.type,
        ...
        if (separator != std::string::npos)
            generated_files.insert(object.logical_path.substr(0, separator) +
                                   "/0000");
    }
    for (const auto &file : iso.files()) {
    ...
    for (const auto &object : plan.objects) {
        ...
        if (!object.canonical_action_id) {
            ...
            if (updated_physical_objects.insert(physical_key).second &&
                ...
                if (node == nullptr)
                    return std::unexpected{
                        transaction_error("ISO9660 package node is missing")};
                auto context = relocation_context(package, plan, object);
                if (!context)
                    return std::unexpected{context.error()};
                auto payload = package_internal::relocate_package_node(
        ...
        if (progress) {
            progress->report({ProgressPhase::writing, completed,
                              plan.objects.size(),
                              "rebuilding ISO9660 portable package graph",
                              output_path.string()});
        }
    }
    std::ranges::sort(prepared.iso_volumes,
                      [](const auto &left, const auto &right) {
                          return std::tie(left.raw_group, left.raw_volume) <
                                 std::tie(right.raw_group, right.raw_volume);
                      });
    prepared.objects.clear();
    for (auto &volume : prepared.iso_volumes) {
        std::ranges::sort(
            volume.objects, [](const auto &left, const auto &right) {
                return std::tuple{left.type, left.name, left.payload.size()} <
                       std::tuple{right.type, right.name, right.payload.size()};
            });
        prepared.objects.insert(prepared.objects.end(), volume.objects.begin(),
                                volume.objects.end());
    }
    std::ranges::sort(prepared.retained_files, {},
                      &detail::PreparedMediaFile::path);
    const auto validator =
        ...
        if (plan.allocation.empty())
            return std::unexpected{transaction_error(
                "ISO9660 package plan has no projected allocation")};
        std::error_code size_error;
        const auto actual_size =
            std::filesystem::file_size(temporary, size_error);
        ...
        if (size_error || actual_size != expected_size ||
            ...
            return std::unexpected{transaction_error(
                size_error
                    ? std::format("cannot inspect rebuilt ISO9660 size: {}",
                                  size_error.message())
...
Result<void> validate_package_result(const std::filesystem::path &temporary,
    ...
    if (media->kind() != MediaKind::sfs)
        return std::unexpected{
            transaction_error("package result is not an SFS image")};
    auto catalog =
        build_object_catalog(*media, 64U * 1024U * 1024U, cancellation);
    if (!catalog)
        return std::unexpected{catalog.error()};
    ...
    for (const auto &object : plan.objects) {
        ...
        if (matches.size() != 1U) {
            return std::unexpected{
                transaction_error("post-write package object does not match "
        ...
        if (*normalized != object.normalized_sha256) {
            return std::unexpected{transaction_error(
                "post-write package object changed normalized identity")};
        }
        if (object.target_link_id) {
            const auto *sample =
                std::get_if<CurrentSmpl>(&matches.front()->object.payload);
...
Result<std::string> serialize_alteration_manifest_template() {
    try {
        ...
        return std::unexpected{transaction_error(
            std::string{"could not serialize alteration manifest template: "} +
            error.what())};
    }
}
...
write_alteration_manifest_template(const std::filesystem::path &output_path,
    ...
    if (!overwrite && std::filesystem::exists(output_path, filesystem_error)) {
        return std::unexpected{
            make_error(ErrorCode::io_open_failed, ErrorCategory::io,
                       "refusing to replace existing alteration manifest: " +
    ...
    if (filesystem_error) {
        return std::unexpected{make_error(
            ErrorCode::io_open_failed, ErrorCategory::io,
...
parse_alteration_manifest(std::string_view json,
    ...
    try {
        ...
        if (*version != "1.0")
            return std::unexpected{
                transaction_error("manifest schema version must be 1.0")};
        if (!root["operations"].is_array() || root["operations"].empty())
            return std::unexpected{transaction_error(
                "manifest.operations must be a non-empty array")};
        AlterationManifest result{*version, {}};
        std::set<std::string> seen;
        for (std::size_t index = 0; index < root["operations"].size();
             ++index) {
            const auto &row = root["operations"][index];
            const auto context =
                "manifest.operations[" + std::to_string(index) + "]";
            if (!row.is_object())
                return std::unexpected{
                    transaction_error(context + " must be an object")};
            ...
            if (!seen.insert(*id).second)
                return std::unexpected{
                    transaction_error("duplicate operation id")};
            if (*type != "delete_volume" && *type != "insert_volume" &&
                *type != "delete_sbnk" && *type != "insert_sbnk" &&
                *type != "insert_waveform" && *type != "delete_waveform" &&
                *type != "delete_program" && *type != "insert_program" &&
                *type != "delete_sbac" && *type != "insert_sbac" &&
                *type != "rename_waveform" && *type != "rename_sbnk" &&
                *type != "rename_sbac") {
                return std::unexpected{
                    transaction_error("operation type is not implemented by "
            ...
            if (row["partition_index"].is_number_integer()) {
                ...
                if (value < 0 || value > 7)
                    return std::unexpected{
                        transaction_error("partition index must be 0..7")};
                selector = PartitionIndex{static_cast<std::uint8_t>(value)};
            } else if (row["partition_index"].is_object() &&
                       row["partition_index"].size() == 1U &&
                       row["partition_index"].contains("operation_ref")) {
                auto reference =
                    required_text(row["partition_index"], "operation_ref",
                                  context + ".partition_index");
                if (!reference)
                    return std::unexpected{reference.error()};
                if (!seen.contains(*reference))
                    return std::unexpected{transaction_error(
                        "operation_ref must name an earlier operation")};
                selector = OperationReference{*reference};
            } else
                return std::unexpected{
                    transaction_error("partition selector is invalid")};
            ...
            if (*type == "delete_volume") {
                if (auto valid = exact_fields(
                        row, {"id", "type", "partition_index", "volume_name"},
                        context);
                    !valid)
                    return std::unexpected{valid.error()};
                ...
                if (auto valid = exact_fields(
                        row, {"id", "type", "partition_index", "volume"},
                        context);
                    !valid)
                    return std::unexpected{valid.error()};
                Json wrapper{
                    ...
                    {"partitions",
                     Json::array({
                         {{"name", "AXK ALTER"},
                          {"volumes", Json::array({row["volume"]})}},
                     })},
                };
                auto parsed =
                    parse_hds_build_manifest(wrapper.dump(), base_directory);
                if (!parsed)
                ...
                if (auto valid =
                        exact_fields(row,
                                     {"id", "type", "partition_index",
                                      "volume_name", "sample_bank_name"},
                                     context);
                    !valid) {
                    return std::unexpected{valid.error()};
                }
                ...
                if (!bank.is_object()) {
                    return std::unexpected{transaction_error(
                        context + ".sample_bank must be an object")};
                }
                const std::set<std::string> required{
                    "name", "waveform_name", "root_key", "key_low", "key_high"};
                const std::set<std::string> optional{"right_waveform_name",
                                                     "level"};
                for (const auto &field : required) {
                    if (!bank.contains(field)) {
                        return std::unexpected{transaction_error(
                            context + ".sample_bank is missing field " +
                            field)};
                    }
                }
                for (const auto &[field, unused] : bank.items()) {
                    ...
                    if (!required.contains(field) &&
                        ...
                        return std::unexpected{transaction_error(
                            context + ".sample_bank has unknown field " +
                            field)};
                    }
                }
                ...
                auto waveform =
                    object_name(bank, "waveform_name", bank_context);
                auto root_key =
                    midi_value(bank, "root_key", bank_context, 0U, true);
                auto key_low =
                    midi_value(bank, "key_low", bank_context, 0U, true);
                auto key_high =
                    midi_value(bank, "key_high", bank_context, 0U, true);
                auto level =
                    midi_value(bank, "level", bank_context, 100U, false);
                if (!name)
                ...
                if (*key_high < *key_low) {
                    return std::unexpected{transaction_error(
                        bank_context + ".key_high must not be below key_low")};
                }
                SampleBankSpec spec;
                ...
                if (bank.contains("right_waveform_name")) {
                    auto right =
                        object_name(bank, "right_waveform_name", bank_context);
                    if (!right)
                ...
                if (*old_name == *new_name)
                    return std::unexpected{
                        transaction_error("new_waveform_name must differ")};
                operation.volume_name = std::move(*volume);
                ...
                auto new_name =
                    object_name(row, "new_sample_bank_name", context);
                if (!volume)
                    return std::unexpected{volume.error()};
                if (!old_name)
                    return std::unexpected{old_name.error()};
                ...
                if (*old_name == *new_name)
                    return std::unexpected{
                        transaction_error("new_sample_bank_name must differ")};
                operation.volume_name = std::move(*volume);
                ...
                if (auto valid =
                        exact_fields(row,
                                     {"id", "type", "partition_index",
                                      "volume_name", "sample_bank_group_name",
                ...
                auto volume = required_text(row, "volume_name", context);
                auto old_name =
                    object_name(row, "sample_bank_group_name", context);
                auto new_name =
                    object_name(row, "new_sample_bank_group_name", context);
                if (!volume)
                    return std::unexpected{volume.error()};
                if (!old_name)
                    return std::unexpected{old_name.error()};
                ...
                if (*old_name == *new_name)
                    return std::unexpected{transaction_error(
                        "new_sample_bank_group_name must differ")};
                operation.volume_name = std::move(*volume);
                ...
                if (auto valid =
                        exact_fields(group, {"name", "member_sample_banks"},
                                     context + ".sample_bank_group");
                    !valid)
                    return std::unexpected{valid.error()};
                auto name =
                    object_name(group, "name", context + ".sample_bank_group");
                if (!name)
                    return std::unexpected{name.error()};
                if (!group["member_sample_banks"].is_array() ||
                    group["member_sample_banks"].empty() ||
                    group["member_sample_banks"].size() > 3U)
                    return std::unexpected{transaction_error(
                        "member_sample_banks must contain 1..3 names")};
                SampleBankGroupSpec spec;
                spec.name = *name;
                for (std::size_t member_index = 0;
                     member_index < group["member_sample_banks"].size();
                     ++member_index) {
                    Json wrapper{
                        {"name", group["member_sample_banks"][member_index]}};
                    auto member = object_name(
                        wrapper, "name",
                        context + ".sample_bank_group.member_sample_banks");
                    if (!member)
                        return std::unexpected{member.error()};
                    if (std::ranges::contains(spec.member_sample_banks,
                                              *member))
                        return std::unexpected{transaction_error(
                            "member_sample_banks must be distinct")};
                ...
                if (auto valid =
                        exact_fields(program, {"number", "assignments"},
                                     context + ".program");
                    !valid)
                    return std::unexpected{valid.error()};
                auto number =
                    program_value(program, "number", context + ".program");
                if (!number)
                    return std::unexpected{number.error()};
                if (!program["assignments"].is_array() ||
                    program["assignments"].size() != 2U)
                    return std::unexpected{transaction_error(
                        "Program requires exactly two assignments")};
                ProgramSpec spec;
                spec.number = *number;
                constexpr std::array<std::string_view, 2> target_fields{
                    "sample_bank_group", "sample_bank"};
                for (std::size_t assignment_index = 0; assignment_index < 2U;
                     ++assignment_index) {
                    const auto &assignment =
                        program["assignments"][assignment_index];
                    const auto field = target_fields[assignment_index];
                    if (auto valid =
                            exact_fields(assignment, {field, "receive_channel"},
                                         context + ".program.assignments");
                        !valid)
                        return std::unexpected{valid.error()};
                    auto target = object_name(assignment, field,
                                              context + ".program.assignments");
                    if (!target)
                        return std::unexpected{target.error()};
                    auto channel =
                        midi_value(assignment, "receive_channel",
                                   context + ".program.assignments", 0U, true);
                    const auto expected_channel =
                        static_cast<std::uint8_t>(assignment_index + 1U);
                    if (!channel || *channel != expected_channel)
                        return std::unexpected{transaction_error(
                            "Program assignments must be SBAC/channel 1 then "
                            "SBNK/channel 2")};
                    spec.assignments.push_back(
                        {assignment_index == 0U ? "SBAC" : "SBNK",
                         std::move(*target), *channel});
                }
                operation.volume_name = std::move(*volume);
                ...
                if (!audio.is_object()) {
                    return std::unexpected{transaction_error(
                        context + ".audio must be an object")};
                }
                const std::set<std::string> required{"path", "waveform_names",
                                                     "root_key"};
                const std::set<std::string> optional{"target_sample_rate"};
                for (const auto &field : required) {
                    if (!audio.contains(field)) {
                        return std::unexpected{transaction_error(
                            context + ".audio is missing field " + field)};
                    }
                }
                for (const auto &[field, unused] : audio.items()) {
                    ...
                    if (!required.contains(field) &&
                        !optional.contains(field)) {
                        return std::unexpected{transaction_error(
                            context + ".audio has unknown field " + field)};
                    }
                }
                if (!audio["waveform_names"].is_array() ||
                    (audio["waveform_names"].size() != 1U &&
                     audio["waveform_names"].size() != 2U)) {
                    return std::unexpected{transaction_error(
                        context +
                        ".audio.waveform_names must contain one or two names")};
                }
                InsertWaveformSpec spec;
                for (std::size_t name_index = 0;
                     name_index < audio["waveform_names"].size();
                     ++name_index) {
                    Json wrapper{{"name", audio["waveform_names"][name_index]}};
                    auto name =
                        object_name(wrapper, "name",
                                    context + ".audio.waveform_names[" +
                ...
                if (!audio_path)
                    return std::unexpected{transaction_error(
                        context + ".audio.path must be valid UTF-8")};
                ...
                if (audio.contains("target_sample_rate")) {
                    ...
                    const auto rate =
                        audio["target_sample_rate"].get<std::int64_t>();
                    if (rate <= 0 ||
                        rate > std::numeric_limits<std::uint32_t>::max()) {
                        return std::unexpected{transaction_error(
                            context +
                            ".audio.target_sample_rate is out of range")};
                    }
        ...
        return std::unexpected{transaction_error(
            std::string{"invalid alteration JSON: "} + error.what())};
    }
}
...
alter_hds(const std::filesystem::path &source_path,
    ...
    if (output_path &&
        ...
        return std::unexpected{
            transaction_error("alteration output must differ from source")};
    }
    ...
    if (progress)
        progress->report({ProgressPhase::writing,
                          0U,
                          manifest.operations.size(),
                          "planning alteration",
                          {}});
    using OperationHandler = Result<OperationReport> (*)(
        TransactionState &, const OperationView &, const CancellationToken &);
    constexpr std::array<OperationHandler,
                         std::variant_size_v<AlterationOperationData>>
        handlers{
            delete_volume,   insert_volume,   delete_sbnk,     insert_sbnk,
            insert_waveform, delete_waveform, rename_waveform, rename_sbnk,
            delete_sbac,     insert_sbac,     rename_sbac,     delete_program,
            insert_program,
        };
    for (std::size_t operation_index = 0;
         operation_index < manifest.operations.size(); ++operation_index) {
        const auto &typed_operation = manifest.operations[operation_index];
        const auto operation = operation_view(typed_operation);
        auto report = handlers[typed_operation.data.index()](state, operation,
                                                             cancellation);
        if (!report)
        ...
        if (progress)
            progress->report(
                {ProgressPhase::writing, operation_index + 1U,
                 manifest.operations.size(), operation.type,
...
Result<PackageImportReport> apply_package_import(
    const std::filesystem::path &target_path,
    std::span<const PortablePackage> packages, const PackageImportPlan &plan,
    const std::filesystem::path &output_path, bool overwrite,
    const CancellationToken &cancellation, ProgressSink *progress) {
    try {
        if (std::filesystem::absolute(target_path).lexically_normal() ==
            std::filesystem::absolute(output_path).lexically_normal()) {
            return std::unexpected{transaction_error(
                "package import output must differ from its source image")};
        }
        if (!overwrite && std::filesystem::exists(output_path)) {
            return std::unexpected{
                make_error(ErrorCode::io_open_failed, ErrorCategory::io,
        ...
        if (!plan.valid())
            return std::unexpected{transaction_error(
                "a conflicting package import plan cannot apply")};
        if (plan.target_kind != MediaKind::sfs &&
            plan.target_kind != MediaKind::fat12_floppy &&
            plan.target_kind != MediaKind::iso9660)
            return std::unexpected{transaction_error(
                "package import target adapter is unsupported")};
        if (packages.size() != plan.package_ids.size())
            return std::unexpected{
                transaction_error("package import inputs do not match the "
                                  "planned package count")};
        for (std::size_t index = 0; index < packages.size(); ++index) {
            if (const auto verified = verify_portable_package(packages[index]);
                !verified)
                return std::unexpected{verified.error()};
            if (packages[index].package_id != plan.package_ids[index]) {
                return std::unexpected{transaction_error(
                    "package import input identity differs from the plan")};
            }
        }
        ...
        if (*snapshot != plan.target_snapshot_id)
            return std::unexpected{transaction_error(
                "package import plan is stale for this target")};

        if (plan.target_kind == MediaKind::fat12_floppy) {
            return apply_fat12_package_import(target_path, packages, plan,
                                              output_path, overwrite,
                                              cancellation, progress);
        }
        if (plan.target_kind == MediaKind::iso9660) {
            return apply_iso9660_package_import(target_path, packages, plan,
                                                output_path, overwrite,
                                                cancellation, progress);
        }

        auto opened =
            open_transaction_state(target_path, cancellation, progress);
        if (!opened)
            return std::unexpected{opened.error()};
        auto state = std::move(*opened);
        ...
        if (*confirmed_snapshot != plan.target_snapshot_id)
            return std::unexpected{
                transaction_error("package import target changed before "
        ...
        for (const auto &destination : plan.destinations) {
            ...
            if (const auto checked = cancellation.check(); !checked)
                return std::unexpected{checked.error()};
            OperationView operation;
            operation.id = std::format("package-destination-{}-{}",
            ...
            if (actual_ids != destination.infrastructure_sfs_ids ||
                inserted->allocated_clusters !=
                    destination.infrastructure_clusters) {
                return std::unexpected{
                    transaction_error("actual destination volume allocation "
            ...
            if (progress) {
                progress->report({ProgressPhase::writing, completed,
                                  plan.objects.size(),
                                  "creating package destination volume",
                                  output_path.string()});
            }
        }
        ...
        for (const auto &object : plan.objects) {
            ...
            if (!has_action(object, PackageImportObjectAction::insert)) {
                if (has_action(object, PackageImportObjectAction::reuse) &&
                    ...
                    if (!object.target_sfs_id || object.object_type != "SBNK") {
                        return std::unexpected{
                            transaction_error("planned reused relocation is "
                                              "not a fixed SBNK object")};
                    }
                    const auto physical_key = std::pair{object.partition_index,
                                                        *object.target_sfs_id};
                    if (updated_reused_objects.emplace(physical_key).second) {
                        const auto &package = packages[object.package_index];
                        const auto *node =
                            package_node(package, object.node_id);
                        if (node == nullptr)
                            return std::unexpected{transaction_error(
                                "package import action node is missing")};
                        auto context =
                            relocation_context(package, plan, object);
                        if (!context)
                            return std::unexpected{context.error()};
                        auto payload = package_internal::relocate_package_node(
                            package, *node, *context);
                        ...
                        if (*normalized != object.normalized_sha256) {
                            return std::unexpected{transaction_error(
                                "relocated reused node differs from its "
                                "planned identity")};
                        }
                        const auto partition =
                            state.partitions.find(object.partition_index);
                        if (partition == state.partitions.end()) {
                            return std::unexpected{transaction_error(
                                "package import partition is invalid")};
                        }
                        if (auto replaced = replace_fixed_object_payload(
                                state, partition->second,
                ...
                if (progress) {
                    progress->report({ProgressPhase::writing, completed,
                                      plan.objects.size(),
                                      "reusing portable package object",
                                      output_path.string()});
                }
                continue;
            }
            ...
            if (node == nullptr)
                return std::unexpected{
                    transaction_error("package import action node is missing")};
            auto context = relocation_context(package, plan, object);
            if (!context)
                return std::unexpected{context.error()};
            auto payload = package_internal::relocate_package_node(
            ...
            if (*normalized != object.normalized_sha256) {
                return std::unexpected{
                    transaction_error("relocated package node differs from its "
                                      "planned identity")};
            }
            const auto partition =
                state.partitions.find(object.partition_index);
            if (partition == state.partitions.end() || !object.target_sfs_id)
                return std::unexpected{transaction_error(
                    "package import partition or SFS ID is invalid")};
            auto &mutable_partition = partition->second;
            auto allocated = allocate_record(
                mutable_partition, std::move(*payload), PayloadKind::object,
                SfsId{*object.target_sfs_id});
            if (!allocated)
                return std::unexpected{allocated.error()};
            const auto inserted =
                mutable_partition.inserted.find(SfsId{*object.target_sfs_id});
            if (inserted == mutable_partition.inserted.end())
                return std::unexpected{transaction_error(
                    "package insertion did not reserve its record")};
            ...
            if (payload_clusters != object.payload_clusters ||
                inserted->second.continuation_clusters.size() !=
                    object.continuation_clusters ||
                allocated->second !=
                    object.payload_clusters + object.continuation_clusters) {
                return std::unexpected{transaction_error(
                    "actual package allocation differs from the import plan")};
            }
            auto directory =
                volume_category(state, mutable_partition, object.volume_name,
                                object.object_type, cancellation);
            if (!directory)
                return std::unexpected{directory.error()};
            if (auto appended = append_directory_entry(
                    state, mutable_partition, *directory,
                    SfsId{*object.target_sfs_id}, object.destination_name,
                    cancellation);
                !appended) {
                return std::unexpected{appended.error()};
            }
            ...
            if (progress) {
                progress->report({ProgressPhase::writing, completed,
                                  plan.objects.size(),
                                  "importing portable package object",
                                  output_path.string()});
            }
        }
        ...
        for (const auto &owner : plan.objects) {
            ...
            for (const auto &edge : package.relationships) {
                ...
                const auto *target =
                    planned_node(plan, owner, edge.target_node_id);
                if (target == nullptr || !owner.target_sfs_id ||
                    !target->target_sfs_id)
                    return std::unexpected{transaction_error(
                        "package relationship lacks a planned SFS endpoint")};
                const auto tuple = std::tuple{owner.partition_index,
                ...
                if (added_edges.emplace(tuple).second &&
                    !std::ranges::contains(
                        ...
                        std::tuple{PartitionIndex{owner.partition_index},
                                   SfsId{*owner.target_sfs_id},
                                   SfsId{*target->target_sfs_id}})) {
                    state.known_edges.emplace_back(
                        PartitionIndex{owner.partition_index},
                        SfsId{*owner.target_sfs_id},
                        SfsId{*target->target_sfs_id});
                }
            }
        ...
        const auto validator = [&](const std::filesystem::path &temporary) {
            return validate_package_result(temporary, packages, plan,
                                           cancellation);
        };
        if (auto published =
                publish(state, output_path, cancellation, overwrite, validator);
            !published) {
            return std::unexpected{published.error()};
        }
        ...
        return std::unexpected{transaction_error(
            std::string{"package import callback failed: "} + error.what())};
    } catch (...) {
        return std::unexpected{
            transaction_error("package import callback failed")};
    }
}
