...
Result<MediaDecode> decode_media_object(std::span<const std::byte> bytes) {
    ...
    if (decoded) {
        ...
        if (std::holds_alternative<CurrentSmpl>(decoded->payload)) {
            const auto pcm_end =
                checked_add(decoded->header.header_size,
...
bool f_directory(std::string_view value) {
    return value.size() == 4U && value[0] == 'F' &&
           std::ranges::all_of(value.substr(1), [](unsigned char ch) {
               return std::isdigit(ch) != 0;
           });
}

std::string
lookup_label(const std::vector<std::pair<std::string, std::string>> &labels,
             std::string_view key) {
    const auto found = std::ranges::find(
        labels, key, &std::pair<std::string, std::string>::first);
...
std::string fallback_label(const MediaObject &object,
                           std::span<const std::byte> bytes) {
    if (object.decoded.header.type == ObjectType::prog &&
        bytes.size() >= 0x88U) {
        const auto display = detail::clean_ascii(bytes.subspan(0x78, 16));
        if (!display.empty() &&
            display != std::format("Pgm {}", object.decoded.header.name))
            return display;
        return {};
    }
...
bool yamaha_object_category(std::string_view value) {
    return value == "SMPL" || value == "SBNK" || value == "SBAC" ||
           value == "PROG" || value == "SEQU" || value == "PRF3";
}
...
void add_category_catalog_issue(detail::IsoMenuLabels &result, std::string code,
    ...
    result.validation_issues.push_back({
        std::move(code),
        std::move(message),
        std::format("CD-ROM group '{}', volume '{}', {} objects", group, volume,
                    category),
...
category_catalog_records(std::span<const std::byte> bytes, std::size_t stride,
                         std::size_t name_width, std::size_t target_offset) {
    std::vector<CategoryCatalogRecord> result;
    for (std::size_t offset = 0; offset + stride <= bytes.size();
         offset += stride) {
        const auto record = bytes.subspan(offset, stride);
        if (std::ranges::all_of(
                record, [](std::byte value) { return value == std::byte{}; }))
            continue;
        result.push_back(
            {clean_label(record.subspan(1, name_width)),
...
shifted_category_catalog_records(std::span<const std::byte> bytes) {
    ...
    if (bytes.size() < 28U)
        return result;
    result.push_back({clean_label(bytes.first(13U)),
                      detail::clean_ascii(bytes.subspan(14U, 11U))});
    for (std::size_t offset = 28U; offset + 32U <= bytes.size();
         offset += 32U) {
        const auto record = bytes.subspan(offset, 32U);
        if (std::ranges::all_of(
                record, [](std::byte value) { return value == std::byte{}; }))
            continue;
        result.push_back({clean_label(record.subspan(1U, 16U)),
                          detail::clean_ascii(record.subspan(18U, 11U))});
    }
    return result;
}

std::size_t
category_catalog_target_count(std::span<const CategoryCatalogRecord> records) {
...
Result<IsoMenuLabels>
read_yamaha_iso_menu_labels(const IsoImage &image,
    ...
    for (const auto &[group, group_volumes] : volumes) {
        const auto table = std::ranges::find(
            image.files(), std::format("{}/0000", group), &IsoFile::path);
        ...
        bool disk_name_seen{};
        for (std::size_t offset = 0; offset + 32U <= bytes->size();
             offset += 32U) {
            ...
            if (label == "_DSKNAME") {
                ...
                const auto expected_id =
                    std::format("F{:03}", group_volumes.size() + 1U);
                ...
                if (id != expected_id) {
                    add_group_catalog_issue(
                        ...
                        std::format(
                            "CD-ROM group menu '{}' references group-label "
                            "file '{}' from _DSKNAME; "
                ...
                const auto label_file = std::ranges::find(
                    image.files(), std::format("{}/{}", group, id),
                    &IsoFile::path);
                if (label_file == image.files().end() ||
                    label_file->is_directory) {
                    add_group_catalog_issue(
                ...
                if (label_file->size != 16U) {
                    add_group_catalog_issue(
                        ...
                        std::format(
                            "CD-ROM group menu '{}' uses a {}-byte group-label "
                            "file '{}'; the Yamaha "
            ...
            if (!id.empty() && !label.empty() && group_volumes.contains(id))
                result.volumes.emplace_back(std::format("{}/{}", group, id),
                                            label);
        }
        if (!disk_name_seen) {
            add_group_catalog_issue(
    ...
    for (const auto &directory : image.files()) {
        ...
        if (!directory.is_directory || parts.size() != 3U ||
            !f_directory(parts[1]) || !yamaha_object_category(parts[2])) {
            continue;
        }
        ...
        for (const auto &file : image.files()) {
            ...
            if (!file.is_directory && child_parts.size() == 4U &&
                child_parts[0] == group && child_parts[1] == volume &&
                child_parts[2] == category && f_directory(child_parts[3]) &&
                child_parts[3] != "F000") {
        ...
        const auto catalog =
            std::ranges::find(image.files(), catalog_path, &IsoFile::path);
        if (catalog == image.files().end() || catalog->is_directory) {
            add_category_catalog_issue(
                result, "ISO_YAMAHA_CATEGORY_CATALOG_MISSING", group, volume,
                category,
        ...
        if (!catalog_bytes) {
            add_category_catalog_issue(
                result, "ISO_YAMAHA_CATEGORY_CATALOG_UNREADABLE", group, volume,
                category,
        ...
        const auto records32 =
            category_catalog_records(*catalog_bytes, 32U, 16U, 18U);
        const auto shifted_records =
            shifted_category_catalog_records(*catalog_bytes);
        const auto score32 = category_catalog_target_count(records32);
        const auto shifted_score =
            category_catalog_target_count(shifted_records);
        const auto &records =
            shifted_score > score32 ? shifted_records : records32;
        if (std::max(shifted_score, score32) == 0U) {
            add_category_catalog_issue(
                result, "ISO_YAMAHA_CATEGORY_CATALOG_ROWS_INVALID", group,
                volume, category,
                std::format("{} objects in CD-ROM volume '{}' have a 0000 "
        ...
        for (const auto &record : records) {
            ...
            if (!catalog_targets.insert(id).second) {
                add_category_catalog_issue(
                    result, "ISO_YAMAHA_CATEGORY_CATALOG_TARGET_DUPLICATE",
                    group, volume, category,
            ...
            const auto target =
                std::ranges::find(image.files(), target_path, &IsoFile::path);
            if (target == image.files().end() || target->is_directory) {
                add_category_catalog_issue(
                    result, "ISO_YAMAHA_CATEGORY_OBJECT_MISSING", group, volume,
                    category,
        ...
        for (const auto &filename : object_files) {
            if (catalog_targets.contains(filename))
                continue;
            add_category_catalog_issue(
                result, "ISO_YAMAHA_CATEGORY_OBJECT_UNCATALOGED", group, volume,
                category,
...
FatImage::objects(std::size_t maximum_object_bytes,
...
Result<std::vector<MediaObject>>
IsoImage::objects(std::size_t maximum_object_bytes,
                  const CancellationToken &cancellation) const {
    ...
    for (const auto &file : files_) {
        ...
        const auto file_offset =
            static_cast<std::uint64_t>(file.extent_sector) *
            detail::iso_sector_size;
        if (file_offset > reader_->size() ||
            file.size > reader_->size() - file_offset)
            continue;
        auto bytes = read_file(file, cancellation);
        if (!bytes)
            return std::unexpected{bytes.error()};
        ...
        auto volume_label =
            lookup_label(volume_labels_, std::format("{}/{}", group, volume));
        pending.push_back(
            {{std::format("iso9660:{}", file.path),
              ...
              {group_label.empty() ? group : group_label,
               group_label.empty() ? LabelStatus::raw_identifier
                                   : LabelStatus::confirmed,
               group_label.empty() ? "ISO9660 raw group identifier"
                                   : "Yamaha CD-ROM menu label"},
              {volume_label.empty() ? volume : volume_label,
               volume_label.empty() ? LabelStatus::raw_identifier
                                    : LabelStatus::confirmed,
               volume_label.empty() ? "ISO9660 raw volume identifier"
                                    : "Yamaha CD-ROM menu label"},
              static_cast<std::uint64_t>(file.extent_sector) *
                  detail::iso_sector_size,
              file.size,
    ...
    const std::map<ObjectType, int> priority{{ObjectType::prog, 0},
                                             ...
                                             {ObjectType::smpl, 3},
                                             {ObjectType::sequ, 4}};
    std::map<std::pair<std::string, std::string>, std::vector<PendingObject *>>
        scopes;
    for (auto &item : pending) {
        if (item.object.volume_label.status != LabelStatus::confirmed)
            scopes[{item.object.raw_group, item.object.raw_volume}].push_back(
                &item);
    }
    std::map<std::string, std::set<std::string>> used;
    for (auto &[scope, items] : scopes) {
        std::ranges::sort(items, [&](const PendingObject *left,
                                     const PendingObject *right) {
            return std::tuple{
                       priority.contains(left->object.decoded.header.type)
                           ? priority.at(left->object.decoded.header.type)
                           : 99,
                       left->object.decoded.header.name, left->object.key} <
                   std::tuple{
                       priority.contains(right->object.decoded.header.type)
        ...
        for (auto *item : items) {
            item->object.volume_label = {
                display, LabelStatus::navigation_aid,
...
Result<StandaloneObject>
StandaloneObject::open(std::shared_ptr<const RandomAccessReader> reader,
    ...
    if (!reader || reader->size() > maximum_object_bytes ||
        ...
        return std::unexpected{detail::media_error(
            ErrorCode::io_unsupported_size,
            "standalone object exceeds the configured size limit",
            source_name)};
    }
    auto bytes = detail::read_bytes(
        *reader, 0, static_cast<std::size_t>(reader->size()), {});
    if (!bytes)
        return std::unexpected{bytes.error()};
    if (!detail::object_prefix(*bytes)) {
        return std::unexpected{detail::media_error(
...
Result<StandaloneObject>
StandaloneObject::open(const std::filesystem::path &path,
                       std::size_t maximum_object_bytes) {
    auto reader = FileReader::open(path);
    if (!reader)
        return std::unexpected{reader.error()};
    return open(std::move(*reader), text::path_to_utf8(path),
...
build_object_catalog(const MediaContainer &container,
    ...
    for (auto &object : *objects) {
        ...
        result.objects.push_back({object.key, PartitionIndex{0}, SfsId{id},
                                  object.scope_key, std::move(object.decoded),
                                  std::move(placement),
                                  std::move(object.raw_payload)});
        if (object.decode_issue) {
            result.issues.push_back({"media_object_decode_failed",
                                     render_error(*object.decode_issue),
...
std::string sanitize_path_component(std::string_view value,
    ...
    for (const auto ch : text) {
        ...
        const auto invalid = byte < 0x20U || byte == 0x7fU ||
                             std::string_view{"/:*?\"|"}.contains(ch);
        if (invalid || ch == '_') {
...
StructuredObjectPath structured_object_path(const MediaObject &object) {
    ...
    auto category = sanitize_path_component(
        object_category(object.decoded.header.type), "objects");
...
std::vector<StructuredObjectPath>
structured_object_paths(std::span<const MediaObject> objects) {
    ...
    for (const auto &object : objects) {
        ...
        const auto &volumes =
            raw_volumes.at({detail::upper_ascii(object.group_label.value),
                            detail::upper_ascii(object.volume_label.value)});
        if (volumes.size() > 1U && !object.raw_volume.empty()) {
            auto group =
                sanitize_path_component(object.group_label.value, "objects");
            auto volume = sanitize_path_component(
                std::format("{} ({})", object.volume_label.value,
                            object.raw_volume),
                "volume");
            auto category = sanitize_path_component(
                object_category(object.decoded.header.type), "objects");
            auto name =
                sanitize_path_component(object.decoded.header.name, "unnamed");
            path.relative_path =
                std::filesystem::path{group} / volume / category / name;
        }
