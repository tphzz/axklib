...
Result<void> validate_waveform(const Waveform &waveform) {
  ...
  if ((channels != 1U && channels != 2U) || (width != 1U && width != 2U) ||
    ...
    return std::unexpected{make_error(
        ...
        "waveform format must be mono/stereo, 8/16-bit PCM with a non-zero sample rate")};
  }
...
Result<Waveform> decode_waveform(const Container &container, const ObjectSnapshot &snapshot,
  ...
  return decode_waveform_payload(*decoded, snapshot.key, container.source_path(),
                                 snapshot.partition, snapshot.sfs_id, snapshot.object.header.name,
                                 *payload);
}
...
Result<Waveform> decode_waveform(const MediaObject &object) {
  ...
  return decode_waveform_payload(*decoded, object.key, object.logical_path, PartitionIndex{0},
                                 SfsId{0}, object.decoded.header.name, object.raw_payload);
}
...
Result<std::vector<std::byte>> wav_bytes(const Waveform &waveform) {
  ...
  const auto block =
      static_cast<std::uint16_t>(waveform.format.channels * waveform.format.sample_width_bytes);
...
Result<void> write_wav_atomic(const std::filesystem::path &path, const Waveform &waveform,
                              bool overwrite) {
...
Result<PreviewEnvelope> build_preview_envelope(const Waveform &waveform, std::size_t bin_count) {
  ...
  const auto used_bins =
      static_cast<std::size_t>(std::min<std::uint64_t>(bin_count, waveform.frame_count));
...
StereoRenderDecision stereo_render_decision(const Waveform &left, const Waveform &right) {
  ...
  return {true, "confirmed-compatible-members", frames, frames - left.frame_count,
          frames - right.frame_count};
}
