...
Result<void> validate_waveform(const Waveform &waveform) {
    ...
    if ((channels != 1U && channels != 2U) || (width != 1U && width != 2U) ||
        waveform.format.sample_rate == 0U) {
    ...
    if (waveform.pcm.size() % block != 0U ||
        waveform.frame_count != waveform.pcm.size() / block) {
        return std::unexpected{make_error(
            ErrorCode::object_malformed, ErrorCategory::audio,
            "waveform frame count and PCM byte length are inconsistent")};
    }
    return {};
}

void le16(std::vector<std::byte> &bytes, std::size_t offset,
          std::uint16_t value) {
    bytes[offset] = static_cast<std::byte>(value & 0xffU);
    bytes[offset + 1U] = static_cast<std::byte>(value >> 8U);
}

void le32(std::vector<std::byte> &bytes, std::size_t offset,
          std::uint32_t value) {
...
std::int32_t sample_value(const Waveform &waveform, std::uint64_t frame,
                          std::uint16_t channel) {
    const auto width = waveform.format.sample_width_bytes;
    const auto index = static_cast<std::size_t>(
        (frame * waveform.format.channels + channel) * width);
    if (width == 1U)
...
static Result<Waveform>
decode_waveform_payload(const CurrentSmpl &decoded, std::string object_key,
                        std::filesystem::path source_path,
                        PartitionIndex partition, SfsId sfs_id,
                        std::string name, std::span<const std::byte> payload) {
    ...
    if (start > payload.size() || size > payload.size() - start) {
        return std::unexpected{
            make_error(ErrorCode::object_malformed, ErrorCategory::audio,
                       "SMPL PCM span lies outside its object")};
    }
    ...
    if (result.stored_sample_width_bytes == 2U) {
        ...
        for (std::size_t offset = 1; offset < limit; offset += 2U) {
            ...
            alternating &=
                std::to_integer<std::uint8_t>(stored[offset]) == expected;
        }
        if (alternating) {
            result.format.sample_width_bytes = 1;
            result.stored_payload_transform =
                "alternating-byte-signed-high-byte";
            ...
            for (std::size_t offset = 0; offset < stored.size(); offset += 2U) {
                result.pcm.push_back(static_cast<std::byte>(
                    (std::to_integer<std::uint8_t>(stored[offset]) + 128U) &
                    0xffU));
            }
        } else {
        ...
        return std::unexpected{make_error(ErrorCode::audio_unsupported_format,
                                          ErrorCategory::audio,
                                          "SMPL sample width is unsupported")};
    }
    result.frame_count = result.pcm.size() / result.format.sample_width_bytes;
    return result;
}

Result<Waveform> decode_waveform(const Container &container,
    ...
    if (decoded == nullptr) {
        return std::unexpected{
            make_error(ErrorCode::invalid_argument, ErrorCategory::audio,
                       "waveform decode requires a SMPL object")};
    }
    auto payload = container.read_record_data(
        snapshot.partition, snapshot.sfs_id,
        std::numeric_limits<std::size_t>::max(), cancellation);
    if (!payload)
        return std::unexpected{payload.error()};
    return decode_waveform_payload(
        *decoded, snapshot.key, container.source_path(), snapshot.partition,
        snapshot.sfs_id, snapshot.object.header.name, *payload);
}
...
Result<Waveform> decode_waveform(const MediaObject &object) {
    ...
    return decode_waveform_payload(
        *decoded, object.key, object.logical_path, PartitionIndex{0}, SfsId{0},
...
Result<std::vector<std::byte>> wav_bytes(const Waveform &waveform) {
    ...
    if (data_size > std::numeric_limits<std::uint32_t>::max() - 36U) {
        return std::unexpected{make_error(ErrorCode::io_unsupported_size,
                                          ErrorCategory::audio,
                                          "PCM is too large for RIFF/WAVE")};
    }
    std::vector<std::byte> result(44U + data_size);
    const auto write_tag = [&](std::size_t offset, std::string_view tag) {
        std::ranges::transform(
            tag, result.begin() + static_cast<std::ptrdiff_t>(offset),
    ...
    const auto block = static_cast<std::uint16_t>(
        waveform.format.channels * waveform.format.sample_width_bytes);
    const auto byte_rate =
        static_cast<std::uint64_t>(waveform.format.sample_rate) * block;
    ...
    le16(result, 34,
         static_cast<std::uint16_t>(waveform.format.sample_width_bytes * 8U));
    write_tag(36, "data");
...
Result<void> write_wav_atomic(const std::filesystem::path &path,
                              const Waveform &waveform, bool overwrite) {
    if (!overwrite && std::filesystem::exists(path)) {
        return std::unexpected{
            make_error(ErrorCode::io_open_failed, ErrorCategory::io,
                       "refusing to replace an existing WAV")};
    }
...
Result<PreviewEnvelope> build_preview_envelope(const Waveform &waveform,
    ...
    if (bin_count == 0U || bin_count > maximum_preview_bins) {
        return std::unexpected{
            make_error(ErrorCode::invalid_argument, ErrorCategory::audio,
                       "preview bin count is outside bounds")};
    }
    PreviewEnvelope result;
    result.frame_count = waveform.frame_count;
    const auto used_bins = static_cast<std::size_t>(
        std::min<std::uint64_t>(bin_count, waveform.frame_count));
    ...
    for (std::size_t bin = 0; bin < used_bins; ++bin) {
        ...
        PreviewBin item{std::numeric_limits<std::int32_t>::max(),
                        std::numeric_limits<std::int32_t>::min()};
        for (auto frame = start; frame < end; ++frame) {
            for (std::uint16_t channel = 0; channel < waveform.format.channels;
                 ++channel) {
...
StereoRenderDecision stereo_render_decision(const Waveform &left,
    ...
    return {true, "confirmed-compatible-members", frames,
            frames - left.frame_count, frames - right.frame_count};
}
...
Result<Waveform> render_stereo(const Waveform &left, const Waveform &right) {
    ...
    if (result.frame_count >
        std::numeric_limits<std::size_t>::max() / (2U * width)) {
        return std::unexpected{
            make_error(ErrorCode::io_unsupported_size, ErrorCategory::audio,
                       "rendered stereo PCM exceeds the addressable size")};
    }
    result.pcm.reserve(static_cast<std::size_t>(result.frame_count) * 2U *
                       width);
    ...
    for (std::uint64_t frame = 0; frame < result.frame_count; ++frame) {
        for (const auto *member : std::array{&left, &right}) {
            if (frame < member->frame_count) {
                ...
                result.pcm.insert(result.pcm.end(), member->pcm.begin() + begin,
                                  member->pcm.begin() + end);
            } else {
                result.pcm.insert(result.pcm.end(), zero.begin(),
                                  zero.begin() + width);
            }
        }
    }
