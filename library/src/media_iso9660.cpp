...
Result<IsoDirectoryRecord> parse_iso_record(std::span<const std::byte> record,
                                            ...
                                            std::uint64_t offset) {
    if (record.size() < 34U ||
        std::to_integer<std::uint8_t>(record[0]) != record.size()) {
    ...
    if (name_length == 1U && std::to_integer<std::uint8_t>(record[33]) == 0U)
        name = ".";
    else if (name_length == 1U &&
             std::to_integer<std::uint8_t>(record[33]) == 1U)
        name = "..";
    else {
    ...
    return IsoDirectoryRecord{extent, size,
                              std::to_integer<std::uint8_t>(record[25]),
                              std::move(name)};
}

Result<void> validate_iso_extent(const RandomAccessReader &reader,
                                 std::uint32_t sector, std::uint32_t size,
                                 std::uint32_t volume_sectors,
                                 std::string_view source,
                                 bool require_physical_bytes = true) {
    const auto start =
        static_cast<std::uint64_t>(sector) * detail::iso_sector_size;
    if (sector >= volume_sectors ||
        (require_physical_bytes &&
         (start > reader.size() || size > reader.size() - start)) ||
        (size != 0U &&
         (static_cast<std::uint64_t>(size) + detail::iso_sector_size - 1U) /
                 detail::iso_sector_size >
             volume_sectors - sector)) {
        return std::unexpected{detail::media_error(
            ErrorCode::allocation_invalid_extent,
            std::format(
                "ISO9660 extent sector {} size {} is outside the volume",
                sector, size),
            source, start)};
    }
    return {};
...
Result<IsoImage>
IsoImage::open(std::shared_ptr<const RandomAccessReader> reader,
    ...
    if (!reader)
        return std::unexpected{detail::media_error(ErrorCode::invalid_argument,
                                                   "ISO reader is null")};
    const auto pvd = detail::read_bytes(
        *reader, detail::iso_pvd_sector * detail::iso_sector_size,
    ...
    if (detail::be32(*pvd, 84) != declared_volume_sectors ||
        detail::le16(*pvd, 128) != detail::iso_sector_size ||
        detail::be16(*pvd, 130) != detail::iso_sector_size ||
        declared_volume_sectors == 0U) {
        return std::unexpected{detail::media_error(
            ErrorCode::container_invalid_geometry,
            "invalid ISO9660 primary volume geometry", source_name)};
    }
    const auto physical_volume_sectors =
        reader->size() / detail::iso_sector_size;
    if (physical_volume_sectors <= detail::iso_pvd_sector) {
        return std::unexpected{detail::media_error(
            ErrorCode::container_truncated,
            "ISO9660 image ends before its data volume", source_name)};
    }
    const bool declared_tail_is_missing =
        declared_volume_sectors > physical_volume_sectors;
    ...
    const auto root = parse_iso_record(
        std::span{*pvd}.subspan(156, root_length), source_name,
        detail::iso_pvd_sector * detail::iso_sector_size + 156U);
    if (!root)
        return std::unexpected{root.error()};
    if (const auto valid =
            validate_iso_extent(*reader, root->extent, root->size,
    ...
    while (!pending.empty()) {
        ...
        const auto bytes =
            detail::read_bytes(*reader,
                               static_cast<std::uint64_t>(directory.extent) *
                                   detail::iso_sector_size,
        ...
        while (offset < bytes->size()) {
            ...
            if (length == 0U) {
                offset = ((offset / detail::iso_sector_size) + 1U) *
                         detail::iso_sector_size;
                continue;
            }
            if (offset % detail::iso_sector_size + length >
                    detail::iso_sector_size ||
            ...
            const auto record = parse_iso_record(
                std::span{*bytes}.subspan(offset, length), source_name,
                static_cast<std::uint64_t>(directory.extent) *
                        detail::iso_sector_size +
                    offset);
            if (!record)
            ...
            if (const auto valid = validate_iso_extent(
                    *reader, record->extent, record->size,
                    declared_volume_sectors, source_name,
...
const std::string &IsoImage::source_name() const noexcept {
    return source_name_;
}
const std::vector<IsoFile> &IsoImage::files() const noexcept { return files_; }
std::span<const std::pair<std::string, std::string>>
IsoImage::group_labels() const noexcept {
    return group_labels_;
}
std::span<const std::pair<std::string, std::string>>
IsoImage::volume_labels() const noexcept {
    return volume_labels_;
}
std::span<const MediaValidationIssue>
IsoImage::validation_issues() const noexcept {
    return validation_issues_;
}

Result<std::vector<std::byte>>
IsoImage::read_file(const IsoFile &file,
