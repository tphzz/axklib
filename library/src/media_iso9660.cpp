...
Result<IsoDirectoryRecord> parse_iso_record(std::span<const std::byte> record,
  ...
  if (record.size() < 34U || std::to_integer<std::uint8_t>(record[0]) != record.size()) {
    return std::unexpected{detail::media_error(ErrorCode::container_invalid_geometry,
                                               "malformed ISO9660 directory record", source,
                                               offset)};
  }
  ...
  if (33U + name_length > record.size()) {
    return std::unexpected{detail::media_error(
        ErrorCode::container_truncated, "truncated ISO9660 directory record name", source, offset)};
  }
  ...
  if (detail::be32(record, 6) != extent || detail::be32(record, 14) != size ||
    ...
    return std::unexpected{detail::media_error(ErrorCode::container_invalid_geometry,
                                               "ISO9660 both-endian directory fields disagree",
                                               source, offset)};
  }
  std::string name;
  ...
  if (name != "." && name != ".." && detail::unsafe_component(name)) {
    return std::unexpected{detail::media_error(ErrorCode::container_invalid_geometry,
                                               std::format("unsafe ISO9660 name '{}'", name),
                                               source, offset)};
  }
...
Result<void> validate_iso_extent(const RandomAccessReader &reader, std::uint32_t sector,
  ...
  if (sector >= volume_sectors ||
      ...
      (size != 0U &&
       (static_cast<std::uint64_t>(size) + detail::iso_sector_size - 1U) / detail::iso_sector_size >
...
Result<IsoImage> IsoImage::open(std::shared_ptr<const RandomAccessReader> reader,
  ...
  if (std::to_integer<std::uint8_t>((*pvd)[0]) != 1U ||
    ...
    return std::unexpected{detail::media_error(
        ...
        "primary ISO9660 volume descriptor not found at sector 16", source_name)};
  }
  ...
  if (detail::be32(*pvd, 84) != declared_volume_sectors ||
    ...
    return std::unexpected{detail::media_error(ErrorCode::container_invalid_geometry,
                                               "invalid ISO9660 primary volume geometry",
                                               source_name)};
  }
  const auto physical_volume_sectors = reader->size() / detail::iso_sector_size;
  if (physical_volume_sectors <= detail::iso_pvd_sector) {
    return std::unexpected{detail::media_error(
        ErrorCode::container_truncated, "ISO9660 image ends before its data volume", source_name)};
  }
  ...
  if (root_length < 34U || 156U + root_length > pvd->size()) {
    return std::unexpected{detail::media_error(ErrorCode::container_invalid_geometry,
                                               "ISO9660 root directory record is malformed",
                                               source_name)};
  }
  ...
  if (const auto valid = validate_iso_extent(*reader, root->extent, root->size,
  ...
  while (!pending.empty()) {
    ...
    const auto bytes = detail::read_bytes(
        *reader, static_cast<std::uint64_t>(directory.extent) * detail::iso_sector_size,
    ...
    while (offset < bytes->size()) {
      ...
      if (!names.insert(detail::upper_ascii(record->name)).second) {
        return std::unexpected{detail::media_error(
            ...
            std::format("duplicate ISO9660 name '{}'", record->name), source_name)};
      }
      ...
      if (const auto valid =
              validate_iso_extent(*reader, record->extent, record->size, declared_volume_sectors,
...
Result<std::vector<std::byte>> IsoImage::read_file(const IsoFile &file,
  ...
  return detail::read_bytes(
      *reader_, static_cast<std::uint64_t>(file.extent_sector) * detail::iso_sector_size, file.size,
      cancellation);
}
