...
std::uint16_t fat12_entry(std::span<const std::byte> fat,
    ...
    return (cluster & 1U) != 0U
               ? static_cast<std::uint16_t>(wide_pair >> 4U)
               : static_cast<std::uint16_t>(wide_pair & 0x0fffU);
}

Result<std::vector<std::uint16_t>>
fat_chain(std::span<const std::byte> fat, const FatGeometry &geometry,
    ...
    if (first < 2U || first >= geometry.data_cluster_count + 2U) {
        return std::unexpected{detail::media_error(
            ...
            std::format("FAT entry '{}' starts at invalid cluster {}", name,
                        first),
            source)};
    }
    ...
    while (true) {
        if (cluster < 2U || cluster >= geometry.data_cluster_count + 2U) {
            return std::unexpected{detail::media_error(
                ...
                std::format(
                    "FAT chain for '{}' leaves the data area at cluster {}",
                    name, cluster),
                source)};
        }
        if (!seen.insert(cluster).second) {
            return std::unexpected{detail::media_error(
                ...
                std::format("FAT chain loop in '{}' at cluster {}", name,
                            cluster),
                source)};
        }
        ...
        if (next == 0xff7U) {
            return std::unexpected{detail::media_error(
                ErrorCode::allocation_invalid_extent,
                std::format("FAT chain for '{}' reaches bad cluster marker",
                            name),
                source)};
        }
        if (next == 0U || next == 1U || (next >= 0xff0U && next <= 0xff6U)) {
            return std::unexpected{detail::media_error(
                ErrorCode::allocation_invalid_extent,
                std::format("FAT chain for '{}' has invalid successor 0x{:03x}",
                            name, next),
                source)};
        }
        cluster = next;
        if (result.size() > geometry.data_cluster_count) {
            return std::unexpected{detail::media_error(
                ErrorCode::allocation_cycle,
                std::format("FAT chain for '{}' exceeds the data cluster count",
                            name),
                source)};
        }
    }
    if (required_bytes && capacity < *required_bytes) {
        return std::unexpected{detail::media_error(
            ...
            std::format(
                "FAT chain for '{}' contains {} bytes but file declares {}",
                name, capacity, *required_bytes),
            source)};
    }
    return result;
}

std::uint64_t fat_cluster_offset(const FatGeometry &geometry,
                                 std::uint16_t cluster) {
    return geometry.data_offset +
           static_cast<std::uint64_t>(cluster - 2U) * geometry.cluster_size();
}
...
Result<void> scan_fat_directory(
    std::vector<FatFile> &files, std::deque<PendingFatDirectory> &pending,
    ...
    for (std::size_t offset = 0; offset + 32U <= bytes.size(); offset += 32U) {
        ...
        const auto path =
            parent.empty() ? name : std::format("{}/{}", parent, name);
        if ((attributes & 0x10U) != 0U) {
            const auto chain = fat_chain(fat, geometry, first_cluster,
                                         std::nullopt, path, source);
            if (!chain)
                return std::unexpected{chain.error()};
            for (const auto cluster : *chain) {
                const auto [found, inserted] =
                    claimed_clusters.emplace(cluster, path);
                if (!inserted) {
                    return std::unexpected{detail::media_error(
                        ErrorCode::allocation_invalid_extent,
                        std::format(
                            "FAT entries '{}' and '{}' share cluster {}",
                            found->second, path, cluster),
                        source)};
                }
            }
            ...
            for (const auto cluster : *chain) {
                const auto block = detail::read_bytes(
                    reader, fat_cluster_offset(geometry, cluster),
        ...
        if (size == 0U)
            continue;
        const auto chain =
            fat_chain(fat, geometry, first_cluster, size, path, source);
        if (!chain)
            return std::unexpected{chain.error()};
        for (const auto cluster : *chain) {
            const auto [found, inserted] =
            ...
            if (!inserted) {
                return std::unexpected{detail::media_error(
                    ErrorCode::allocation_invalid_extent,
                    std::format("FAT entries '{}' and '{}' share cluster {}",
                                found->second, path, cluster),
                    source)};
            }
        }
        files.push_back({path, name, directory_offset + offset, first_cluster,
                         size, *chain,
                         fat_cluster_offset(geometry, first_cluster)});
    }
    return {};
...
Result<FatImage>
FatImage::open(std::shared_ptr<const RandomAccessReader> reader,
    ...
    if (!power_of_two(geometry.bytes_per_sector) ||
        geometry.bytes_per_sector < 512U || geometry.bytes_per_sector > 4096U ||
    ...
    geometry.data_cluster_count =
        static_cast<std::uint32_t>((geometry.total_sectors - metadata_sectors) /
                                   geometry.sectors_per_cluster);
    if (geometry.data_cluster_count == 0U ||
        geometry.data_cluster_count >= 4085U) {
    ...
    geometry.fat_offset =
        static_cast<std::uint64_t>(geometry.reserved_sectors) *
        geometry.bytes_per_sector;
    geometry.root_offset = static_cast<std::uint64_t>(
    ...
    geometry.data_offset =
        ...
        static_cast<std::uint64_t>(root_sectors) * geometry.bytes_per_sector;
    const auto fat_size = static_cast<std::size_t>(geometry.sectors_per_fat) *
                          geometry.bytes_per_sector;
    const auto fat = detail::read_bytes(*reader, geometry.fat_offset, fat_size,
                                        cancellation);
    if (!fat)
        return std::unexpected{fat.error()};
    if (fat_size < 3U ||
        std::to_integer<std::uint8_t>((*fat)[0]) != geometry.media_descriptor) {
        return std::unexpected{detail::media_error(
            ErrorCode::container_invalid_geometry,
    ...
    for (std::uint8_t index = 1; index < geometry.fat_count; ++index) {
        const auto copy = detail::read_bytes(
            *reader,
            geometry.fat_offset + static_cast<std::uint64_t>(index) * fat_size,
            fat_size, cancellation);
        if (!copy)
            return std::unexpected{copy.error()};
        if (*copy != *fat) {
            return std::unexpected{
                detail::media_error(ErrorCode::container_backup_mismatch,
                                    "FAT copies disagree", source_name)};
        }
    }
    const auto root_size =
        static_cast<std::size_t>(root_sectors) * geometry.bytes_per_sector;
    const auto root = detail::read_bytes(*reader, geometry.root_offset,
                                         root_size, cancellation);
    if (!root)
        return std::unexpected{root.error()};
    std::vector<FatFile> files;
    std::deque<PendingFatDirectory> pending;
    std::unordered_map<std::uint16_t, std::string> claimed_clusters;
    if (const auto scan = scan_fat_directory(
            files, pending, *root, {}, geometry.root_offset, *fat, geometry,
    ...
    while (!pending.empty()) {
        ...
        pending.pop_front();
        if (!visited_directories.insert(detail::upper_ascii(directory.path))
                 .second) {
            return std::unexpected{detail::media_error(
                ErrorCode::allocation_cycle,
                std::format("duplicate or cyclic FAT directory '{}'",
                            directory.path),
                source_name)};
        }
        if (const auto scan = scan_fat_directory(
                files, pending, directory.bytes, directory.path,
...
const std::string &FatImage::source_name() const noexcept {
    return source_name_;
}
const std::vector<FatFile> &FatImage::files() const noexcept { return files_; }

Result<std::vector<std::byte>>
FatImage::read_file(const FatFile &file,
    ...
    for (const auto cluster : file.clusters) {
        const auto take =
            std::min<std::size_t>(remaining, geometry_.cluster_size());
        const auto bytes =
            detail::read_bytes(*reader_, fat_cluster_offset(geometry_, cluster),
                               take, cancellation);
        if (!bytes)
