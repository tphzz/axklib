...
Result<std::vector<std::uint16_t>> fat_chain(std::span<const std::byte> fat,
  ...
  if (required_bytes && capacity < *required_bytes) {
    return std::unexpected{
        detail::media_error(ErrorCode::container_truncated,
                            std::format("FAT chain for '{}' contains {} bytes but file declares {}",
...
Result<void> scan_fat_directory(std::vector<FatFile> &files,
  ...
  for (std::size_t offset = 0; offset + 32U <= bytes.size(); offset += 32U) {
    ...
    if (!dot && detail::unsafe_component(name)) {
      return std::unexpected{detail::media_error(
          ErrorCode::container_invalid_geometry,
          std::format("unsafe FAT directory name '{}'", name), source, directory_offset + offset)};
    }
...
Result<FatImage> FatImage::open(std::shared_ptr<const RandomAccessReader> reader,
  ...
  if (!power_of_two(geometry.bytes_per_sector) || geometry.bytes_per_sector < 512U ||
    ...
    return std::unexpected{detail::media_error(ErrorCode::container_invalid_geometry,
                                               "invalid or unsupported FAT BPB geometry",
                                               source_name)};
  }
  ...
  if (metadata_sectors >= geometry.total_sectors ||
      static_cast<std::uint64_t>(geometry.total_sectors) * geometry.bytes_per_sector >
          reader->size()) {
    return std::unexpected{detail::media_error(
        ErrorCode::container_truncated, "FAT geometry exceeds the input image", source_name)};
  }
  ...
  if (fat_size < 3U || std::to_integer<std::uint8_t>((*fat)[0]) != geometry.media_descriptor) {
    return std::unexpected{detail::media_error(ErrorCode::container_invalid_geometry,
                                               "FAT media descriptor disagrees with the BPB",
                                               source_name)};
  }
  for (std::uint8_t index = 1; index < geometry.fat_count; ++index) {
    const auto copy = detail::read_bytes(
        *reader, geometry.fat_offset + static_cast<std::uint64_t>(index) * fat_size, fat_size,
        cancellation);
    if (!copy)
      return std::unexpected{copy.error()};
    if (*copy != *fat) {
      return std::unexpected{detail::media_error(ErrorCode::container_backup_mismatch,
...
Result<std::vector<std::byte>> FatImage::read_file(const FatFile &file,
  ...
  for (const auto cluster : file.clusters) {
    ...
    const auto bytes =
        detail::read_bytes(*reader_, fat_cluster_offset(geometry_, cluster), take, cancellation);
    if (!bytes)
