...
prepare_transfer(const SavedObjectTransferSpec &transfer, const CancellationToken &cancellation) {
    ...
    if (media->kind() != MediaKind::fat12_floppy) {
      return std::unexpected{
          make_error(ErrorCode::unsupported_profile, ErrorCategory::unsupported,
                     "whole-source transfer requires one Yamaha FAT12 floppy image")};
    }
    for (const auto &object : *media_objects) {
      if (object.decoded.header.type == ObjectType::unknown || object.decode_issue) {
        return std::unexpected{make_error(
            ...
            "whole-source transfer requires every Yamaha floppy object to decode cleanly")};
      }
      ...
      if (!by_key.contains(key)) {
        return std::unexpected{
            make_error(ErrorCode::object_missing, ErrorCategory::object,
                       "transfer root object key does not exist in the source image: " + key)};
      }
    ...
    while (!pending.empty()) {
      ...
      for (const auto &relationship : graph.relationships) {
        ...
        if (relationship.quality != RelationshipQuality::known || !relationship.target_key) {
          return std::unexpected{make_error(
              relationship.candidate_keys.size() > 1U ? ErrorCode::relationship_ambiguous
                                                      : ErrorCode::relationship_unresolved,
              ErrorCategory::relationship,
              "saved-object transfer requires a known dependency for " + relationship.type)};
        }
...
Result<std::vector<detail::PreparedMediaObject>>
prepare_authored(const VolumeSpec &volume, const CancellationToken &cancellation) {
...
Result<void> validate_written_image(const detail::PreparedMediaImage &prepared,
  ...
  if (!media)
    return std::unexpected{media.error()};
  const auto expected_kind = prepared.manifest.format == MediaImageFormat::fat12_floppy
