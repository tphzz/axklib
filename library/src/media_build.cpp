...
bool dependency_relationship(const Relationship &relationship) {
    ...
    return (relationship.type == "PROG_ASSIGNMENT_TO_SBNK" ||
...
prepare_transfer(const SavedObjectTransferSpec &transfer,
    ...
    if (transfer.selection == SavedObjectSelection::all) {
        ...
        for (const auto &object : *media_objects) {
            if (object.decoded.header.type == ObjectType::unknown ||
                object.decode_issue) {
    ...
    for (const auto &key : selected) {
        ...
        result.push_back({object.decoded.header.type,
...
Result<std::vector<detail::PreparedMediaObject>>
prepare_authored(const VolumeSpec &volume,
                 const CancellationToken &cancellation) {
    PartitionSpec partition{"MEDIA", {volume}};
    constexpr PartitionGeometry staging_geometry{
        0, 3, 2'000'000, 1'999'999, 999'999, 2, 1, 3, 512};
    auto records = detail::prepare_partition_records(
        partition, staging_geometry, 1, cancellation);
    if (!records)
    ...
    for (auto &record : *records) {
        ...
        result.push_back(
            {header->type, std::move(header->name), std::move(record.payload)});
    }
    std::ranges::sort(result, [](const auto &left, const auto &right) {
        return std::tuple{left.type, left.name, left.payload.size()} <
               std::tuple{right.type, right.name, right.payload.size()};
...
Result<void> validate_written_image(const detail::PreparedMediaImage &prepared,
    ...
    const auto payload_less = [](const std::vector<std::byte> &left,
    ...
    std::multiset<std::vector<std::byte>, decltype(payload_less)> expected{
        payload_less};
    std::multiset<std::vector<std::byte>, decltype(payload_less)> actual{
        payload_less};
    ...
    if (prepared.manifest.format == MediaImageFormat::fat12_floppy) {
        ...
        for (const auto &retained : prepared.retained_files) {
            const auto found =
                std::ranges::find(fat.files(), retained.path, &FatFile::path);
        ...
        for (const auto &retained : prepared.retained_files) {
            const auto found =
                std::ranges::find(iso.files(), retained.path, &IsoFile::path);
...
Result<detail::PreparedMediaImage>
detail::prepare_media_image(const MediaBuildManifest &manifest,
                            const CancellationToken &cancellation) {
    ...
    auto objects =
        manifest.transfer
            ? prepare_transfer(*manifest.transfer, cancellation)
...
Result<WrittenMediaImage> detail::write_prepared_media_image(
    const PreparedMediaImage &prepared,
    ...
    if (auto validated =
            validate_written_image(prepared, *temporary, cancellation);
        !validated)
    ...
    if (filesystem_error) {
        return std::unexpected{
            make_error(ErrorCode::io_read_failed, ErrorCategory::io,
                       "could not determine written media image size")};
    }
    if (overwrite)
    ...
    cleanup.release();
    return WrittenMediaImage{output_path, prepared.manifest.format, size,
...
write_media_image(const MediaBuildManifest &manifest,
    ...
    return detail::write_prepared_media_image(*prepared, output_path, overwrite,
                                              cancellation);
}
