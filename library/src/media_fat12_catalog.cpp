...
FloppyCatalogInspection inspect_yamaha_floppy_catalog(const FatImage &image, const CancellationToken &cancellation) {
    std::vector<FloppyCatalogFile> files;
    for (const auto &file : image.files())
        files.push_back({file.path, file.size});
    return inspect_yamaha_floppy_catalog(files, [&](std::size_t index, std::size_t limit) {
        return image.read_file_prefix(image.files()[index], limit, cancellation);
    });
}

FloppyCatalogInspection inspect_yamaha_floppy_catalog(std::span<const FloppyCatalogFile> files,
                                                      const FloppyPrefixReader &read_prefix) {
    FloppyCatalogInspection result;
    const auto yamaha = std::ranges::find_if(files, [](const FloppyCatalogFile &file) {
        return file.path.find('/') == std::string::npos && is_yamaha_floppy_catalog_path(file.path);
    });
    if (yamaha == files.end()) {
        result.issues.push_back(issue("FLOPPY_CATALOG_MISSING", "Floppy member has no root YAMAHA.SYM catalog", "\\",
                                      "Treat this as a standalone recovery source, not a trusted disk-set member"));
        return result;
    }
    const auto bytes = read_prefix(static_cast<std::size_t>(yamaha - files.begin()), 9767U);
    if (!bytes) {
    ...
    for (const auto &file : files) {
        if (&file == &*yamaha || file.path.find('/') != std::string::npos)
            continue;
        const auto slot = yamaha_floppy_filename_slot(file.path);
        if (slot)
    ...
    for (const auto &entry : catalog->files) {
        ...
        if (physical == files_by_slot.end()) {
            result.issues.push_back(
                issue("FLOPPY_CATALOG_SLOT_MISSING",
                      std::format("YAMAHA.SYM slot {} ('{}') has no matching file", entry.slot, entry.logical_path),
        ...
        if (physical->second.size() != 1U) {
            result.issues.push_back(issue("FLOPPY_CATALOG_SLOT_DUPLICATE",
                                          std::format("YAMAHA.SYM slot {} maps to more than one file", entry.slot),
                                          entry.logical_path, "Repair duplicate numeric filename extensions"));
            continue;
        }
        ...
        if (!expected_type)
            continue;
        const auto prefix = read_prefix(static_cast<std::size_t>(&file - files.data()), 0x42U);
        const auto header =
        ...
        if (!header || header->type != *expected_type) {
            result.issues.push_back(issue(
                "FLOPPY_CATALOG_OBJECT_TYPE_MISMATCH",
                std::format("Catalog path '{}' disagrees with file '{}''s object type", entry.logical_path, file.path),
                entry.logical_path, "Use unverified recovery; do not join this disk to a trusted set"));
        }
    }

    for (const auto &[slot, candidates] : files_by_slot) {
        if (!catalog_slots.contains(slot)) {
            result.issues.push_back(
                issue("FLOPPY_CATALOG_FILE_UNLISTED",
                      std::format("File '{}' uses uncataloged slot {}", candidates.front()->path, slot),
                      candidates.front()->path, "Use unverified recovery or repair YAMAHA.SYM"));
        }
    }
