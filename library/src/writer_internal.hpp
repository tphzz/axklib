...
Result<std::vector<std::byte>> prepare_smpl_payload(const WaveformSpec &spec,
                                                    const ImportedAudio &audio,
                                                    std::uint32_t link_id);
Result<std::vector<std::byte>>
prepare_sbnk_payload(const SampleBankSpec &spec,
...
Result<std::vector<PreparedRecord>> prepare_partition_records(
...
Result<PreparedMediaImage>
prepare_media_image(const MediaBuildManifest &manifest,
                    const CancellationToken &cancellation);
Result<WrittenMediaImage> write_prepared_media_image(
    const PreparedMediaImage &image, const std::filesystem::path &output_path,
