...
constexpr std::size_t second_fat_offset =
    first_fat_offset + sectors_per_fat * sector_size;
...
void apply_yamaha_floppy_profile(std::span<std::byte> bytes) {
    constexpr std::array<std::byte, 3> jump{std::byte{0xeb}, std::byte{0x58},
                                            std::byte{0x90}};
    ...
    std::ranges::transform(
        oem_name, bytes.begin() + oem_name_offset,
    ...
    std::ranges::transform(
        filesystem_name, bytes.begin() + filesystem_name_offset,
        [](char character) { return static_cast<std::byte>(character); });
...
extern "C" DRESULT disk_read(BYTE drive, BYTE *buffer, LBA_t sector,
                             UINT count) {
    if (drive != 0U || active_disk == nullptr || buffer == nullptr ||
        count == 0U || sector > sector_count || count > sector_count - sector) {
        return RES_PARERR;
    }
...
extern "C" DRESULT disk_write(BYTE drive, const BYTE *buffer, LBA_t sector,
                              UINT count) {
    if (drive != 0U || active_disk == nullptr || buffer == nullptr ||
        count == 0U || sector > sector_count || count > sector_count - sector) {
        return RES_PARERR;
...
Result<void> write_fat12_image(const PreparedMediaImage &image,
    ...
    const MKFS_PARM options{static_cast<BYTE>(FM_FAT | FM_SFD), 2, 0, 224,
                            sector_size};
    auto status =
        f_mkfs("", &options, work.data(), static_cast<UINT>(work.size()));
    if (status != FR_OK) {
        reset_disk();
        return std::unexpected{
            fatfs_error("could not format FAT12 image", status)};
    }
    ...
    if (status != FR_OK) {
        reset_disk();
        return std::unexpected{
            fatfs_error("could not mount formatted FAT12 image", status)};
    }
    ...
    const auto write_file =
        ...
        FIL file{};
        auto write_status = f_open(&file, std::string{filename}.c_str(),
                                   FA_CREATE_NEW | FA_WRITE);
        if (write_status == FR_OK) {
            UINT written{};
            write_status = f_write(&file, payload.data(),
                                   static_cast<UINT>(payload.size()), &written);
        ...
        if (write_status != FR_OK)
            return std::unexpected{
                fatfs_error("could not write FAT12 file", write_status)};
        return {};
    };
    for (const auto &retained : image.retained_files) {
        ...
        if (auto written = write_file(retained.path, retained.payload);
            !written) {
            f_mount(nullptr, "", 0);
            reset_disk();
    ...
    for (std::size_t index = 0; index < image.objects.size(); ++index) {
        ...
        if (!stems.insert(stem).second) {
            ...
            stem.resize(std::min<std::size_t>(
                stem.size(), 8U - std::min<std::size_t>(suffix.size(), 7U)));
            stem += suffix;
            if (!stems.insert(stem).second) {
                f_mount(nullptr, "", 0);
                reset_disk();
        ...
        auto filename =
            stem + "." +
            std::to_string(1000U + static_cast<unsigned int>(suffix_index))
                .substr(1);
        ...
        if (auto written = write_file(filename, image.objects[index].payload);
            !written) {
            f_mount(nullptr, "", 0);
            reset_disk();
            return std::unexpected{written.error()};
