...
Result<std::uint64_t> cluster_offset(std::uint32_t partition_start_sector,
                                     std::uint32_t sector_size, std::uint32_t sectors_per_cluster,
...
Result<Superblock> parse_superblock(std::span<const std::byte> bytes) {
  ...
  for (std::size_t index = 0; index < result.partition_entries.size(); ++index) {
    ...
    result.partition_entries[index] = {PartitionIndex{static_cast<std::uint8_t>(index)}, *start,
                                       *count};
  }
  return result;
}
...
std::vector<AllocationMismatchRange> mismatch_ranges(std::span<const std::byte> left,
...
Result<std::vector<Extent>> direct_extents(std::span<const std::byte> bytes,
  ...
  for (std::uint16_t index = 0; index < extent_count; ++index) {
    ...
    if (!cluster || !count || !bytes_count) {
      return std::unexpected{!cluster ? cluster.error()
                                      : (!count ? count.error() : bytes_count.error())};
    }
    if (*cluster == 0 || *count == 0 || *bytes_count == 0) {
      return std::unexpected{make_error(ErrorCode::allocation_invalid_extent,
...
continuation_extents(const RandomAccessReader &image, const Partition &partition,
                     std::uint32_t sector_size, std::uint32_t list_cluster,
                     std::uint16_t expected_count, std::vector<std::uint32_t> &list_clusters,
...
Result<std::vector<std::byte>> read_logical_prefix(const RandomAccessReader &image,
                                                   const Partition &partition,
                                                   std::uint32_t sector_size,
                                                   const IndexRecord &record, std::size_t limit,
                                                   const OpenOptions &options) {
  ...
  for (const auto &extent : record.extents) {
    ...
    const auto capacity =
        checked_multiply(extent.cluster_count,
    ...
    const auto count = static_cast<std::size_t>(
        std::min<std::uint64_t>({extent.byte_count, *capacity, wanted - result.size()}));
    const auto offset = cluster_offset(partition.start_sector, sector_size,
                                       partition.sectors_per_cluster, extent.cluster_offset);
    if (!offset) {
      return std::unexpected{offset.error()};
    }
  ...
  for (const auto &record : partition.records) {
    ...
    if (!inserted) {
      ...
      partition.diagnostics.push_back(make_error(
          ...
          "multiple SFS directory records claim the same directory ID", std::move(context)));
    }
    if (record.data_size > options.max_directory_bytes) {
      ...
      partition.diagnostics.push_back(make_error(
          ...
          "directory payload exceeds the configured traversal bound", std::move(context)));
    }
  }
  ...
  std::function<void(std::uint32_t)> visit = [&](std::uint32_t directory_id) {
    ...
    for (const auto &entry : found->second->directory_entries) {
      ...
      if (color == 1) {
        ...
        partition.diagnostics.push_back(
            make_error(ErrorCode::relationship_cycle, ErrorCategory::relationship,
                       "directory child links contain a cycle", std::move(context)));
      } else if (color == 0) {
...
Result<Partition> parse_partition(const RandomAccessReader &image,
                                  const PartitionEntry &table_entry, std::uint32_t sector_size,
                                  const OpenOptions &options) {
  Partition result;
  ...
  const auto header = read_bytes(image, *start, static_cast<std::size_t>(partition_header_size),
  ...
  const auto raw_index_bytes =
      cluster_bytes ? checked_multiply(*cluster_bytes, result.directory_index_span_clusters)
  ...
  const auto index_offset = cluster_offset(
      result.start_sector, sector_size, result.sectors_per_cluster, result.directory_index_cluster);
  if (!index_offset) {
  ...
  for (std::size_t block = 0; block + index_block_size <= index_data->size();
    ...
    for (std::size_t slot = 0; slot < records_per_index_block; ++slot) {
      ...
      if (std::all_of(bytes.begin(), bytes.begin() + 4,
      ...
      if (!parsed) {
        result.diagnostics.push_back(partition_error(
            ErrorCode::object_malformed, "nonempty SFS index slot has an invalid record header",
            result.index, *index_offset + relative));
        continue;
      }
      IndexRecord record;
      record.sfs_id = SfsId{
          static_cast<std::uint32_t>((block / index_block_size) * records_per_index_block + slot)};
      ...
      Result<std::vector<Extent>> extents =
          std::unexpected{make_error(ErrorCode::allocation_invalid_extent,
                                     ErrorCategory::allocation, "extent parser was not selected")};
      if (record.extent_count <= direct_extent_limit) {
        ...
        if (list_cluster) {
          extents =
              continuation_extents(image, result, sector_size, *list_cluster, record.extent_count,
      ...
      for (const auto &extent : record.extents) {
        ...
        if (extent.cluster_offset >= result.cluster_count ||
          ...
          result.diagnostics.push_back(
              partition_error(ErrorCode::allocation_invalid_extent,
                              "SFS extent lies outside the partition cluster range", result.index,
      ...
      if (prefix) {
        ...
        if (record.payload_kind == PayloadKind::directory && record.data_size > prefix->size() &&
          ...
          const auto expanded =
              read_logical_prefix(image, result, sector_size, record,
  ...
  const auto bitmap_offset = cluster_offset(result.start_sector, sector_size,
                                            result.sectors_per_cluster, result.bitmap_cluster);
  if (!bitmap_offset) {
    return std::unexpected{bitmap_offset.error()};
  }
  const auto stored = read_bytes(image, *bitmap_offset, static_cast<std::size_t>(bitmap_bytes_u64),
                                 options.cancellation);
  if (!stored) {
  ...
  const auto first_payload_u64 =
      checked_add(result.directory_index_cluster, result.directory_index_span_clusters);
  if (first_payload_u64 && *first_payload_u64 <= std::numeric_limits<std::uint32_t>::max() &&
    ...
    const auto free = calculate_sfs_free_space(
        result.cluster_count, static_cast<std::uint32_t>(*first_payload_u64),
        result.allocation.stored_used_cluster_count, static_cast<std::uint32_t>(*cluster_bytes));
    if (free) {
...
Container::read_record_data(PartitionIndex partition_index, SfsId record_id,
  ...
  const auto partition =
      std::find_if(partitions_.begin(), partitions_.end(),
  ...
  const auto record =
      std::find_if(partition->records.begin(), partition->records.end(),
  ...
  const auto data = read_logical_prefix(*reader_, *partition, superblock_.sector_size_bytes,
...
Result<SfsFreeSpace> calculate_sfs_free_space(std::uint32_t cluster_count,
  ...
  if (first_payload_cluster > cluster_count || cluster_size_bytes == 0) {
    return std::unexpected{
        make_error(ErrorCode::container_invalid_geometry, ErrorCategory::allocation,
  ...
  return SfsFreeSpace{cluster_count,      first_payload_cluster, allocated_cluster_count,
                      free_clusters,      cluster_size_bytes,    *free_bytes,
                      *free_bytes / 1024U};
}
...
Result<Container> open_image(std::shared_ptr<const RandomAccessReader> image,
  ...
  if (!result.backup_superblock_matches_) {
    ...
    result.diagnostics_.push_back(
        make_error(ErrorCode::container_backup_mismatch, ErrorCategory::container,
  ...
  for (const auto &entry : primary->partition_entries) {
    ...
    const auto partition =
        parse_partition(*result.reader_, entry, primary->sector_size_bytes, options);
    if (!partition) {
