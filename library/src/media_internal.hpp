...
[[nodiscard]] Error
media_error(ErrorCode code, std::string message, std::string_view source = {},
...
[[nodiscard]] std::uint16_t le16(std::span<const std::byte> bytes,
                                 std::size_t offset);
[[nodiscard]] std::uint32_t le32(std::span<const std::byte> bytes,
                                 std::size_t offset);
...
[[nodiscard]] Result<IsoMenuLabels>
