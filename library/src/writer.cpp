...
OrderedJson authored_starter_volume(std::string_view name) {
    OrderedJson result = OrderedJson::object();
    result["name"] = name;
    result["waveforms"] = OrderedJson::array({{{"id", "tone"},
                                               {"name", "Authored Tone"},
                                               {"path", "tone.wav"},
                                               {"root_key", 60}}});
...
Result<OrderedJson> manifest_template(BuildManifestKind kind) {
    ...
    return std::unexpected{
        manifest_error("unknown build manifest template kind")};
}
...
Result<void> fields(const Json &value, std::string_view context,
    ...
    if (!value.is_object()) {
        return std::unexpected{
            manifest_error(std::string{context} + " must be a JSON object")};
    }
    ...
    for (const auto &[name, ignored] : value.items()) {
        ...
        if (!allowed.contains(name)) {
            return std::unexpected{manifest_error(
                std::string{context} + " has unknown field: " + name)};
        }
    }
    return {};
}
...
Result<std::uint64_t> integer(const Json &value, std::string_view context,
                              std::uint64_t minimum, std::uint64_t maximum) {
    if (!value.is_number_unsigned() && !value.is_number_integer()) {
        return std::unexpected{
            manifest_error(std::string{context} + " must be an integer")};
    }
    if (value.is_number_integer() && value.get<std::int64_t>() < 0) {
        return std::unexpected{manifest_error(
            std::string{context} + " is outside its supported range")};
    }
...
Result<WaveformSpec> waveform(const Json &value, std::string context,
                              const std::filesystem::path &base) {
    if (auto valid = fields(value, context, {"id", "name", "path", "root_key"},
                            {"target_sample_rate"});
        !valid)
    ...
    WaveformSpec result{
        *id, *name, *source, static_cast<std::uint8_t>(*root), {}};
    if (value.contains("target_sample_rate")) {
        auto rate = integer(value["target_sample_rate"],
                            context + ".target_sample_rate", 1,
...
Result<SampleBankSpec> sample_bank(const Json &value, std::string context,
                                   const std::filesystem::path &base) {
    if (auto valid =
            fields(value, context, {"name", "root_key", "key_low", "key_high"},
                   {"level", "waveform_id", "right_waveform_id",
                    "interleaved_audio_path", "left_waveform_name",
                    "right_waveform_name", "target_sample_rate"});
        !valid) {
        return std::unexpected{valid.error()};
    }
    const bool direct = value.contains("waveform_id");
    const bool interleaved = value.contains("interleaved_audio_path");
    if (direct == interleaved ||
        (interleaved && value.contains("right_waveform_id")) ||
        (direct && (value.contains("left_waveform_name") ||
                    value.contains("right_waveform_name") ||
                    value.contains("target_sample_rate")))) {
        return std::unexpected{manifest_error(
            context + " has an invalid audio source field combination")};
    }
    auto name = text(value["name"], context + ".name");
    auto root = integer(value["root_key"], context + ".root_key", 0, 127);
    ...
    if (*high < *low)
        return std::unexpected{
            manifest_error(context + ".key_high precedes key_low")};
    ...
    const auto optional_text =
        [&](std::string_view field) -> Result<std::optional<std::string>> {
    ...
    if (interleaved) {
        auto source = path(value["interleaved_audio_path"],
                           context + ".interleaved_audio_path", base);
        if (!source)
            return std::unexpected{source.error()};
...
Result<VolumeSpec> volume(const Json &value, std::string context,
                          const std::filesystem::path &base) {
    if (auto valid =
            fields(value, context, {"name", "waveforms", "sample_banks"},
    ...
    if (!value["waveforms"].is_array() || !value["sample_banks"].is_array()) {
        return std::unexpected{manifest_error(
            context + " waveform and sample-bank fields must be arrays")};
    }
    auto name = text(value["name"], context + ".name");
    if (!name)
        return std::unexpected{name.error()};
    ...
    for (std::size_t index = 0; index < value["waveforms"].size(); ++index) {
        ...
        if (!waveform_ids.insert(item->id).second)
            return std::unexpected{
                manifest_error(context + " has duplicate waveform ids")};
    ...
    for (std::size_t index = 0; index < value["sample_banks"].size(); ++index) {
        auto item = sample_bank(
            value["sample_banks"][index],
            context + ".sample_banks[" + std::to_string(index) + "]", base);
        if (!item)
            return std::unexpected{item.error()};
        if (!bank_names.insert(item->name).second)
            return std::unexpected{
                manifest_error(context + " has duplicate sample-bank names")};
        if (item->waveform_id && !waveform_ids.contains(*item->waveform_id))
            return std::unexpected{
                manifest_error(context + " references an unknown waveform")};
        if (item->right_waveform_id &&
            (!waveform_ids.contains(*item->right_waveform_id) ||
             item->right_waveform_id == item->waveform_id))
            return std::unexpected{manifest_error(
                context + " has an invalid right waveform reference")};
        result.sample_banks.push_back(std::move(*item));
    }
    const auto &groups = value.contains("sample_bank_groups")
                             ? value["sample_bank_groups"]
                             : Json::array();
    if (!groups.is_array()) {
        return std::unexpected{
            manifest_error(context + ".sample_bank_groups must be an array")};
    }
    std::set<std::string> group_names;
    for (std::size_t index = 0; index < groups.size(); ++index) {
        const auto group_context =
            context + ".sample_bank_groups[" + std::to_string(index) + "]";
        const auto &row = groups[index];
        if (auto valid = fields(row, group_context, {"name"},
                                {"member_sample_bank", "member_sample_banks"});
            !valid) {
            return std::unexpected{valid.error()};
        }
        ...
        if (singular == plural) {
            return std::unexpected{manifest_error(
                group_context + " must contain exactly one member field")};
        }
        ...
        if (!group_names.insert(*group_name).second) {
            return std::unexpected{manifest_error(
                context + " has duplicate sample-bank group names")};
        }
        SampleBankGroupSpec group{*group_name, {}};
        if (singular) {
            auto member = text(row["member_sample_bank"],
                               group_context + ".member_sample_bank");
            if (!member)
            ...
            if (!row["member_sample_banks"].is_array()) {
                return std::unexpected{manifest_error(
                    group_context + ".member_sample_banks must be an array")};
            }
            for (std::size_t member_index = 0;
                 member_index < row["member_sample_banks"].size();
                 ++member_index) {
                auto member = text(row["member_sample_banks"][member_index],
                                   group_context + ".member_sample_banks[" +
        ...
        const std::set<std::string> unique_members{
            group.member_sample_banks.begin(), group.member_sample_banks.end()};
        if (group.member_sample_banks.empty() ||
            group.member_sample_banks.size() > 3U ||
            unique_members.size() != group.member_sample_banks.size()) {
            return std::unexpected{manifest_error(
                group_context + " must contain 1..3 distinct members")};
        }
        for (const auto &member : group.member_sample_banks) {
            if (!bank_names.contains(member)) {
                return std::unexpected{manifest_error(
                    group_context + " references an unknown sample bank")};
            }
        }
        result.sample_bank_groups.push_back(std::move(group));
    }
    const auto &programs =
        value.contains("programs") ? value["programs"] : Json::array();
    if (!programs.is_array()) {
        return std::unexpected{
            manifest_error(context + ".programs must be an array")};
    }
    std::set<std::uint64_t> program_numbers;
    for (std::size_t index = 0; index < programs.size(); ++index) {
        const auto program_context =
            context + ".programs[" + std::to_string(index) + "]";
        const auto &row = programs[index];
        if (auto valid =
                fields(row, program_context, {"number", "assignments"});
            !valid) {
            return std::unexpected{valid.error()};
        }
        auto number =
            integer(row["number"], program_context + ".number", 1, 128);
        if (!number)
            return std::unexpected{number.error()};
        if (!program_numbers.insert(*number).second) {
            return std::unexpected{
                manifest_error(context + " has duplicate Program numbers")};
        }
        if (!row["assignments"].is_array()) {
            return std::unexpected{manifest_error(
                program_context + ".assignments must be an array")};
        }
        ProgramSpec program{static_cast<std::uint8_t>(*number), {}};
        for (std::size_t assignment_index = 0;
             assignment_index < row["assignments"].size(); ++assignment_index) {
            const auto assignment_context = program_context + ".assignments[" +
                                            std::to_string(assignment_index) +
                                            "]";
            const auto &assignment = row["assignments"][assignment_index];
            if (auto valid =
                    fields(assignment, assignment_context, {"receive_channel"},
            ...
            if (bank == group)
                return std::unexpected{manifest_error(
                    assignment_context + " must contain exactly one target")};
            const auto target_field =
                bank ? "sample_bank" : "sample_bank_group";
            auto target = text(assignment[target_field],
                               assignment_context + "." + target_field);
            auto channel =
                integer(assignment["receive_channel"],
                        assignment_context + ".receive_channel", 1, 16);
            if (!target)
                return std::unexpected{target.error()};
            if (!channel)
                return std::unexpected{channel.error()};
            if ((bank && !bank_names.contains(*target)) ||
                (group && !group_names.contains(*target))) {
                return std::unexpected{manifest_error(
                    assignment_context + " references an unknown target")};
            }
            program.assignments.push_back(
                {bank ? "SBNK" : "SBAC", *target,
...
Result<HdsBuildManifest> parse(const Json &root,
                               const std::filesystem::path &base) {
    if (auto valid = fields(root, "manifest",
                            {"schema_version", "size_bytes", "partitions"});
        !valid)
        return std::unexpected{valid.error()};
    ...
    if (*version != "1.0")
        return std::unexpected{
            manifest_error("manifest.schema_version must be '1.0'")};
    auto size = integer(root["size_bytes"], "manifest.size_bytes",
                        minimum_hds_size, maximum_hds_size);
    if (!size)
        return std::unexpected{size.error()};
    if (*size % 512U != 0U)
        return std::unexpected{
            manifest_error("manifest.size_bytes must be a multiple of 512")};
    if (!root["partitions"].is_array() || root["partitions"].empty() ||
        root["partitions"].size() > 8U)
        return std::unexpected{
            manifest_error("manifest.partitions must contain 1..8 partitions")};
    ...
    for (std::size_t index = 0; index < root["partitions"].size(); ++index) {
        ...
        const auto context =
            "manifest.partitions[" + std::to_string(index) + "]";
        ...
        if (!row["volumes"].is_array() || row["volumes"].empty())
            return std::unexpected{
                manifest_error(context + ".volumes must be a non-empty array")};
        PartitionSpec partition{*name, {}};
        for (std::size_t volume_index = 0; volume_index < row["volumes"].size();
             ++volume_index) {
...
Result<MediaBuildManifest> parse_media(const Json &root,
                                       const std::filesystem::path &base) {
    ...
    if (transfer == authored) {
        return std::unexpected{
            manifest_error("manifest must contain exactly one of transfer or "
                           "authored_volume")};
    }
    ...
    if (*format == "fat12_floppy") {
        ...
        return std::unexpected{manifest_error(
            "manifest.format must be 'fat12_floppy' or 'iso9660'")};
    }
    if (transfer) {
        const auto &row = root["transfer"];
        if (auto valid = fields(row, "manifest.transfer", {"source_path"},
                                {"selection", "root_object_keys"});
            !valid) {
            return std::unexpected{valid.error()};
        }
        auto source =
            path(row["source_path"], "manifest.transfer.source_path", base);
        if (!source)
            return std::unexpected{source.error()};
        ...
        if (row.contains("selection")) {
            ...
            if (*value == "all") {
                ...
                return std::unexpected{manifest_error(
                    "manifest.transfer.selection must be 'roots' or 'all'")};
            }
        }
        const bool has_roots = row.contains("root_object_keys");
        if (selection == SavedObjectSelection::all && has_roots) {
            return std::unexpected{
                manifest_error("manifest.transfer.root_object_keys must be "
                               "omitted when selection is 'all'")};
        }
        if (selection == SavedObjectSelection::roots &&
            (!has_roots || !row["root_object_keys"].is_array() ||
             row["root_object_keys"].empty())) {
            return std::unexpected{
                manifest_error("manifest.transfer.root_object_keys must be a "
                               "non-empty array for root selection")};
        }
        SavedObjectTransferSpec spec{*source, {}, selection};
        if (selection == SavedObjectSelection::roots) {
            std::set<std::string> keys;
            for (std::size_t index = 0; index < row["root_object_keys"].size();
                 ++index) {
                auto key = text(row["root_object_keys"][index],
                                "manifest.transfer.root_object_keys[" +
                                    std::to_string(index) + "]");
                if (!key)
                    return std::unexpected{key.error()};
                if (!keys.insert(*key).second)
                    return std::unexpected{
                        manifest_error("manifest.transfer.root_object_keys "
        ...
        auto parsed =
            volume(root["authored_volume"], "manifest.authored_volume", base);
        if (!parsed)
            return std::unexpected{parsed.error()};
    ...
    if (root.contains("iso")) {
        ...
        if (auto valid = fields(iso, "manifest.iso",
                                {"volume_id", "raw_group", "group_name",
                                 "raw_volume", "volume_name"});
            !valid) {
            return std::unexpected{valid.error()};
        }
        ...
        return std::unexpected{
            manifest_error("manifest.iso is required for ISO9660 output")};
    }
    return result;
}
...
parse_hds_build_manifest(std::string_view json,
    ...
    try {
        ...
        return std::unexpected{manifest_error(
            std::string{"invalid HDS manifest JSON: "} + error.what())};
    }
}
...
parse_media_build_manifest(std::string_view json,
    ...
    try {
        ...
        return std::unexpected{manifest_error(
            std::string{"invalid media manifest JSON: "} + error.what())};
    }
}
...
load_media_build_manifest(const std::filesystem::path &path) {
    ...
    if (!input)
        return std::unexpected{make_error(ErrorCode::io_open_failed,
                                          ErrorCategory::io,
                                          "could not open media manifest")};
    std::ostringstream contents;
    contents << input.rdbuf();
    if (!input && !input.eof())
        return std::unexpected{make_error(ErrorCode::io_read_failed,
                                          ErrorCategory::io,
                                          "could not read media manifest")};
...
Result<std::string> serialize_build_manifest_template(BuildManifestKind kind) {
    try {
        ...
        return std::unexpected{manifest_error(
            std::string{"could not serialize build manifest template: "} +
            error.what())};
    }
}
...
write_build_manifest_template(BuildManifestKind kind,
    ...
    if (!overwrite && std::filesystem::exists(output_path, filesystem_error)) {
        return std::unexpected{
            make_error(ErrorCode::io_open_failed, ErrorCategory::io,
                       "refusing to replace existing build manifest: " +
                           text::path_to_utf8(output_path))};
    }
    if (filesystem_error) {
        return std::unexpected{
            make_error(ErrorCode::io_open_failed, ErrorCategory::io,
                       "could not inspect build manifest output path")};
    }
        ...
        if (!output) {
            ...
            return std::unexpected{
                make_error(ErrorCode::io_read_failed, ErrorCategory::io,
                           "could not write temporary build manifest")};
        }
    }
    if (overwrite)
    ...
    if (filesystem_error) {
        ...
        return std::unexpected{
            make_error(ErrorCode::io_open_failed, ErrorCategory::io,
                       "could not publish build manifest atomically")};
    }
    return {};
}

Result<std::vector<PartitionGeometry>>
plan_hds_geometry(const HdsBuildManifest &manifest) {
    if (manifest.partitions.empty() || manifest.partitions.size() > 8U ||
        manifest.size_bytes < minimum_hds_size ||
        manifest.size_bytes > maximum_hds_size ||
        manifest.size_bytes % 512U != 0U) {
        return std::unexpected{manifest_error(
            "HDS manifest geometry is outside the writer profile")};
    }
    constexpr std::uint64_t start = 3;
    constexpr std::uint64_t maximum_slot = 1'073'741'824 / 512U - 1U;
    const auto slot = std::min((manifest.size_bytes / 512U - (start - 1U)) /
                                   manifest.partitions.size(),
                               maximum_slot);
    const auto sectors = slot - 1U;
    if (sectors < 2045U)
        return std::unexpected{
            manifest_error("partition slots are too small for SFS")};
    ...
    for (std::size_t index = 0; index < manifest.partitions.size(); ++index) {
        ...
        result.push_back({static_cast<std::uint8_t>(index),
                          start + index * slot, slot, sectors, clusters, bitmap,
