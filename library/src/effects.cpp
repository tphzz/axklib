...
constexpr std::string_view table_note = "Table value-label transform; use as Likely display metadata.";
constexpr std::string_view validated_note = "Validated Program Effect display transform.";

const generated::EffectTypeData *type_data(std::uint16_t raw_type) {
    const auto found = std::ranges::find(generated::effect_types, raw_type, &generated::EffectTypeData::raw_type);
    return found == generated::effect_types.end() ? nullptr : &*found;
}

const generated::EffectParameterData *parameter_data(std::uint16_t raw_type, std::uint8_t parameter_number) {
    const auto found = std::ranges::find_if(generated::effect_parameters, [&](const auto &row) {
        return row.raw_type == raw_type && row.parameter_number == parameter_number;
    });
...
std::string signed_label(std::int32_t value) { return value > 0 ? std::format("+{}", value) : std::to_string(value); }
...
std::string tenths_ms(std::int32_t tenths) {
    return tenths % 10 == 0 ? std::format("{}ms", tenths / 10)
...
std::optional<std::string> numeric_display(std::string_view source, std::uint8_t raw_value, std::int32_t index) {
    ...
    if (source.starts_with("Delay_01_") || source.starts_with("DelayTime_01_") || source == "Initial_Delay")
...
std::optional<std::string_view> enum_label(std::string_view table, std::int32_t index) {
    if (index < 0)
        return std::nullopt;
    const auto found = std::ranges::find_if(generated::effect_enum_values, [&](const auto &row) {
        return row.table == table && row.index == static_cast<std::uint32_t>(index);
    });
    return found == generated::effect_enum_values.end() ? std::nullopt : std::optional<std::string_view>{found->label};
}

ModelRequirement shared_requirement() {
    return {"a4000_a5000", {"a4000", "a5000"}, "Likely", "A4000/A5000 shared effect table or Effect1-3 slot."};
}

ModelRequirement a5000_requirement() { return {"a5000_only", {"a5000"}, "Likely", "A5000-only effect feature."}; }

ModelRequirement unknown_requirement() {
    return {"unknown", {}, "Unknown", "No model compatibility rule is known for this value."};
}

} // namespace

std::optional<EffectProfile> parse_effect_profile(std::string_view value) noexcept {
...
std::optional<EffectTypeInfo> effect_type_info(std::uint16_t raw_type, EffectProfile profile) noexcept {
    if (!effect_type_supported(raw_type, profile))
        return std::nullopt;
    const auto *row = type_data(raw_type);
    return EffectTypeInfo{row->raw_type, row->printed_number, row->printed_label, row->ui_label, row->validated};
}

std::optional<EffectParameterInfo> effect_parameter_info(std::uint16_t raw_type, std::uint8_t parameter_number,
...
EffectDisplayValue format_effect_parameter(std::optional<std::uint16_t> raw_type, std::uint8_t parameter_number,
    ...
    if (parameter == nullptr)
        return {{}, std::nullopt, "Unknown", {}, "No effect parameter table row for this raw type/parameter."};
    if (!index)
        return {{},
                std::nullopt,
                "Unknown",
    ...
    if (*index < 0)
        return {{}, index, "Unknown", std::string{parameter->table_source}, "Computed table index is below zero."};
    if (const auto label = enum_label(parameter->value_source, *index))
        return {std::string{*label}, index, "Likely",
                std::format("{}:{}", parameter->table_source, parameter->value_source), std::string{table_note}};
    const auto has_enum_table = std::ranges::any_of(
        generated::effect_enum_values, [&](const auto &row) { return row.table == parameter->value_source; });
    if (has_enum_table)
        return {{},
                index,
                "Unknown",
    ...
    if (const auto numeric = numeric_display(parameter->value_source, *raw_value, *index))
        return {*numeric, index, "Likely", std::format("{}:{}", parameter->table_source, parameter->value_source),
                std::string{table_note}};
    return {{},
            index,
            "Unknown",
            std::string{parameter->table_source},
            std::format("No table display labels for value source {}.", parameter->value_source)};
}
...
ModelRequirement effect_output_destination_requirement(std::optional<std::uint8_t> raw_value) {
    if (!raw_value)
        return unknown_requirement();
