...
bool fits_u16(std::size_t value) {
...
std::optional<std::uint16_t> read_u16(std::span<const std::byte> source,
                                      std::size_t offset) {
...
std::optional<std::uint32_t> read_u32(std::span<const std::byte> source,
                                      std::size_t offset) {
...
bool safe_archive_path(std::string_view path) {
    if (path.empty() || path.front() == '/' || path.back() == '/' ||
        !text::is_valid_utf8(path) || path.contains('\\') ||
        path.contains('\0')) {
        return false;
    }
    std::size_t begin{};
    while (begin <= path.size()) {
        ...
        const auto component = path.substr(begin, end == std::string_view::npos
                                                      ? path.size() - begin
                                                      : end - begin);
...
Result<void> validate_entries(const std::vector<ArchiveEntry> &entries,
                              const ArchiveLimits &limits) {
    if (limits.maximum_archive_bytes < end_header_size)
        return std::unexpected{
            archive_error("configured package archive limit is too small")};
    if (entries.empty())
    ...
    if (entries.size() > limits.maximum_entries || !fits_u16(entries.size()))
        return std::unexpected{archive_error(
            "package archive entry count exceeds the configured limit")};
    if (entries.front().path != "manifest.json")
        return std::unexpected{archive_error(
            "manifest.json must be the first package archive entry")};
    ...
    for (const auto &entry : entries) {
        ...
        if (!previous.empty() && entry.path <= previous)
            return std::unexpected{archive_error(
                "package archive entries are not in lexical path order")};
        previous = entry.path;
        if (!paths.emplace(entry.path).second)
            return std::unexpected{archive_error(
                "package archive contains a duplicate entry path")};
        if (entry.bytes.size() > limits.maximum_entry_bytes ||
            !fits_u32(entry.bytes.size()))
            return std::unexpected{archive_error(
                "package archive entry exceeds the configured size limit")};
        if (entry.bytes.size() > limits.maximum_total_bytes - total)
            return std::unexpected{archive_error(
                "package archive expanded size exceeds the configured limit")};
        total += entry.bytes.size();
        const auto entry_archive_size =
            static_cast<std::uint64_t>(local_header_size) +
            central_header_size + entry.path.size() * 2U + entry.bytes.size();
        if (entry_archive_size > limits.maximum_archive_bytes - archive_size)
            return std::unexpected{archive_error(
                "package archive exceeds the configured archive limit")};
        archive_size += entry_archive_size;
    }
    return {};
}

void compress_sha256_block(std::array<std::uint32_t, 8> &state,
    ...
    for (std::size_t index = 16U; index < words.size(); ++index) {
        const auto low = std::rotr(words[index - 15U], 7) ^
                         std::rotr(words[index - 15U], 18) ^
                         (words[index - 15U] >> 3U);
        const auto high = std::rotr(words[index - 2U], 17) ^
...
class Sha256State {
  public:
    void update(std::span<const std::byte> bytes) {
        ...
        while (!bytes.empty()) {
            const auto count =
                std::min(bytes.size(), block_.size() - block_size_);
    ...
    [[nodiscard]] Sha256Digest finish() {
        ...
        for (std::size_t index = 0; index < state_.size(); ++index) {
            ...
            result[index * 4U + 1U] =
                static_cast<std::byte>(state_[index] >> 16U);
            result[index * 4U + 2U] =
                static_cast<std::byte>(state_[index] >> 8U);
...
Result<Sha256Digest> sha256_reader(const RandomAccessReader &reader,
    ...
    for (std::uint64_t offset = 0U; offset < reader.size();) {
        ...
        const auto count = static_cast<std::size_t>(
            std::min<std::uint64_t>(buffer.size(), reader.size() - offset));
...
Result<std::vector<std::byte>> write_archive(std::vector<ArchiveEntry> entries,
                                             const ArchiveLimits &limits) {
    ...
    for (const auto &entry : entries) {
        if (!fits_u32(result.size()))
            return std::unexpected{archive_error(
                "package archive exceeds the ZIP32 offset range")};
        const auto size = static_cast<std::uint32_t>(entry.bytes.size());
        const auto checksum = crc32(entry.bytes);
        central.push_back(
            {&entry, checksum, static_cast<std::uint32_t>(result.size())});
    ...
    if (!fits_u32(result.size()))
        return std::unexpected{
            archive_error("package archive exceeds the ZIP32 offset range")};
    ...
    if (!fits_u32(central_size))
        return std::unexpected{
            archive_error("package archive central directory exceeds ZIP32")};
...
Result<ArchiveInspection> inspect_archive(const RandomAccessReader &reader,
                                          const CancellationToken &cancellation,
                                          const ArchiveLimits &limits) {
    const auto read_bytes =
        ...
        if (offset > reader.size() || size > reader.size() - offset)
            return std::unexpected{
                archive_error("package archive metadata is truncated")};
    ...
    if (archive_size < end_header_size)
        return std::unexpected{archive_error(
            "package archive is truncated before the end record")};
    ...
    if (!signature || !disk || !central_disk || !disk_entries ||
        !total_entries || !central_size || !central_offset || !comment_size ||
        *signature != end_signature || *disk != 0U || *central_disk != 0U ||
        *disk_entries != *total_entries || *comment_size != 0U) {
        return std::unexpected{
            archive_error("package archive has an invalid ZIP end record")};
    }
    ...
    if (*central_size > limits.maximum_directory_bytes)
        return std::unexpected{archive_error(
            "package archive central directory exceeds the configured limit")};
    if (static_cast<std::uint64_t>(*central_offset) + *central_size !=
        end_offset)
        return std::unexpected{archive_error(
            "package archive central directory bounds are invalid")};
    ...
    for (std::size_t index = 0; index < *total_entries; ++index) {
        if (cursor > central->size() ||
            central_header_size > central->size() - cursor)
            return std::unexpected{archive_error(
                "package archive central directory is truncated")};
        ...
        if (!central_sig || !made_by || !needed || !flags || !method || !time ||
            !date || !checksum || !compressed || !expanded || !name_size ||
            !extra_size || !entry_comment || !start_disk ||
            !internal_attributes || !external_attributes || !local_offset ||
            *central_sig != central_signature || *made_by != zip_version ||
            *needed != zip_version || *flags != utf8_flag ||
            *method != stored_method || *time != fixed_time ||
            *date != fixed_date || *compressed != *expanded ||
            *name_size == 0U || *extra_size != 0U || *entry_comment != 0U ||
            *start_disk != 0U || *internal_attributes != 0U ||
            *external_attributes != 0U) {
            return std::unexpected{archive_error(
                "package archive central entry violates the profile")};
        }
        const auto record_size = central_header_size + *name_size;
        if (record_size > central->size() - cursor)
            return std::unexpected{archive_error(
                "package archive central entry name is truncated")};
        const auto name_bytes = std::span{*central}.subspan(
            cursor + central_header_size, *name_size);
        std::string path(reinterpret_cast<const char *>(name_bytes.data()),
                         name_bytes.size());
        if (!safe_archive_path(path))
            return std::unexpected{archive_error(
                "package archive contains an invalid entry path")};
        directory.push_back(
            {std::move(path), *checksum, *expanded, *local_offset});
        cursor += record_size;
    }
    if (cursor != central->size())
        return std::unexpected{archive_error(
            "package archive central directory contains trailing data")};
    ...
    for (const auto &item : directory) {
        ...
        if (item.local_offset != expected_local_offset)
            return std::unexpected{archive_error(
                "package archive local entries are not contiguous")};
        if (item.size > limits.maximum_entry_bytes ||
            item.size > limits.maximum_total_bytes - total)
            return std::unexpected{archive_error(
                "package archive expanded size exceeds the configured limit")};
        total += item.size;
        ...
        if (!local_sig || !needed || !flags || !method || !time || !date ||
            !checksum || !compressed || !expanded || !name_size ||
            !extra_size || *local_sig != local_signature ||
            *needed != zip_version || *flags != utf8_flag ||
            *method != stored_method || *time != fixed_time ||
            *date != fixed_date || *checksum != item.checksum ||
            *compressed != item.size || *expanded != item.size ||
            *name_size != item.path.size() || *extra_size != 0U) {
            return std::unexpected{archive_error(
                "package archive local entry disagrees with its directory")};
        }
        auto local_name = read_bytes(
            static_cast<std::uint64_t>(item.local_offset) + local_header_size,
            *name_size);
        if (!local_name)
            return std::unexpected{local_name.error()};
        const std::string_view local_name_text(
            reinterpret_cast<const char *>(local_name->data()),
            local_name->size());
        if (local_name_text != item.path)
            return std::unexpected{
                archive_error("package archive local entry name disagrees")};
        const auto data_offset = static_cast<std::uint64_t>(item.local_offset) +
                                 local_header_size +
                                 static_cast<std::uint64_t>(*name_size);
        if (data_offset > *central_offset ||
            item.size > *central_offset - data_offset)
            return std::unexpected{
                archive_error("package archive entry data is truncated")};
        result.entries.push_back(
            {item.path, item.checksum, item.size, data_offset});
        if (item.path == "manifest.json") {
            ...
            if (crc32(*manifest) != item.checksum)
                return std::unexpected{
                    archive_error("package manifest CRC-32 mismatch")};
    ...
    if (expected_local_offset != *central_offset)
        return std::unexpected{
            archive_error("package archive contains undeclared local data")};
    if (result.entries.empty() ||
        result.entries.front().path != "manifest.json" ||
...
read_archive(std::span<const std::byte> archive, const ArchiveLimits &limits) {
    ...
    for (std::size_t index = 0; index < *total_entries; ++index) {
        ...
        const auto name_bytes =
            archive.subspan(cursor + central_header_size, *name_size);
        std::string path(reinterpret_cast<const char *>(name_bytes.data()),
                         name_bytes.size());
        if (!safe_archive_path(path))
            return std::unexpected{archive_error(
    ...
    for (const auto &item : directory) {
        ...
        if (local > *central_offset ||
            local_header_size > *central_offset - local)
            return std::unexpected{
                archive_error("package archive local entry is truncated")};
        ...
        if (data_offset > *central_offset ||
            ...
            return std::unexpected{
                archive_error("package archive entry data is truncated")};
        const auto name_bytes =
            archive.subspan(local + local_header_size, *name_size);
        ...
        if (crc32(data) != item.checksum)
            return std::unexpected{
                archive_error("package archive entry CRC-32 mismatch")};
        entries.push_back(
            {item.path, std::vector<std::byte>{data.begin(), data.end()}});
        expected_local_offset = data_offset + item.size;
    }
    if (expected_local_offset != *central_offset)
        return std::unexpected{
