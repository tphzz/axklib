...
Result<void> validate_entries(const std::vector<ArchiveEntry> &entries,
  ...
  if (entries.size() > limits.maximum_entries || !fits_u16(entries.size()))
    return std::unexpected{
        archive_error("package archive entry count exceeds the configured limit")};
  ...
  for (const auto &entry : entries) {
    ...
    if (!previous.empty() && entry.path <= previous)
      return std::unexpected{
          archive_error("package archive entries are not in lexical path order")};
    previous = entry.path;
    ...
    if (entry.bytes.size() > limits.maximum_entry_bytes || !fits_u32(entry.bytes.size()))
      return std::unexpected{
          archive_error("package archive entry exceeds the configured size limit")};
    if (entry.bytes.size() > limits.maximum_total_bytes - total)
      return std::unexpected{
          archive_error("package archive expanded size exceeds the configured limit")};
    total += entry.bytes.size();
    const auto entry_archive_size = static_cast<std::uint64_t>(local_header_size) +
                                    central_header_size + entry.path.size() * 2U +
                                    entry.bytes.size();
...
void compress_sha256_block(std::array<std::uint32_t, 8> &state,
  ...
  for (std::size_t index = 16U; index < words.size(); ++index) {
    const auto low = std::rotr(words[index - 15U], 7) ^ std::rotr(words[index - 15U], 18) ^
                     (words[index - 15U] >> 3U);
    const auto high = std::rotr(words[index - 2U], 17) ^ std::rotr(words[index - 2U], 19) ^
                      (words[index - 2U] >> 10U);
...
Result<Sha256Digest> sha256_reader(const RandomAccessReader &reader,
  ...
  for (std::uint64_t offset = 0U; offset < reader.size();) {
    ...
    const auto count =
        static_cast<std::size_t>(std::min<std::uint64_t>(buffer.size(), reader.size() - offset));
...
Result<ArchiveInspection> inspect_archive(const RandomAccessReader &reader,
                                          const CancellationToken &cancellation,
                                          const ArchiveLimits &limits) {
  const auto read_bytes = [&](std::uint64_t offset,
  ...
  if (!signature || !disk || !central_disk || !disk_entries || !total_entries || !central_size ||
      !central_offset || !comment_size || *signature != end_signature || *disk != 0U ||
      *central_disk != 0U || *disk_entries != *total_entries || *comment_size != 0U) {
  ...
  if (*central_size > limits.maximum_directory_bytes)
    return std::unexpected{
        archive_error("package archive central directory exceeds the configured limit")};
  ...
  for (std::size_t index = 0; index < *total_entries; ++index) {
    ...
    if (!central_sig || !made_by || !needed || !flags || !method || !time || !date || !checksum ||
        !compressed || !expanded || !name_size || !extra_size || !entry_comment || !start_disk ||
        !internal_attributes || !external_attributes || !local_offset ||
        *central_sig != central_signature || *made_by != zip_version || *needed != zip_version ||
        *flags != utf8_flag || *method != stored_method || *time != fixed_time ||
        *date != fixed_date || *compressed != *expanded || *name_size == 0U || *extra_size != 0U ||
        *entry_comment != 0U || *start_disk != 0U || *internal_attributes != 0U ||
  ...
  if (cursor != central->size())
    return std::unexpected{
        archive_error("package archive central directory contains trailing data")};
  ...
  for (const auto &item : directory) {
    ...
    if (!local_sig || !needed || !flags || !method || !time || !date || !checksum || !compressed ||
        !expanded || !name_size || !extra_size || *local_sig != local_signature ||
        *needed != zip_version || *flags != utf8_flag || *method != stored_method ||
        *time != fixed_time || *date != fixed_date || *checksum != item.checksum ||
        *compressed != item.size || *expanded != item.size || *name_size != item.path.size() ||
        *extra_size != 0U) {
      return std::unexpected{
          archive_error("package archive local entry disagrees with its directory")};
    }
    auto local_name =
        read_bytes(static_cast<std::uint64_t>(item.local_offset) + local_header_size, *name_size);
    if (!local_name)
      return std::unexpected{local_name.error()};
    const std::string_view local_name_text(reinterpret_cast<const char *>(local_name->data()),
                                           local_name->size());
    if (local_name_text != item.path)
      return std::unexpected{archive_error("package archive local entry name disagrees")};
    const auto data_offset = static_cast<std::uint64_t>(item.local_offset) + local_header_size +
...
Result<std::vector<ArchiveEntry>> read_archive(std::span<const std::byte> archive,
                                               const ArchiveLimits &limits) {
  ...
  for (const auto &item : directory) {
    ...
    const std::string_view local_name(reinterpret_cast<const char *>(name_bytes.data()),
                                      name_bytes.size());
