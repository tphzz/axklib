...
constexpr std::string_view yamaha_iso_system_id =
    "APPLE COMPUTER, INC., TYPE: 0002";

void little16(std::span<std::byte> bytes, std::size_t offset,
              std::uint16_t value) {
    bytes[offset] = static_cast<std::byte>(value & 0xffU);
    bytes[offset + 1U] = static_cast<std::byte>((value >> 8U) & 0xffU);
}

void big16(std::span<std::byte> bytes, std::size_t offset,
           std::uint16_t value) {
    bytes[offset] = static_cast<std::byte>((value >> 8U) & 0xffU);
    bytes[offset + 1U] = static_cast<std::byte>(value & 0xffU);
}

void both16(std::span<std::byte> bytes, std::size_t offset,
            std::uint16_t value) {
    little16(bytes, offset, value);
    big16(bytes, offset + 2U, value);
}

void little32(std::span<std::byte> bytes, std::size_t offset,
              std::uint32_t value) {
...
void big32(std::span<std::byte> bytes, std::size_t offset,
           std::uint32_t value) {
    for (std::size_t index = 0; index < 4U; ++index)
        bytes[offset + index] =
...
void both32(std::span<std::byte> bytes, std::size_t offset,
            std::uint32_t value) {
...
std::vector<std::byte> directory_record(const IsoNode &node,
...
std::vector<std::byte> label_file(std::string_view label) {
    ...
    std::ranges::transform(
        label.substr(0, result.size()), result.begin(),
        [](char character) { return static_cast<std::byte>(character); });
    return result;
}
...
std::uint8_t catalog_hash(std::span<const std::byte> value) {
    ...
    std::uint8_t result{};
    for (const auto byte :
         value.first(std::min<std::size_t>(value.size(), 16U))) {
        const auto character = std::to_integer<std::uint8_t>(byte);
        if (character == 0U)
            break;
        result = static_cast<std::uint8_t>((result ^ table[result & 3U]) +
                                           character);
    }
    return result;
}

std::vector<std::byte> catalog_record(std::string_view display_name,
                                      std::string_view filename) {
    std::vector<std::byte> result(32U);
    std::fill_n(result.begin() + 1, 16U, std::byte{' '});
    std::ranges::transform(
        display_name.substr(0, 16U), result.begin() + 1,
        [](char character) { return static_cast<std::byte>(character); });
    result[0] =
        static_cast<std::byte>(catalog_hash(std::span{result}.subspan(1, 16U)));
    std::ranges::transform(
        filename.substr(0, 11U), result.begin() + 18,
        [](char character) { return static_cast<std::byte>(character); });
    result[17] = static_cast<std::byte>(
        catalog_hash(std::as_bytes(std::span{filename})));
    return result;
}
...
std::vector<std::byte> disk_name_record(std::string_view filename) {
    ...
    std::ranges::transform(marker, result.begin() + 1, [](char character) {
...
void append_path_record(std::vector<std::byte> &table, const IsoNode &node,
    ...
    const std::string_view name = node.name.empty()
                                      ? std::string_view{"\0", 1}
    ...
    std::ranges::transform(
        name, table.begin() + static_cast<std::ptrdiff_t>(offset + 8U),
        [](char character) { return static_cast<std::byte>(character); });
}
...
Result<std::uint32_t>
checked_iso9660_sector_count(std::size_t directory_count,
                             std::span<const std::uint64_t> file_sizes) {
    constexpr std::uint64_t initial_sector_count = 20U;
    constexpr std::uint64_t maximum_sector_count =
...
Result<void> write_iso9660_image(const PreparedMediaImage &image,
    ...
    if (volumes.empty()) {
        volumes.push_back({manifest.raw_group, manifest.group_name,
                           manifest.raw_volume, manifest.volume_name,
                           image.objects});
    }
    if (volumes.empty())
    ...
    std::map<std::string, std::vector<const PreparedIsoVolume *>, std::less<>>
        groups;
    std::set<std::pair<std::string, std::string>> volume_ids;
    std::map<std::string, std::string, std::less<>> group_labels;
    for (const auto &volume : volumes) {
        if (!iso_identifier(volume.raw_group, 8U) ||
            volume.raw_volume.size() != 4U || volume.raw_volume[0] != 'F' ||
            volume.raw_volume == "F000" ||
            !std::ranges::all_of(
                std::string_view{volume.raw_volume}.substr(1),
                [](unsigned char value) { return std::isdigit(value) != 0; }) ||
            volume.group_name.empty() || volume.group_name.size() > 16U ||
            volume.volume_name.empty() || volume.volume_name.size() > 16U ||
        ...
        const auto [label, inserted] =
            group_labels.emplace(volume.raw_group, volume.group_name);
    ...
    for (const auto &[raw_group, group_volumes] : groups) {
        for (std::size_t index = 0; index < group_volumes.size(); ++index) {
            if (group_volumes[index]->raw_volume !=
                std::format("F{:03}", index + 1U)) {
                return std::unexpected{make_error(
                    ErrorCode::unsupported_profile, ErrorCategory::unsupported,
    ...
    const auto add_directory = [&](std::string name,
                                   std::size_t parent) -> Result<std::size_t> {
        ...
        if (std::ranges::find_if(nodes, [&](const auto &node) {
            ...
            return std::unexpected{make_error(ErrorCode::manifest_invalid,
    ...
    const auto add_file = [&](std::string name, std::vector<std::byte> data,
        ...
        if (!iso_identifier(name, 31U) ||
            std::ranges::find_if(nodes, [&](const auto &node) {
                return node.parent == parent && node.name == name;
            }) != nodes.end()) {
            return std::unexpected{
                make_error(ErrorCode::manifest_invalid, ErrorCategory::manifest,
    ...
    const auto ensure_directory =
        [&](std::string_view path) -> Result<std::size_t> {
        std::size_t parent{};
        std::size_t begin{};
    ...
    for (const auto &[raw_group, group_volumes] : groups) {
        ...
        for (const auto *volume : group_volumes) {
            const auto record =
                catalog_record(volume->volume_name, volume->raw_volume);
            group_catalog.insert(group_catalog.end(), record.begin(),
                                 record.end());
        }
        const auto group_label_filename =
            std::format("F{:03}", group_volumes.size() + 1U);
        const auto name_record = disk_name_record(group_label_filename);
        group_catalog.insert(group_catalog.end(), name_record.begin(),
                             name_record.end());
        if (auto added = add_file("0000", std::move(group_catalog), *group);
            !added)
            return std::unexpected{added.error()};
        std::vector<std::pair<const PreparedIsoVolume *, std::size_t>>
            volume_nodes;
        ...
        if (auto added =
                add_file(group_label_filename,
                         label_file(group_labels.at(raw_group)), *group);
            !added) {
            return std::unexpected{added.error()};
        }
        for (const auto &[volume, volume_node] : volume_nodes) {
            std::map<std::string, std::vector<const PreparedMediaObject *>>
                categories;
            for (const auto &object : volume->objects)
                categories[category(object.type)].push_back(&object);
            for (auto &[name, objects] : categories) {
                std::ranges::sort(
                    objects, [](const auto *left, const auto *right) {
                ...
                for (std::size_t index = 0; index < objects.size(); ++index) {
                    ...
                    const auto record =
                        catalog_record(objects[index]->name, filename);
                    catalog_bytes.insert(catalog_bytes.end(), record.begin(),
                                         record.end());
                    filenames.push_back(std::move(filename));
                }
                if (auto added = add_file("0000", std::move(catalog_bytes),
                                          *category_node);
                    !added)
                    return std::unexpected{added.error()};
                for (std::size_t index = 0; index < objects.size(); ++index) {
                    if (auto added =
                            add_file(std::move(filenames[index]),
                                     objects[index]->payload, *category_node);
                        !added) {
                        return std::unexpected{added.error()};
                    }
                }
    ...
    for (const auto &retained : image.retained_files) {
        ...
        if (auto added = add_file(std::string{name}, retained.payload, *parent);
            !added)
            return std::unexpected{added.error()};
    }
    ...
    const auto directory_count = static_cast<std::size_t>(
        std::ranges::count(nodes, true, &IsoNode::directory));
    const auto sector_count =
        checked_iso9660_sector_count(directory_count, file_sizes);
    if (!sector_count)
    ...
    for (auto &node : nodes) {
        if (node.directory)
            continue;
        node.sector = next_sector;
        next_sector += static_cast<std::uint32_t>(
            node.data.size() / sector_size +
    ...
    std::vector<std::byte> image_bytes(static_cast<std::size_t>(*sector_count) *
                                       sector_size);
    ...
    for (std::size_t index = 0; index < directory_indices.size(); ++index)
        path_numbers.emplace(directory_indices[index],
    ...
    for (const auto index : directory_indices) {
        const auto parent_number = index == 0
                                       ? std::uint16_t{1}
                                       : path_numbers.at(nodes[index].parent);
    ...
    std::ranges::copy(little_path,
                      image_bytes.begin() + little_path_sector * sector_size);
    std::ranges::copy(big_path,
                      image_bytes.begin() + big_path_sector * sector_size);

    for (const auto index : directory_indices) {
        const auto &node = nodes[index];
        const auto directory_offset =
            static_cast<std::size_t>(node.sector) * sector_size;
        std::size_t offset{};
        ...
        const auto parent =
            directory_record(nodes[index == 0 ? 0 : node.parent], dotdot);
        std::ranges::copy(self,
                          image_bytes.begin() +
                              static_cast<std::ptrdiff_t>(directory_offset));
        offset += self.size();
        std::ranges::copy(
            parent, image_bytes.begin() +
    ...
    const auto root_record =
        directory_record(nodes[0], std::array<std::byte, 1>{std::byte{0}});
    ...
    pvd[881] = std::byte{1};
    auto terminator =
        std::span{image_bytes}.subspan(17U * sector_size, sector_size);
