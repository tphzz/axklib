...
bool object_prefix(std::span<const std::byte> bytes) {
    ...
    return std::ranges::equal(
        bytes.first(object_magic.size()), object_magic, {},
...
MediaContainer::MediaContainer(MediaStorage storage)
...
const MediaStorage &MediaContainer::storage() const noexcept {
    return storage_;
}

std::span<const MediaValidationIssue>
MediaContainer::validation_issues() const noexcept {
...
MediaContainer::objects(std::size_t maximum_object_bytes,
    ...
    auto catalog =
        build_object_catalog(*sfs, maximum_object_bytes, cancellation);
    if (!catalog)
        return std::unexpected{catalog.error()};
    std::vector<MediaObject> result;
    for (const auto &item : catalog->objects) {
        const auto placement = item.placement;
        auto bytes = sfs->read_record_data(item.partition, item.sfs_id,
        ...
        result.push_back(
            {item.key,
             placement
                 ? std::format("{}/{}/{}", placement->volume_name,
             ...
             {placement ? placement->volume_name : std::string{},
              LabelStatus::confirmed, "SFS volume directory"},
             0,
             bytes->size(),
             item.object,
             std::move(*bytes),
...
Result<MediaContainer> open_media(const std::filesystem::path &path,
    ...
    const auto prefix_size = static_cast<std::size_t>(
        std::min<std::uint64_t>((*reader)->size(), 0x9000U));
    ...
    if (detail::object_prefix(*prefix)) {
        auto object = StandaloneObject::open(std::move(*reader),
    ...
    if (prefix->size() >=
            detail::iso_pvd_sector * detail::iso_sector_size + 6U &&
        std::to_integer<std::uint8_t>((
            *prefix)[detail::iso_pvd_sector * detail::iso_sector_size]) == 1U &&
        detail::clean_ascii(std::span{*prefix}.subspan(
            detail::iso_pvd_sector * detail::iso_sector_size + 1U, 5)) ==
            "CD001") {
        auto iso = IsoImage::open(std::move(*reader), text::path_to_utf8(path),
                                  cancellation);
        if (!iso)
    ...
    if (prefix->size() >= 512U && detail::le16(*prefix, 0x0b) >= 512U &&
        ...
        auto fat = FatImage::open(std::move(*reader), text::path_to_utf8(path),
                                  cancellation);
        if (!fat)
