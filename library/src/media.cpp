...
Error media_error(ErrorCode code, std::string message, std::string_view source, std::optional<std::uint64_t> offset) {
...
Result<std::vector<std::byte>> read_bytes(const RandomAccessReader &reader, std::uint64_t offset, std::size_t size,
...
std::uint32_t le32(std::span<const std::byte> bytes, std::size_t offset) {
    return static_cast<std::uint32_t>(le16(bytes, offset)) | static_cast<std::uint32_t>(le16(bytes, offset + 2)) << 16U;
}

std::uint16_t be16(std::span<const std::byte> bytes, std::size_t offset) {
    const auto value = static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(bytes[offset])) << 8U |
...
std::uint32_t be32(std::span<const std::byte> bytes, std::size_t offset) {
    return static_cast<std::uint32_t>(be16(bytes, offset)) << 16U | static_cast<std::uint32_t>(be16(bytes, offset + 2));
}
...
std::string upper_ascii(std::string value) {
    std::ranges::transform(value, value.begin(), [](unsigned char ch) { return static_cast<char>(std::toupper(ch)); });
    return value;
}
...
template <typename T> const T *variant_ptr(const MediaStorage &storage) { return std::get_if<T>(&storage); }
...
Result<std::vector<MediaObject>> MediaContainer::objects(MediaObjectReadMode mode, std::size_t maximum_object_bytes,
    ...
    for (const auto &item : catalog->objects) {
        ...
        result.push_back(
            {item.key,
             placement
                 ? std::format("{}/{}/{}", placement->volume_name, placement->category_name, placement->entry_name)
                 : item.key,
             item.scope_key,
             {},
             {},
