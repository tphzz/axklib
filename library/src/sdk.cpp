...
result<T> protect(Function &&function) noexcept {
    try {
...
result<std::filesystem::path> checked_path(const std::string &value,
                                           std::string label) {
    if (value.empty())
...
result<std::vector<PackageRootSelector>>
internal_roots(const std::vector<package_root_selector> &roots) {
    std::vector<PackageRootSelector> result_value;
    result_value.reserve(roots.size());
    for (const auto &root : roots) {
        if (root.partition_index &&
            *root.partition_index > std::numeric_limits<std::uint8_t>::max())
            return invalid_argument(
                "package root partition index is out of range");
        PackageRootSelector item;
        item.kind = internal_root_kind(root.kind);
        if (root.partition_index)
            item.partition_index =
                static_cast<std::uint8_t>(*root.partition_index);
...
package_issue_info public_package_issue(const PackageIssue &issue) {
    return {issue.code, issue.message, issue.fatal};
}
...
package_allocation_info
public_package_allocation(const PackageAllocationDelta &allocation) {
    return {
...
const std::vector<ContentNode> *find_children(const ContentTree &tree,
                                              std::string_view parent) {
    if (parent.empty())
        return &tree.roots;
    const std::vector<ContentNode> *result_value{};
    const auto visit = [&](const auto &self,
                           const std::vector<ContentNode> &nodes) -> void {
...
object_info public_object(const ObjectSnapshot &item) {
    ...
    } else if (const auto *bank =
                   std::get_if<CurrentSbnk>(&item.object.payload)) {
        value.member_count = bank->right.has_value() ? 2U : 1U;
    } else if (const auto *group =
                   std::get_if<CurrentSbac>(&item.object.payload)) {
        value.member_count = group->active_slot_count;
    } else if (const auto *program =
                   std::get_if<CurrentProg>(&item.object.payload)) {
...
content_page(const std::shared_ptr<const image_state> &state,
    ...
    if (children == nullptr)
        return invalid_argument(
            "content node is not part of this image session");
    ...
    for (std::uint64_t index = 0; index < count; ++index)
        output.items.push_back(
            public_node((*children)[static_cast<std::size_t>(begin + index)]));
    return output;
}

result<page<object_info>>
object_page(const std::shared_ptr<const image_state> &state,
    ...
    const auto begin =
        std::min<std::uint64_t>(offset, state->catalog.objects.size());
    const auto remaining =
        static_cast<std::uint64_t>(state->catalog.objects.size()) - begin;
    const auto count = std::min(limit, remaining);
    output.items.reserve(static_cast<std::size_t>(count));
    for (std::uint64_t index = 0; index < count; ++index)
        output.items.push_back(public_object(
            state->catalog.objects[static_cast<std::size_t>(begin + index)]));
    return output;
}
...
relationship_page(const std::shared_ptr<const image_state> &state,
                  std::uint64_t offset, std::uint64_t limit) {
    if (!state)
        return invalid_argument("image session is not open");
    if (limit == 0U)
    ...
    output.items.reserve(static_cast<std::size_t>(count));
    for (std::uint64_t index = 0; index < count; ++index)
        output.items.push_back(public_relationship(
            state->graph
                .relationships[static_cast<std::size_t>(begin + index)]));
    return output;
}
...
validation_issue_page(const std::shared_ptr<const image_state> &state,
                      std::uint64_t offset, std::uint64_t limit) {
    if (!state)
        return invalid_argument("image session is not open");
    if (limit == 0U)
    ...
    for (std::uint64_t index = 0; index < count; ++index) {
        const auto &issue =
            state->validation.issues[static_cast<std::size_t>(begin + index)];
        output.items.push_back(
            {issue.code, validation_severity_name(issue.severity),
             issue.message, issue.sampler_path, issue.object_key});
    }
    return output;
}
...
operation_context &
operation_context::operator=(operation_context &&) noexcept = default;
...
result<image> image::open(const std::string &utf8_path,
    ...
    return protect<image>([&]() -> result<image> {
        ...
        auto catalog = build_object_catalog(*container, 64U * 1024U * 1024U,
...
result<snapshot> image::make_snapshot() const {
    return protect<snapshot>([&]() -> result<snapshot> {
        ...
        snapshot output;
        output.impl_ =
            std::make_unique<snapshot::impl>(snapshot::impl{impl_->state});
        return output;
    });
}

result<page<content_node>>
image::content_children(const std::string &parent_id, std::uint64_t offset,
    ...
    return protect<page<content_node>>([&]() -> result<page<content_node>> {
        ...
        return content_page(impl_ ? impl_->state : nullptr, parent_id, offset,
                            limit);
    });
}

result<page<object_info>> image::objects(std::uint64_t offset,
                                         std::uint64_t limit,
...
result<page<relationship_info>>
image::relationships(std::uint64_t offset, std::uint64_t limit,
                     operation_context &context) const {
    return protect<page<relationship_info>>(
        [&]() -> result<page<relationship_info>> {
            ...
            const auto cancellation =
                context.impl_->cancellation.token().check();
            if (!cancellation)
                return public_error(cancellation.error());
            return relationship_page(impl_ ? impl_->state : nullptr, offset,
                                     limit);
        });
}
...
result<page<validation_issue>>
image::validation_issues(std::uint64_t offset, std::uint64_t limit,
                         operation_context &context) const {
    return protect<page<validation_issue>>(
        [&]() -> result<page<validation_issue>> {
            ...
            if (!cancellation)
                return public_error(cancellation.error());
            return validation_issue_page(impl_ ? impl_->state : nullptr, offset,
                                         limit);
        });
}

result<waveform_preview> image::preview(const std::string &object_key,
    ...
    return protect<waveform_preview>([&]() -> result<waveform_preview> {
        ...
        if (object_key.empty() || bin_count == 0U ||
            ...
            return invalid_argument(
                "waveform key and nonzero bin count are required");
        const auto found = std::ranges::find(impl_->state->catalog.objects,
                                             object_key, &ObjectSnapshot::key);
        if (found == impl_->state->catalog.objects.end())
            return error{error_code::object_missing,
                         ...
                         {}};
        auto waveform = decode_waveform(impl_->state->container, *found,
                                        context.impl_->cancellation.token());
        if (!waveform)
            return public_error(waveform.error());
        auto envelope = build_preview_envelope(
            *waveform, static_cast<std::size_t>(bin_count));
        if (!envelope)
...
image::waveform_pcm(const std::string &object_key,
                    operation_context &context) const {
    return protect<std::vector<std::uint8_t>>(
        [&]() -> result<std::vector<std::uint8_t>> {
            if (!impl_ || !impl_->state)
                return invalid_argument("image session is not open");
            if (!context.impl_)
            ...
            const auto found =
                std::ranges::find(impl_->state->catalog.objects, object_key,
                                  &ObjectSnapshot::key);
            if (found == impl_->state->catalog.objects.end())
                return error{error_code::object_missing,
                             error_category::object,
                             ...
                             {}};
            auto waveform =
                decode_waveform(impl_->state->container, *found,
                                context.impl_->cancellation.token());
            if (!waveform)
                return public_error(waveform.error());
...
image::export_audio(const std::string &utf8_output_directory,
    ...
    return protect<export_summary>([&]() -> result<export_summary> {
        ...
        if (!context.impl_)
            return invalid_argument("operation context is not initialized");
        auto directory =
            checked_path(utf8_output_directory, "output directory");
        if (!directory)
            return directory.error();
        auto plan = build_export_plan(
            impl_->state->container, impl_->state->catalog, impl_->state->graph,
            context.impl_->cancellation.token());
        if (!plan)
        ...
        export_summary output{audio->written_files.size(),
                              audio->warnings.size()};
...
portable_package &
portable_package::operator=(portable_package &&) noexcept = default;

result<portable_package> portable_package::open(const std::string &utf8_path,
                                                operation_context &context) {
    return protect<portable_package>([&]() -> result<portable_package> {
        ...
        if (!path)
            return path.error();
        auto opened = inspect_portable_package(
            *path, context.impl_->cancellation.token());
        if (!opened)
            return public_error(opened.error());
        portable_package result_value;
        result_value.impl_ =
            std::make_unique<impl>(impl{std::move(*opened), std::move(*path)});
        return result_value;
    });
}

result<package_export_summary>
portable_package::export_from(const std::string &utf8_source_path,
                              const std::vector<package_root_selector> &roots,
                              const std::string &utf8_output_path,
                              const write_options &options,
                              operation_context &context) {
    return protect<package_export_summary>(
        [&]() -> result<package_export_summary> {
            if (!context.impl_)
                return invalid_argument("operation context is not initialized");
            ...
            auto media =
                open_media(*source, context.impl_->cancellation.token());
            if (!media)
                return public_error(media.error());
            context.impl_->report({ProgressPhase::exporting, 0U, 1U,
                                   "exporting portable package",
                                   text::path_to_utf8(*output)});
            auto published = export_portable_package(
                *media, *selectors, *output, options.overwrite,
                context.impl_->cancellation.token());
            if (!published)
                return public_error(published.error());
            context.impl_->report({ProgressPhase::exporting, 1U, 1U,
...
result<std::vector<package_issue_info>> portable_package::issues() const {
    return protect<std::vector<package_issue_info>>(
        [&]() -> result<std::vector<package_issue_info>> {
            if (!impl_)
                return invalid_argument("portable package is not open");
            std::vector<package_issue_info> result_value;
            result_value.reserve(impl_->package.issues.size());
            std::ranges::transform(impl_->package.issues,
                                   std::back_inserter(result_value),
                                   public_package_issue);
            return result_value;
        });
}
...
package_import_plan::~package_import_plan() = default;
package_import_plan::package_import_plan(package_import_plan &&) noexcept =
    default;
package_import_plan &
package_import_plan::operator=(package_import_plan &&) noexcept = default;

result<package_import_plan>
package_import_plan::create(const std::string &utf8_target_path,
    ...
    return protect<package_import_plan>([&]() -> result<package_import_plan> {
        if (!context.impl_)
            return invalid_argument("operation context is not initialized");
        auto target =
            checked_path(utf8_target_path, "package import target path");
        if (!target)
            return target.error();
        ...
        for (const auto &path_value : utf8_package_paths) {
            ...
            if (!path)
                return path.error();
            auto package = open_portable_package(
                *path, context.impl_->cancellation.token());
            if (!package)
        ...
        for (const auto &destination : request.root_destinations) {
            if (destination.package_index >
                ...
                return invalid_argument(
                    "package destination index is out of range");
            }
            PackageRootDestination item;
            item.package_index =
                static_cast<std::size_t>(destination.package_index);
            item.root_index = static_cast<std::size_t>(destination.root_index);
            if (destination.partition_index)
                item.partition_index =
                    static_cast<std::uint8_t>(*destination.partition_index);
        ...
        for (const auto &rename : request.renames) {
            ...
            internal_request.policy.renames.push_back(
                {static_cast<std::size_t>(rename.package_index), rename.node_id,
                 rename.destination_name});
        }

        auto planned = plan_package_import(*target, packages, internal_request,
                                           context.impl_->cancellation.token());
        if (!planned)
...
result<package_import_summary> package_import_plan::summary() const {
    return protect<package_import_summary>(
        [&]() -> result<package_import_summary> {
            if (!impl_)
                return invalid_argument(
                    "package import plan is not initialized");
...
result<std::vector<package_issue_info>> package_import_plan::warnings() const {
    return protect<std::vector<package_issue_info>>(
        [&]() -> result<std::vector<package_issue_info>> {
            ...
            std::ranges::transform(impl_->plan.warnings,
                                   std::back_inserter(result_value),
                                   public_package_issue);
            return result_value;
        });
...
result<std::vector<package_conflict_info>>
package_import_plan::conflicts() const {
    return protect<std::vector<package_conflict_info>>(
        [&]() -> result<std::vector<package_conflict_info>> {
            if (!impl_)
                return invalid_argument(
                    "package import plan is not initialized");
...
result<std::vector<package_action_info>> package_import_plan::actions() const {
    return protect<std::vector<package_action_info>>(
        [&]() -> result<std::vector<package_action_info>> {
            if (!impl_)
                return invalid_argument(
                    "package import plan is not initialized");
            std::vector<package_action_info> result_value;
            result_value.reserve(impl_->plan.objects.size());
            std::ranges::transform(impl_->plan.objects,
                                   std::back_inserter(result_value),
                                   public_package_action);
            return result_value;
        });
}

result<std::vector<package_allocation_info>>
package_import_plan::allocation() const {
    return protect<std::vector<package_allocation_info>>(
        [&]() -> result<std::vector<package_allocation_info>> {
            if (!impl_)
                return invalid_argument(
                    "package import plan is not initialized");
            ...
            std::ranges::transform(impl_->plan.allocation,
                                   std::back_inserter(result_value),
                                   public_package_allocation);
            return result_value;
        });
}

result<package_import_result>
package_import_plan::apply(const std::string &utf8_output_path,
                           const write_options &options,
                           operation_context &context) {
    return protect<package_import_result>(
        [&]() -> result<package_import_result> {
            if (!impl_)
                return invalid_argument(
                    "package import plan is not initialized");
            ...
            if (impl_->owner != std::this_thread::get_id())
                return invalid_argument(
                    "package import plan used from a different thread");
            auto output =
                checked_path(utf8_output_path, "package import output path");
            if (!output)
                return output.error();
            auto applied = apply_package_import(
                impl_->target_path, impl_->packages, impl_->plan, *output,
...
result<page<content_node>>
snapshot::content_children(const std::string &parent_id, std::uint64_t offset,
...
result<page<object_info>> snapshot::objects(std::uint64_t offset,
                                            std::uint64_t limit) const {
...
result<page<relationship_info>>
snapshot::relationships(std::uint64_t offset, std::uint64_t limit) const {
    return protect<page<relationship_info>>([&]() {
...
result<page<validation_issue>>
snapshot::validation_issues(std::uint64_t offset, std::uint64_t limit) const {
    return protect<page<validation_issue>>([&]() {
        return validation_issue_page(impl_ ? impl_->state : nullptr, offset,
                                     limit);
    });
...
result<build_plan>
build_plan::from_manifest(const std::string &utf8_manifest_path,
                          operation_context &context) {
    return protect<build_plan>([&]() -> result<build_plan> {
        ...
        if (auto manifest = load_hds_build_manifest(*path); manifest) {
            ...
            output.impl_ = std::make_unique<impl>(
                impl{std::move(*manifest), std::move(*geometry),
                     std::this_thread::get_id()});
        } else {
            ...
            output.impl_ = std::make_unique<impl>(impl{
                std::move(*media_manifest), {}, std::this_thread::get_id()});
        }
        return output;
    });
}
...
plan_summary build_plan::summary() const noexcept {
    ...
    return {0U, 0U,
            media.format == MediaImageFormat::fat12_floppy ? 1'474'560U : 0U,
            true};
}

result<void> build_plan::apply(const std::string &utf8_output_path,
                               const write_options &options,
                               operation_context &context) {
    return protect<void>([&]() -> result<void> {
        if (!impl_)
        ...
        if (!path)
            return path.error();
        if (const auto *hds = std::get_if<HdsBuildManifest>(&impl_->manifest)) {
            auto written = write_hds_image(*hds, *path, options.overwrite,
                                           context.impl_->cancellation.token());
            if (!written)
                return public_error(written.error());
        } else {
            auto written = write_media_image(
                std::get<MediaBuildManifest>(impl_->manifest), *path,
                options.overwrite, context.impl_->cancellation.token());
            if (!written)
                return public_error(written.error());
        }
...
result<transaction>
transaction::from_manifest(const std::string &utf8_source_path,
...
plan_summary transaction::summary() const noexcept {
    if (!impl_)
        return {};
    return {0U, impl_->plan.operations.size(), 0U,
            !impl_->plan.operations.empty()};
}

result<void> transaction::apply(const std::string &utf8_output_path,
                                const write_options &options,
                                operation_context &context) {
    return protect<void>([&]() -> result<void> {
        if (!impl_)
        ...
        if (!options.overwrite && std::filesystem::exists(*output))
            return error{error_code::io_open_failed,
                         error_category::io,
...
std::string render_error(const error &failure) {
    ...
    return axk::render_error(Error{static_cast<ErrorCode>(failure.code),
                                   static_cast<ErrorCategory>(failure.category),
