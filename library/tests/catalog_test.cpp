...
TEST(RelationshipGraph, MatchesMaintainedFixtureCountsAndSamplerHierarchy) {
  ...
  EXPECT_TRUE(std::ranges::all_of(graph.bitmap_comparisons,
                                  [](const auto &row) { return row.status == "match"; }));
  ...
  const auto bank_category = std::ranges::find(volume.children, std::string{"Sample Banks"},
                                               &axk::ContentNode::display_name);
  ASSERT_NE(bank_category, volume.children.end());
  const auto group = std::ranges::find(bank_category->children, std::string{"B New SmpBank"},
                                       &axk::ContentNode::display_name);
...
TEST(RelationshipGraph, PrefersUniqueCrossFolderLinkBeforeDuplicateLocalNames) {
  ...
  catalog.objects.push_back({"bank", axk::PartitionIndex{0}, axk::SfsId{1}, "iso:GROUP",
                             std::move(bank), bank_placement});
...
TEST(ProgramRelationships, UsesIsoBasisForMissingVisibleOffSampleBankGroup) {
  ...
  catalog.objects.push_back({"program", axk::PartitionIndex{0}, axk::SfsId{1}, "iso:GROUP",
                             std::move(program),
...
TEST(ProgramRelationships, KeepsVisibleOffAmbiguousGroupsAsDiagnosticCandidates) {
  ...
  for (const auto key : {"group-a", "group-b"}) {
    ...
    catalog.objects.push_back(
        {key, axk::PartitionIndex{0}, axk::SfsId{1}, "iso:GROUP", std::move(group), {}});
  }
  axk::CurrentProg current_program;
  axk::ProgAssignment assignment;
  ...
  program.payload = std::move(current_program);
  catalog.objects.push_back(
      {"program", axk::PartitionIndex{0}, axk::SfsId{2}, "iso:GROUP", std::move(program), {}});
...
TEST(WaveformOrphans, AllowsOnlyCompleteExactUnreferencedClassification) {
  ...
  const auto bank = object_named(*catalog, axk::ObjectType::sbnk, "sine wave");
  ASSERT_NE(bank, nullptr);
  std::erase_if(catalog->objects,
                [&](const axk::ObjectSnapshot &item) { return item.key == bank->key; });
