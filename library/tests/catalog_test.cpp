...
const axk::ObjectSnapshot *object_named(const axk::ObjectCatalog &catalog,
                                        axk::ObjectType type,
                                        std::string_view name) {
    const auto found = std::ranges::find_if(
        catalog.objects, [&](const axk::ObjectSnapshot &item) {
            return item.object.header.type == type &&
                   item.object.header.name == name;
        });
...
TEST(ObjectCatalog, DerivesExactVolumeAndCategoryPlacementFromDirectories) {
    const auto container =
        axk::open_image(fixture("HD00_512_single_sbnk_authored.hds"));
    ...
    const auto *sample =
        object_named(*catalog, axk::ObjectType::smpl, "sine wave");
    ...
    const auto source_payload = container->read_record_data(
        sample->partition, sample->sfs_id, 64U * 1024U * 1024U);
...
TEST(RelationshipGraph, MatchesMaintainedFixtureCountsAndSamplerHierarchy) {
    ...
    EXPECT_TRUE(
        std::ranges::all_of(graph.bitmap_comparisons, [](const auto &row) {
    ...
    const auto bank_category =
        std::ranges::find(volume.children, std::string{"Sample Banks"},
                          &axk::ContentNode::display_name);
    ASSERT_NE(bank_category, volume.children.end());
    const auto group =
        std::ranges::find(bank_category->children, std::string{"B New SmpBank"},
                          &axk::ContentNode::display_name);
...
TEST(RelationshipGraph, PreservesDuplicateMemberCandidatesWithoutKnownEdge) {
    ...
    const auto graph = axk::build_relationship_graph(*catalog);
    const auto bank =
        object_named(*catalog, axk::ObjectType::sbnk, "sine wave");
    ...
    const auto validation =
        axk::validate_semantics(*container, *catalog, graph);
...
TEST(RelationshipGraph, PrefersUniqueCrossFolderLinkBeforeDuplicateLocalNames) {
    ...
    bank.header.name = "Bank";
    bank.payload = std::move(current_bank);
    catalog.objects.push_back({"bank", axk::PartitionIndex{0}, axk::SfsId{1},
                               "iso:GROUP", std::move(bank), bank_placement});

    const auto add_sample = [&](std::string key, std::string name,
                                std::uint32_t link, std::string folder) {
        ...
        catalog.objects.push_back({std::move(key), axk::PartitionIndex{0},
                                   axk::SfsId{2}, "iso:GROUP",
...
TEST(ProgramRelationships, UsesIsoBasisForMissingVisibleOffSampleBankGroup) {
    ...
    catalog.objects.push_back(
        {"program", axk::PartitionIndex{0}, axk::SfsId{1}, "iso:GROUP",
         std::move(program),
         axk::ObjectPlacement{axk::PartitionIndex{0}, "GROUP", axk::SfsId{1},
                              "Volume", "PROG", "001", "GROUP/F001"}});

    const auto graph = axk::build_relationship_graph(catalog);
    ASSERT_EQ(graph.relationships.size(), 1U);
    EXPECT_EQ(graph.relationships.front().assignment_state,
              axk::AssignmentState::visible_off);
...
TEST(ProgramRelationships, KeepsSourceLoadReceiveChannelUnknown) {
    axk::ObjectCatalog catalog;
    const axk::ObjectPlacement placement{axk::PartitionIndex{0},
                                         "GROUP",
                                         axk::SfsId{1},
                                         "Volume",
                                         "SBNK",
    ...
    catalog.objects.push_back({"bank", axk::PartitionIndex{0}, axk::SfsId{1},
                               "iso:GROUP", std::move(bank), placement});
    ...
    catalog.objects.push_back({"program", axk::PartitionIndex{0}, axk::SfsId{2},
                               "iso:GROUP", std::move(program),
                               std::move(program_placement)});

    const auto graph = axk::build_relationship_graph(catalog);
    ASSERT_EQ(graph.relationships.size(), 1U);
    EXPECT_EQ(graph.relationships.front().assignment_state,
              axk::AssignmentState::source_load);
    EXPECT_EQ(graph.relationships.front().receive_channel_display, "unknown");
}

TEST(ProgramRelationships,
     ResolvesUnknownKindsByUniqueNameWithoutInventingACategory) {
    axk::ObjectCatalog catalog;
    axk::DecodedObject bank;
    bank.header.type = axk::ObjectType::sbnk;
...
TEST(ProgramRelationships,
     KeepsVisibleOffAmbiguousGroupsAsDiagnosticCandidates) {
    axk::ObjectCatalog catalog;
    ...
    EXPECT_EQ(graph.relationships.front().candidate_keys.size(), 2U);
    EXPECT_EQ(graph.relationships.front().quality,
              axk::RelationshipQuality::tentative);
    EXPECT_EQ(graph.relationships.front().basis,
...
TEST(RelationshipGraph, ResolvesActiveSfsGroupMemberWithinItsOwningVolume) {
    axk::ObjectCatalog catalog;
    for (const auto &[key, volume] :
         {std::pair{"bank-a", 1U}, std::pair{"bank-b", 2U}}) {
        axk::DecodedObject bank;
        bank.header.type = axk::ObjectType::sbnk;
        bank.header.name = "Duplicate Bank";
        bank.payload = axk::CurrentSbnk{};
        catalog.objects.push_back({key, axk::PartitionIndex{0},
                                   axk::SfsId{volume}, "partition:0",
                                   std::move(bank),
    ...
    catalog.objects.push_back({"group", axk::PartitionIndex{0}, axk::SfsId{3},
                               "partition:0", std::move(group),
                               axk::ObjectPlacement{axk::PartitionIndex{0},
                                                    "Disk",
                                                    axk::SfsId{2},
                                                    "Volume",
    ...
    EXPECT_EQ(graph.relationships.front().quality,
              axk::RelationshipQuality::known);
    EXPECT_EQ(graph.relationships.front().basis,
              "active-sbac-slot-name+same-volume");
    EXPECT_EQ(graph.relationships.front().candidate_keys.size(), 2U);
}

TEST(ProgramRelationships,
     IdentifiesOneSameVolumeVisibleOffDiagnosticCandidate) {
    axk::ObjectCatalog catalog;
    for (const auto &[key, volume] :
         {std::pair{"group-a", 1U}, std::pair{"group-b", 2U}}) {
        axk::DecodedObject group;
        group.header.type = axk::ObjectType::sbac;
        group.header.name = "Duplicate Group";
        group.payload = axk::CurrentSbac{};
        catalog.objects.push_back(
            {key, axk::PartitionIndex{0}, axk::SfsId{volume}, "partition:0",
             std::move(group),
             axk::ObjectPlacement{axk::PartitionIndex{0}, "Disk",
                                  axk::SfsId{volume}, "Volume", "SBAC",
    ...
    program.payload = std::move(current_program);
    catalog.objects.push_back(
        {"program", axk::PartitionIndex{0}, axk::SfsId{3}, "partition:0",
         std::move(program),
...
TEST(ProgramRelationships, MarksVisibleOffRepeatOfActiveTargetAsDuplicate) {
    ...
    EXPECT_EQ(graph.relationships[0].assignment_state,
              axk::AssignmentState::active);
    EXPECT_EQ(graph.relationships[1].assignment_state,
              axk::AssignmentState::duplicate_not_active);
}
...
TEST(ProgramRelationships, KeepsVisibleOffRowsOutOfNavigableContent) {
    ...
    bank.payload = axk::CurrentSbnk{};
    catalog.objects.push_back({"p0:sfs10",
                               axk::PartitionIndex{0},
                               axk::SfsId{10},
                               "partition:0",
                               std::move(bank),
    ...
    program.header.name = "001";
    program.payload = std::move(current_program);
    catalog.objects.push_back({"p0:sfs11",
                               axk::PartitionIndex{0},
                               axk::SfsId{11},
                               "partition:0",
                               std::move(program),
