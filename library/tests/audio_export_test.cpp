...
TEST(AudioExport, ProjectsLogicalBanksToTheirKnownWaveformVolume) {
  ...
  const auto waveform_volume = std::ranges::find_if(
      plan->volumes, [](const auto &volume) { return !volume.waveforms.empty(); });
...
TEST(AudioExport, RetainsLikelyMembersAsGraphMetadataWithoutWritingSfzRegions) {
  ...
  EXPECT_TRUE(std::ranges::all_of(plan->volumes.front().sample_banks, [](const auto &bank) {
    return bank.members.size() == 1U &&
           bank.members.front().quality == axk::RelationshipQuality::likely &&
...
TEST(AudioExport, PrefersRenderedStereoAndEmitsSamplerParametersToSfz) {
  ...
  EXPECT_TRUE(
      std::filesystem::is_regular_file(output / "partition_00_hd1/Vol 1/Invalid Tune (2).sfz"));
  std::filesystem::remove_all(output, error);
}
