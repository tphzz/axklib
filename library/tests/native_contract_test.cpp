...
nlohmann::json read_contract(const std::string &filename) {
    const std::string path =
        std::string{AXK_SOURCE_ROOT} + "/library/tests/contracts/" + filename;
...
TEST(NativeContracts, HasCanonicalInventoryAndUniqueEntries) {
    ...
    const auto commands =
        baseline.at("commands").get<std::vector<std::string>>();
    const auto command_set =
        std::set<std::string>{commands.begin(), commands.end()};
    EXPECT_EQ(commands.size(), command_set.size());

    const auto profiles =
        baseline.at("writer_profiles").get<std::vector<std::string>>();
    const auto profile_set =
        std::set<std::string>{profiles.begin(), profiles.end()};
...
TEST(NativeContracts, PinsTheBuildContractAndRetainedMediaCommands) {
    ...
    const auto quality_commands =
        build.at("quality_commands").get<std::vector<std::string>>();
    const std::set<std::string> quality_command_set{quality_commands.begin(),
                                                    quality_commands.end()};
...
TEST(NativeContracts, SemanticContractsAreUniqueAndContainNoRegenerationData) {
    ...
    for (const auto &contract : document.at("contracts")) {
        EXPECT_TRUE(
            identifiers.insert(contract.at("id").get<std::string>()).second);
        EXPECT_FALSE(contract.contains("regeneration_command"));
        EXPECT_EQ(contract.at("input_sha256").get<std::string>().size(), 64U);
        EXPECT_EQ(
            contract.at("expected_semantic_sha256").get<std::string>().size(),
            64U);
    }
}
