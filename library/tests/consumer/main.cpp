...
int main() {
  ...
  auto exported = axk::portable_package::export_from(AXK_TEST_FIXTURE, {selector},
  ...
  if (!package_summary || package_summary->package_id != exported->package_id ||
  ...
  auto import_plan = axk::package_import_plan::create(output.string(), {exported->output_path},
                                                      import_request, context);
  if (!import_plan)
    return 9;
  ...
  if (!imported_objects ||
      std::none_of(imported_objects->items.begin(), imported_objects->items.end(),
