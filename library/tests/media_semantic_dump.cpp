...
int main(int argc, char **argv) {
    ...
    for (const auto &object : catalog->objects) {
        ...
        const auto size = media == media_by_key.end() ? static_cast<std::uint64_t>(object.object.header.header_size) +
                                                            object.object.header.payload_bytes_0x1c
                                                      : static_cast<std::uint64_t>(media->second->raw_payload.size());
        object_rows.emplace_back(object_type(object.object.header.type), object.object.header.name, offset, size);
    }
    std::ranges::sort(object_rows);

    std::vector<std::tuple<std::string, std::string, std::string, std::string, std::string>> relationship_rows;
    for (const auto &relationship : graph.relationships) {
        ...
        if (relationship.target_key) {
            ...
            if (target_entry != catalog_by_key.end()) {
                ...
                target_identity =
                    std::format("{}:{}", object_type(target->object.header.type), target->object.header.name);
            }
        }
    ...
    for (const auto &row : trees)
        tree_rows.emplace_back(row[0].get<std::string>(), row[1].get<std::string>(), row[2].get<std::string>());
