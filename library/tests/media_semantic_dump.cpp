...
void append_tree_rows(nlohmann::ordered_json &rows,
...
int main(int argc, char **argv) {
    ...
    for (const auto &object : catalog->objects) {
        ...
        const auto offset =
            media == media_by_key.end() ? 0U : media->second->data_offset;
        const auto size =
            media == media_by_key.end()
        ...
        object_rows.emplace_back(object_type(object.object.header.type),
                                 object.object.header.name, offset, size);
    }
    ...
    for (const auto &relationship : graph.relationships) {
        ...
        if (relationship.target_key) {
            const auto target_entry =
                catalog_by_key.find(*relationship.target_key);
            if (target_entry != catalog_by_key.end()) {
                const auto target = target_entry->second;
                target_identity = std::format(
                    "{}:{}", object_type(target->object.header.type),
                    target->object.header.name);
            }
        }
        relationship_rows.emplace_back(
            std::format("{}:{}", object_type(source->object.header.type),
                        source->object.header.name),
            std::move(target_identity), relationship.type,
            std::string{axk::relationship_quality_name(relationship.quality)},
            relationship.basis);
    }
    ...
    for (const auto &row : trees)
        tree_rows.emplace_back(row[0].get<std::string>(),
