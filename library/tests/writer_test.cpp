...
TEST(HdsManifest, ParsesStrictSchemaAndResolvesRelativeAudioPaths) {
    ...
    EXPECT_EQ(parsed->partitions[0].volumes[0].waveforms[0].path,
              "/project/audio/tone.wav");
...
TEST(HdsManifest, RejectsUnknownFieldsReferencesAndInvalidGeometry) {
    ...
    reference.replace(reference.find("\"tone\",\"root_key\"", 200), 19,
                      "\"missing\",\"root_key\"");
...
TEST(BuildManifestTemplate, EmitsParseableHdsFloppyAndIsoStarters) {
    const auto hds =
        axk::serialize_build_manifest_template(axk::BuildManifestKind::hds);
    ...
    EXPECT_TRUE(
        parsed_hds->partitions.front().volumes.front().waveforms.empty());
    EXPECT_TRUE(
        parsed_hds->partitions.front().volumes.front().sample_banks.empty());

    const auto floppy = axk::serialize_build_manifest_template(
        axk::BuildManifestKind::fat12_floppy);
    ASSERT_TRUE(floppy) << floppy.error().message;
    const auto parsed_floppy =
        axk::parse_media_build_manifest(*floppy, "manifest-root");
    ...
    EXPECT_EQ(parsed_floppy->authored_volume->waveforms.front().path.filename(),
              "tone.wav");

    const auto iso =
        axk::serialize_build_manifest_template(axk::BuildManifestKind::iso9660);
    ...
    EXPECT_FALSE(axk::serialize_build_manifest_template(
        static_cast<axk::BuildManifestKind>(
            std::numeric_limits<std::uint8_t>::max())));
}

TEST(BuildManifestTemplate, PublishesAtomicallyAndRequiresExplicitOverwrite) {
    const auto root = std::filesystem::temp_directory_path() /
                      "axklib-build-manifest-template-test";
    const auto path = root / "nested" / "image.json";
    std::error_code error;
    std::filesystem::remove_all(root, error);

    ASSERT_TRUE(
        axk::write_build_manifest_template(axk::BuildManifestKind::hds, path));
    ASSERT_TRUE(std::filesystem::is_regular_file(path));
    EXPECT_FALSE(axk::write_build_manifest_template(
        axk::BuildManifestKind::iso9660, path));
    ASSERT_TRUE(axk::write_build_manifest_template(
        axk::BuildManifestKind::iso9660, path, true));
...
TEST(HdsGeometry, CoversEveryPartitionCountAtOneAndTwoGiBBoundaries) {
    for (std::uint8_t count = 1; count <= 8; ++count) {
        axk::HdsBuildManifest value{"1.0",
                                    count == 1 ? axk::minimum_hds_size
                                               : axk::maximum_hds_size,
                                    {}};
...
TEST(AudioImport, PreservesNativePcm16AndChoosesOnlyHardwareRates) {
    ...
    const auto path =
        std::filesystem::temp_directory_path() / "axklib-writer-import.wav";
    std::error_code error;
...
TEST(HdsWriter, AtomicallyWritesAndReopensFreshEmptyVolumeImage) {
    ...
    const auto path =
        std::filesystem::temp_directory_path() / "axklib-native-empty.hds";
    std::error_code error;
    std::filesystem::remove(path, error);
...
TEST(HdsWriter, WritesMonoSampleBankAndRoundTripsExactPhysicalPcm) {
    ...
    const auto audio_path =
        std::filesystem::temp_directory_path() / "axklib-writer-bank.wav";
    const auto image_path =
        std::filesystem::temp_directory_path() / "axklib-writer-bank.hds";
    std::error_code error;
    ...
    ASSERT_TRUE(catalog);
    const auto type = [](const axk::ObjectSnapshot &item) {
        return item.object.header.type;
    };
    EXPECT_EQ(std::ranges::count(catalog->objects, axk::ObjectType::smpl, type),
              1U);
    EXPECT_EQ(std::ranges::count(catalog->objects, axk::ObjectType::sbnk, type),
              1U);
    const auto sample =
        std::ranges::find(catalog->objects, axk::ObjectType::smpl, type);
    ASSERT_NE(sample, catalog->objects.end());
    const auto decoded = axk::decode_waveform(*reopened, *sample);
    ASSERT_TRUE(decoded);
    EXPECT_TRUE(std::ranges::equal(
        source.pcm,
        std::span<const std::byte>{decoded->pcm}.first(source.pcm.size())));
    std::filesystem::remove(audio_path, error);
    std::filesystem::remove(image_path, error);
}
...
TEST(HdsWriter, CancellationPublishesNoImageOrTemporarySibling) {
    ...
    manifest_value.partitions.push_back({"hd1", {std::move(volume)}});
    const auto path =
        std::filesystem::temp_directory_path() / "axklib-cancelled-writer.hds";
    ...
    cancellation.cancel();
    const auto written =
        axk::write_hds_image(manifest_value, path, false, cancellation.token());
...
TEST(HdsWriter, OverwriteReplacesSymlinkWithoutFollowingItsTarget) {
    ...
    manifest_value.partitions.push_back({"hd1", {std::move(volume)}});
    const auto root =
        std::filesystem::temp_directory_path() / "axklib-writer-symlink";
...
TEST(MediaManifest, ParsesStrictAuthoredAndTransferModes) {
    ...
    EXPECT_EQ(parsed->authored_volume->waveforms.front().path,
              "/project/tone.wav");
    ...
    const auto whole =
        axk::parse_media_build_manifest(whole_source, "/project");
    ...
    conflicting_selection.replace(
        conflicting_selection.find("\"selection\":\"all\""), 17,
    ...
    invalid.insert(invalid.find("\"authored_volume\""),
                   "\"transfer\":{\"source_path\":\"source.hds\",\"root_object_"
                   "keys\":[\"x\"]},");
...
TEST(MediaWriter, WritesDeterministicFat12AndIso9660ImagesAndReopensExactPcm) {
    ...
    source.pcm = {std::byte{},     std::byte{},     std::byte{0xe8},
                  std::byte{0x03}, std::byte{0x18}, std::byte{0xfc}};
    const auto root =
        std::filesystem::temp_directory_path() / "axklib-media-writer";
    ...
    for (const auto format : {axk::MediaImageFormat::fat12_floppy,
        ...
        const auto extension =
            format == axk::MediaImageFormat::fat12_floppy ? ".ima" : ".iso";
        ...
        if (format == axk::MediaImageFormat::fat12_floppy) {
            ...
            EXPECT_EQ((std::string_view{first_bytes.data(), 3U}),
                      (std::string_view{"\xeb\x58\x90", 3U}));
            EXPECT_EQ((std::string_view{first_bytes.data() + 3U, 8U}),
                      "WINIMAGE");
            ...
            EXPECT_EQ((std::string_view{first_bytes.data() + 43U, 11U}),
                      "           ");
            EXPECT_EQ((std::string_view{first_bytes.data() + 54U, 8U}),
                      "FAT12   ");
            EXPECT_EQ(static_cast<unsigned char>(first_bytes[512]), 0xf0U);
            EXPECT_EQ(static_cast<unsigned char>(first_bytes[512U + 9U * 512U]),
                      0xf0U);
        } else {
            ASSERT_GE(first_bytes.size(), (16U * 2048U) + 40U);
            const std::string_view system_id{
                first_bytes.data() + (16U * 2048U) + 8U, 32U};
            ...
            const auto find_file = [&](std::string_view path) {
                return std::ranges::find(iso->files(), path,
                                         &axk::IsoFile::path);
            };
            const auto expected_catalog = [](std::string_view name,
                                             std::byte name_hash) {
                std::vector<std::byte> result(32U);
                std::fill_n(result.begin() + 1, 16U, std::byte{' '});
                std::ranges::transform(
                    name, result.begin() + 1, [](char character) {
            ...
            EXPECT_EQ(*group_label_bytes,
                      (std::vector<std::byte>{
                          ...
                          std::byte{'t'}, std::byte{' '}, std::byte{'G'},
                          std::byte{'r'}, std::byte{'o'}, std::byte{'u'},
            ...
            auto expected_group_catalog =
                expected_catalog("Test Volume", std::byte{0xd8});
            std::vector<std::byte> expected_disk_name(32U);
            expected_disk_name[0] = std::byte{0xe1};
            std::ranges::transform(std::string_view{"_DSKNAME"},
                                   expected_disk_name.begin() + 1,
                                   [](char character) {
                                       return static_cast<std::byte>(character);
                                   });
            expected_disk_name[17] = std::byte{0x5e};
            std::ranges::transform(std::string_view{"F002"},
                                   expected_disk_name.begin() + 18,
                                   [](char character) {
                                       return static_cast<std::byte>(character);
                                   });
            expected_group_catalog.insert(expected_group_catalog.end(),
                                          expected_disk_name.begin(),
                                          expected_disk_name.end());
            EXPECT_EQ(*volume_catalog_bytes, expected_group_catalog);

            for (const auto &[path, name, expected_hash] :
                 std::array{std::tuple{"GROUP/F001/SMPL/0000", "Wave",
                                       std::byte{0xfa}},
                            std::tuple{"GROUP/F001/SBNK/0000", "Bank",
                                       std::byte{0xc7}}}) {
                ...
                EXPECT_EQ(*catalog_bytes,
                          expected_catalog(name, expected_hash));
            }
            ...
            const auto catalog_offset =
                static_cast<std::size_t>(sample_catalog->extent_sector) * 2048U;
            ...
            missing_tail_bytes.resize(
                static_cast<std::size_t>(sample_object->extent_sector) * 2048U);
            const auto missing_tail_path = root / "missing-object-tail.iso";
            std::ofstream missing_tail_output{
                missing_tail_path, std::ios::binary | std::ios::trunc};
            missing_tail_output.write(
                missing_tail_bytes.data(),
            ...
            std::ranges::copy_n(
                shifted_bytes.begin() +
                    static_cast<std::ptrdiff_t>(catalog_offset + 4U),
                shifted_record.size(), shifted_record.begin());
            std::ranges::copy(shifted_record,
                              shifted_bytes.begin() +
                                  static_cast<std::ptrdiff_t>(catalog_offset));
            std::ranges::fill_n(
                shifted_bytes.begin() +
                    static_cast<std::ptrdiff_t>(catalog_offset + 28U),
                4U, '\0');
            const auto shifted_path = root / "shifted-category.iso";
            std::ofstream shifted_output{shifted_path,
                                         std::ios::binary | std::ios::trunc};
            shifted_output.write(
                shifted_bytes.data(),
            ...
            std::ranges::copy(std::string_view{"F099"},
                              damaged_bytes.data() + catalog_offset + 18U);
            const auto damaged_path = root / "damaged-category.iso";
            std::ofstream damaged_output{damaged_path,
                                         std::ios::binary | std::ios::trunc};
            damaged_output.write(
                damaged_bytes.data(),
        ...
        const auto sample = std::ranges::find(
            objects->begin(), objects->end(), axk::ObjectType::smpl,
...
TEST(MediaWriter, AuthoredIsoReopensCompleteProgramHierarchy) {
    ...
    source.pcm = {std::byte{},     std::byte{},     std::byte{0x34},
                  std::byte{0x12}, std::byte{0xcc}, std::byte{0xed},
                  std::byte{},     std::byte{}};
    const auto root =
        std::filesystem::temp_directory_path() / "axklib-media-graph";
    ...
    EXPECT_EQ(std::ranges::count(catalog->objects, axk::ObjectType::sbnk, type),
              2U);
    EXPECT_EQ(std::ranges::count(catalog->objects, axk::ObjectType::sbac, type),
              1U);
    EXPECT_EQ(std::ranges::count(catalog->objects, axk::ObjectType::prog, type),
              1U);
    const auto graph = axk::build_relationship_graph(*catalog);
    const auto relationship_count = [&](std::string_view relationship_type) {
        return std::ranges::count(graph.relationships, relationship_type,
                                  &axk::Relationship::type);
    };
...
TEST(MediaWriter, AllowsObjectEmptyIsoPackageStagingTargetOnly) {
    const auto root =
        std::filesystem::temp_directory_path() / "axklib-empty-media-staging";
    ...
    ASSERT_FALSE(floppy);
    EXPECT_NE(floppy.error().message.find("at least one Yamaha object"),
              std::string::npos);
    std::filesystem::remove_all(root, error);
}
...
TEST(MediaWriter, SavedObjectTransferAddsKnownSampleBankDependencies) {
    ...
    const auto root =
        std::filesystem::temp_directory_path() / "axklib-media-transfer";
    const auto audio_path = root / "tone.wav";
    ...
    const auto bank_object = std::ranges::find(
        *source_objects, axk::ObjectType::sbnk,
        [](const auto &object) { return object.decoded.header.type; });
    ...
    transfer.transfer =
        axk::SavedObjectTransferSpec{source_path, {bank_object->key}};
    ...
    EXPECT_EQ(std::ranges::count(*output_objects, axk::ObjectType::smpl,
                                 [](const auto &object) {
                                     return object.decoded.header.type;
                                 }),
              1U);

    const auto program_object = std::ranges::find(
        *source_objects, axk::ObjectType::prog,
        [](const auto &object) { return object.decoded.header.type; });
    ASSERT_NE(program_object, source_objects->end());
    const auto program_path = root / "program.ima";
    transfer.transfer =
        axk::SavedObjectTransferSpec{source_path, {program_object->key}};
    ...
    whole.transfer = axk::SavedObjectTransferSpec{
        program_path, {}, axk::SavedObjectSelection::all};
    ...
    EXPECT_EQ(sorted_payloads(*program_objects),
              sorted_payloads(*whole_objects));
