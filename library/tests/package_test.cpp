...
std::filesystem::path fixture(std::string_view name) {
    return std::filesystem::path{AXK_SOURCE_ROOT} /
           "tests/fixtures/images/sampler-authored" / name;
}
...
axk::PackageRootSelector root(axk::PackageRootKind kind, std::string volume,
                              std::string object = {}) {
...
axk::PackageRootDestination destination(std::size_t package_index,
                                        std::string volume) {
...
axk::Result<axk::PackageBuild>
standalone_smpl_package(std::vector<std::byte> payload) {
    auto object = axk::StandaloneObject::open(
...
axk::VolumeSpec graph_volume(const std::filesystem::path &audio_path) {
    ...
    volume.programs.push_back(
        {1U, {{"SBAC", "Graph Group", 1U}, {"SBNK", "Direct Bank", 2U}}});
    return volume;
}

axk::VolumeSpec single_bank_volume(const std::filesystem::path &audio_path,
    ...
    volume.waveforms.push_back(
        {"wave", std::move(waveform_name), audio_path, 60U, {}});
...
axk::Result<std::vector<axk::PortablePackage>>
mixed_source_packages(const std::filesystem::path &root_path) {
    ...
    if (const auto written =
            axk::write_wav_atomic(graph_audio, tiny_waveform(1000));
        !written)
        return std::unexpected{written.error()};
    if (const auto written =
            axk::write_wav_atomic(iso_audio, tiny_waveform(2000));
        !written)
        return std::unexpected{written.error()};

    axk::HdsBuildManifest sfs_manifest{"1.0", 4U * 1024U * 1024U, {}};
    sfs_manifest.partitions.push_back({"P1", {graph_volume(graph_audio)}});
    if (const auto written = axk::write_hds_image(sfs_manifest, sfs_path);
        !written)
        return std::unexpected{written.error()};
    auto sfs = axk::open_media(sfs_path);
    if (!sfs)
        return std::unexpected{sfs.error()};
    const std::vector program_root{
    ...
    iso_manifest.authored_volume =
        single_bank_volume(iso_audio, "ISO Volume", "ISO Wave", "ISO Bank");
    if (const auto written = axk::write_media_image(iso_manifest, iso_path);
        !written)
        return std::unexpected{written.error()};
...
axk::Result<std::vector<std::string>>
normalized_media_signature(const axk::MediaContainer &media) {
    ...
    for (const auto &object : *objects) {
        auto profile = axk::package_internal::build_relocation_profile(
            object.decoded, object.raw_payload);
        if (!profile)
            return std::unexpected{profile.error()};
        result.push_back(std::format(
            "{}\0{}\0{}", object.decoded.header.raw_type,
            object.decoded.header.name,
            axk::package_internal::hex_digest(
                axk::package_internal::sha256(profile->normalized_payload))));
    }
    std::ranges::sort(result);
    return result;
}

void expect_package_audio(const axk::MediaContainer &media,
    ...
    for (const auto &package : packages) {
        for (const auto &node : package.nodes) {
            ...
            const auto object =
                std::ranges::find_if(*objects, [&](const auto &candidate) {
                    return candidate.decoded.header.type ==
                               axk::ObjectType::smpl &&
            ...
            EXPECT_EQ(axk::package_internal::hex_digest(
                          axk::package_internal::sha256(waveform->pcm)),
                      *node.audio_sha256)
                << node.name;
        }
    }
...
nlohmann::json archive_manifest(std::span<const std::byte> archive) {
    ...
    if (!entries)
        return {};
    const auto manifest =
        std::ranges::find(*entries, std::string{"manifest.json"},
                          &axk::package_internal::ArchiveEntry::path);
    if (manifest == entries->end())
        return {};
    return nlohmann::json::parse(
        std::string_view{reinterpret_cast<const char *>(manifest->bytes.data()),
                         manifest->bytes.size()});
}
...
void expect_schema_object_shape(const nlohmann::json &schema,
    ...
    EXPECT_EQ(object_keys(schema.at("properties")),
              string_set(schema.at("required")));
...
void expect_external_iso_tools_accept(const std::filesystem::path &path) {
    ...
    const auto isoinfo = shell_quote(AXK_TEST_ISOINFO) + " -i " +
                         shell_quote(path) + " -f >/dev/null 2>&1";
    EXPECT_EQ(std::system(isoinfo.c_str()), 0) << "isoinfo rejected " << path;
#endif
#ifdef AXK_TEST_XORRISO
    const auto xorriso = shell_quote(AXK_TEST_XORRISO) + " -indev " +
                         shell_quote(path) + " -toc >/dev/null 2>&1";
    EXPECT_EQ(std::system(xorriso.c_str()), 0) << "xorriso rejected " << path;
#endif
}

template <typename Mutation>
std::vector<std::byte> mutate_manifest(std::span<const std::byte> archive,
                                       Mutation mutation) {
    ...
    const std::string text(
        reinterpret_cast<const char *>(manifest->bytes.data()),
        manifest->bytes.size());
    ...
    json["package_id"] = axk::package_internal::hex_digest(
        axk::package_internal::sha256(source));
    const auto output = json.dump() + '\n';
    const auto bytes = std::as_bytes(std::span{output});
    manifest->bytes.assign(bytes.begin(), bytes.end());
    const auto rewritten =
        axk::package_internal::write_archive(std::move(*entries));
...
TEST(PortablePackage, ExportsAndVerifiesTypedSmplFromFat12) {
    ...
    EXPECT_NE(built->package.nodes.front().semantic_sha256,
              built->package.nodes.front().normalized_sha256);
    EXPECT_NE(built->package.nodes.front().audio_sha256,
              built->package.nodes.front().normalized_sha256);
    EXPECT_EQ(
        *built->package.nodes.front().semantic_sha256,
        "f0bc9a18c0c14d35e0cb81c375fd43ef8cd6528bb52eb29aee67ef8bbfe82b82");
    EXPECT_EQ(
        *built->package.nodes.front().audio_sha256,
    ...
    EXPECT_EQ(
        built->package.package_id,
        "f3ce5bc88b01566c80b9dbfe1d630869ec65b90b0a9c4674ef0406a662f9ca4a");

    const auto reopened =
        axk::open_portable_package(built->archive, "TEST.axksmpl");
...
TEST(PortablePackage,
     BuildsStrictSbnkClosureAndDeduplicatesMultiRootDependencies) {
    auto source = axk::open_media(fixture("HD00_512_single_sbnk_authored.hds"));
    ASSERT_TRUE(source) << source.error().message;
    const std::vector bank_root{
        root(axk::PackageRootKind::sbnk, "New Volume", "sine wave")};
    ...
    EXPECT_EQ(std::ranges::count(bank->package.nodes, std::string{"SBNK"},
                                 &axk::PackageNode::object_type),
              1);
    EXPECT_EQ(std::ranges::count(bank->package.nodes, std::string{"SMPL"},
                                 &axk::PackageNode::object_type),
              1);

    std::vector bundle_roots{
        root(axk::PackageRootKind::sbnk, "New Volume", "sine wave"),
...
TEST(PortablePackage,
     IgnoresAmbiguousInactiveProgramDiagnosticsButKeepsExactRows) {
    ...
    EXPECT_FALSE(axk::package_internal::portable_inactive_program_relationship(
        relationship));
    ...
    EXPECT_TRUE(axk::package_internal::portable_inactive_program_relationship(
        relationship));
}
...
TEST(PortablePackage, ExportsACompleteSupportedVolumeWithOnePayloadPerDigest) {
    ...
    ASSERT_TRUE(image) << image.error().message;
    const axk::MediaContainer media{std::move(*image)};
    const std::vector volume_root{
        root(axk::PackageRootKind::volume, "FAT root")};
    ...
    const auto reopened =
        axk::open_portable_package(volume->archive, "volume.axkvol");
    ASSERT_TRUE(reopened) << reopened.error().message;
    EXPECT_EQ(reopened->nodes.size(), 1U);
}

TEST(PortablePackage,
     TypedSuffixFollowsSelectedRootRatherThanDependencyClosure) {
    ...
    waveform.format = {1U, 2U, 44100U};
    waveform.frame_count = 4U;
    waveform.pcm = {std::byte{},     std::byte{},     std::byte{0xe8},
    ...
    authored_volume.sample_bank_groups.push_back(
        {"Graph Group 2", {"Grouped Bank 2"}});
    authored_volume.programs.push_back(
        {2U, {{"SBAC", "Graph Group 2", 1U}, {"SBNK", "Direct Bank 2", 2U}}});
    ...
    const std::vector cases{
        Case{axk::PackageRootKind::volume, "", axk::PackageKind::volume,
             ".axkvol", 5U},
        Case{axk::PackageRootKind::prog, "001", axk::PackageKind::program,
             ".axkprg", 5U},
        Case{axk::PackageRootKind::sbac, "Graph Group", axk::PackageKind::sbac,
             ".axksbac", 3U},
        Case{axk::PackageRootKind::sbnk, "Grouped Bank", axk::PackageKind::sbnk,
             ".axksbnk", 2U},
        Case{axk::PackageRootKind::smpl, "Graph Wave", axk::PackageKind::smpl,
             ".axksmpl", 1U},
    };
    for (const auto &test : cases) {
        const std::vector selectors{
            root(test.root_kind, "Graph Volume", test.object_name)};
        ...
        EXPECT_GE(built->package.nodes.size(), test.minimum_nodes)
            << test.extension;
        const auto published = axk::publish_portable_package(
            *built,
            output_root / (std::string{"typed"} + std::string{test.extension}));
        ASSERT_TRUE(published)
            << test.extension << ": " << published.error().message;
        const auto inspected =
            axk::inspect_portable_package(published->output_path);
        ASSERT_TRUE(inspected)
            << test.extension << ": " << inspected.error().message;
        ...
        const auto reopened =
            axk::open_portable_package(published->output_path);
        ASSERT_TRUE(reopened)
            << test.extension << ": " << reopened.error().message;
    ...
    const auto published_bundle =
        axk::publish_portable_package(*bundle, output_root / "typed.axkpkg");
    ASSERT_TRUE(published_bundle) << published_bundle.error().message;
    const auto inspected_bundle =
        axk::inspect_portable_package(published_bundle->output_path);
    ...
    const auto verified_bundle =
        axk::open_portable_package(published_bundle->output_path);
    ...
    const auto same_type_bundle =
        axk::build_portable_package(*source, same_type_bundle_roots);
    ...
    EXPECT_EQ(std::ranges::count(same_type_bundle->package.nodes,
                                 std::string{"SMPL"},
                                 &axk::PackageNode::object_type),
              1);
    ...
    const auto original_program =
        axk::build_portable_package(*source, program_root);
    const auto copied_program =
        axk::build_portable_package(*copied_source, program_root);
    ASSERT_TRUE(original_program) << original_program.error().message;
    ASSERT_TRUE(copied_program) << copied_program.error().message;
    EXPECT_EQ(copied_program->package.package_id,
              original_program->package.package_id);
    ...
    auto sequence_object = axk::StandaloneObject::open(
        std::make_shared<axk::MemoryReader>(std::move(sequence_payload)),
        "sequence.bin");
    ...
    const std::vector sequence{
        root(axk::PackageRootKind::sequ, "Standalone object", "Sequence")};
    const auto unsupported =
        axk::build_portable_package(sequence_media, sequence);
...
TEST(PortablePackage, SbacRelationshipOrdinalsPreserveSourceSlotOrder) {
    ...
    volume.sample_bank_groups.push_back(
        {"Ordered Group", {"Z Bank", "A Bank"}});
    volume.programs.push_back(
        {1U, {{"SBAC", "Ordered Group", 1U}, {"SBNK", "Direct Bank", 2U}}});
    axk::HdsBuildManifest manifest{"1.0", 4U * 1024U * 1024U, {}};
    ...
    const std::vector roots{
        root(axk::PackageRootKind::sbac, "Slot Order", "Ordered Group")};
    ...
    const auto target_name = [&](const axk::PackageRelationship &edge) {
        const auto target =
            std::ranges::find(built->package.nodes, edge.target_node_id,
                              &axk::PackageNode::node_id);
...
TEST(PortablePackage,
     NormativeJsonSchemaMatchesCanonicalManifestShapeAndEnums) {
    ...
    std::ifstream schema_input{std::filesystem::path{AXK_SOURCE_ROOT} /
                               "docs/axklib/portable-package-v1.schema.json"};
    ...
    expect_schema_object_shape(schema.at("properties").at("provenance"),
                               manifest.at("provenance"));
    ...
    for (const auto &item : manifest.at("objects")) {
        ...
        expect_schema_object_shape(definitions.at("placementHint"),
                                   item.at("placement_hint"));
        for (const auto &relocation : item.at("relocations"))
            expect_schema_object_shape(definitions.at("relocation"),
                                       relocation);
    }
    ...
    EXPECT_EQ(string_set(schema.at("properties").at("package_kind").at("enum")),
              (std::set<std::string>{"bundle", "program", "sbnk", "sbac",
                                     "sequence", "smpl", "volume"}));
    EXPECT_EQ(
        string_set(
            definitions.at("root").at("properties").at("kind").at("enum")),
        (std::set<std::string>{"prog", "sbnk", "sbac", "sequ", "smpl",
                               "volume"}));
    EXPECT_EQ(string_set(definitions.at("object")
                             .at("properties")
                             .at("object_type")
                             .at("enum")),
              (std::set<std::string>{"PRF3", "PROG", "SBAC", "SBNK", "SEQU",
                                     "SMPL"}));
    EXPECT_EQ(
        string_set(definitions.at("relocation")
                       .at("properties")
                       .at("role")
                       .at("enum")),
        (std::set<std::string>{"PROG_ASSIGNMENT_HANDLE", "SBAC_SLOT_HANDLE",
                               "SBNK_GROUP_MEMBERSHIP", "SBNK_LEFT_MEMBER_LINK",
                               "SBNK_PROGRAM_BITMAP", "SBNK_RIGHT_MEMBER_LINK",
...
TEST(PortablePackage, ExportsCompleteVolumeWithRelocatableNonzeroSbacHandles) {
    ...
    ASSERT_TRUE(source) << source.error().message;
    const std::vector volume_root{
        root(axk::PackageRootKind::volume, "New Volume")};
    const auto volume = axk::build_portable_package(*source, volume_root);
    ASSERT_TRUE(volume) << volume.error().message;
    EXPECT_TRUE(axk::verify_portable_package(volume->package));
    EXPECT_TRUE(
        std::ranges::any_of(volume->package.nodes, [](const auto &node) {
            return node.object_type == "SBAC" &&
                   std::ranges::any_of(
                       node.relocations, [](const auto &relocation) {
                           return relocation.role == "SBAC_SLOT_HANDLE" &&
                                  relocation.expected_hex != "00000000";
                       });
        }));
}
...
TEST(PortablePackage, ManifestKindOverridesARecognizedWrongFilenameExtension) {
    ...
    ASSERT_TRUE(built) << built.error().message;
    const auto reopened =
        axk::open_portable_package(built->archive, "renamed.axkprg");
    ASSERT_TRUE(reopened) << reopened.error().message;
    ...
    const auto bundle_named =
        axk::open_portable_package(built->archive, "bundle.axkpkg");
...
TEST(PortablePackage,
     RejectsMissingAmbiguousAndUnsupportedSelectorsWithoutArchive) {
    auto image = axk::FatImage::open(
        std::make_shared<axk::MemoryReader>(fat_fixture()), "fixture.ima");
    ASSERT_TRUE(image) << image.error().message;
    const axk::MediaContainer media{std::move(*image)};
    const std::vector missing{
        root(axk::PackageRootKind::smpl, "FAT root", "MISSING")};
    EXPECT_FALSE(axk::build_portable_package(media, missing));
    const std::vector sequence{
        root(axk::PackageRootKind::sequ, "FAT root", "TEST")};
...
TEST(PortablePackage, RejectsRehashedSemanticTampering) {
    ...
    const auto wrong_kind =
        mutate_manifest(built->archive, [](nlohmann::json &manifest) {
            manifest["package_kind"] = "program";
        });
    ASSERT_FALSE(wrong_kind.empty());
    EXPECT_FALSE(axk::open_portable_package(wrong_kind, "TEST.axkprg"));

    const auto wrong_node = mutate_manifest(built->archive, [](nlohmann::json
                                                                   &manifest) {
    ...
    const auto wrong_relocation =
        mutate_manifest(built->archive, [](nlohmann::json &manifest) {
            manifest["objects"][0]["relocations"][0]["offset"] = 1;
        });
...
TEST(PortablePackage, RevalidatesInMemoryPackagesBeforePlanning) {
    ...
    changed_id.package_id.front() =
        changed_id.package_id.front() == '0' ? '1' : '0';
...
TEST(PortablePackage, PublishesWithDerivedTypedSuffixAndReopensFromPath) {
    ...
    const auto published =
        axk::export_portable_package(media, roots, output_root / "TEST", false);
    ...
    EXPECT_EQ(published->size_bytes,
              std::filesystem::file_size(published->output_path));
    const auto inspected =
        axk::inspect_portable_package(published->output_path);
    ...
    const auto archive_summary =
        axk::package_internal::inspect_archive(archive_reader);
    ...
    corrupted_archive[static_cast<std::size_t>(payload_offset)] ^=
        std::byte{0x01};
    const auto corrupted_path = output_root / "CORRUPTED.axksmpl";
    ASSERT_TRUE(write_file(corrupted_path, corrupted_archive));
    const auto inspected_corruption =
        axk::inspect_portable_package(corrupted_path);
    ...
    for (const auto &entry : std::filesystem::directory_iterator(output_root))
        EXPECT_EQ(entry.path().filename().string().find(".package."),
                  std::string::npos);
    std::filesystem::remove_all(output_root, error);
}
...
TEST(PortablePackage, RejectsWrongOutputSuffixAndPreservesExistingTarget) {
    ...
    EXPECT_FALSE(axk::export_portable_package(media, roots,
                                              output_root / "wrong.axkprg"));
    EXPECT_FALSE(axk::export_portable_package(media, roots,
                                              output_root / "wrong.axkpkg"));
    EXPECT_FALSE(
        axk::export_portable_package(media, roots, output_root / "wrong.zip"));
    ...
    const auto rejected =
        axk::export_portable_package(media, roots, target, false);
    ...
    const auto replaced =
        axk::export_portable_package(media, roots, target, true);
...
TEST(PortablePackage,
     CancellationAndInvalidBuildPublishNothingAndCleanTemporaryFiles) {
    ...
    cancellation.cancel();
    const auto cancelled = axk::publish_portable_package(
        *built, output_root / "cancelled", false, cancellation.token());
    ...
    const auto invalid =
        axk::publish_portable_package(*built, output_root / "invalid");
...
TEST(PortablePackage,
     ConcurrentNoOverwritePublishersProduceOneCompletePackage) {
    ...
    auto publish = [&] {
        return axk::publish_portable_package(*built, target);
    };
    ...
    const auto reopened =
        axk::open_portable_package(output_root / "shared.axksmpl");
    ASSERT_TRUE(reopened) << reopened.error().message;
    EXPECT_EQ(reopened->package_id, built->package.package_id);
...
TEST(PortablePackage, NormalizationIgnoresOnlyDeclaredRelocationBytes) {
    ...
    const auto original = axk::package_internal::build_relocation_profile(
        *decoded, node.raw_payload);
    ...
    const auto relocated_profile =
        axk::package_internal::build_relocation_profile(*relocated_decoded,
                                                        relocated);
    ASSERT_TRUE(relocated_profile) << relocated_profile.error().message;
    EXPECT_NE(axk::package_internal::sha256(relocated),
              axk::package_internal::sha256(node.raw_payload));
    EXPECT_EQ(relocated_profile->normalized_payload,
    ...
    const auto unknown_profile =
        axk::package_internal::build_relocation_profile(*unknown_decoded,
                                                        unknown_changed);
    ASSERT_TRUE(unknown_profile) << unknown_profile.error().message;
    EXPECT_NE(unknown_profile->normalized_payload,
              original->normalized_payload);
}

TEST(PortablePackage,
     RelocationProfilesCoverEveryAdmittedObjectAndOnlyDeclaredBytes) {
    const auto output_root =
        publication_root("axklib-package-relocation-profiles");
    ...
    const std::map<std::string, std::string, std::less<>> destination_names{
        {"PROG", "Reloc Program"},
        {"SBAC", "Reloc Group"},
        {"Grouped Bank", "Reloc Grouped"},
        {"Direct Bank", "Reloc Direct"},
        {"SMPL", "Reloc Wave"},
    };
    const auto destination_name =
        [&](const axk::PackageNode &node) -> const std::string & {
    ...
    const auto target_node =
        [&](std::string_view node_id) -> const axk::PackageNode & {
        const auto found = std::ranges::find(built->package.nodes, node_id,
                                             &axk::PackageNode::node_id);
    ...
    const auto in_range = [](std::size_t offset, std::size_t begin,
                             std::size_t width) {
    ...
    for (const auto &node : built->package.nodes) {
        ...
        ASSERT_TRUE(decoded) << decoded.error().message;
        const auto profile = axk::package_internal::build_relocation_profile(
            *decoded, node.raw_payload);
        ASSERT_TRUE(profile) << profile.error().message;
        ASSERT_EQ(profile->relocations.size(), node.relocations.size());
        for (std::size_t index = 0; index < profile->relocations.size();
             ++index) {
            EXPECT_EQ(profile->relocations[index].offset,
                      node.relocations[index].offset);
            EXPECT_EQ(profile->relocations[index].width,
                      node.relocations[index].width);
            EXPECT_EQ(profile->relocations[index].role,
                      node.relocations[index].role);
            EXPECT_EQ(profile->relocations[index].mask_hex,
                      node.relocations[index].mask_hex);
            EXPECT_EQ(profile->relocations[index].expected_hex,
                      node.relocations[index].expected_hex);
        }
        ...
        if (node.object_type == "SMPL") {
            ...
            for (std::size_t index = 0; index < group->slots.size(); ++index) {
                ...
                const auto edge =
                    std::ranges::find(built->package.relationships,
            ...
            const auto nonzero_profile =
                axk::package_internal::build_relocation_profile(
                    *nonzero_decoded, nonzero_handles);
            ASSERT_TRUE(nonzero_profile) << nonzero_profile.error().message;
            EXPECT_EQ(nonzero_profile->normalized_payload,
            ...
            for (std::size_t index = 0; index < group->slots.size(); ++index) {
                ...
                mutable_nonzero_profile.relocations[index].edge_ids =
                    node.relocations[index].edge_ids;
            }
            auto nonzero_node = node;
            nonzero_node.raw_payload = std::move(nonzero_handles);
            nonzero_node.relocations =
                std::move(mutable_nonzero_profile.relocations);
            ...
            for (const auto &edge : built->package.relationships) {
                if (edge.source_node_id == node.node_id) {
                    nonzero_context.edge_target_names.emplace(
                        edge.edge_id, target_node(edge.target_node_id).name);
                }
            }
            const auto normalized =
                axk::package_internal::relocate_package_node(
            ...
            ASSERT_TRUE(normalized_decoded)
                << normalized_decoded.error().message;
            const auto *normalized_group =
                std::get_if<axk::CurrentSbac>(&normalized_decoded->payload);
            ASSERT_NE(normalized_group, nullptr);
            EXPECT_TRUE(std::ranges::all_of(
                normalized_group->slots,
                [](const auto &slot) { return slot.raw_handle == 0U; }));
        } else if (node.object_type == "PROG") {
            const auto *program =
                std::get_if<axk::CurrentProg>(&decoded->payload);
            ...
            for (std::size_t relocation_index = 0;
                 relocation_index < portable_rows.size(); ++relocation_index) {
                const auto row_index = portable_rows[relocation_index];
                const auto offset = 0x130U + row_index * 0x38U;
                const auto value =
                    static_cast<std::uint32_t>(0x50607080U + relocation_index);
                nonzero_handles[offset] = static_cast<std::byte>(value >> 24U);
                nonzero_handles[offset + 1U] =
                    static_cast<std::byte>(value >> 16U);
                nonzero_handles[offset + 2U] =
                    static_cast<std::byte>(value >> 8U);
                nonzero_handles[offset + 3U] = static_cast<std::byte>(value);
                EXPECT_EQ(profile->relocations[relocation_index].offset,
                          offset);
                EXPECT_EQ(profile->relocations[relocation_index].width, 4U);
                EXPECT_EQ(profile->relocations[relocation_index].role,
                          "PROG_ASSIGNMENT_HANDLE");
                EXPECT_TRUE(
                    profile->relocations[relocation_index].mask_hex.empty());
                ASSERT_EQ(node.relocations[relocation_index].edge_ids.size(),
                          1U);
                const auto edge = std::ranges::find(
                    built->package.relationships,
            ...
            ASSERT_TRUE(normalized_decoded)
                << normalized_decoded.error().message;
            const auto *normalized_program =
                std::get_if<axk::CurrentProg>(&normalized_decoded->payload);
            ASSERT_NE(normalized_program, nullptr);
            for (const auto index : portable_rows)
                EXPECT_EQ(normalized_program->assignments[index].raw_handle,
                          0U);
            ...
            const auto empty_index = static_cast<std::size_t>(
                std::distance(program->assignments.begin(), empty_row));
            ...
            const auto invalid_profile =
                axk::package_internal::build_relocation_profile(
                    *invalid_decoded, invalid_handle);
            ASSERT_TRUE(invalid_profile) << invalid_profile.error().message;
            EXPECT_NE(invalid_profile->normalized_payload,
                      profile->normalized_payload);
        } else {
        ...
        for (const auto &relocation : profile->relocations) {
            ...
            if (relocation.mask_hex.empty()) {
                for (std::size_t index = 0; index < width; ++index) {
                    ...
                    ASSERT_TRUE(changed_decoded)
                        << relocation.role << '[' << index << ']';
                    const auto changed_profile =
                        axk::package_internal::build_relocation_profile(
                            *changed_decoded, changed);
                    ASSERT_TRUE(changed_profile)
                        << changed_profile.error().message;
                    EXPECT_EQ(changed_profile->normalized_payload,
                              profile->normalized_payload)
                        << relocation.role << '[' << index << ']';
                }
            } else {
                ...
                const auto declared_decoded =
                    axk::decode_object(declared_changed);
                ASSERT_TRUE(declared_decoded);
                const auto declared_profile =
                    axk::package_internal::build_relocation_profile(
                        *declared_decoded, declared_changed);
                ASSERT_TRUE(declared_profile);
                EXPECT_EQ(declared_profile->normalized_payload,
                          profile->normalized_payload);

                auto undeclared_mask_changed = node.raw_payload;
                undeclared_mask_changed[first] ^= std::byte{0x02};
                const auto undeclared_mask_decoded =
                    axk::decode_object(undeclared_mask_changed);
                ASSERT_TRUE(undeclared_mask_decoded);
                const auto undeclared_mask_profile =
                    axk::package_internal::build_relocation_profile(
                        *undeclared_mask_decoded, undeclared_mask_changed);
                ASSERT_TRUE(undeclared_mask_profile);
                EXPECT_NE(undeclared_mask_profile->normalized_payload,
                          profile->normalized_payload);
            }
        }
        ...
        if (node.object_type == "SBNK") {
            ...
            context.grouped = std::ranges::any_of(
                built->package.relationships,
                [&](const axk::PackageRelationship &edge) {
                    return edge.role == "SBAC_SLOT_TO_SBNK" &&
        ...
        const auto relocated = axk::package_internal::relocate_package_node(
            built->package, node, context);
        ASSERT_TRUE(relocated) << node.object_type << ' ' << node.name << ": "
        ...
        for (std::size_t offset = 0; offset < relocated->size(); ++offset) {
            ...
            for (const auto &relocation : node.relocations)
                allowed = allowed ||
                          in_range(offset, relocation.offset, relocation.width);
            for (const auto &edge : built->package.relationships) {
                if (edge.source_node_id != node.node_id)
                    continue;
                if (edge.role == "SBNK_LEFT_MEMBER_TO_SMPL")
                    ...
                    allowed =
                        allowed ||
                        in_range(offset, 0x14cU + edge.ordinal * 0x14U, 16U);
                else if (edge.role == "PROG_ASSIGNMENT_TO_SBAC" ||
                         edge.role == "PROG_ASSIGNMENT_TO_SBNK")
                    allowed =
                        allowed ||
                        in_range(offset, 0x120U + edge.ordinal * 0x38U, 16U);
            }
            EXPECT_TRUE(allowed) << node.object_type << ' ' << node.name
                                 << " changed byte " << offset;
        }
        if (node.object_type == "SMPL") {
            ...
            ASSERT_TRUE(relocated_decoded) << relocated_decoded.error().message;
            const auto *group =
                std::get_if<axk::CurrentSbac>(&relocated_decoded->payload);
            ASSERT_NE(group, nullptr);
            EXPECT_TRUE(std::ranges::all_of(group->slots, [](const auto &slot) {
                return slot.raw_handle == 0U;
            }));
        } else if (node.object_type == "PROG") {
            const auto relocated_decoded = axk::decode_object(*relocated);
            ASSERT_TRUE(relocated_decoded) << relocated_decoded.error().message;
            const auto *program =
                std::get_if<axk::CurrentProg>(&relocated_decoded->payload);
            ASSERT_NE(program, nullptr);
            EXPECT_TRUE(std::ranges::all_of(
                program->assignments, [](const auto &assignment) {
    ...
    EXPECT_EQ(object_types,
              (std::set<std::string>{"PROG", "SBAC", "SBNK", "SMPL"}));
    std::filesystem::remove_all(output_root, error);
}
...
TEST(PackageImportPlanner, ReservesOneSfsObjectForSharedIncomingRoots) {
    ...
    axk::PackageImportRequest request;
    request.root_destinations = {destination(0U, "New Volume"),
                                 destination(1U, "New Volume")};
    const std::vector packages{built->package, built->package};
    const auto first = axk::plan_package_import(
        fixture("HD00_512_single_sbnk_authored.hds"), packages, request);
    const auto second = axk::plan_package_import(
        fixture("HD00_512_single_sbnk_authored.hds"), packages, request);
    ...
    EXPECT_EQ(
        std::ranges::count_if(first->objects,
                              [](const auto &object) {
    ...
    EXPECT_EQ(
        ...
        1);
    const auto inserted =
        std::ranges::find_if(first->objects, [](const auto &object) {
            return std::ranges::contains(
                object.actions, axk::PackageImportObjectAction::insert);
        });
    const auto reused =
        std::ranges::find_if(first->objects, [](const auto &object) {
            return std::ranges::contains(object.actions,
    ...
    const auto expect_rejected_canonical = [&](auto mutate) {
        auto tampered = *first;
        const auto tampered_reuse =
            std::ranges::find_if(tampered.objects, [](const auto &object) {
                return object.canonical_action_id.has_value();
            });
    ...
    expect_rejected_canonical(
        [](auto &object) { object.volume_name = "Other Volume"; });
    expect_rejected_canonical(
        [](auto &object) { object.object_type = "SBNK"; });
    expect_rejected_canonical(
        [](auto &object) { object.destination_name = "Other Name"; });
    expect_rejected_canonical([](auto &object) {
        object.normalized_sha256.front() =
            object.normalized_sha256.front() == '0' ? '1' : '0';
    });
...
TEST(PackageImportPlanner,
     CollectsIncomingNameConflictsAndAcceptsExplicitRename) {
    auto image = axk::FatImage::open(
        std::make_shared<axk::MemoryReader>(fat_fixture()), "fixture.ima");
    ASSERT_TRUE(image) << image.error().message;
    ...
    ASSERT_NE(original->package.nodes.front().normalized_sha256,
              changed->package.nodes.front().normalized_sha256);

    const std::vector packages{original->package, changed->package};
    axk::PackageImportRequest conflicting;
    conflicting.root_destinations = {destination(0U, "New Volume"),
                                     destination(1U, "New Volume")};
    const auto rejected = axk::plan_package_import(
        fixture("HD00_512_single_sbnk_authored.hds"), packages, conflicting);
    ASSERT_TRUE(rejected) << rejected.error().message;
    EXPECT_FALSE(rejected->valid());
    EXPECT_TRUE(
        std::ranges::any_of(rejected->conflicts, [](const auto &conflict) {
            return conflict.code == "SFS_NAME_CONFLICT";
        }));
    EXPECT_EQ(std::ranges::count_if(rejected->conflicts,
              ...
              2);
    EXPECT_GE(
        std::ranges::count_if(rejected->objects,
                              [](const auto &object) {
                                  return std::ranges::contains(
                                      object.actions,
    ...
    renamed.policy.renames.push_back(
        {1U, changed->package.nodes.front().node_id, "TEST ALT"});
    const auto accepted = axk::plan_package_import(
        fixture("HD00_512_single_sbnk_authored.hds"), packages, renamed);
    ASSERT_TRUE(accepted) << accepted.error().message;
    ASSERT_TRUE(accepted->valid());
    EXPECT_EQ(
        std::ranges::count_if(accepted->objects,
                              [](const auto &object) {
                                  return std::ranges::contains(
                                      object.actions,
                                      axk::PackageImportObjectAction::insert);
    ...
    EXPECT_TRUE(std::ranges::any_of(accepted->objects, [](const auto &object) {
        return object.destination_name == "TEST ALT" &&
               std::ranges::contains(object.actions,
                                     axk::PackageImportObjectAction::rename);
    }));

    const std::vector equal_pcm_packages{original->package, original->package};
    auto distinct_names = conflicting;
    distinct_names.policy.renames.push_back(
        {1U, original->package.nodes.front().node_id, "TEST ALT"});
    const auto name_sensitive =
        axk::plan_package_import(fixture("HD00_512_single_sbnk_authored.hds"),
                                 equal_pcm_packages, distinct_names);
    ASSERT_TRUE(name_sensitive) << name_sensitive.error().message;
    ASSERT_TRUE(name_sensitive->valid()) << conflict_summary(*name_sensitive);
    EXPECT_EQ(
        std::ranges::count_if(name_sensitive->objects,
                              [](const auto &object) {
                                  return std::ranges::contains(
                                      object.actions,
                                      axk::PackageImportObjectAction::insert);
    ...
    const auto fat_root =
        publication_root("axklib-package-fat12-name-conflicts");
    ...
    fat_conflicting.root_destinations = {destination(0U, "FAT root"),
                                         destination(1U, "FAT root")};
    const auto fat_rejected =
        axk::plan_package_import(fat_target, packages, fat_conflicting);
    ASSERT_TRUE(fat_rejected) << fat_rejected.error().message;
    EXPECT_FALSE(fat_rejected->valid());
    EXPECT_TRUE(
        std::ranges::any_of(fat_rejected->conflicts, [](const auto &conflict) {
            return conflict.code == "FAT12_NAME_CONFLICT";
        }));

    auto fat_distinct_names = fat_conflicting;
    fat_distinct_names.policy.renames.push_back(
        {1U, original->package.nodes.front().node_id, "TEST ALT"});
    const auto fat_name_sensitive = axk::plan_package_import(
        fat_target, equal_pcm_packages, fat_distinct_names);
    ASSERT_TRUE(fat_name_sensitive) << fat_name_sensitive.error().message;
    ASSERT_TRUE(fat_name_sensitive->valid())
        << conflict_summary(*fat_name_sensitive);
    EXPECT_EQ(
        std::ranges::count_if(fat_name_sensitive->objects,
                              [](const auto &object) {
                                  return std::ranges::contains(
                                      object.actions,
                                      axk::PackageImportObjectAction::insert);
...
TEST(PackageImportPlanner, ReportsInsufficientSfsAndFat12CapacityBeforeApply) {
    const auto output_root =
        publication_root("axklib-package-capacity-conflict");
    ...
    source_volume.waveforms.push_back(
        {"large", "Large Wave", audio_path, 60U, {}});
    axk::HdsBuildManifest source_manifest{"1.0", 4U * 1024U * 1024U, {}};
    ...
    ASSERT_TRUE(source) << source.error().message;
    const std::vector selectors{
        root(axk::PackageRootKind::smpl, "Source", "Large Wave")};
    const auto built = axk::build_portable_package(*source, selectors);
    ASSERT_TRUE(built) << built.error().message;
    const std::vector packages{built->package};
    axk::PackageImportRequest request;
    ...
    const auto fat_plan =
        axk::plan_package_import(fat_target, packages, fat_request);
    ASSERT_TRUE(fat_plan) << fat_plan.error().message;
    EXPECT_FALSE(fat_plan->valid());
    EXPECT_TRUE(
        std::ranges::any_of(fat_plan->conflicts, [](const auto &conflict) {
            return conflict.code == "FAT12_CLUSTER_EXHAUSTED";
        }));
    std::filesystem::remove_all(output_root, error);
}
...
TEST(PackageImportPlanner, RejectsFat12RootExhaustionAndInvalidExistingChains) {
    ...
    for (std::size_t index = 0U; index < 224U; ++index)
        full.retained_files.push_back({std::format("R{:07}.DAT", index),
                                       {static_cast<std::byte>(index)}});
    const auto full_written =
        axk::detail::write_fat12_image(full, full_target, {});
    ...
    const auto full_plan =
        axk::plan_package_import(full_target, packages, request);
    ASSERT_TRUE(full_plan) << full_plan.error().message;
    EXPECT_FALSE(full_plan->valid());
    EXPECT_TRUE(
        std::ranges::any_of(full_plan->conflicts, [](const auto &conflict) {
            return conflict.code == "FAT12_ROOT_ENTRY_EXHAUSTED";
        }));

    const auto invalid_bytes = fat_fixture(2U);
    ASSERT_TRUE(write_file(invalid_target, invalid_bytes));
    const auto invalid_plan =
        axk::plan_package_import(invalid_target, packages, request);
...
TEST(PackageImportPlanner, ReportsSfsObjectAndDirectoryCapacityBeforeApply) {
    ...
    for (std::size_t index = 0U; index < package_count; ++index) {
        ...
        request.policy.renames.push_back({index,
                                          built->package.nodes.front().node_id,
                                          std::format("W{:04}", index)});
    }
    ...
    EXPECT_FALSE(plan->valid());
    EXPECT_TRUE(std::ranges::any_of(plan->conflicts, [](const auto &conflict) {
        return conflict.code == "SFS_OBJECT_ID_EXHAUSTED";
    }));
    EXPECT_TRUE(std::ranges::any_of(plan->conflicts, [](const auto &conflict) {
        return conflict.code == "SFS_DIRECTORY_CAPACITY_EXHAUSTED";
    }));
...
TEST(PackageImportPlanner, ReportsIsoDirectoryCapacityBeforeApply) {
    ...
    target_manifest.authored_volume = single_bank_volume(
        audio_path, "Target Volume", "Target Wave", "Target Bank");
    ...
    EXPECT_FALSE(plan->valid());
    EXPECT_TRUE(std::ranges::any_of(plan->conflicts, [](const auto &conflict) {
        return conflict.code == "ISO9660_DIRECTORY_CAPACITY_EXHAUSTED";
    }));
...
TEST(MediaWriter, RejectsIsoSectorCountOverflowWithoutAllocatingAnImage) {
    ...
    const std::array<std::uint64_t, 1> boundary{
        (static_cast<std::uint64_t>(maximum) - 21U) * 2048U};
    const auto accepted =
        axk::detail::checked_iso9660_sector_count(1U, boundary);
    ASSERT_TRUE(accepted) << accepted.error().message;
    EXPECT_EQ(*accepted, maximum);

    const std::array<std::uint64_t, 1> overflow{
        (static_cast<std::uint64_t>(maximum) - 20U) * 2048U};
    const auto rejected =
        axk::detail::checked_iso9660_sector_count(1U, overflow);
    ASSERT_FALSE(rejected);
...
TEST(PackageImportApply, AtomicallyInsertsAndThenReusesAnExactSmpl) {
    ...
    EXPECT_TRUE(std::ranges::contains(plan->objects.front().actions,
                                      axk::PackageImportObjectAction::insert));

    const auto first_output = output_root / "first.hds";
    const auto applied = axk::apply_package_import(source_path, packages, *plan,
                                                   first_output, false);
    ...
    EXPECT_EQ(imported.front()->sfs_id.value,
              *plan->objects.front().target_sfs_id);
    const auto *sample =
        std::get_if<axk::CurrentSmpl>(&imported.front()->object.payload);
    ASSERT_NE(sample, nullptr);
    EXPECT_EQ(sample->link_id.value, *plan->objects.front().target_link_id);

    const auto repeat_plan =
        axk::plan_package_import(first_output, packages, request);
    ASSERT_TRUE(repeat_plan) << repeat_plan.error().message;
    ASSERT_TRUE(repeat_plan->valid());
    ASSERT_EQ(repeat_plan->objects.size(), 1U);
    EXPECT_TRUE(std::ranges::contains(repeat_plan->objects.front().actions,
                                      axk::PackageImportObjectAction::reuse));
    ...
    const auto repeated = axk::apply_package_import(
        first_output, packages, *repeat_plan, second_output, false);
    ASSERT_TRUE(repeated) << repeated.error().message;
    EXPECT_EQ(read_file(second_output), read_file(first_output));
    std::filesystem::remove_all(output_root, error);
}

TEST(PackageImportApply,
     AppliesVolumeLocalReuseAndPartitionIsolationInOnePlan) {
    const auto output_root =
        publication_root("axklib-package-import-scope-matrix");
    const auto target_path = output_root / "target.hds";
    ...
    manifest.partitions.push_back(
        {"P1", {std::move(volume_a), std::move(volume_b)}});
    manifest.partitions.push_back({"P2", {std::move(volume_c)}});
    ASSERT_TRUE(axk::write_hds_image(manifest, target_path));

    const auto built = fat_smpl_package();
    ASSERT_TRUE(built) << built.error().message;
    const std::vector packages{built->package, built->package, built->package,
                               built->package};
    axk::PackageImportRequest request;
    request.root_destinations = {
        destination(0U, 0U, "Volume A"), destination(1U, 0U, "Volume A"),
    ...
    ASSERT_TRUE(plan->valid());
    EXPECT_EQ(
        std::ranges::count_if(plan->objects,
                              [](const auto &object) {
                                  return std::ranges::contains(
                                      object.actions,
                                      axk::PackageImportObjectAction::insert);
    ...
    EXPECT_EQ(
        std::ranges::count_if(plan->allocation,
                              [](const auto &allocation) {
                                  return allocation.inserted_object_count == 1U;
                              }),
        3);

    const auto applied =
        axk::apply_package_import(target_path, packages, *plan, output_path);
    ASSERT_TRUE(applied) << applied.error().message;
    auto reopened = axk::open_media(output_path);
    ASSERT_TRUE(reopened) << reopened.error().message;
    const auto catalog = axk::build_object_catalog(*reopened);
    ASSERT_TRUE(catalog) << catalog.error().message;
    const auto count_sample = [&](std::uint8_t partition,
                                  std::string_view volume) {
        return std::ranges::count_if(catalog->objects, [&](const auto &object) {
            return object.partition.value == partition && object.placement &&
                   object.placement->volume_name == volume &&
                   object.object.header.raw_type == "SMPL" &&
                   object.object.header.name == "TEST";
        });
    };
    ...
    const auto repeat_plan =
        axk::plan_package_import(output_path, packages, request);
    ASSERT_TRUE(repeat_plan) << repeat_plan.error().message;
    ASSERT_TRUE(repeat_plan->valid());
    EXPECT_TRUE(
        std::ranges::all_of(repeat_plan->objects, [](const auto &object) {
            return std::ranges::contains(
                       object.actions, axk::PackageImportObjectAction::reuse) &&
                   !std::ranges::contains(
                       object.actions, axk::PackageImportObjectAction::insert);
        }));
    ASSERT_TRUE(axk::apply_package_import(output_path, packages, *repeat_plan,
                                          repeated_path, false));
    EXPECT_EQ(read_file(output_path), read_file(repeated_path));
}

TEST(PackageImportApply,
     RelocatesACompleteRenamedSbnkClosureAndReopensItsGraph) {
    ...
    for (const auto &node : built->package.nodes) {
        request.policy.renames.push_back(
            {0U, node.node_id,
             node.object_type == "SBNK" ? "Bank Copy" : "Wave Copy"});
    }
    const auto plan = axk::plan_package_import(source_path, packages, request);
    ASSERT_TRUE(plan) << plan.error().message;
    ASSERT_TRUE(plan->valid());
    ...
    EXPECT_TRUE(std::ranges::all_of(plan->objects, [](const auto &object) {
        return std::ranges::contains(object.actions,
                                     axk::PackageImportObjectAction::rename) &&
               std::ranges::contains(
                   object.actions, axk::PackageImportObjectAction::relocate) &&
               std::ranges::contains(object.actions,
                                     axk::PackageImportObjectAction::insert);
    }));
    ...
    const auto applied =
        axk::apply_package_import(source_path, packages, *plan, output);
    ASSERT_TRUE(applied) << applied.error().message;
    ...
    const auto bank =
        std::ranges::find_if(catalog->objects, [](const auto &object) {
            return object.placement &&
                   object.placement->volume_name == "New Volume" &&
                   object.object.header.raw_type == "SBNK" &&
                   object.object.header.name == "Bank Copy";
        });
    const auto sample =
        std::ranges::find_if(catalog->objects, [](const auto &object) {
            return object.placement &&
                   object.placement->volume_name == "New Volume" &&
                   object.object.header.raw_type == "SMPL" &&
                   object.object.header.name == "Wave Copy";
        });
    ASSERT_NE(bank, catalog->objects.end());
    ASSERT_NE(sample, catalog->objects.end());
    const auto *decoded_bank =
        std::get_if<axk::CurrentSbnk>(&bank->object.payload);
    const auto *decoded_sample =
        std::get_if<axk::CurrentSmpl>(&sample->object.payload);
    ...
    EXPECT_TRUE(
        std::ranges::any_of(children, [&](const axk::Relationship *edge) {
    ...
    const auto repeat_plan =
        axk::plan_package_import(output, packages, request);
    ASSERT_TRUE(repeat_plan) << repeat_plan.error().message;
    ASSERT_TRUE(repeat_plan->valid());
    EXPECT_TRUE(
        std::ranges::all_of(repeat_plan->objects, [](const auto &object) {
    ...
    ASSERT_TRUE(
        axk::apply_package_import(output, packages, *repeat_plan, repeated));
...
TEST(PackageImportApply, PreservesStereoClosureAndExactPhysicalPcm) {
    ...
    left.frame_count = 4U;
    left.pcm = {std::byte{0x01}, std::byte{0x00}, std::byte{0x02},
                std::byte{0x00}, std::byte{0x03}, std::byte{0x00},
                std::byte{0x04}, std::byte{0x00}};
    auto right = left;
    right.pcm = {std::byte{0xf1}, std::byte{0xff}, std::byte{0xf2},
                 std::byte{0xff}, std::byte{0xf3}, std::byte{0xff},
                 std::byte{0xf4}, std::byte{0xff}};
    ...
    ASSERT_TRUE(source) << source.error().message;
    const std::vector bank_root{
        root(axk::PackageRootKind::sbnk, "Stereo Source", "Stereo Bank")};
    const auto built = axk::build_portable_package(*source, bank_root);
    ASSERT_TRUE(built) << built.error().message;
    ...
    const auto source_left =
        std::ranges::find_if(source_catalog->objects, [](const auto &object) {
            return object.placement &&
                   object.placement->volume_name == "Stereo Source" &&
                   object.object.header.raw_type == "SMPL" &&
                   object.object.header.name == "Stereo L";
        });
    const auto source_right =
        std::ranges::find_if(source_catalog->objects, [](const auto &object) {
            return object.placement &&
                   object.placement->volume_name == "Stereo Source" &&
                   object.object.header.raw_type == "SMPL" &&
                   object.object.header.name == "Stereo R";
        });
    ASSERT_NE(source_left, source_catalog->objects.end());
    ASSERT_NE(source_right, source_catalog->objects.end());
    const auto source_left_waveform =
        axk::decode_waveform(source_container, *source_left);
    const auto source_right_waveform =
        axk::decode_waveform(source_container, *source_right);
    ...
    ASSERT_TRUE(plan->valid());
    ASSERT_TRUE(
        axk::apply_package_import(source_path, packages, *plan, output_path));

    auto reopened = axk::open_media(output_path);
    ASSERT_TRUE(reopened) << reopened.error().message;
    const auto catalog = axk::build_object_catalog(*reopened);
    ASSERT_TRUE(catalog) << catalog.error().message;
    const auto imported_bank =
        std::ranges::find_if(catalog->objects, [](const auto &object) {
            return object.placement &&
                   object.placement->volume_name == "Stereo Imported" &&
                   object.object.header.raw_type == "SBNK" &&
                   object.object.header.name == "Stereo Bank";
        });
    const auto imported_left =
        std::ranges::find_if(catalog->objects, [](const auto &object) {
            return object.placement &&
                   object.placement->volume_name == "Stereo Imported" &&
                   object.object.header.raw_type == "SMPL" &&
    ...
    const auto *decoded_bank =
        std::get_if<axk::CurrentSbnk>(&imported_bank->object.payload);
    ASSERT_NE(decoded_bank, nullptr);
    ...
    EXPECT_TRUE(
        std::ranges::any_of(children, [&](const axk::Relationship *edge) {
            return edge->type == "SBNK_RIGHT_MEMBER_TO_SMPL" &&
                   edge->quality == axk::RelationshipQuality::known &&
    ...
    fat_request.root_destinations.push_back(destination(0U, "FAT root"));
    const auto fat_plan =
        axk::plan_package_import(fat_target_path, packages, fat_request);
    ASSERT_TRUE(fat_plan) << fat_plan.error().message;
    ASSERT_TRUE(fat_plan->valid()) << conflict_summary(*fat_plan);
    ASSERT_TRUE(axk::apply_package_import(fat_target_path, packages, *fat_plan,
                                          fat_output_path, false));
    ...
    const auto fat_bank =
        std::ranges::find_if(fat_catalog->objects, [](const auto &object) {
            return object.object.header.raw_type == "SBNK" &&
                   object.object.header.name == "Stereo Bank";
        });
    ASSERT_NE(fat_bank, fat_catalog->objects.end());
    const auto fat_graph = axk::build_relationship_graph(*fat_catalog);
    EXPECT_EQ(std::ranges::count_if(
                  fat_graph.children(fat_bank->key),
...
TEST(PackageImportApply,
     CreatesAnExplicitDestinationWithFullyPlannedScaffolding) {
    const auto output_root =
        publication_root("axklib-package-import-new-volume");
    std::error_code error;
    std::filesystem::remove_all(output_root, error);
    auto image = axk::FatImage::open(
        std::make_shared<axk::MemoryReader>(fat_fixture()), "fixture.ima");
...
TEST(PackageImportApply, ImportsACompleteProgramGroupBankAndWaveformGraph) {
    ...
    const auto program_node =
        std::ranges::find(built->package.nodes, std::string{"PROG"},
                          &axk::PackageNode::object_type);
    ...
    invalid_program_name.policy.renames.push_back(
        {0U, program_node->node_id, "1"});
    const std::vector packages{built->package};
    const auto rejected =
        axk::plan_package_import(source_path, packages, invalid_program_name);
    ASSERT_TRUE(rejected) << rejected.error().message;
    EXPECT_FALSE(rejected->valid());
    EXPECT_TRUE(
        std::ranges::any_of(rejected->conflicts, [](const auto &conflict) {
            return conflict.code == "SFS_PROGRAM_SLOT_INVALID" &&
                   conflict.message.find("001 through 128") !=
                       std::string::npos;
        }));
    ...
    const auto in_imported_volume = [](const auto &object) {
        return object.placement &&
               object.placement->volume_name == "Imported Graph";
    };
    EXPECT_EQ(std::ranges::count_if(catalog->objects, in_imported_volume), 5);
    const auto grouped_bank =
        std::ranges::find_if(catalog->objects, [&](const auto &object) {
            return in_imported_volume(object) &&
                   object.object.header.raw_type == "SBNK" &&
                   object.object.header.name == "Grouped Bank";
        });
    const auto direct_bank =
        std::ranges::find_if(catalog->objects, [&](const auto &object) {
            return in_imported_volume(object) &&
                   object.object.header.raw_type == "SBNK" &&
    ...
    const auto *grouped =
        std::get_if<axk::CurrentSbnk>(&grouped_bank->object.payload);
    const auto *direct =
        std::get_if<axk::CurrentSbnk>(&direct_bank->object.payload);
    ASSERT_NE(grouped, nullptr);
    ASSERT_NE(direct, nullptr);
    EXPECT_NE(grouped->sample_flags & 1U, 0U);
    EXPECT_TRUE(direct->linked_program_numbers ==
                std::vector<std::uint8_t>{1U});
    const auto graph = axk::build_relationship_graph(*catalog);
    for (const auto &object : plan->objects) {
        const auto actual =
            std::ranges::find_if(catalog->objects, [&](const auto &snapshot) {
                return object.target_sfs_id &&
        ...
        const auto expected_children = std::ranges::count_if(
            built->package.relationships, [&](const auto &edge) {
                return edge.source_node_id == object.node_id;
            });
        ASSERT_NE(actual, catalog->objects.end())
            << object.object_type << ' ' << object.destination_name;
        const auto actual_children = graph.children(actual->key);
        const auto package_children =
            std::ranges::count_if(actual_children, [&](const auto *edge) {
                return std::ranges::any_of(
                    built->package.relationships, [&](const auto &expected) {
                        return expected.source_node_id == object.node_id &&
                               expected.role == edge->type;
                    });
            });
    ...
    const auto reexported =
        axk::build_portable_package(*reopened, imported_program_root);
    ...
    const auto reexported_program =
        std::ranges::find(reexported->package.nodes, std::string{"PROG"},
                          &axk::PackageNode::object_type);
    ASSERT_NE(reexported_program, reexported->package.nodes.end());
    ASSERT_EQ(reexported_program->relocations.size(),
              program_node->relocations.size());
...
TEST(PackageImportApply, CancellationAtEveryGraphBoundaryPublishesNothing) {
    const auto output_root =
        publication_root("axklib-package-import-cancellation-boundaries");
    const auto audio_path = output_root / "tone.wav";
    const auto source_path = output_root / "source.hds";
    std::error_code error;
    std::filesystem::remove_all(output_root, error);
    ...
    for (std::uint64_t boundary = 0U; boundary <= plan->objects.size();
         ++boundary) {
        SCOPED_TRACE(boundary);
        axk::CancellationSource cancellation;
        CancellingPackageProgress progress{cancellation, boundary};
        const auto output_path =
            output_root / ("cancelled-" + std::to_string(boundary) + ".hds");
        const auto applied =
            axk::apply_package_import(source_path, packages, *plan, output_path,
                                      false, cancellation.token(), &progress);
    ...
    for (const auto &entry : std::filesystem::directory_iterator(output_root)) {
        EXPECT_TRUE(entry.path() == audio_path || entry.path() == source_path)
            << entry.path().string();
    }
    std::filesystem::remove_all(output_root, error);
}
...
TEST(PackageImportApply, MergesPlannedGraphMetadataIntoReusedSampleBanks) {
    const auto output_root =
        publication_root("axklib-package-import-reused-banks");
    const auto audio_path = output_root / "tone.wav";
    const auto source_path = output_root / "source.hds";
    const auto target_path = output_root / "target.hds";
    const auto output_path = output_root / "imported.hds";
    ...
    EXPECT_EQ(
        ...
        2);
    const auto grouped_bank =
        std::ranges::find_if(plan->objects, [](const auto &object) {
            return object.object_type == "SBNK" &&
                   object.destination_name == "Grouped Bank";
        });
    const auto direct_bank =
        std::ranges::find_if(plan->objects, [](const auto &object) {
            return object.object_type == "SBNK" &&
                   object.destination_name == "Direct Bank";
        });
    ...
    EXPECT_TRUE(direct_bank->target_program_numbers ==
                std::vector<std::uint8_t>{1U});
    for (const auto *bank : {&*grouped_bank, &*direct_bank}) {
        EXPECT_TRUE(std::ranges::contains(
            bank->actions, axk::PackageImportObjectAction::reuse));
        EXPECT_TRUE(std::ranges::contains(
            bank->actions, axk::PackageImportObjectAction::relocate));
        EXPECT_FALSE(std::ranges::contains(
            bank->actions, axk::PackageImportObjectAction::insert));
    }
    ...
    auto catalog = axk::build_object_catalog(*reopened);
    ASSERT_TRUE(catalog) << catalog.error().message;
    const auto imported_grouped =
        std::ranges::find_if(catalog->objects, [](const auto &object) {
            return object.placement &&
                   object.placement->volume_name == "Target Volume" &&
                   object.object.header.raw_type == "SBNK" &&
                   object.object.header.name == "Grouped Bank";
        });
    const auto imported_direct =
        std::ranges::find_if(catalog->objects, [](const auto &object) {
            return object.placement &&
                   object.placement->volume_name == "Target Volume" &&
                   object.object.header.raw_type == "SBNK" &&
    ...
    const auto *grouped =
        std::get_if<axk::CurrentSbnk>(&imported_grouped->object.payload);
    const auto *direct =
        std::get_if<axk::CurrentSbnk>(&imported_direct->object.payload);
    ASSERT_NE(grouped, nullptr);
    ASSERT_NE(direct, nullptr);
    EXPECT_NE(grouped->sample_flags & 1U, 0U);
    EXPECT_TRUE(direct->linked_program_numbers ==
    ...
    EXPECT_TRUE(
        std::ranges::all_of(repeat_plan->objects, [](const auto &object) {
            return std::ranges::contains(
                   ...
                   !std::ranges::contains(
                       object.actions,
                       axk::PackageImportObjectAction::relocate);
        }));
    ASSERT_TRUE(axk::apply_package_import(output_path, packages, *repeat_plan,
                                          repeated_path, false));
    EXPECT_EQ(read_file(output_path), read_file(repeated_path));
...
TEST(PackageImportApply,
     RejectsTamperedStaleAndCancelledPlansWithoutPublication) {
    ...
    ASSERT_TRUE(std::filesystem::copy_file(
        fixture("HD00_512_single_sbnk_authored.hds"), mutable_target));
    const auto plan =
        axk::plan_package_import(mutable_target, packages, request);
    ASSERT_TRUE(plan) << plan.error().message;
    ASSERT_TRUE(plan->valid());
    ...
    EXPECT_FALSE(axk::apply_package_import(mutable_target, packages, tampered,
                                           tampered_output, false));
    EXPECT_FALSE(std::filesystem::exists(tampered_output));

    {
        std::fstream target{mutable_target,
    ...
    const auto stale = axk::apply_package_import(mutable_target, packages,
                                                 *plan, stale_output, false);
    ASSERT_FALSE(stale);
    ...
    const auto cancelled = axk::apply_package_import(
        mutable_target, packages, *plan, cancelled_output, false,
        cancellation.token());
    ASSERT_FALSE(cancelled);
    EXPECT_EQ(cancelled.error().code, axk::ErrorCode::operation_cancelled);
...
TEST(PackageRegressionMatrix,
     MixesSfsFat12AndIsoSourcesIntoSfsDeterministically) {
    const auto output_root =
        publication_root("axklib-package-mixed-sfs-matrix");
    const auto target_path = output_root / "target.hds";
    ...
    axk::PackageImportRequest request;
    request.root_destinations = {destination(0U, "Mixed"),
                                 destination(1U, "Mixed"),
                                 destination(2U, "Mixed")};
    const auto plan = axk::plan_package_import(target_path, *packages, request);
    ASSERT_TRUE(plan) << plan.error().message;
    ASSERT_TRUE(plan->valid());
    ASSERT_TRUE(axk::verify_package_import_plan(*plan));
    ...
    const auto report =
        axk::apply_package_import(target_path, *packages, *plan, first_path);
    ...
    std::vector<axk::PortablePackage> reversed_packages{
        (*packages)[2], (*packages)[1], (*packages)[0]};
    axk::PackageImportRequest reversed_request;
    reversed_request.root_destinations = {destination(0U, "Mixed"),
                                          destination(1U, "Mixed"),
                                          destination(2U, "Mixed")};
    const auto reversed_plan = axk::plan_package_import(
        target_path, reversed_packages, reversed_request);
    ...
    ASSERT_TRUE(axk::apply_package_import(target_path, reversed_packages,
                                          *reversed_plan, reversed_path));
    ...
    const auto repeat_plan =
        axk::plan_package_import(first_path, *packages, request);
    ASSERT_TRUE(repeat_plan) << repeat_plan.error().message;
    ASSERT_TRUE(repeat_plan->valid());
    EXPECT_TRUE(
        std::ranges::all_of(repeat_plan->objects, [](const auto &object) {
    ...
    ASSERT_TRUE(axk::apply_package_import(first_path, *packages, *repeat_plan,
                                          repeated_path, false));
...
TEST(PackageRegressionMatrix,
     MixesSfsFat12AndIsoSourcesIntoFat12Deterministically) {
    const auto output_root =
        publication_root("axklib-package-mixed-fat12-matrix");
    ...
    auto seed = axk::FatImage::open(
        std::make_shared<axk::MemoryReader>(fat_fixture()), "seed.ima");
    ...
    target_prepared.objects.push_back(
        {seed_objects->front().decoded.header.type,
         seed_objects->front().decoded.header.name,
         seed_objects->front().raw_payload});
    const std::vector retained_file_payload{std::byte{'r'}, std::byte{'e'},
                                            std::byte{'t'}, std::byte{'a'},
                                            std::byte{'i'}, std::byte{'n'}};
    target_prepared.retained_files.push_back(
        {"README.TXT", retained_file_payload});
    ASSERT_TRUE(axk::detail::write_prepared_media_image(
        target_prepared, target_path, false, {}));
    ...
    const auto target_profile = axk::package_internal::build_relocation_profile(
        target_objects->front().decoded, target_objects->front().raw_payload);
    ...
    EXPECT_EQ((*packages)[1].nodes.front().normalized_sha256,
              axk::package_internal::hex_digest(axk::package_internal::sha256(
                  target_profile->normalized_payload)));

    axk::PackageImportRequest request;
    request.root_destinations = {destination(0U, "FAT root"),
                                 destination(1U, "FAT root"),
    ...
    EXPECT_TRUE(std::ranges::any_of(*first_objects, [&](const auto &object) {
        return object.decoded.header.raw_type == "SMPL" &&
               object.decoded.header.name == "TEST" &&
               object.raw_payload == retained_payload;
    }));
    const auto &first_fat = std::get<axk::FatImage>(first->storage());
    const auto retained_file = std::ranges::find(
        first_fat.files(), std::string{"README.TXT"}, &axk::FatFile::path);
    ...
    std::vector<axk::PortablePackage> reversed_packages{
        (*packages)[2], (*packages)[1], (*packages)[0]};
    axk::PackageImportRequest reversed_request;
    reversed_request.root_destinations = {destination(0U, "FAT root"),
                                          destination(1U, "FAT root"),
                                          destination(2U, "FAT root")};
    const auto reversed_plan = axk::plan_package_import(
        target_path, reversed_packages, reversed_request);
...
TEST(PackageImportApply, RebuildsFat12GraphAndRepeatsByteIdentically) {
    ...
    const std::vector packages{built->package, built->package};
    axk::PackageImportRequest request;
    request.root_destinations = {destination(0U, 0U, "FAT root"),
                                 destination(1U, 0U, "FAT root")};
    ...
    const auto cancelled =
        axk::apply_package_import(target_path, packages, *plan, cancelled_path,
    ...
    EXPECT_TRUE(std::ranges::any_of(*media_objects, [&](const auto &object) {
        return object.decoded.header.name == "TEST" &&
               object.raw_payload == retained_payload;
    }));
    const auto catalog = axk::build_object_catalog(*reopened);
    ASSERT_TRUE(catalog) << catalog.error().message;
    const auto bank =
        std::ranges::find_if(catalog->objects, [](const auto &object) {
            return object.object.header.raw_type == "SBNK" &&
                   object.object.header.name == "sine wave";
        });
    ASSERT_NE(bank, catalog->objects.end());
    const auto graph = axk::build_relationship_graph(*catalog);
    EXPECT_TRUE(std::ranges::any_of(
        graph.children(bank->key), [](const axk::Relationship *edge) {
            return edge->type == "SBNK_LEFT_MEMBER_TO_SMPL" &&
                   edge->quality == axk::RelationshipQuality::known &&
                   edge->target_key.has_value();
        }));
...
TEST(PackageRegressionMatrix,
     MixesSfsFat12AndIsoSourcesIntoIso9660Deterministically) {
    const auto output_root =
        publication_root("axklib-package-mixed-iso9660-matrix");
    ...
    target_manifest.raw_volume = "F001";
    target_manifest.authored_volume = single_bank_volume(
        target_audio, "Target Volume", "Target Wave", "Target Bank");
    auto prepared = axk::detail::prepare_media_image(target_manifest, {});
    ASSERT_TRUE(prepared) << prepared.error().message;
    const std::vector retained_payload{std::byte{'r'}, std::byte{'e'},
                                       std::byte{'t'}, std::byte{'a'},
                                       std::byte{'i'}, std::byte{'n'}};
    prepared->retained_files.push_back({"README", retained_payload});
    ASSERT_TRUE(axk::detail::write_prepared_media_image(*prepared, target_path,
                                                        false, {}));
    ...
    std::map<std::pair<std::string, std::string>, std::vector<std::byte>>
        retained_objects;
    for (const auto &object : *target_objects) {
        retained_objects.emplace(std::pair{object.decoded.header.raw_type,
                                           object.decoded.header.name},
                                 object.raw_payload);
    }
    ...
    for (std::size_t package_index = 0U; package_index < packages->size();
         ++package_index) {
        auto target_destination =
            destination(package_index, 0U, "Target Volume");
    ...
    EXPECT_EQ(plan->allocation.front().directory_growth_bytes, 256U);
    EXPECT_TRUE(std::ranges::all_of(plan->objects, [](const auto &object) {
        return object.payload_clusters == 0U && object.payload_sectors == 1U &&
               object.continuation_clusters == 0U;
    }));
    ...
    const auto report =
        axk::apply_package_import(target_path, *packages, *plan, first_path);
    ASSERT_TRUE(report) << report.error().message;
    EXPECT_EQ(std::filesystem::file_size(first_path),
    ...
    const auto retained = std::ranges::find(
        first_iso.files(), std::string{"README"}, &axk::IsoFile::path);
    ...
    for (std::size_t package_index = 0U;
         package_index < reversed_packages.size(); ++package_index) {
        auto target_destination =
            destination(package_index, 0U, "Target Volume");
        target_destination.group_name = "Target Group";
        target_destination.raw_group = "GROUP";
        target_destination.raw_volume = "F001";
        reversed_request.root_destinations.push_back(
            std::move(target_destination));
    }
    const auto reversed_plan = axk::plan_package_import(
        target_path, reversed_packages, reversed_request);
    ASSERT_TRUE(reversed_plan) << reversed_plan.error().message;
    ...
    EXPECT_EQ(repeat_plan->allocation.front().projected_image_sectors,
              plan->allocation.front().projected_image_sectors);
    EXPECT_EQ(repeat_plan->allocation.front().projected_image_size_bytes,
...
TEST(PackageImportApply,
     RebuildsIsoWithVolumeLocalReuseAndByteIdenticalRepeat) {
    ...
    auto target_prepared =
        axk::detail::prepare_media_image(target_manifest, {});
    ...
    const auto target_written = axk::detail::write_prepared_media_image(
        *target_prepared, target_path, false, {});
    ...
    const auto shared_creation = axk::plan_package_import(
        target_path, packages, shared_creation_request);
    ...
    EXPECT_EQ(
        std::ranges::count_if(shared_creation->objects,
                              [](const auto &object) {
                                  return std::ranges::contains(
                                      object.actions,
                                      axk::PackageImportObjectAction::insert);
    ...
    EXPECT_TRUE(std::ranges::any_of(plan->warnings, [](const auto &warning) {
        return warning.code == "ISO9660_CROSS_VOLUME_DUPLICATE";
    }));

    auto conflicting_request = request;
    const auto sample_node =
        std::ranges::find_if(packages[0].nodes, [](const auto &node) {
            return node.object_type == "SMPL";
        });
    ASSERT_NE(sample_node, packages[0].nodes.end());
    conflicting_request.policy.renames.push_back(
        {0U, sample_node->node_id, "Graph Wave"});
    const auto conflicting =
        axk::plan_package_import(target_path, packages, conflicting_request);
    ASSERT_TRUE(conflicting) << conflicting.error().message;
    EXPECT_FALSE(conflicting->valid());
    EXPECT_TRUE(
        std::ranges::any_of(conflicting->conflicts, [](const auto &conflict) {
            return conflict.code == "ISO9660_NAME_CONFLICT";
        }));

    auto invalid_creation_request = request;
    invalid_creation_request.root_destinations[1].raw_volume = "F003";
    const auto invalid_creation = axk::plan_package_import(
        target_path, packages, invalid_creation_request);
    ASSERT_TRUE(invalid_creation) << invalid_creation.error().message;
    EXPECT_FALSE(invalid_creation->valid());
    EXPECT_TRUE(std::ranges::any_of(
        invalid_creation->conflicts, [](const auto &conflict) {
    ...
    const auto cancelled = axk::apply_package_import(
        target_path, packages, *plan, cancelled_path, false,
    ...
    ASSERT_TRUE(objects) << objects.error().message;
    for (const auto raw_volume :
         {std::string_view{"F001"}, std::string_view{"F002"}}) {
        EXPECT_TRUE(std::ranges::any_of(*objects, [&](const auto &object) {
            return object.raw_group == "GROUP" &&
                   object.raw_volume == raw_volume &&
                   object.decoded.header.type == axk::ObjectType::sbnk &&
                   object.decoded.header.name == "sine wave";
        }));
        EXPECT_TRUE(std::ranges::any_of(*objects, [&](const auto &object) {
            return object.raw_group == "GROUP" &&
                   object.raw_volume == raw_volume &&
    ...
    EXPECT_TRUE(std::ranges::any_of(*objects, [](const auto &object) {
        return object.raw_volume == "F001" &&
               object.decoded.header.name == "Graph Wave";
    }));
    const auto &reopened_iso = std::get<axk::IsoImage>(reopened->storage());
    const auto retained = std::ranges::find(
        reopened_iso.files(), std::string{"README"}, &axk::IsoFile::path);
    ...
    const auto repeat_plan =
        axk::plan_package_import(output_path, packages, repeat_request);
    ASSERT_TRUE(repeat_plan) << repeat_plan.error().message;
    ASSERT_TRUE(repeat_plan->valid());
    EXPECT_TRUE(
        std::ranges::all_of(repeat_plan->objects, [](const auto &object) {
