...
std::vector<std::byte> continuation_fixture(bool cycle) {
  ...
  const std::vector<std::byte> logical(bytes.begin() + static_cast<std::ptrdiff_t>(old_payload),
                                       bytes.begin() +
                                           static_cast<std::ptrdiff_t>(old_payload + 96U));
...
std::shared_ptr<SparseReader> sparse_geometry_fixture(std::uint64_t total_sectors,
  ...
  const auto slot_span =
      std::min<std::uint64_t>((total_sectors - 2U) / partition_count, 2'097'151U);
  for (std::uint8_t index = 0; index < partition_count; ++index) {
    ...
    EXPECT_TRUE(
        superblock_writer.write_be32(0xa8U + index * 8U, static_cast<std::uint32_t>(start)));
    EXPECT_TRUE(
        superblock_writer.write_be32(0xacU + index * 8U, static_cast<std::uint32_t>(count)));
...
TEST(SfsReader, MatchesPythonOracleOnSamplerAuthoredFixtures) {
  ...
  const auto root =
      std::filesystem::path{AXK_SOURCE_ROOT} / "tests/fixtures/images/sampler-authored";
...
TEST(SfsReader, ReportsMissingDirectoryTargetsAndChildCycles) {
  ...
  EXPECT_TRUE(std::any_of(missing_diagnostics.begin(), missing_diagnostics.end(),
  ...
  EXPECT_TRUE(
      std::any_of(cycle_diagnostics.begin(), cycle_diagnostics.end(), [](const axk::Error &error) {
