...
TEST(Iso9660Reader, RejectsMultiExtentAndUsesOnlyPrimaryTreeNames) {
  ...
  const auto unsupported = axk::IsoImage::open(
      std::make_shared<axk::MemoryReader>(std::move(multi_extent)), "multi-extent.iso");
  ...
  const auto image =
      axk::IsoImage::open(std::make_shared<axk::MemoryReader>(std::move(hybrid)), "hybrid.iso");
  ASSERT_TRUE(image) << image.error().message;
  EXPECT_NE(std::ranges::find(image->files(), "GROUP", &axk::IsoFile::path), image->files().end());
  EXPECT_EQ(std::ranges::find(image->files(), "Mapped Vol", &axk::IsoFile::path),
            image->files().end());
}
