...
TEST(ReportSchema, MatchesStableColumnCountAndClassificationRules) {
    const std::vector<axk::ReportRow> rows{
        {{"object_type", "SMPL"},
         ...
         {"optional", nullptr}},
        {{"object_type", "SBNK"}, {"quality", "Likely"}, {"code", "WARN"}, {"raw_offset", nullptr}},
    };
    axk::ReportSchemaOptions options;
    options.source_command = "axklib inventory";
    options.library_version = "test";
    const auto schema = axk::make_report_schema("example", rows, std::move(options));
    EXPECT_EQ(schema.row_count, 2U);
    EXPECT_EQ(schema.quality_counts, (std::vector<std::pair<std::string, std::uint64_t>>{{"Known", 1}, {"Likely", 1}}));
    EXPECT_EQ(schema.issue_code_counts, (std::vector<std::pair<std::string, std::uint64_t>>{{"OK", 1}, {"WARN", 1}}));
    EXPECT_EQ(schema.object_type_counts,
              (std::vector<std::pair<std::string, std::uint64_t>>{{"SBNK", 1}, {"SMPL", 1}}));
    const auto raw = std::ranges::find(schema.columns, "raw_offset", &axk::ReportColumnSchema::name);
...
TEST(ReportWriter, PreservesJsonOrderAndPythonCsvEscapingWithoutPartialOverwrite) {
    const std::vector<axk::ReportRow> rows{
        {{"name", "A, \"quoted\""}, {"count", std::int64_t{-2}}, {"active", true}},
        {{"name", "line\nbreak"}, {"count", std::uint64_t{3}}, {"active", false}},
    };
    const auto root = std::filesystem::temp_directory_path() / "axklib-report-test";
    std::error_code error;
    std::filesystem::remove_all(root, error);
    ASSERT_TRUE(axk::write_report_json(root / "rows.json", rows));
    ASSERT_TRUE(axk::write_report_csv(root / "rows.csv", rows));
    EXPECT_EQ(read(root / "rows.csv"), "name,count,active\r\n\"A, "
                                       "\"\"quoted\"\"\",-2,True\r\n\"line\nbreak\",3,False\r\n");
