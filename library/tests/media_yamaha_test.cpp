...
TEST(MediaPaths, SanitizesTraversalAndPlatformReservedCharacters) {
    EXPECT_EQ(axk::sanitize_path_component("../unsafe/name", "fallback"),
              "unsafe_name");
    EXPECT_EQ(axk::sanitize_path_component("..", "fallback"), "fallback");
    EXPECT_EQ(axk::sanitize_path_component("A:B*C?", "fallback"), "A_B_C");
    EXPECT_EQ(axk::sanitize_path_component("  Partition      A  ", "fallback"),
              "Partition A");
...
TEST(MediaPaths, DisambiguatesDuplicateDisplayedIsoVolumesByRawIdentifier) {
    ...
    EXPECT_EQ(paths[0].relative_path.generic_string(),
              "ORGANS/Or11 Argent (F001)/SMPL/TEST");
    EXPECT_EQ(paths[1].relative_path.generic_string(),
              "ORGANS/Or11 Argent (F002)/SMPL/TEST");
}
