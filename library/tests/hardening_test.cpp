...
std::vector<std::byte> seed(std::string_view relative) {
    const auto path = std::filesystem::path{AXK_SOURCE_ROOT} / "library/fuzz/corpus" / relative;
...
TEST(Hardening, EveryFocusedFuzzerSeedHasANormalTestReplay) {
    const auto corpus = std::filesystem::path{AXK_SOURCE_ROOT} / "library/fuzz/corpus";
    const auto fuzz_root = std::filesystem::path{AXK_SOURCE_ROOT} / "library/fuzz";
    ...
    std::ifstream cmake_stream{std::filesystem::path{AXK_SOURCE_ROOT} / "library/CMakeLists.txt"};
    ...
    for (const auto &harness : manifest.at("harnesses")) {
        ...
        for (const auto &entry : std::filesystem::directory_iterator{corpus_directory}) {
            ...
            if (target == "bytes") {
                ...
                static_cast<void>(
                    axk::IsoImage::open(std::make_shared<axk::MemoryReader>(std::move(envelope)), "seed.iso"));
