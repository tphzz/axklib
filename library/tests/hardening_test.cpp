...
TEST(Hardening, ReplaysEveryMaintainedMalformedSeedWithoutPartialSuccess) {
    ...
    EXPECT_FALSE(
        axk::open_image(std::move(sfs_reader), "seed.hds").has_value());
...
TEST(Hardening, EveryFocusedFuzzerSeedHasANormalTestReplay) {
    const auto corpus =
        std::filesystem::path{AXK_SOURCE_ROOT} / "library/fuzz/corpus";
    const auto fuzz_root =
        std::filesystem::path{AXK_SOURCE_ROOT} / "library/fuzz";
    ...
    std::ifstream cmake_stream{std::filesystem::path{AXK_SOURCE_ROOT} /
    ...
    for (const auto &support : manifest.at("support_sources")) {
        ...
        EXPECT_NE(cmake.find("add_executable(axk_sfs_seed_builder fuzz/" +
                             source + ")"),
                  std::string::npos)
            << source;
    }
    ...
    for (const auto &harness : manifest.at("harnesses")) {
        ...
        const auto corpus_directory =
            corpus / harness.at("corpus").get<std::string>();
        registered_sources.insert(source);
        const auto directly_registered =
            cmake.find("axk_add_fuzzer(" + target + " " + source + ")") !=
            std::string::npos;
        const auto typed_registered =
            ...
            cmake.find(target.substr(std::string{"object_"}.size())) !=
                std::string::npos;
        ...
        for (const auto &entry :
             std::filesystem::directory_iterator{corpus_directory}) {
            ...
            if (target == "bytes") {
                ...
                static_cast<void>(
                    axk::open_image(std::move(reader), "seed.hds"));
                ...
                std::ranges::transform(
                    type, type.begin(), [](unsigned char value) {
                        return static_cast<char>(std::toupper(value));
                    });
                const auto envelope =
                    axk::fuzz::typed_object_envelope(bytes, type);
                ...
                static_cast<void>(axk::FatImage::open(
                    std::make_shared<axk::MemoryReader>(std::move(envelope)),
                    "seed.ima"));
                ...
                static_cast<void>(
                    axk::package_internal::inspect_archive(reader));
                static_cast<void>(axk::package_internal::read_archive(bytes));
                static_cast<void>(
                    axk::open_portable_package(bytes, "seed.axkpkg"));
            } else {
