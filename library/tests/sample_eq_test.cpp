...
TEST(SampleEq, SharedReferencesCoverFreshSamplesBankStateAndExplicitEdits) {
    ...
    for (const auto &reference : references) {
        ...
        member.name = "Member";
        member = axk::detail::apply_sample_bank_parameter_overrides(member, parameters);
        const auto member_payload =
            axk::detail::prepare_sbnk_payload(member, {"Wave", 0x100U, 44'100U, 400U}, {}, true);
        ASSERT_TRUE(member_payload);
        EXPECT_EQ(std::to_integer<unsigned>((*member_payload)[0x0d0U]) & 1U, 1U);
        expect_vector(std::span{*member_payload}.subspan(0x152U, 10U), reference);

        const auto words = reference.at("coefficients_q13").get<std::array<std::int16_t, 5>>();
        std::array<std::byte, 10> stored;
        for (std::size_t index = 0; index < words.size(); ++index) {
            const auto raw = static_cast<std::uint16_t>(words[index]);
            stored[index * 2U] = static_cast<std::byte>(raw >> 8U);
            stored[index * 2U + 1U] = static_cast<std::byte>(raw & 0xffU);
        }
        ...
        for (const auto &edit : edits) {
            for (const auto &initial : {flat, stored}) {
                auto payload = *fresh;
                std::ranges::copy(initial, payload.begin() + 0x152U);
                axk::SampleParameters unrelated;
                unrelated.level = 126U;
                ASSERT_TRUE(axk::detail::apply_sample_parameters_to_payload(payload, unrelated));
                EXPECT_TRUE(std::ranges::equal(initial, std::span{payload}.subspan(0x152U, 10U)));
