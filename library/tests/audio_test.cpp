...
TEST(Audio, DecodesExactCurrentPcmAndWritesDeterministicWave) {
  ...
  ASSERT_TRUE(catalog);
  const auto sample =
      std::ranges::find(catalog->objects, std::string{"p0:sfs9"}, &axk::ObjectSnapshot::key);
  ...
  EXPECT_TRUE(
      std::ranges::equal(std::span{*wav}.subspan(44), std::span<const std::byte>{waveform->pcm}));
