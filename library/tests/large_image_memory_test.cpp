...
int main() {
...
#if defined(__APPLE__)
  // Darwin's peak RSS charges sparse-file writeback pages to this process. The
  // Mach physical footprint still catches a retained whole-image allocation.
  task_vm_info_data_t vm_info{};
  mach_msg_type_number_t vm_info_count = TASK_VM_INFO_COUNT;
  const auto usage_ok =
      ::task_info(::mach_task_self(), TASK_VM_INFO, reinterpret_cast<task_info_t>(&vm_info),
  ...
  std::cout << "2 GiB / 8-partition inventory memory: " << measured_memory_bytes / kibibyte
            << " KiB\n";
  return 0;
}
