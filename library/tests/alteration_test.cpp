...
axk::HdsBuildManifest bank_source_manifest(const std::filesystem::path &audio_path) {
  axk::HdsBuildManifest result{"1.0", 4U * 1024U * 1024U, {}};
...
void mark_cluster_used(const std::filesystem::path &path, const axk::Partition &partition,
...
void mark_cluster_free(const std::filesystem::path &path, const axk::Partition &partition,
                       std::uint32_t cluster) {
  std::fstream image{path, std::ios::binary | std::ios::in | std::ios::out};
  ASSERT_TRUE(image);
  const auto byte_index = cluster / 8U;
  ...
  for (const auto offset : offsets) {
    ...
    ASSERT_TRUE(image);
    value =
        static_cast<char>(static_cast<unsigned char>(value) & static_cast<unsigned char>(~mask));
    image.seekp(static_cast<std::streamoff>(offset));
    image.write(&value, 1);
    ASSERT_TRUE(image);
  }
...
TEST(AlterationManifest, ParsesStrictSampleBankOperations) {
  ...
  ASSERT_TRUE(parsed) << parsed.error().message;
  const auto *insert = std::get_if<axk::InsertSampleBankOperation>(&parsed->operations[1].data);
...
TEST(Alteration, DeleteVolumeDryRunMatchesApplyAndPreservesSource) {
  ...
  const auto &root_record = *std::ranges::find(reopened->partitions()[0].records, axk::SfsId{1},
  ...
  EXPECT_FALSE(std::ranges::any_of(root_record.directory_entries,
...
TEST(Alteration, RejectsSourceBitmapThatExposesLiveExtentsAsFree) {
  ...
  const auto live = std::ranges::find_if(partition.records, [](const auto &record) {
...
TEST(Alteration, RejectsSharedGroupMemberAndNonzeroRenameHandle) {
  const auto root = std::filesystem::temp_directory_path() / "axklib-alteration-group-safety";
...
TEST(Alteration, DeleteThenInsertSampleBankReusesRecordAndAllocation) {
  ...
  const auto bank = std::ranges::find_if(catalog->objects, [](const auto &object) {
    return object.placement && object.placement->entry_name == "New Bank";
  });
...
TEST(Alteration, QueuedWaveformAndSampleBankInsertionUsesEvolvingState) {
  ...
  expected_stored_pcm.insert(expected_stored_pcm.end(), expected_waveform.pcm.begin(),
...
TEST(Alteration, WritesAndReopensFortyEightExtentContinuationList) {
  ...
  const auto inserted = std::ranges::find_if(
      records, [](const auto &record) { return record.object_name == "Large Wave"; });
