...
TEST(CurrentLookups, ExposesCanonicalParameterAndProgramLabels) {
  ...
  EXPECT_EQ(axk::current_label(axk::CurrentLookup::sample_control_function_ui_labels, 4),
            "Cutoff Bias");
...
TEST(CurrentSmpl, MatchesPythonOracleOnSamplerAuthoredObject) {
  ...
  const auto payload = container->read_record_data(axk::PartitionIndex{0}, axk::SfsId{9},
...
TEST(CurrentSbnk, MatchesPythonOracleAndPreservesInactiveRightLane) {
  ...
  const auto container = axk::open_image(path);
  ASSERT_TRUE(container);
  const auto payload = container->read_record_data(axk::PartitionIndex{0}, axk::SfsId{10},
                                                   std::numeric_limits<std::size_t>::max());
  ASSERT_TRUE(payload);
  const auto decoded = axk::decode_object(*payload);
  ASSERT_TRUE(decoded);
...
TEST(CurrentSbac, MatchesPythonOracleSlotsAndBitmapBoundaries) {
  ...
  const auto container = axk::open_image(path);
  ASSERT_TRUE(container);
  const auto payload = container->read_record_data(axk::PartitionIndex{0}, axk::SfsId{23},
                                                   std::numeric_limits<std::size_t>::max());
  ASSERT_TRUE(payload);
  const auto decoded = axk::decode_object(*payload);
  ASSERT_TRUE(decoded);
