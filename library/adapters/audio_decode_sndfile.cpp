...
Result<SndfileHandle> open_sndfile(const std::filesystem::path &path,
                                   SF_INFO &info) {
#ifdef _WIN32
    SndfileHandle source{sf_wchar_open(path.c_str(), SFM_READ, &info),
                         &sf_close};
#else
    const auto path_text = path.string();
    SndfileHandle source{sf_open(path_text.c_str(), SFM_READ, &info),
                         &sf_close};
#endif
    if (!source) {
...
Result<SndfileHandle> open_verified(const std::filesystem::path &path,
    ...
    if (info.frames < 0 ||
        static_cast<std::uint64_t>(info.frames) != expected.frames ||
        info.channels < 0 ||
        static_cast<std::size_t>(info.channels) != expected.channels ||
        info.samplerate < 0 ||
        static_cast<std::uint32_t>(info.samplerate) != expected.sample_rate) {
        return std::unexpected{audio_error(
            "audio source metadata changed while it was being read")};
    }
    return source;
}
...
Result<SourceAudioInfo> inspect_sndfile(const std::filesystem::path &path,
    ...
    if (info.channels < 1 || info.channels > 2 ||
        ...
        return std::unexpected{audio_error(
            "audio source channel count does not match the import")};
    }
    if (info.frames <= 0 || info.samplerate <= 0) {
        return std::unexpected{audio_error(
            "audio source must contain frames at a valid sample rate")};
    }
    ...
    return SourceAudioInfo{
        ...
        .reduces_precision = subtype != SF_FORMAT_PCM_U8 &&
...
decode_sndfile_pcm16(const std::filesystem::path &path,
    ...
    const auto read =
        sf_readf_short(source->get(), samples.data(), info.frames);
    if (read != info.frames) {
        return std::unexpected{
            audio_error("audio source ended before its declared frame count")};
    }
    return samples;
}
...
decode_sndfile_float64(const std::filesystem::path &path,
    ...
    const auto read =
        sf_readf_double(source->get(), samples.data(), info.frames);
    if (read != info.frames) {
        return std::unexpected{
            audio_error("audio source ended before its declared frame count")};
    }
