...
Result<std::vector<double>> resample_vhq(std::span<const double> samples, std::size_t channels,
    ...
    const auto estimated_frames =
        static_cast<std::size_t>(std::ceil(static_cast<double>(input_frames) * output_rate / source_rate)) + 16U;
    ...
    const auto error = soxr_oneshot(static_cast<double>(source_rate), static_cast<double>(output_rate),
                                    static_cast<unsigned>(channels), samples.data(), input_frames, &input_done,
                                    converted.data(), estimated_frames, &output_done, &io, &quality, nullptr);
