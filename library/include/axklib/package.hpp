...
struct PackageImportPolicy {
  PackageWaveformReuseScope sfs_waveform_reuse_scope{PackageWaveformReuseScope::volume};
...
AXK_API Result<PackagePublication>
publish_portable_package(const PackageBuild &build, const std::filesystem::path &output_path,
                         bool overwrite = false, const CancellationToken &cancellation = {});

AXK_API Result<PackagePublication>
export_portable_package(const MediaContainer &source, std::span<const PackageRootSelector> roots,
