...
struct ContentNode {
  ...
  ContentNode(std::string id, std::string type, std::string display, std::string key = {},
      ...
      : node_id(std::move(id)), node_type(std::move(type)), display_name(std::move(display)),
        object_key(std::move(key)), object_type(std::move(stored_object_type)),
        quality(stored_quality), basis(std::move(stored_basis)), notes(std::move(stored_notes)),
        details(std::move(stored_details)), children(std::move(stored_children)) {}
};
...
AXK_API ContentTree build_content_tree(std::string source_path, const ObjectCatalog &catalog,
                                       const RelationshipGraph &graph,
                                       bool include_default_programs = false);
AXK_API WaveformOrphanReport analyze_waveform_orphans(const Container &container,
