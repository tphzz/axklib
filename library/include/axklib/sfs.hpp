...
public:
  ...
  read_record_data(PartitionIndex partition, SfsId record, std::size_t maximum_bytes,
...
private:
  ...
  friend AXK_API Result<Container> open_image(std::shared_ptr<const RandomAccessReader>,
                                              std::filesystem::path, const OpenOptions &);
};

AXK_API Result<SfsFreeSpace> calculate_sfs_free_space(std::uint32_t cluster_count,
