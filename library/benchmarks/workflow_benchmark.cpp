...
int main(int argc, char **argv) try {
  ...
  measurements.push_back(
      measure("cold_open", [&] { container = require(axk::open_image(source), "open"); }));
  axk::ObjectCatalog catalog;
  measurements.push_back(measure(
      "inventory", [&] { catalog = require(axk::build_object_catalog(container), "inventory"); }));
  ...
  measurements.push_back(measure("alteration_apply", [&] {
    static_cast<void>(
        require(axk::alter_hds(source, alteration, scratch / "altered.hds"), "alteration apply"));
  }));
