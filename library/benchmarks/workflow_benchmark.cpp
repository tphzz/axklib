...
int main(int argc, char **argv) try {
    ...
    measurements.push_back(
        measure("inventory", [&] { catalog = require(axk::build_object_catalog(container), "inventory"); }));
    axk::RelationshipGraph graph;
    measurements.push_back(measure("relationships", [&] { graph = axk::build_relationship_graph(catalog); }));
    ...
    for (const auto &measurement : measurements) {
        ...
        const auto throughput =
            measurement.wall_ms <= 0.0 ? 0.0 : static_cast<double>(bytes) / (measurement.wall_ms / 1000.0);
