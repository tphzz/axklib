...
from axklib.alteration import alter_hds, load_alteration_manifest, parse_alteration_manifest
...
def test_language_neutral_manifest_covers_every_maintained_operation() -> None:
    fixture = (
        Path(__file__).resolve().parents[3]
        / "tests/fixtures/manifests/alteration/all-operations.json"
    )
    manifest = load_alteration_manifest(fixture)
    assert [operation.type for operation in manifest.operations] == [
        "delete_volume",
        "insert_volume",
        "delete_sbnk",
        "insert_sbnk",
    ...
    assert manifest.operations[11].program_number == 128
