...
Maintained 1.44 MB Yamaha floppies commonly use the values in the
`Sampler-media example` column. The generated image is the same FAT12 capacity
and allocation shape, but its informational BPB media/CHS fields come from the
pinned FatFs formatter and are listed separately. The reader validates actual
filesystem geometry; it does not require one CHS decoration.

| Field | Boot offset | Size/type | Sampler-media example | Generated value |
| --- | ---: | --- | ---: | ---: |
...
| Media descriptor | `0x15` | u8 | `0xf0` | `0xf8` |
| Sectors per FAT | `0x16` | u16le | `9` | `9` |
| Sectors per track | `0x18` | u16le | `18` | `63` |
| Heads | `0x1a` | u16le | `2` | `255` |
...
| --- | ---: | --- |
| `0x00` | 3 | Boot jump `eb fe 90`. |
| `0x03` | 8 | OEM name `MSDOS5.0`. |
| `0x24` | 1 | BIOS drive number `0x80`. |
| `0x26` | 1 | Extended boot signature `0x29`. |
| `0x27` | 4 | Volume serial `0x5c210b40`, u32le. |
| `0x2b` | 11 | Boot-sector label `NO NAME` padded with spaces. This is not a root-directory volume-label entry. |
| `0x36` | 8 | Filesystem text `FAT` padded with spaces; FAT type still comes from cluster count. |
