...
std::string normalize_newlines(std::string value) {
    ...
    for (std::size_t index = 0; index < value.size(); ++index) {
        if (value[index] == '\r' && index + 1U < value.size() &&
            value[index + 1U] == '\n')
            continue;
...
TEST(ContentId, ReusesEqualContentAndRejectsInjectedShortPrefixCollision) {
    const auto fake = [](std::span<const std::byte> bytes) {
        const auto tail = bytes.front() == std::byte{1} ? std::string(28U, '0')
                                                        : std::string(28U, '1');
    ...
    EXPECT_NE(collision.error().message.find("distinct WAV contents"),
              std::string::npos);
}
...
TEST(Cli11Adapter, ReturnsParserExitCodesWithoutProcessExitMacros) {
    ...
    std::array invalid{const_cast<char *>("axklib"),
                       const_cast<char *>("--not-an-option")};
    EXPECT_NE(axk::cli::run(static_cast<int>(invalid.size()), invalid.data()),
              0);
}
...
TEST(Cli11Adapter, RejectsInvalidUtf8AndRepeatedScalarOptions) {
    ...
    EXPECT_EQ(axk::cli::run(static_cast<int>(invalid_utf8.size()),
                            invalid_utf8.data()),
              2);

    std::array repeated{
        const_cast<char *>("axklib"),     const_cast<char *>("info"),
        const_cast<char *>("--format"),   const_cast<char *>("json"),
        const_cast<char *>("--format"),   const_cast<char *>("tree"),
        const_cast<char *>("missing.hds")};
    EXPECT_NE(axk::cli::run(static_cast<int>(repeated.size()), repeated.data()),
              0);
}
...
TEST(Cli11Adapter, ExposesOnlyMaintainedCommandInventory) {
    ...
    for (const auto command : {"info", "inventory", "validate", "relationships",
                               "coverage", "create", "alter", "orphans",
                               "objects", "corpus", "extract", "package"})
        EXPECT_NE(output.find(command), std::string::npos) << command;
    for (const auto command : {"create-hds", "object-json", "tree",
                               "extract-wav", "export", "preview"}) {
    ...
    const auto fixture = std::filesystem::path{AXK_SOURCE_ROOT} /
                         "library/tests/fixtures/cli/help.txt";
...
TEST(Cli11Adapter, ExposesCompletePortablePackageCommandFamily) {
    ...
    for (const auto command :
         {"export", "inspect", "verify", "plan-import", "import"})
        EXPECT_NE(output.find(command), std::string::npos) << command;
}
...
TEST(Cli11Adapter, WritesStarterBuildManifestsWithoutSilentReplacement) {
    ...
    testing::internal::CaptureStdout();
    ASSERT_EQ(
        run_cli({"axklib", "create", "manifest", "hds", "-o", output.string()}),
        0);
    EXPECT_NE(testing::internal::GetCapturedStdout().find("kind=hds"),
              std::string::npos);
    ...
    EXPECT_TRUE(parsed.at("partitions")
                    .at(0)
                    .at("volumes")
                    .at(0)
                    .at("waveforms")
    ...
    testing::internal::CaptureStderr();
    EXPECT_EQ(
        run_cli({"axklib", "create", "manifest", "iso", "-o", output.string()}),
        2);
    EXPECT_NE(
        testing::internal::GetCapturedStderr().find("refusing to replace"),
        std::string::npos);

    EXPECT_EQ(run_cli({"axklib", "create", "manifest", "iso", "-o",
                       output.string(), "--overwrite"}),
              0);
...
TEST(Cli11Adapter, WritesStarterAlterationManifestWithoutSilentReplacement) {
    const auto root = std::filesystem::temp_directory_path() /
                      "axklib-cli-alter-manifest-template";
    const auto output = root / "transaction.json";
    std::error_code error;
    std::filesystem::remove_all(root, error);

    ASSERT_EQ(run_cli({"axklib", "alter", "manifest", "-o", output.string()}),
              0);
    ASSERT_TRUE(std::filesystem::is_regular_file(output));
    const auto parsed = axk::load_alteration_manifest(output);
    ASSERT_TRUE(parsed) << parsed.error().message;

    EXPECT_EQ(run_cli({"axklib", "alter", "manifest", "-o", output.string()}),
              2);
    EXPECT_EQ(run_cli({"axklib", "alter", "manifest", "-o", output.string(),
                       "--overwrite"}),
              0);
...
TEST(Cli11Adapter, PortablePackageRoundTripPlansAndImportsAtomically) {
    ...
    testing::internal::CaptureStdout();
    ASSERT_EQ(run_cli({"axklib", "package", "export", fixture.string(),
                       "--root", "sbnk=sine wave", "--partition", "0",
                       "--group", "New Partition", "--volume", "New Volume",
                       "-o", package_stem.string(), "--format", "json"}),
              0);
    const auto export_output = testing::internal::GetCapturedStdout();
    EXPECT_NE(export_output.find("\"package_kind\":\"sbnk\""),
              std::string::npos);
    EXPECT_NE(export_output.find("\"required_extension\":\".axksbnk\""),
              std::string::npos);
    ...
    const auto sdk_export = axk::portable_package::export_from(
        fixture.string(), {sdk_selector}, (root / "sdk-sine-bank").string(), {},
        sdk_context);
    ASSERT_TRUE(sdk_export) << sdk_export.error().message;
    EXPECT_EQ(sdk_export->package_id,
              cli_export_json.at("package_id").get<std::string>());
    ...
    for (const auto command : {"inspect", "verify"}) {
        testing::internal::CaptureStdout();
        EXPECT_EQ(run_cli({"axklib", "package", command, package.string(),
                           "--format", "json"}),
                  0);
        const auto output = testing::internal::GetCapturedStdout();
        EXPECT_NE(output.find("\"valid\":true"), std::string::npos) << command;
        EXPECT_NE(output.find("\"package_kind\":\"sbnk\""), std::string::npos)
            << command;
        const auto expected_verification =
            std::string{"\"payloads_verified\":"} +
            (std::string_view{command} == "verify" ? "true" : "false");
        EXPECT_NE(output.find(expected_verification), std::string::npos)
            << command;
    }

    std::ofstream{manifest}
        << R"({"schema_version":"1.0","size_bytes":1048576,"partitions":[{"name":"Target","volumes":[{"name":"Imported","waveforms":[],"sample_banks":[]}]}]})";
    ASSERT_EQ(run_cli({"axklib", "create", "hds", manifest.string(), "-o",
                       target.string()}),
              0);
    ASSERT_TRUE(std::filesystem::is_regular_file(target));
    const std::string destination =
        R"({"package":0,"root":0,"partition":0,"volume":"Imported"})";

    testing::internal::CaptureStdout();
    ASSERT_EQ(run_cli({"axklib", "package", "plan-import", target.string(),
                       package.string(), "--destination", destination,
                       "--format", "json"}),
              0);
    ...
    auto sdk_plan = axk::package_import_plan::create(
        target.string(), {package.string()}, sdk_request, sdk_context);
    ...
    EXPECT_EQ(sdk_plan_summary->plan_id,
              cli_plan_json.at("plan_id").get<std::string>());
    EXPECT_EQ(sdk_plan_summary->target_snapshot_id,
              cli_plan_json.at("target_snapshot_id").get<std::string>());
    EXPECT_EQ(sdk_plan_summary->object_count,
              cli_plan_json.at("objects").size());
#endif

    testing::internal::CaptureStdout();
    ASSERT_EQ(run_cli({"axklib", "package", "import", target.string(),
                       package.string(), "--destination", destination, "-o",
                       imported.string(), "--format", "json"}),
              0);
    ...
    testing::internal::CaptureStderr();
    EXPECT_NE(run_cli({"axklib", "package", "import", target.string(),
                       package.string(), "--destination", destination, "-o",
                       imported.string()}),
              0);
...
TEST(Cli11Adapter,
     PortablePackageRejectsBadSelectorsMappingsAndConflictsCleanly) {
    const auto fixture = std::filesystem::path{AXK_SOURCE_ROOT} /
                         "tests/fixtures/images" /
                         "sampler-authored/HD00_512_single_sbnk_authored.hds";
    const auto root =
        std::filesystem::temp_directory_path() / "axklib-cli-package-errors";
    ...
    testing::internal::CaptureStderr();
    EXPECT_EQ(run_cli({"axklib", "package", "export", fixture.string(),
                       "--root", "nonsense=x", "-o", (root / "bad").string()}),
              2);
    EXPECT_NE(testing::internal::GetCapturedStderr().find("root kind"),
              std::string::npos);
    ...
    testing::internal::CaptureStderr();
    EXPECT_EQ(run_cli({"axklib", "package", "plan-import", target.string(),
                       package.string(), "--destination", "not-json"}),
              2);
    EXPECT_NE(testing::internal::GetCapturedStderr().find("destination JSON"),
              std::string::npos);

    testing::internal::CaptureStdout();
    EXPECT_EQ(
        run_cli({"axklib", "package", "import", target.string(),
                 package.string(), "--destination",
                 R"({"package":0,"root":0,"partition":0,"volume":"Missing"})",
...
TEST(CliReports, EmitsBothAllocationMismatchDirections) {
    ...
    EXPECT_EQ(std::get<std::string>(rows[0][3].second.value),
              "stored-used-without-index-extent");
    EXPECT_EQ(std::get<std::uint64_t>(rows[0][6].second.value), 3U);
    EXPECT_EQ(std::get<std::string>(rows[1][3].second.value),
              "index-extent-references-free-cluster");
...
TEST(Cli11Adapter, ExtractSfzWritesAudioInstrumentsAndVolumeGraph) {
    ...
    const auto output = std::filesystem::temp_directory_path() /
                        "axklib-cli-extract-integration-test";
    std::error_code error;
    std::filesystem::remove_all(output, error);
    std::vector<std::string> arguments{"axklib",       "extract",        "sfz",
                                       "file",         fixture.string(), "-o",
                                       output.string()};
    std::vector<char *> argv;
    std::ranges::transform(arguments, std::back_inserter(argv),
                           [](auto &value) { return value.data(); });
...
TEST(CliArchitecture, KeepsCli11AndJsonAtTheirOwnedBoundaries) {
    ...
    for (const auto &entry :
         std::filesystem::directory_iterator{root / "commands"}) {
        if (entry.path().extension() != ".cpp")
            continue;
        const auto source = read(entry.path());
        EXPECT_EQ(source.find("CLI/CLI.hpp"), std::string::npos)
            << entry.path();
        EXPECT_EQ(source.find("\"schema_version\""), std::string::npos)
            << entry.path();
    }
    for (const auto &entry :
         std::filesystem::directory_iterator{root / "schema"}) {
        if (entry.path().extension() == ".hpp") {
            EXPECT_EQ(read(entry.path()).find("nlohmann"), std::string::npos)
                << entry.path();
        }
    }
}
