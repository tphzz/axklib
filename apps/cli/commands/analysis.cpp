...
int run_relationships_request(const axk::cli::RelationshipsRequest &request) {
    ...
    auto relation_schema =
        write_cli_report(request.output_directory, "relationships", rows,
    ...
    const std::array<std::pair<std::string_view, std::string_view>, 4>
        detail_reports{{
            ...
            {"current_sbnk_program_bitmap_crosscheck",
            ...
            for (const auto &source : loaded.loaded) {
                for (const auto &row : source.graph.relationships) {
                    if ((prefix.ends_with('_') &&
                         row.type.starts_with(prefix)) ||
                        row.type == prefix)
                        selected.push_back(
                            relationship_report_row(source, row));
                }
            }
        }
        auto schema = write_cli_report(
            request.output_directory, std::string{name}, selected,
            "axklib relationships", request.overwrite);
        if (!schema)
    ...
    auto load_schema =
        write_cli_report(request.output_directory, "load_errors", loaded.errors,
                         "axklib relationships", request.overwrite);
    if (!load_schema)
    ...
    auto summary_schema = axk::make_report_schema(
        "relationship_summary", std::span{&summary, 1U}, summary_options);
                         ...
                         return std::ranges::any_of(row, [](const auto &field) {
                             return field.first == "quality" &&
                                    std::get_if<std::string>(
                                        &field.second.value) != nullptr &&
                                    std::get<std::string>(field.second.value) ==
                                        "Tentative";
                         });
                     })
...
int run_coverage_request(const axk::cli::CoverageRequest &request) {
    ...
    if (auto written = axk::write_report_csv(
            request.output_directory / "coverage_summary.csv", summary_rows, {},
            request.overwrite);
        !written)
        return report_failure(written.error());
    if (auto written = axk::write_report_object(request.output_directory /
    ...
    summary_options.library_version = std::string{axk::version()};
    auto summary_schema = axk::make_report_schema(
        "coverage_summary", summary_rows, std::move(summary_options));
    if (auto written =
            axk::write_report_schema(request.output_directory / "_schemas" /
    ...
    if (auto index = axk::write_report_schema_index(
        ...
        !index)
        return report_failure(index.error());
    std::cout << "relationships=" << rows.size()
              << " load_errors=" << loaded.errors.size() << '\n';
    return loaded.errors.empty() ? 0 : 3;
}

axk::ContentTree cli_content_tree(const CliLoaded &loaded,
...
std::string sfs_selector_component(const axk::ContentNode &node) {
    if (node.node_type == "partition") {
        ...
        const auto raw_index = separator == std::string::npos
                                   ? std::string{}
                                   : node.node_id.substr(separator + 1U);
        ...
        for (const auto value : partition_name) {
            ...
            if (std::isspace(byte) != 0) {
                ...
                const bool retained = std::isalnum(byte) != 0 || value == '.' ||
                                      value == '_' || value == '-';
                safe.push_back(retained ? value : '_');
                prior_space = false;
            }
        }
        while (!safe.empty() && (safe.front() == '.' || safe.front() == '_' ||
                                 safe.front() == '-'))
            safe.erase(safe.begin());
        while (!safe.empty() &&
               (safe.back() == '.' || safe.back() == '_' || safe.back() == '-'))
            safe.pop_back();
        return std::format(
            "partition_{:0>2}_{}", raw_index,
...
std::string first_media_object_directory(const CliLoaded &loaded,
                                         const axk::ContentNode &node) {
    if (!node.object_key.empty()) {
        const auto object = std::ranges::find(loaded.objects, node.object_key,
                                              &axk::MediaObject::key);
        if (object != loaded.objects.end()) {
            auto directory =
                std::filesystem::path{object->logical_path}.parent_path();
...
std::string selector_component(const CliLoaded &loaded,
                               const axk::ContentNode &node) {
...
axk::cli::schema::info_v1::NodeOutput compatible_tree_node_output(
    const CliLoaded &loaded, const axk::ContentNode &node,
    std::string parent_path = {}, std::string parent_id = {},
    ...
    const auto object_key = node.object_key.empty()
                                ? std::string{}
    ...
    if (!node.object_key.empty())
        ...
        node_id = "partition:None";
               ...
               node.node_type == "volume") {
        node_id = std::format("volume:None:{}",
                              first_media_object_directory(loaded, node));
    } else if (loaded.media.kind() == axk::MediaKind::iso9660 &&
               node.node_type == "category" && parent_type == "volume") {
    ...
    } else if (loaded.media.kind() == axk::MediaKind::sfs &&
               node.node_type == "volume") {
        const auto separator = parent_id.find(':');
        const auto partition_index = separator == std::string::npos
                                         ? std::string{}
                                         : parent_id.substr(separator + 1U);
        node_id =
            std::format("volume:{}:{}", partition_index, node.display_name);
    } else if (loaded.media.kind() == axk::MediaKind::sfs &&
               node.node_type == "category" && parent_type == "volume") {
        const auto volume_prefix = parent_id.find(':');
        const auto volume_tail = volume_prefix == std::string::npos
    ...
    for (const auto &child : node.children)
        children.push_back(compatible_tree_node_output(
            loaded, child, selector, node_id, node.node_type));
    const bool counted = node.node_type == "partition" ||
                         node.node_type == "volume" ||
    ...
    return {
        ...
        .count = counted ? std::optional<std::uint64_t>{node.children.size()}
                         : std::nullopt,
...
std::string tree_type_label(const axk::ContentNode &node) {
    ...
    if (node.node_type == "unresolved" ||
        node.node_type == "relationship_target" ||
...
void render_tree_text(const axk::ContentNode &node, std::size_t depth,
...
void render_tree_paths(const CliLoaded &loaded, const axk::ContentNode &node,
    ...
    if (node.node_type == "volume")
        ...
        scope = "sbnk";
    if (!scope.empty()) {
        std::cout << axk::text::path_to_utf8(loaded.path) << '\t' << scope
                  << '\t' << selector << '\t' << node.display_name << '\t'
                  << node.object_type << '\t'
                  << (node.object_key.empty()
                          ? std::string{}
                          : public_object_key(loaded, node.object_key))
                  << '\n';
    }
...
int run_info_request(const axk::cli::InfoRequest &request) {
    ...
    if (request.format == "json") {
        ...
        for (const auto &source : loaded.loaded) {
            const auto tree =
                cli_content_tree(source, request.show_default_programs);
            ...
            for (const auto &root : tree.roots)
                projected.roots.push_back(
                    compatible_tree_node_output(source, root));
    ...
    for (const auto &error : loaded.errors) {
        ...
        std::cout << path << "\tERROR\tAXKLIB_CONTAINER_OPEN_FAILED\t"
                  << message << '\n';
    }
    ...
    for (const auto &source : loaded.loaded) {
        ...
        for (const auto &root : tree.roots) {
            if (request.format == "paths")
                ...
                render_tree_text(root, 1U, {},
                                 root.node_id == tree.roots.back().node_id,
                                 request);
        }
    }
    return loaded.errors.empty() ? 0 : 1;
