...
std::string sfs_selector_component(const axk::ContentNode &node) {
  if (node.node_type == "partition") {
    ...
    const auto raw_index =
        separator == std::string::npos ? std::string{} : node.node_id.substr(separator + 1U);
...
axk::cli::schema::info_v1::NodeOutput compatible_tree_node_output(const CliLoaded &loaded,
  ...
  if (!node.object_key.empty())
    ...
    const auto partition_index =
        separator == std::string::npos ? std::string{} : parent_id.substr(separator + 1U);
  ...
  for (const auto &child : node.children)
    children.push_back(
        compatible_tree_node_output(loaded, child, selector, node_id, node.node_type));
  const bool counted =
