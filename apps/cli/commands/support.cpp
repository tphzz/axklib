...
load_semantic_snapshot(const std::filesystem::path &path) {
    ...
    return SemanticSnapshot{std::move(*container), std::move(*catalog),
                            std::move(graph)};
}
...
expand_cli_paths(const std::vector<std::filesystem::path> &inputs) {
    static const std::set<std::string> extensions{".hda", ".hds", ".ima",
                                                  ".img", ".iso"};
    std::vector<std::filesystem::path> result;
    for (const auto &path : inputs) {
        std::error_code error;
        if (std::filesystem::is_directory(path, error)) {
            for (std::filesystem::recursive_directory_iterator it{path, error},
                 end;
                 it != end && !error; it.increment(error)) {
                if (it->is_regular_file(error) &&
                    extensions.contains(
                        axk::text::path_to_utf8(it->path().extension())))
    ...
    std::ranges::sort(result, {}, [](const auto &path) {
        return axk::text::path_to_utf8(path);
    });
    ...
    for (const auto &path : expand_cli_paths(inputs)) {
        ...
        if (!catalog) {
            result.errors.push_back({{"path", axk::text::path_to_utf8(path)},
                                     {"error_code", static_cast<std::uint64_t>(
                                                        catalog.error().code)},
        ...
        auto graph = axk::build_relationship_graph(*catalog);
        result.loaded.push_back({path, std::move(*media), std::move(*objects),
                                 std::move(*catalog), std::move(graph)});
    }
    return result;
}

axk::Result<void> prepare_report_directory(const std::filesystem::path &path,
                                           bool overwrite) {
    std::error_code error;
...
write_cli_report(const std::filesystem::path &output, std::string name,
    ...
    if (auto written =
            axk::write_report_json(output / (name + ".json"), rows, overwrite);
        !written)
        return std::unexpected{written.error()};
    ...
    if (name == "inventory_objects")
        ...
        options.semantic_notes =
            "Decode issues use stable code/severity/quality fields.";
...
std::string public_object_key(const CliLoaded &loaded,
    ...
    const auto object =
        std::ranges::find(loaded.objects, native_key, &axk::MediaObject::key);
    if (object == loaded.objects.end())
        return std::string{native_key};
    if (loaded.media.kind() == axk::MediaKind::fat12_floppy)
        return std::format("{}:{}",
                           axk::text::path_to_utf8(loaded.path.filename()),
                           object->logical_path);
    if (loaded.media.kind() == axk::MediaKind::iso9660)
        return std::format("{}:iso9660:{}",
                           axk::text::path_to_utf8(loaded.path.filename()),
                           object->logical_path);
    return std::format("{}:standalone-object",
                       axk::text::path_to_utf8(loaded.path.filename()));
}
...
std::string public_scope_key(const CliLoaded &loaded,
    ...
    if (loaded.media.kind() == axk::MediaKind::sfs)
        return std::format("{}:partition:{}",
                           axk::text::path_to_utf8(loaded.path),
    ...
    if (loaded.media.kind() == axk::MediaKind::standalone_object)
        return std::format("{}:standalone-object",
                           axk::text::path_to_utf8(loaded.path));
    const auto object =
        std::ranges::find(loaded.objects, item.key, &axk::MediaObject::key);
    return object == loaded.objects.end()
               ? std::format("{}:iso", axk::text::path_to_utf8(loaded.path))
               : std::format("{}:{}", axk::text::path_to_utf8(loaded.path),
                             object->scope_key);
}

axk::ReportRow inventory_row(const CliLoaded &loaded,
                             const axk::ObjectSnapshot &item) {
    const auto media_object =
        std::ranges::find(loaded.objects, item.key, &axk::MediaObject::key);
    ...
    if (item.object.header.type == axk::ObjectType::smpl) {
        ...
        decoded_fields =
            "fine_tune;loop_length;loop_mode;loop_start;root_key;sample_rate";
    ...
    const auto field_count =
        decoded_fields.empty()
            ...
            : static_cast<unsigned int>(
                  std::ranges::count(decoded_fields, ';') + 1);
    std::uint64_t payload_offset{};
    if (sfs) {
        const auto &container =
            std::get<axk::Container>(loaded.media.storage());
    ...
    if (fat && media_object != loaded.objects.end()) {
        ...
        if (image != nullptr) {
            const auto found =
                std::ranges::find(image->files(), media_object->logical_path,
                                  &axk::FatFile::path);
    ...
    return {
        ...
        {"partition_index", sfs ? axk::ReportValue{static_cast<std::uint64_t>(
                                      item.partition.value)}
                                : axk::ReportValue{""}},
        {"sfs_id",
         sfs ? axk::ReportValue{static_cast<std::uint64_t>(item.sfs_id.value)}
             : axk::ReportValue{""}},
        ...
        {"iso_volume_label_source",
         iso && media_object != loaded.objects.end() &&
                 media_object->volume_label.status ==
                     axk::LabelStatus::confirmed
             ? axk::ReportValue{"yamaha-cdrom-menu-label"}
             : axk::ReportValue{""}},
        {"fat_directory_offset",
         fat_metadata != nullptr
             ? axk::ReportValue{fat_metadata->directory_offset}
             : axk::ReportValue{""}},
        {"fat_first_cluster", fat_metadata != nullptr
                                  ? axk::ReportValue{static_cast<std::uint64_t>(
        ...
        {"fat_stored_payload_offset",
         fat && media_object != loaded.objects.end()
             ? axk::ReportValue{media_object->data_offset +
                                item.object.header.header_size}
             : axk::ReportValue{""}}};
}

axk::ReportRow relationship_report_row(const CliLoaded &loaded,
    ...
    const auto source = std::ranges::find(
        loaded.catalog.objects, row.source_key, &axk::ObjectSnapshot::key);
    if (source != loaded.catalog.objects.end() &&
        (row.type == "SBNK_LEFT_MEMBER_TO_SMPL" ||
         row.type == "SBNK_RIGHT_MEMBER_TO_SMPL")) {
        if (const auto *bank =
                std::get_if<axk::CurrentSbnk>(&source->object.payload)) {
            const bool right = row.type == "SBNK_RIGHT_MEMBER_TO_SMPL";
            const auto *member =
                right && bank->right ? &*bank->right : &bank->left;
            raw_fields =
                std::format("SBNK+{} member {}; name={}; link_id=0x{:08x}",
                            right ? "right" : "left", right ? "right" : "left",
                            member->sample_name, member->smpl_link_id);
            if (row.basis == "sbnk-member-link+name")
                notes = "Current SBNK member name and member link ID match "
                        "exactly one same-scope SMPL "
                        "object.";
        }
    } else if (source != loaded.catalog.objects.end() &&
               row.type == "SBAC_SLOT_TO_SBNK") {
        if (const auto *group =
                std::get_if<axk::CurrentSbac>(&source->object.payload)) {
            std::size_t index{};
            if (row.target_key) {
                const auto target_object =
                    std::ranges::find(loaded.catalog.objects, *row.target_key,
                                      &axk::ObjectSnapshot::key);
                if (target_object != loaded.catalog.objects.end()) {
                    const auto found = std::ranges::find(
                        group->slots, target_object->object.header.name,
                        &axk::SbacSlot::name);
                    if (found != group->slots.end())
                        index = static_cast<std::size_t>(
                            std::distance(group->slots.begin(), found));
                }
            }
            const auto offset = index < group->slots.size()
                                    ? group->slots[index].offset
                                    : 0x14cU;
        ...
        raw_fields = std::format(
            "PROG assignment {} at 0x{:03x}", *row.assignment_index,
    ...
    if (row.assignment_state == axk::AssignmentState::visible_off)
        ...
        diagnostic = row.basis.starts_with("sbnk-member-link-id-only")
                         ? "sbnk-member-link"
                         : "ambiguous-target";
    else if (row.quality == axk::RelationshipQuality::unknown)
        diagnostic = "missing-target";
    return {
        {"key", std::format("{}|{}|{}|{}", source_key, row.type,
        ...
        {"scope_key", std::format("{}:{}", axk::text::path_to_utf8(loaded.path),
                                  row.scope_key)},
        ...
        {"active_assignment_state",
         row.assignment_index
             ? std::string{axk::assignment_state_name(row.assignment_state)}
             : std::string{}},
