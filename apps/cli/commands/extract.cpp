...
std::string safe_display_path_name(std::string_view value,
...
axk::Result<void>
retarget_export_plan(axk::ExportPlan &plan,
    ...
    for (auto &volume : plan.volumes) {
        volume.relative_root = preserve_volume_roots
                                   ? selection_root / volume.relative_root
                                   : selection_root;
        ...
        for (auto &waveform : volume.waveforms) {
            ...
            waveform_paths.emplace(waveform.object_key,
        ...
        for (auto &bank : volume.sample_banks) {
            ...
            if (bank.rendered_wav_path && bank.members.size() == 2U) {
                ...
                if (left != volume.waveforms.end() &&
                    ...
                    auto stereo =
                        axk::render_stereo(left->waveform, right->waveform);
                    if (!stereo)
                    ...
                    if (!bytes)
                        return std::unexpected{bytes.error()};
                    auto pooled = pooled_paths.allocate(
                        volume.relative_root, "rendered",
                        safe_display_path_name(bank.display_name, "sample"),
                        *bytes);
                    if (!pooled)
                        return std::unexpected{pooled.error()};
...
bool selector_scope_matches(const axk::ContentNode &node,
...
void find_selector_nodes(const CliLoaded &loaded, const axk::ContentNode &node,
    ...
    const auto component = loaded.media.kind() == axk::MediaKind::sfs
                               ...
                               : node.display_name;
    const auto selector = parent_path.empty()
                              ? component
...
std::filesystem::path selection_root(std::string_view scope,
...
axk::Result<void> write_volume_graph(const std::filesystem::path &path,
                                     std::string_view contents,
                                     bool overwrite) {
    if (!overwrite && std::filesystem::exists(path)) {
        return std::unexpected{axk::make_error(
            axk::ErrorCode::io_open_failed, axk::ErrorCategory::io,
        ...
        return std::unexpected{temporary.error()};
    {
        ...
        if (!output) {
            ...
            return std::unexpected{axk::make_error(
                axk::ErrorCode::io_read_failed, axk::ErrorCategory::io,
...
void filter_export_plan(axk::ExportPlan &plan, const CliLoaded &source,
    ...
    if (scope == "volume") {
        std::erase_if(plan.volumes, [&](const auto &volume) {
            return axk::text::path_to_utf8(volume.relative_root) !=
                   selector_path;
        });
        return;
    }
    ...
    while (changed) {
        ...
        for (const auto &row : source.graph.relationships) {
            if (!row.target_key)
                continue;
            if (programs.contains(row.source_key) &&
            ...
            if (groups.contains(row.source_key) &&
    ...
    for (auto &volume : plan.volumes) {
        std::erase_if(volume.sample_banks, [&](const auto &bank) {
            return !banks.contains(bank.object_key);
        });
        ...
        std::erase_if(volume.waveforms, [&](const auto &waveform) {
            return !waveforms.contains(waveform.object_key);
        });
        std::erase_if(volume.sample_bank_groups, [&](const auto &group) {
            return !groups.contains(group.object_key);
        });
        for (auto &group : volume.sample_bank_groups) {
            std::erase_if(group.member_bank_keys, [&](const auto &key) {
                return !banks.contains(key);
            });
            std::erase_if(group.relationship_bank_keys, [&](const auto &key) {
                return !banks.contains(key);
            });
        }
        std::erase_if(volume.programs, [&](const auto &program) {
            return !programs.contains(program.object_key);
        });
    }
    std::erase_if(plan.volumes, [](const auto &volume) {
...
int run_extract_request(const axk::cli::ExtractRequest &request) {
    ...
    std::error_code error;
    if (std::filesystem::exists(request.output_directory, error) &&
        !request.overwrite &&
    ...
    const auto selectors = request.scope == "file"
        ...
        for (const auto &source : loaded.loaded) {
            ...
            if (request.scope != "file") {
                for (const auto &root : tree.roots)
                    find_selector_nodes(source, root, request.scope, selector,
                                        {}, matches);
                if (matches.empty())
                    continue;
                found = true;
            }
            auto plan = axk::build_export_plan(source.media, source.catalog,
                                               source.graph);
            if (!plan)
                return report_failure(plan.error());
            if (request.scope != "file")
                filter_export_plan(*plan, source, request.scope, selector,
                                   matches.front()->object_key);
            auto root = selection_root(request.scope, selector);
            if (request.scope == "file") {
                root /= safe_display_path_name(
                    axk::text::path_to_utf8(source.path.stem()), "source");
            }
            if (auto retargeted = retarget_export_plan(
                    *plan, root, request.scope == "file", pooled_paths);
                !retargeted) {
                return report_failure(retargeted.error());
            }
            std::ranges::move(plan->decode_errors,
                              std::back_inserter(combined.decode_errors));
            for (const auto &volume : plan->volumes) {
                auto graph =
                    axk::cli::schema::export_v1::serialize_volume_graph(
                        volume, source.graph, source.path,
                        media_kind_text(source.media.kind()));
                if (!graph)
                    return report_failure(graph.error());
                const auto path = request.output_directory /
                                  volume.relative_root / "volume.axklib.json";
                const auto [existing, inserted] =
                    volume_graphs.emplace(path, *graph);
                if (!inserted && existing->second != *graph) {
                    std::cerr
                        << "error: distinct volume graphs share output path: "
                        << axk::text::path_to_utf8(path) << '\n';
                    return 1;
                }
            }
            std::ranges::move(plan->volumes,
                              std::back_inserter(combined.volumes));
        }
        if (!found) {
            std::cerr << "selector path not found for " << request.scope << ": "
                      << selector
                      << ". Run `axklib info --format paths` and copy the path "
                         "column.\n";
            return 4;
        }
    }
    if (!request.overwrite) {
        const auto existing =
            std::ranges::find_if(volume_graphs, [](const auto &entry) {
    ...
    auto audio = axk::write_export_audio(combined, request.output_directory,
                                         request.overwrite);
    if (!audio)
    ...
    if (request.sfz) {
        auto sfz = axk::write_sfz(combined, request.output_directory,
                                  request.overwrite);
        if (!sfz)
    ...
    for (const auto &[path, graph] : volume_graphs) {
        if (auto written = write_volume_graph(path, graph, request.overwrite);
            !written)
    ...
    const auto waveform_count = std::accumulate(
        ...
        [](std::size_t count, const auto &volume) {
            return count + volume.waveforms.size();
        });
    std::cout << "waveforms=" << waveform_count
              << " written_files=" << written_files
              << " selection_graphs=" << volume_graphs.size()
              << " sfz_files=" << sfz_count
...
axk::ReportRow coverage_summary(const CliLoadResult &loaded,
    ...
    for (const auto &source : loaded.loaded) {
        for (const auto &row : source.graph.relationships) {
            ++qualities[std::string{
                axk::relationship_quality_name(row.quality)}];
            ++types[row.type];
    ...
    const auto ignored =
        static_cast<std::uint64_t>(program_ignored_detail_rows(loaded).size());
    ...
    return {
        {"relationship_count",
         static_cast<std::uint64_t>(relationships.size())},
