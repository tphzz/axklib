        ...
        for (auto &bank : volume.sample_banks) {
            ...
            if (bank.rendered_wav_path && bank.members.size() == 2U) {
                ...
                if (left != volume.waveforms.end() && right != volume.waveforms.end()) {
                    ...
                    if (!bytes)
                        return std::unexpected{bytes.error()};
                    auto pooled = pooled_paths.allocate(volume.relative_root, "rendered",
...
void find_selector_nodes(const CliLoaded &loaded, const axk::ContentNode &node, std::string_view scope,
                         std::string_view wanted, std::string parent_path,
...
axk::Result<void> write_volume_graph(const std::filesystem::path &path, std::string_view contents, bool overwrite) {
    if (!overwrite && std::filesystem::exists(path)) {
        return std::unexpected{
            axk::make_error(axk::ErrorCode::io_open_failed, axk::ErrorCategory::io,
                            "refusing to replace an existing volume graph: " + axk::text::path_to_utf8(path))};
    }
...
int run_extract_request(const axk::cli::ExtractRequest &request) {
    ...
    if (std::filesystem::exists(request.output_directory, error) && !request.overwrite &&
        std::filesystem::directory_iterator{request.output_directory, error} != std::filesystem::directory_iterator{}) {
    ...
    for (const auto &selector : selectors) {
        ...
        for (const auto &source : loaded.loaded) {
            ...
            if (auto retargeted = retarget_export_plan(*plan, root, request.scope == "file", pooled_paths);
                !retargeted) {
            ...
            for (const auto &volume : plan->volumes) {
                auto graph = axk::cli::schema::export_v1::serialize_volume_graph(volume, source.graph, source.path,
                ...
                if (!inserted && existing->second != *graph) {
                    std::cerr << "error: distinct volume graphs share output path: " << axk::text::path_to_utf8(path)
                              << '\n';
                    return 1;
                }
            }
    ...
    if (!request.overwrite) {
        const auto existing =
            std::ranges::find_if(volume_graphs, [](const auto &entry) { return std::filesystem::exists(entry.first); });
        if (existing != volume_graphs.end()) {
            std::cerr << "error: volume graph already exists: " << axk::text::path_to_utf8(existing->first) << '\n';
            return 1;
        }
    }
    ...
    std::cout << "waveforms=" << waveform_count << " written_files=" << written_files
              ...
              << " decode_errors=" << combined.decode_errors.size() << " load_errors=" << loaded.errors.size() << '\n';
    return loaded.errors.empty() ? 0 : 1;
}

axk::ReportRow coverage_summary(const CliLoadResult &loaded, std::span<const axk::ReportRow> relationships) {
