...
int run_objects_request(const axk::cli::ObjectsRequest &request) {
    if (!request.output_directory)
        return 2;
    if (const auto ready = prepare_report_directory(*request.output_directory,
                                                    request.overwrite);
        !ready)
    ...
    for (const auto &source : loaded.loaded) {
        for (const auto &item : source.catalog.objects) {
            if (request.object_type &&
                object_type_text(item.object.header.type) !=
                    *request.object_type)
                continue;
            rows.push_back(inventory_row(source, item));
        }
    }
    auto schema = write_cli_report(*request.output_directory, "objects", rows,
                                   "axklib objects", request.overwrite);
    if (!schema)
        return report_failure(schema.error());
    const std::array schemas{*schema};
    if (auto index = axk::write_report_schema_index(
            *request.output_directory / "_schemas" / "schema_index.json",
            schemas, request.overwrite);
        !index)
        return report_failure(index.error());
    std::cout << "objects=" << rows.size()
...
int run_inventory_request(const axk::cli::InventoryRequest &request) {
        ...
        for (const auto &issue : source.catalog.issues) {
            issues.push_back(
                {{"source_path", axk::text::path_to_utf8(source.path)},
                 ...
                 {"object_key",
                  issue.sfs_id ? std::format("p{}:sfs{}", issue.partition.value,
                                             issue.sfs_id->value)
                               : std::string{}},
                 {"object_type", ""},
                 {"object_name", ""},
    ...
    auto object_schema =
        write_cli_report(request.output_directory, "inventory_objects", rows,
                         "axklib inventory", request.overwrite);
    if (!object_schema)
        return report_failure(object_schema.error());
    auto issue_schema =
        write_cli_report(request.output_directory, "decode_issues", issues,
                         "axklib inventory", request.overwrite);
    if (!issue_schema)
    ...
    for (const auto &row : loaded.errors)
        load_errors.emplace_back(
            axk::ReportValue::Object{row.begin(), row.end()});
    axk::ReportRow summary{
        {"input_count", static_cast<std::uint64_t>(loaded.loaded.size() +
                                                   loaded.errors.size())},
    ...
    if (auto written = axk::write_report_object(request.output_directory /
    ...
    auto summary_schema = axk::make_report_schema(
        "inventory_summary", std::span{&summary, 1U}, summary_options);
    if (auto written =
            axk::write_report_schema(request.output_directory / "_schemas" /
...
const axk::ObjectSnapshot *catalog_object(const CliLoaded &loaded,
                                          std::string_view key) {
    const auto found = std::ranges::find(loaded.catalog.objects, key,
                                         &axk::ObjectSnapshot::key);
    return found == loaded.catalog.objects.end() ? nullptr : &*found;
}

const axk::MediaObject *media_object(const CliLoaded &loaded,
                                     std::string_view key) {
    const auto found =
        std::ranges::find(loaded.objects, key, &axk::MediaObject::key);
    return found == loaded.objects.end() ? nullptr : &*found;
}

const axk::FatFile *fat_file_metadata(const CliLoaded &loaded,
                                      const axk::MediaObject *object) {
    if (object == nullptr)
        return nullptr;
    const auto *fat = std::get_if<axk::FatImage>(&loaded.media.storage());
    if (fat == nullptr)
        return nullptr;
    const auto found = std::ranges::find(fat->files(), object->logical_path,
                                         &axk::FatFile::path);
    return found == fat->files().end() ? nullptr : &*found;
}

std::uint64_t sfs_payload_offset(const CliLoaded &loaded,
                                 const axk::ObjectSnapshot &item) {
    if (loaded.media.kind() != axk::MediaKind::sfs)
        return 0U;
    const auto &container = std::get<axk::Container>(loaded.media.storage());
    const auto partition =
        std::ranges::find(container.partitions(), item.partition.value,
                          [](const auto &row) { return row.index.value; });
    if (partition == container.partitions().end())
        return 0U;
    const auto record =
        std::ranges::find(partition->records, item.sfs_id.value,
    ...
    for (const auto &source : loaded.loaded) {
        for (const auto &relation : source.graph.relationships) {
            ...
            if (sbac_item == nullptr)
                continue;
            const auto *sbac =
                std::get_if<axk::CurrentSbac>(&sbac_item->object.payload);
            if (sbac == nullptr)
                continue;
            const auto matched =
                relation.target_key
                    ? catalog_object(source, *relation.target_key)
                    : nullptr;
            ...
            if (slot == nullptr) {
                const auto named =
                    std::ranges::find_if(sbac->slots, [](const auto &item) {
                        return !item.name.empty();
                    });
                if (named == sbac->slots.end())
                    continue;
                slot_index = static_cast<std::size_t>(
                    std::distance(sbac->slots.begin(), named));
                slot = &*named;
            }
            ...
            const auto *matched_media =
                matched == nullptr ? nullptr
                                   : media_object(source, matched->key);
            ...
            const bool fat =
                source.media.kind() == axk::MediaKind::fat12_floppy;
            const auto match_notes =
                relation.quality == axk::RelationshipQuality::known
                    ? "Input consistency: counted SBAC slot name uniquely "
                      "matches a "
                      "same-scope SBNK header name. The companion 32-bit slot "
                      "word "
            ...
            rows.push_back({
                ...
                {"sbac_partition_index",
                 optional_unsigned(sfs, sbac_item->partition.value)},
                {"sbac_sfs_id",
                 optional_unsigned(sfs, sbac_item->sfs_id.value)},
                {"sbac_fat_file",
                 fat && sbac_media != nullptr ? sbac_media->logical_path : ""},
                {"sbac_payload_offset",
                ...
                {"sbac_payload_size", sbac_media != nullptr
                                          ? static_cast<std::uint64_t>(
                                                sbac_media->raw_payload.size())
                                          : std::uint64_t{0}},
                {"sbac_slot_count_0x144",
                 static_cast<std::uint64_t>(sbac->active_slot_count)},
                {"slot_index", static_cast<std::uint64_t>(slot_index)},
                {"slot_offset", static_cast<std::uint64_t>(slot->offset)},
                {"slot_sbnk_name", slot->name},
                {"slot_raw_handle_0x10",
                 static_cast<std::uint64_t>(slot->raw_handle)},
                {"match_method", relation.basis},
                {"match_quality",
                 std::string{axk::relationship_quality_name(relation.quality)}},
                {"match_notes", match_notes},
                {"candidate_count",
                 static_cast<std::uint64_t>(relation.candidate_keys.size())},
                {"candidate_object_keys", joined_strings(candidate_keys)},
                {"candidate_fat_files",
                 fat ? joined_strings(candidate_files) : ""},
                {"candidate_names", joined_strings(candidate_names)},
                {"matched_sbnk_object_key",
                 matched == nullptr ? ""
                                    : public_object_key(source, matched->key)},
                ...
                {"matched_sbnk_fat_file", fat && matched_media != nullptr
                                              ? matched_media->logical_path
                                              : ""},
                {"matched_sbnk_payload_offset",
                 optional_unsigned(matched != nullptr &&
                                       (sfs || matched_media != nullptr),
                                   matched == nullptr ? 0U
                                   : sfs ? sfs_payload_offset(source, *matched)
                                         : matched_media->data_offset)},
                {"matched_sbnk_name",
                 matched == nullptr ? "" : matched->object.header.name},
                {"notes", ""},
                {"sbac_iso_extent_sector",
                 optional_unsigned(iso && sbac_media != nullptr,
                                   sbac_media == nullptr
                                       ? 0U
                                       : sbac_media->data_offset / 2048U)},
                {"sbac_iso_data_offset",
                 optional_unsigned(
                     iso && sbac_media != nullptr,
                     sbac_media == nullptr ? 0U : sbac_media->data_offset)},
                {"sbac_iso_file_size",
                 optional_unsigned(iso && sbac_media != nullptr,
                                   sbac_media == nullptr ? 0U
                                                         : sbac_media->size)},
                {"sbac_iso_recovery_quality",
                 iso ? "clean-iso9660-object" : ""},
                ...
                {"matched_sbnk_iso_extent_sector",
                 optional_unsigned(iso && matched_media != nullptr,
                                   matched_media == nullptr
                                       ? 0U
                                       : matched_media->data_offset / 2048U)},
                ...
                {"matched_sbnk_fat_cluster_count",
                 optional_unsigned(matched_fat != nullptr,
                                   matched_fat == nullptr
                                       ? 0U
                                       : matched_fat->clusters.size())},
                {"matched_sbnk_fat_file_size",
                 optional_unsigned(
                     fat && matched_media != nullptr,
                     matched_media == nullptr ? 0U : matched_media->size)},
...
std::vector<axk::ReportRow> bitmap_detail_rows(const CliLoadResult &loaded) {
    static constexpr std::string_view notes =
        "Validated standalone assignment rows support SBNK+0x0c0..0x0cf as "
        "four big-endian "
        "program-link bitmap words for direct PROG->SBNK/sample assignments. "
        "PROG->SBAC "
        "assignments are reported separately as indirection and are not "
        "expected to set child "
        "SBNK bits.";
    std::vector<axk::ReportRow> rows;
    for (const auto &source : loaded.loaded) {
        for (const auto &comparison : source.graph.bitmap_comparisons) {
            ...
            if (item == nullptr)
                continue;
            const auto *bank =
                std::get_if<axk::CurrentSbnk>(&item->object.payload);
            if (bank == nullptr)
                continue;
            ...
            for (const auto &relation : source.graph.relationships) {
                if (relation.type != "PROG_ASSIGNMENT_TO_SBNK" ||
                ...
                const auto ambiguous =
                    ...
                    std::ranges::find(relation.candidate_keys, item->key) !=
                        relation.candidate_keys.end();
                if (!direct && !ambiguous)
                    continue;
                const auto *program_item =
                    catalog_object(source, relation.source_key);
                if (program_item == nullptr)
                    continue;
                const auto *program = std::get_if<axk::CurrentProg>(
                    &program_item->object.payload);
                if (program == nullptr ||
                    *relation.assignment_index >= program->assignments.size())
                    continue;
                const auto &assignment =
                    program->assignments[*relation.assignment_index];
                const auto detail =
                    std::format("{}@slot{}:kind0x{:02x}:flag0x{:02x}",
            ...
            direct_details.erase(
                std::unique(direct_details.begin(), direct_details.end()),
                direct_details.end());
            ambiguous_programs.erase(std::unique(ambiguous_programs.begin(),
                                                 ambiguous_programs.end()),
                                     ambiguous_programs.end());
            ambiguous_details.erase(
                std::unique(ambiguous_details.begin(), ambiguous_details.end()),
            ...
            rows.push_back({
                ...
                {"sbnk_partition_index",
                 optional_unsigned(sfs, item->partition.value)},
                {"sbnk_sfs_id", optional_unsigned(sfs, item->sfs_id.value)},
                {"sbnk_fat_file",
                 fat && object != nullptr ? object->logical_path : ""},
                {"sbnk_payload_offset",
                ...
                {"linked_programs_001_032_bitmap_0x0c0",
                 static_cast<std::uint64_t>(
                     bank->linked_program_bitmap_words[0])},
                {"linked_programs_033_064_bitmap_0x0c4",
                 static_cast<std::uint64_t>(
                     bank->linked_program_bitmap_words[1])},
                {"linked_programs_065_096_bitmap_0x0c8",
                 static_cast<std::uint64_t>(
                     bank->linked_program_bitmap_words[2])},
                {"linked_programs_097_128_bitmap_0x0cc",
                 static_cast<std::uint64_t>(
                     bank->linked_program_bitmap_words[3])},
                {"bitmap_programs",
                 joined_programs(comparison.bitmap_programs)},
                {"direct_prog_assignment_programs",
                 joined_programs(comparison.direct_assignment_programs)},
                {"direct_prog_assignment_details",
                 joined_strings(direct_details)},
                {"ambiguous_direct_assignment_programs",
                 joined_strings(ambiguous_programs)},
                {"ambiguous_direct_assignment_details",
                 joined_strings(ambiguous_details)},
                ...
                {"quality",
                 comparison.status == "match" ? "Known" : "Tentative"},
    ...
    for (const auto &source : loaded.loaded) {
        for (const auto &relation : source.graph.relationships) {
            ...
            if (program_item == nullptr)
                continue;
            const auto *program =
                std::get_if<axk::CurrentProg>(&program_item->object.payload);
            if (program == nullptr ||
                *relation.assignment_index >= program->assignments.size())
                continue;
            const auto &assignment =
            ...
            const auto *target_media =
                target == nullptr ? nullptr : media_object(source, target->key);
            ...
            for (const auto &key : relation.candidate_keys) {
                ...
                if (const auto *candidate = catalog_object(source, key)) {
                    candidate_names.push_back(candidate->object.header.name);
                    candidate_categories.push_back(
                        object_type_text(candidate->object.header.type));
                }
                if (const auto *object = media_object(source, key))
                    candidate_files.push_back(object->logical_path);
            }
            std::ranges::sort(candidate_categories);
            candidate_categories.erase(std::unique(candidate_categories.begin(),
                                                   candidate_categories.end()),
                                       candidate_categories.end());
            axk::ReportValue child_count{nullptr};
            if (target != nullptr &&
                target->object.header.type == axk::ObjectType::sbac) {
                child_count = static_cast<std::uint64_t>(std::ranges::count_if(
                    source.graph.relationships, [&](const auto &row) {
                        return row.source_key == target->key &&
            ...
            rows.push_back({
                ...
                {"prog_object_key",
                 public_object_key(source, program_item->key)},
                {"prog_partition_index",
                 optional_unsigned(sfs, program_item->partition.value)},
                {"prog_sfs_id",
                 optional_unsigned(sfs, program_item->sfs_id.value)},
                {"prog_fat_file", fat && program_media != nullptr
                                      ? program_media->logical_path
                                      : ""},
                {"prog_payload_offset",
                 optional_unsigned(
                     sfs || program_media != nullptr,
                ...
                {"prog_payload_size",
                 program_media != nullptr
                     ? static_cast<std::uint64_t>(
                           program_media->raw_payload.size())
                     : std::uint64_t{0}},
                {"assignment_index",
                 static_cast<std::uint64_t>(*relation.assignment_index)},
                {"assignment_offset",
                 static_cast<std::uint64_t>(
                     0x120U + *relation.assignment_index * 0x38U)},
                {"assignment_name", assignment.name},
                {"assignment_raw_handle_0x10",
                 static_cast<std::uint64_t>(assignment.raw_handle)},
                {"assignment_kind_byte_0x14",
                 static_cast<std::uint64_t>(assignment.kind)},
                {"assignment_flag_byte_0x15",
                 static_cast<std::uint64_t>(assignment.flags)},
                {"assignment_output1_byte_0x1d",
                 static_cast<std::uint64_t>(
                     std::to_integer<std::uint8_t>(assignment.raw_row[0x1d]))},
                {"assignment_rch_assign_gate_byte_0x28",
                 static_cast<std::uint64_t>(
                     std::to_integer<std::uint8_t>(assignment.raw_row[0x28]))},
                {"assignment_rch_assign_display",
                 relation.receive_channel_display},
                {"selector_expected_category", expected},
                {"assignment_row_state", "decoded-row"},
                {"active_assignment_state",
                 std::string{
                     axk::assignment_state_name(relation.assignment_state)}},
                {"match_method", relation.basis},
                {"match_quality",
                 std::string{axk::relationship_quality_name(relation.quality)}},
                ...
                {"matched_target_type",
                 target == nullptr
                     ? ""
                     : object_type_text(target->object.header.type)},
                {"matched_target_object_key",
                 target == nullptr ? ""
                                   : public_object_key(source, target->key)},
                ...
                {"matched_target_sfs_id",
                 optional_unsigned(sfs && target != nullptr,
                                   target == nullptr ? 0U
                                                     : target->sfs_id.value)},
                {"matched_target_fat_file", fat && target_media != nullptr
                                                ? target_media->logical_path
                                                : ""},
                {"matched_target_payload_offset",
                 optional_unsigned(target != nullptr &&
                                       (sfs || target_media != nullptr),
                                   target == nullptr ? 0U
                                   : sfs ? sfs_payload_offset(source, *target)
                                         : target_media->data_offset)},
                {"matched_target_name",
                 target == nullptr ? "" : target->object.header.name},
                ...
                {"prog_iso_extent_sector",
                 optional_unsigned(iso && program_media != nullptr,
                                   program_media == nullptr
                                       ? 0U
                                       : program_media->data_offset / 2048U)},
                ...
                {"prog_fat_cluster_count",
                 optional_unsigned(program_fat != nullptr,
                                   program_fat == nullptr
                                       ? 0U
                                       : program_fat->clusters.size())},
                {"prog_fat_file_size",
                 optional_unsigned(
                     fat && program_media != nullptr,
                     program_media == nullptr ? 0U : program_media->size)},
...
std::vector<axk::ReportRow>
program_ignored_detail_rows(const CliLoadResult &loaded) {
    std::vector<axk::ReportRow> rows;
    for (const auto &source : loaded.loaded) {
        for (const auto &item : source.catalog.objects) {
            const auto *program =
                std::get_if<axk::CurrentProg>(&item.object.payload);
            if (program == nullptr)
                continue;
            std::set<std::size_t> represented;
            for (const auto &relation : source.graph.relationships) {
                if (relation.source_key == item.key &&
                    relation.type.starts_with("PROG_ASSIGNMENT_TO_") &&
                    relation.assignment_index) {
            ...
            for (std::size_t index = 0; index < program->assignments.size();
                ...
                const bool name_match = std::ranges::any_of(
                    source.catalog.objects, [&](const auto &target) {
                        return target.scope_key == item.scope_key &&
                               target.object.header.name == assignment.name;
                    });
                std::string reason;
                if (!known_kind && !name_match) {
                    reason = "ignored-reserved-or-tail-slot-no-known-kind-and-"
                             "no-name-match";
                ...
                rows.push_back({
                    ...
                    {"assignment_offset",
                     static_cast<std::uint64_t>(0x120U + index * 0x38U)},
...
int run_orphans_request(const axk::cli::OrphansRequest &request) {
    ...
    for (const auto &path : expand_cli_paths(request.paths)) {
        ...
        const auto report = axk::analyze_waveform_orphans(
            snapshot->container, snapshot->catalog, snapshot->graph);
        for (const auto &row : report.rows) {
            rows.push_back({
                {"source_path", axk::text::path_to_utf8(path)},
                {"partition_index",
                 static_cast<std::uint64_t>(row.partition.value)},
                ...
                {"referencing_sample_banks",
                 joined_strings(row.referencing_sample_banks)},
                {"basis", row.basis},
                {"notes", row.notes},
            });
        }
        summaries.push_back({
            ...
            {"referenced_count",
             static_cast<std::uint64_t>(report.referenced_count)},
    ...
    auto row_schema =
        write_cli_report(request.output_directory, "waveform_orphans", rows,
                         "axklib", request.overwrite);
    if (!row_schema)
        return report_failure(row_schema.error());
    auto summary_schema =
        write_cli_report(request.output_directory, "waveform_orphan_summary",
    ...
    for (const auto &summary : summaries) {
        const auto text = [&](std::string_view name) {
            ...
            if (const auto *value =
                ...
                return *value;
            if (const auto *value =
                    std::get_if<std::uint64_t>(&found->second.value))
                return std::to_string(*value);
            return std::string{};
        };
        std::cout << "image=" << text("source_path")
                  << " waveforms=" << text("waveform_count")
                  << " referenced=" << text("referenced_count")
                  << " known_unreferenced=" << text("known_unreferenced_count")
                  << " ambiguous_or_unresolved="
                  << text("ambiguous_or_unresolved_count") << '\n';
    }
    std::cout << "reports written to "
              << axk::text::path_to_utf8(request.output_directory) << '\n';
    return 0;
...
std::uint64_t
mismatch_cluster_count(std::span<const axk::AllocationMismatchRange> ranges) {
    std::uint64_t result{};
    for (const auto &range : ranges)
        result += static_cast<std::uint64_t>(range.end_cluster) -
                  range.start_cluster + 1U;
    return result;
}

std::vector<axk::ReportRow>
allocation_summary_rows(const std::filesystem::path &path,
                        const axk::Container &container) {
    std::vector<axk::ReportRow> rows;
    for (const auto &partition : container.partitions()) {
        const auto cluster_size =
            static_cast<std::uint64_t>(partition.sectors_per_cluster) *
            container.superblock().sector_size_bytes;
        ...
        rows.push_back({
            ...
            {"partition_index",
             static_cast<std::uint64_t>(partition.index.value)},
            ...
            {"sectors_per_cluster",
             static_cast<std::uint64_t>(partition.sectors_per_cluster)},
            {"cluster_count",
             static_cast<std::uint64_t>(partition.cluster_count)},
            {"bitmap_offset",
             (static_cast<std::uint64_t>(partition.start_sector) +
            ...
            {"index_offset",
             (static_cast<std::uint64_t>(partition.start_sector) +
              static_cast<std::uint64_t>(partition.directory_index_cluster) *
                 ...
                 container.superblock().sector_size_bytes},
            {"scanned_index_bytes",
             (first_object - partition.directory_index_cluster) * cluster_size},
            {"valid_index_record_count",
             static_cast<std::uint64_t>(partition.records.size())},
            {"invalid_extent_record_count",
             static_cast<std::uint64_t>(
                 allocation.invalid_extent_record_count)},
            ...
            {"reserved_cluster_count",
             free ? static_cast<std::uint64_t>(free->reserved_cluster_count)
                  : first_payload},
            {"sampler_free_cluster_count",
             free ? static_cast<std::uint64_t>(free->free_cluster_count)
                  : std::uint64_t{0}},
            ...
            {"extent_total_mismatch_count",
             static_cast<std::uint64_t>(
                 allocation.extent_total_mismatch_count)},
...
std::vector<axk::ReportRow>
allocation_extent_rows(const std::filesystem::path &path,
                       const axk::Container &container) {
    std::vector<axk::ReportRow> rows;
    for (const auto &partition : container.partitions()) {
        for (const auto &record : partition.records) {
            for (std::size_t index = 0; index < record.extents.size();
                 ++index) {
                ...
                rows.push_back({
                    ...
                    {"cluster_offset",
                     static_cast<std::uint64_t>(extent.cluster_offset)},
                    {"cluster_count",
                     static_cast<std::uint64_t>(extent.cluster_count)},
                    {"byte_count",
                     static_cast<std::uint64_t>(extent.byte_count)},
...
allocation_mismatch_rows(const std::filesystem::path &path,
    ...
    std::vector<axk::ReportRow> rows;
    const auto append =
        [&](const axk::Partition &partition, std::string direction,
            ...
            for (const auto &range : ranges) {
                rows.push_back({
                    ...
                    {"direction", direction},
                    {"start_cluster",
                     static_cast<std::uint64_t>(range.start_cluster)},
                    {"end_cluster",
                     static_cast<std::uint64_t>(range.end_cluster)},
                    {"cluster_count",
                     static_cast<std::uint64_t>(range.end_cluster) -
                         range.start_cluster + 1U},
                });
...
std::vector<axk::ReportRow>
volume_validation_rows(const std::filesystem::path &path,
                       const axk::Container &container,
    ...
    for (const auto &partition : container.partitions()) {
        ...
        for (const auto &entry : root->directory_entries) {
            ...
            if (!volume->parent_directory_id ||
                ...
                continue;
            volumes.try_emplace(VolumeKey{partition.index.value,
                                          volume->sfs_id.value, partition.name,
                                          entry.name});
        }
    }
    for (const auto &item : catalog.objects) {
        if (item.placement) {
            volumes[{item.partition.value,
                     item.placement->volume_directory.value,
                     item.placement->partition_name,
    ...
    for (const auto &[key, objects] : volumes) {
        ...
        const auto &[partition_index, directory_id, partition_name,
                     volume_name] = key;
        const auto partition = std::ranges::find(
            container.partitions(), partition_index,
            [](const auto &item) { return item.index.value; });
        const axk::IndexRecord *volume_record{};
        if (partition != container.partitions().end()) {
            const auto found = std::ranges::find_if(
                partition->records, [&](const auto &record) {
        ...
        if (partition != container.partitions().end() &&
            ...
            for (const auto &category_entry :
                 volume_record->directory_entries) {
                ...
                const auto category = std::ranges::find(
                    partition->records, category_entry.link_id.value,
                ...
                for (const auto &entry : category->directory_entries) {
                    ...
                    const auto target = std::ranges::find(
                        partition->records, entry.link_id.value,
                        [](const auto &record) { return record.sfs_id.value; });
        ...
        const auto details = std::format(
            "visible alternating-byte compatibility artifact object entries: "
            "total={}, SMPL={}, "
            ...
            artifact_count, artifact_smpl_count,
            artifact_counts[axk::ObjectType::sbnk],
            artifact_counts[axk::ObjectType::sbac],
            artifact_counts[axk::ObjectType::prog]);
        if (artifact_count != 0U) {
            detail_issues.push_back({
                {"source_image", axk::text::path_to_utf8(path)},
                {"partition_index",
                 static_cast<std::uint64_t>(partition_index)},
                {"partition_name", partition_name},
                {"volume_name", volume_name},
                {"volume_path", "/" + volume_name},
                {"severity", "warning"},
                {"issue_type",
                 "visible-alternating-byte-compatibility-artifact-objects"},
                {"category_code", ""},
                {"category_name", ""},
                ...
                {"target_sfs_id", ""},
                {"target_payload_kind",
                 "alternating-byte-compatibility-object"},
        ...
        const auto classification =
            allocation_issues != 0U ? "volume-likely-corrupt"
            : warning_count != 0U
        ...
        rows.push_back({
            ...
            {"allocation_issue_count",
             static_cast<std::uint64_t>(allocation_issues)},
            {"validation_status", validation_status},
            {"volume_classification", classification},
            {"quality_summary", warning_count != 0U ? details
                                : allocation_issues == 0U
                                    ? "category directory entries and optional "
...
std::optional<std::uint32_t> little_u32(std::span<const std::byte> bytes,
                                        std::size_t offset) {
...
std::optional<std::uint16_t> little_u16(std::span<const std::byte> bytes,
                                        std::size_t offset) {
...
std::vector<axk::ReportRow>
validate_export_directory(const std::filesystem::path &root) {
    std::vector<axk::ReportRow> issues;
    if (!std::filesystem::exists(root)) {
        issues.push_back(export_validation_issue(
            "error", "EXPORT_DIR_NOT_FOUND", "Export directory does not exist.",
            "export", root));
        return issues;
    }
    std::error_code iteration_error;
    for (std::filesystem::recursive_directory_iterator
             iterator{root, iteration_error},
         end;
        ...
        if (!iterator->is_regular_file() ||
            ...
            std::ranges::find(iterator->path(), "_schemas") !=
                iterator->path().end())
            continue;
        Json record;
        try {
            ...
            issues.push_back(export_validation_issue(
                ...
                std::string{"Sidecar JSON could not be parsed: "} +
                    error.what(),
        ...
        if (schema == "axklib.wave_sidecar.v2") {
            static constexpr std::array sections{
                "identity",   "audio",      "playback", "relationships",
            ...
            if (wav_path.is_absolute() ||
                std::ranges::find(wav_path, "..") != wav_path.end()) {
                issues.push_back(export_validation_issue(
                    "error", "EXPORT_SIDECAR_PATH_ESCAPE",
                    "v2 sidecar audio.wav_path must be relative and stay "
                    "inside the export root.",
                    "sidecar", iterator->path(), object_key));
                continue;
            }
            ...
            if (!record.contains("wav_path"))
                continue;
            static constexpr std::array required{"source_container",
                                                 "object_key",
                                                 "wav_path",
                                                 "sample_rate",
                                                 "channels",
        ...
        wav.read(reinterpret_cast<char *>(bytes.data()),
        ...
        if (wav.gcount() != static_cast<std::streamsize>(bytes.size()) ||
            ...
            std::string_view{reinterpret_cast<const char *>(bytes.data() + 8U),
                             4U} != "WAVE") {
            issues.push_back(export_validation_issue(
                "error", "EXPORT_WAV_BAD_HEADER",
                "Referenced WAV could not be opened: invalid WAVE header",
                "export", wav_path, object_key));
            continue;
        }
        const std::array observed{
            std::pair{"sample_rate",
                      static_cast<std::uint64_t>(*little_u32(bytes, 24U))},
            std::pair{"channels",
                      static_cast<std::uint64_t>(*little_u16(bytes, 22U))},
            std::pair{"sample_width_bytes",
                      static_cast<std::uint64_t>(*little_u16(bytes, 34U) / 8U)},
...
axk::ReportRow media_validation_issue(const CliLoaded &source,
...
std::string media_object_report_path(const CliLoaded &source,
                                     std::string_view object_key) {
    if (const auto *item = catalog_object(source, object_key);
        item != nullptr && item->placement) {
        ...
        std::string path =
            std::format("partition {}", placement.partition.value);
        for (const auto &component :
             {std::string_view{placement.volume_name}, category,
    ...
    return object == nullptr ? public_object_key(source, object_key)
                             : object->logical_path;
}

std::string media_object_group_path(const CliLoaded &source,
                                    std::string_view object_key) {
    ...
    if (category_separator == std::string::npos)
        return path;
    const auto category = path.substr(
        category_separator + 1U, filename_separator - category_separator - 1U);
    static constexpr std::array object_categories{"PROG", "SBAC", "SBNK",
                                                  "SMPL", "SEQU", "PRF3"};
    if (std::ranges::find(object_categories, category) ==
        object_categories.end())
        return path;
    return path.substr(0U, category_separator);
}

std::string active_program_assignment_label(const CliLoaded &source,
                                            const axk::Relationship &row) {
    const auto assignment_name =
        !row.assignment_name.empty()
            ? row.assignment_name
            : row.target_key.value_or("unnamed assignment");
    if (!row.assignment_index)
        return std::format("{}: {}",
                           media_object_report_path(source, row.source_key),
                           assignment_name);
...
std::string relationship_issue_path(const CliLoaded &source,
                                    const axk::Relationship &row) {
...
ambiguous_relationship_message(const axk::Relationship &row) {
    if (row.basis == "assignment-visible-off-same-volume-sbac-diagnostic" ||
        ...
        const auto target =
            row.basis == "assignment-visible-off-same-volume-sbac-diagnostic"
                ? "sample-bank group"
                : "sample-bank";
        return {
            std::format(
                "Visible/off Program assignment row names a {} with one "
                "same-volume "
                "diagnostic candidate plus other duplicate-name candidates; "
                "this is "
                "decoded Program inventory, not active Program content loss.",
                target),
            "Use relationships.csv candidate fields when auditing off rows; "
            "the same-volume "
            "candidate is diagnostic only and must not create an active "
            "Program child."};
    }
    if (row.assignment_state == axk::AssignmentState::visible_off)
        return {"Visible/off Program assignment row has multiple possible "
                "local targets; this is "
                "decoded Program inventory, not active Program content loss.",
                "Use relationships.csv candidate fields only when auditing off "
                "rows; do not treat "
                "this warning as a missing active Program child."};
    if (row.type == "PROG_ASSIGNMENT_TO_SBAC")
        return {"Program assignment to a sample-bank group has multiple "
                "possible targets.",
                "Verify the sampler-visible Program assignment and group "
                "target before promotion."};
    if (row.type == "PROG_ASSIGNMENT_TO_SBNK")
        return {"Direct Program assignment has multiple possible sample-bank "
                "targets.",
                "Verify the sampler-visible Program assignment target before "
                "promotion."};
    if (row.type == "SBAC_SLOT_TO_SBNK")
        return {"Sample-bank group slot has multiple possible sample-bank "
                "member targets.",
                "Inspect duplicate same-name sample-bank candidates before "
                "using this group slot as "
                "authoritative."};
    if (row.basis == "sbnk-member-link-id-only-iso-cross-folder-name-mismatch")
        return {"Sample-bank member link ID points to one physical waveform in "
                "another ISO object "
                "folder, but the member name does not confirm it.",
                "Inspect the waveform candidate before treating this member "
                "link as exact."};
    if (row.basis == "sbnk-member-link-id-only-name-mismatch")
        return {"Sample-bank member link ID points to one physical waveform, "
                "but the member name "
                "does not confirm it.",
                "Inspect the waveform candidate before treating this member "
                "link as exact."};
    if (row.type.starts_with("SBNK_") && row.type.ends_with("_TO_SMPL"))
        return {"Sample-bank member link has multiple possible physical "
                "waveform targets.",
                "Inspect candidate waveform objects before treating this "
                "sample-bank member link as "
                "exact."};
    if (row.basis.starts_with("sbnk-program-link-bitmap-")) {
        std::string message;
        if (row.basis.find("disambiguates-ambiguous-direct-assignment") !=
            std::string::npos)
            message = "Sample-bank Program-link bitmap points to one Program "
                      "from an ambiguous "
                      "direct-assignment set.";
        else if (row.basis.find("known-direct-assignment-missing-bitmap") !=
                 std::string::npos)
            message = "Known direct Program assignment is missing from the "
                      "sample-bank Program-link bitmap.";
        else if (row.basis.find(
                     "nondefault-flag-direct-assignment-without-bitmap") !=
                 std::string::npos)
            message = "Nondefault direct Program assignment is missing from "
                      "the sample-bank Program-link "
                      "bitmap.";
        else
            message = "Sample-bank Program-link bitmap differs from resolved "
                      "direct Program assignments.";
        return {
            std::move(message),
            "Use this as bitmap consistency data only; do not treat it as "
            "Program content loss "
            "unless another public rule proves the bitmap is authoritative."};
    }
    if (row.type == "SBNK_PROGRAM_BITMAP_TO_PROG")
        return {"Sample-bank Program-link bitmap maps to multiple possible "
                "Program slots.",
                "Use this as bitmap consistency data only until the Program "
                "target is disambiguated."};
...
std::pair<std::string, std::string>
missing_relationship_message(const axk::Relationship &row) {
    if (row.assignment_state == axk::AssignmentState::active)
        return {"Active Program assignment references a missing local target.",
                "Inspect the Program assignment and source object group; "
                "user-facing info may show "
                "an unresolved placeholder instead of a normal Program child."};
    if (row.assignment_state == axk::AssignmentState::visible_off) {
        const auto expected = row.type == "PROG_ASSIGNMENT_TO_SBAC"
                                  ? "sample-bank group"
                                  : "sample-bank";
        return {std::format("Visible/off Program assignment row names a "
                            "missing local {} target; "
                            "this is decoded Program inventory, not active "
                            "Program content loss.",
                            expected),
                "Keep this row as diagnostic/off-row data unless "
                "sampler-visible checks prove it "
    ...
    if (row.assignment_state == axk::AssignmentState::source_load)
        return {
            ...
            "Keep the selector as diagnostic source data until sampler-loaded "
            "placement or "
            "another public rule proves a target."};
    if (row.type.starts_with("SBNK_") && row.type.ends_with("_TO_SMPL"))
        return {"Sample-bank member link does not resolve to a physical "
                "waveform target.",
                "Inspect the object group before treating this sample-bank "
                "member as complete."};
    return {"Relationship target could not be resolved.",
            "Inspect the relationship row and decoded source object before "
            "treating the target as "
            "present."};
}
...
std::vector<axk::ReportRow>
validate_media_details(const CliLoaded &source,
    ...
    if (include_object_checks) {
        for (const auto &issue : source.media.validation_issues()) {
            issues.push_back(media_validation_issue(
                source, "error", issue.code, issue.message, "container",
                issue.sampler_path, {}, "Confirmed", issue.basis,
                issue.recommended_next_check));
        }
        for (const auto &object : source.objects) {
            const auto required =
                static_cast<std::uint64_t>(object.decoded.header.header_size) +
            ...
            issues.push_back(media_validation_issue(
                ...
                std::format(
                    "Object header requires {} bytes but payload has {} bytes.",
                    required, object.raw_payload.size()),
                "object", {}, public_object_key(source, object.key), "Known",
                "validation"));
        }
    }
    ...
    std::map<MemberGroup, std::vector<const axk::Relationship *>>
        grouped_members;
    ...
    for (const auto &row : source.graph.relationships) {
        ...
        if (active != reachable.end()) {
            for (const auto *program_row : active->second)
                grouped_active_labels[group].insert(
                    active_program_assignment_label(source, *program_row));
        }
    ...
    for (const auto &[group, rows] : grouped_members) {
        ...
        if (group.second) {
            ...
            issues.push_back(media_validation_issue(
                ...
                std::format("{} sample-bank member link(s) across {} sample "
                            "bank(s) do not resolve to "
                            "physical sample objects and are reachable from "
                            "active Program assignments.",
                            member_count, bank_count),
                "relationship",
                std::format("{} | {}", group.first, active_summary),
                *source_keys.begin(), "Unknown",
                "SBNK member target aggregation",
                "Treat the affected Program/sample-bank path as incomplete "
                "until the missing physical "
                "sample objects are found or the source is confirmed partially "
                "loadable."));
        } else {
            issues.push_back(media_validation_issue(
                ...
                "SBNK member target aggregation",
                "Inspect the sample-bank member links before treating this "
                "object group as complete."));
        }
    }
    for (const auto &row : source.graph.relationships) {
        ...
        if (row.quality == axk::RelationshipQuality::tentative) {
            ...
            issues.push_back(media_validation_issue(
                source, "warning", tentative_relationship_code(row),
                std::move(message), "relationship",
                relationship_issue_path(source, row),
                public_object_key(source, row.source_key), "Tentative",
                row.basis, std::move(next_check)));
        } else if (row.quality == axk::RelationshipQuality::unknown) {
            auto [message, next_check] = missing_relationship_message(row);
            issues.push_back(media_validation_issue(
                source, "warning", missing_relationship_code(row),
                std::move(message), "relationship",
                relationship_issue_path(source, row),
                public_object_key(source, row.source_key), "Unknown", row.basis,
                std::move(next_check)));
        }
    }
    std::ranges::sort(issues, {}, [](const axk::ReportRow &row) {
        const auto value = [&](std::string_view key) -> std::string {
            const auto found =
                std::ranges::find(row, key, &axk::ReportRow::value_type::first);
            return found == row.end()
                       ? std::string{}
                       : std::get<std::string>(found->second.value);
        };
...
int run_validate_request(const axk::cli::ValidateRequest &request) {
    if (!request.exports && request.paths.empty()) {
        std::cerr
            << "validate requires input paths unless --exports is supplied\n";
        return 2;
    }
    if (const auto ready = prepare_report_directory(request.output_directory,
                                                    request.overwrite);
    ...
    if (request.exports) {
        ...
        for (const auto &issue : issues) {
            const auto code = std::ranges::find(
                issue, "code",
                &std::pair<std::string, axk::ReportValue>::first);
            if (code != issue.end())
    ...
    const auto loaded =
        request.exports ? CliLoadResult{} : load_cli_paths(request.paths);
    ...
    for (const auto &source : loaded.loaded) {
        ...
        if (source.media.kind() != axk::MediaKind::sfs) {
            ...
            for (auto &issue : source_issues) {
                ...
                if (code != issue.end())
                    ++issue_counts[std::get<std::string>(code->second.value)];
                const auto severity = std::ranges::find(
                    issue, "severity",
                    &std::pair<std::string, axk::ReportValue>::first);
                if (severity != issue.end()) {
                    const auto &value =
                        std::get<std::string>(severity->second.value);
        ...
        const auto &container =
            std::get<axk::Container>(source.media.storage());
        const auto report =
            axk::validate_semantics(container, source.catalog, source.graph);
        for (const auto &issue : report.issues) {
            if (issue.code.starts_with("REL_"))
                continue;
            const auto severity =
                issue.severity == axk::ValidationSeverity::error     ? "error"
            ...
            if (issue.severity == axk::ValidationSeverity::error ||
                (request.policy == "strict" &&
                 issue.severity == axk::ValidationSeverity::warning))
                failed = true;
            issues.push_back({
                {"severity", severity},
                {"code", issue.code},
        ...
        std::ranges::move(source_summaries,
                          std::back_inserter(allocation_summaries));
        auto source_extents = allocation_extent_rows(path, container);
        std::ranges::move(source_extents,
                          std::back_inserter(allocation_extents));
        auto source_mismatches =
            allocation_mismatch_rows(path, container.partitions());
        std::ranges::move(source_mismatches,
                          std::back_inserter(allocation_mismatches));
        const auto prior_issue_count = issues.size();
        auto source_volumes = volume_validation_rows(
            path, container, source.catalog, volume_issues, issues);
    ...
    axk::ReportRow validation_summary{
        {"policy", policy},
        {"failed", failed},
    ...
    for (const auto &row : volumes) {
        const auto text = [&](std::string_view key) -> std::string {
            const auto found = std::ranges::find(
                row, key, &std::pair<std::string, axk::ReportValue>::first);
            return found == row.end()
                       ? std::string{}
                       : std::get<std::string>(found->second.value);
        };
        const auto number = [&](std::string_view key) -> std::uint64_t {
            ...
            return found == row.end()
                       ? 0U
                       : std::get<std::uint64_t>(found->second.value);
        };
        ...
        malformed_category_entry_count +=
            number("malformed_category_entry_count");
        allocation_issue_count += number("allocation_issue_count");
    }
    axk::ReportRow volume_summary{
        {"source_image", request.paths.size() == 1U
                             ? axk::text::path_to_utf8(request.paths.front())
                             : ""},
    ...
    const auto report = [&](std::string name,
                            const std::vector<axk::ReportRow> &rows) -> bool {
        auto schema =
            write_cli_report(request.output_directory, std::move(name), rows,
                             "axklib", request.overwrite);
        if (!schema)
            return false;
    ...
    if (auto written = axk::write_report_object(
            request.output_directory / "validation_summary.json",
    ...
    auto validation_summary_schema = axk::make_report_schema(
        "validation_summary", std::span{&validation_summary, 1U},
        schema_options);
    if (auto written = axk::write_report_schema(
            request.output_directory / "_schemas" /
...
write_csv_schema(const std::filesystem::path &output, std::string name,
    ...
    if (auto written = axk::write_report_csv(output / (name + ".csv"), rows, {},
                                             overwrite);
        !written)
    ...
    if (auto written = axk::write_report_schema(
            output / "_schemas" / (name + ".schema.json"), schema, overwrite);
        !written)
        return std::unexpected{written.error()};
    return schema;
}
...
int run_corpus_audit_request(const axk::cli::CorpusAuditRequest &request) {
    ...
    for (const auto &path : paths) {
        ...
        std::ranges::transform(
            suffix, suffix.begin(), [](const unsigned char value) {
        ...
        manifest.push_back(
            {{"path", axk::text::path_to_utf8(path)},
             {"exists", exists},
             {"is_file",
              exists && std::filesystem::is_regular_file(path, error)},
    ...
    for (const auto &source : loaded.loaded) {
        ...
        if (const auto *container =
                std::get_if<axk::Container>(&source.media.storage())) {
            const auto report = axk::validate_semantics(
                *container, source.catalog, source.graph);
        ...
        if (!request.skip_wave_smoke) {
            ...
            for (const auto &item : source.catalog.objects) {
                ...
                axk::Result<axk::Waveform> waveform =
                    ...
                    const auto object = std::ranges::find(
                        source.objects, item.key, &axk::MediaObject::key);
                    if (object == source.objects.end())
                        return std::unexpected{axk::make_error(
                            axk::ErrorCode::object_malformed,
            ...
            wave_decoded +=
                std::min(successful,
                         static_cast<std::uint64_t>(request.wave_smoke_limit));
        }
    }
    ...
    axk::ReportRow summary{
        {"input_count", static_cast<std::uint64_t>(loaded.loaded.size() +
                                                   loaded.errors.size())},
        {"loaded_container_count",
         static_cast<std::uint64_t>(loaded.loaded.size())},
        {"load_error_count", static_cast<std::uint64_t>(loaded.errors.size())},
        {"relationship_load_error_count",
         static_cast<std::uint64_t>(loaded.errors.size())},
        {"object_count", static_cast<std::uint64_t>(inventory.size())},
        {"validation_issue_count",
         static_cast<std::uint64_t>(validation_issues.size())},
    ...
    if (auto written = axk::write_report_object(request.output_directory /
                                                    "corpus_audit_summary.json",
                                                summary, request.overwrite);
        !written)
        return report_failure(written.error());
    if (auto written = axk::write_report_json(request.output_directory /
                                                  "input_manifest.json",
    ...
    auto summary_schema = axk::make_report_schema(
        "corpus_audit_summary", summary_rows, base_schema_options);
    if (auto written =
            axk::write_report_schema(request.output_directory / "_schemas" /
    ...
    const auto schema_for = [&](std::string name,
                                std::span<const axk::ReportRow> report_rows,
                                bool json_already = false) -> bool {
        axk::Result<axk::ReportSchemaManifest> result =
            json_already ? [&]() -> axk::Result<axk::ReportSchemaManifest> {
            if (auto written = axk::write_report_csv(
                    request.output_directory / (name + ".csv"), report_rows, {},
                    request.overwrite);
                !written)
                return std::unexpected{written.error()};
            axk::ReportSchemaOptions options;
            ...
            : write_csv_schema(request.output_directory, std::move(name),
    ...
    if (!schema_for("input_manifest", manifest, true) ||
        ...
        !schema_for("relationships", relationships) ||
        !schema_for("wave_smoke_issues", wave_issues))
        return 2;
    if (auto index = axk::write_report_schema_index(
            request.output_directory / "_schemas" / "schema_index.json",
            schemas, request.overwrite);
        !index)
        return report_failure(index.error());
    std::cout << "containers=" << loaded.loaded.size()
              << " objects=" << inventory.size()
              << " validation_issues=" << validation_issues.size()
              << " relationships=" << relationships.size()
              << " wave_smoke=" << wave_decoded << '/' << wave_issues.size()
              << '\n';
    std::cout << "reports written to "
              << axk::text::path_to_utf8(request.output_directory) << '\n';
    if (!loaded.errors.empty())
