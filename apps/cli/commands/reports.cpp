...
int run_objects_request(const axk::cli::ObjectsRequest &request) {
    ...
    auto schema = write_cli_report(*request.output_directory, "objects", rows, "axklib objects", request.overwrite);
    if (!schema)
...
int run_inventory_request(const axk::cli::InventoryRequest &request) {
    ...
    auto object_schema =
        write_cli_report(request.output_directory, "inventory_objects", rows, "axklib inventory", request.overwrite);
    if (!object_schema)
        return report_failure(object_schema.error());
    auto issue_schema =
        write_cli_report(request.output_directory, "decode_issues", issues, "axklib inventory", request.overwrite);
    if (!issue_schema)
    ...
    axk::ReportRow summary{{"input_count", static_cast<std::uint64_t>(loaded.loaded.size() + loaded.errors.size())},
    ...
    if (auto written =
            axk::write_report_object(request.output_directory / "inventory_summary.json", summary, request.overwrite);
        !written)
    ...
    auto summary_schema = axk::make_report_schema("inventory_summary", std::span{&summary, 1U}, summary_options);
    if (auto written = axk::write_report_schema(request.output_directory / "_schemas" / "inventory_summary.schema.json",
...
std::vector<axk::ReportRow> bitmap_detail_rows(const CliLoadResult &loaded) {
    static constexpr std::string_view notes = "Validated standalone assignment rows support SBNK+0x0c0..0x0cf as "
                                              "four big-endian "
    ...
    for (const auto &source : loaded.loaded) {
        for (const auto &relation : source.graph.relationships) {
            ...
            rows.push_back({
                ...
                {"candidate_fat_files", fat ? joined_strings(candidate_files) : ""},
                {"candidate_names", joined_strings(candidate_names)},
                {"matched_target_type", target == nullptr ? "" : object_type_text(target->object.header.type)},
    ...
    for (const auto &source : loaded.loaded) {
        for (const auto &item : source.catalog.objects) {
            ...
            for (std::size_t index = 0; index < program->assignments.size(); ++index) {
                ...
                const bool name_match = std::ranges::any_of(source.catalog.objects, [&](const auto &target) {
...
int run_orphans_request(const axk::cli::OrphansRequest &request) {
    ...
    for (const auto &path : expand_cli_paths(request.paths)) {
        ...
        const auto report = axk::analyze_waveform_orphans(snapshot->container, snapshot->catalog, snapshot->graph);
    ...
    auto row_schema = write_cli_report(request.output_directory, "waveform_orphans", rows, "axklib", request.overwrite);
    if (!row_schema)
        return report_failure(row_schema.error());
    auto summary_schema =
        write_cli_report(request.output_directory, "waveform_orphan_summary", summaries, "axklib", request.overwrite);
    if (!summary_schema)
    ...
    for (const auto &summary : summaries) {
        const auto text = [&](std::string_view name) {
            const auto found = std::ranges::find(summary, name, &std::pair<std::string, axk::ReportValue>::first);
    ...
    for (const auto &partition : container.partitions()) {
        ...
        for (const auto &entry : root->directory_entries) {
            ...
            if (!volume->parent_directory_id || volume->parent_directory_id->value != root->directory_id->value)
                continue;
            volumes.try_emplace(VolumeKey{partition.index.value, volume->sfs_id.value, partition.name, entry.name});
        }
    }
    ...
    for (const auto &[key, objects] : volumes) {
        ...
        rows.push_back({
            ...
            {"quality_summary", warning_count != 0U       ? details
                                : allocation_issues == 0U ? "category directory entries and optional "
...
axk::ReportRow media_validation_issue(const CliLoaded &source, std::string severity, std::string code,
...
std::pair<std::string, std::string> ambiguous_relationship_message(const axk::Relationship &row) {
    if (row.basis == "assignment-visible-off-same-volume-sbac-diagnostic" ||
        ...
        return {std::format("Visible/off Program assignment row names a {} with one "
                            "same-volume "
    ...
    if (row.basis.starts_with("sbnk-program-link-bitmap-")) {
        ...
        return {std::move(message), "Use this as bitmap consistency data only; do not treat it as "
...
std::vector<axk::ReportRow> validate_media_details(const CliLoaded &source, bool include_object_checks = true) {
    std::vector<axk::ReportRow> issues;
    ...
    for (const auto &[group, rows] : grouped_members) {
        ...
        if (group.second) {
            ...
            issues.push_back(media_validation_issue(source, "error", "REL_ACTIVE_PROGRAM_SBNK_MEMBER_TARGET_MISSING",
                                                    ...
                                                    "relationship", std::format("{} | {}", group.first, active_summary),
                                                    *source_keys.begin(), "Unknown", "SBNK member target aggregation",
            ...
            issues.push_back(media_validation_issue(source, "warning", "REL_SBNK_MEMBER_TARGET_MISSING",
                                                    std::format("{} sample-bank member link(s) across {} sample "
                                                                "bank(s) do not resolve to "
...
int run_validate_request(const axk::cli::ValidateRequest &request) {
    ...
    for (const auto &source : loaded.loaded) {
        ...
        auto source_volumes = volume_validation_rows(path, container, source.catalog, volume_issues, issues);
    ...
    axk::ReportRow volume_summary{
        {"source_image", request.paths.size() == 1U ? axk::text::path_to_utf8(request.paths.front()) : ""},
    ...
    const auto report = [&](std::string name, const std::vector<axk::ReportRow> &rows) -> bool {
        auto schema = write_cli_report(request.output_directory, std::move(name), rows, "axklib", request.overwrite);
        if (!schema)
            return false;
    ...
    if (auto written =
            axk::write_report_schema(request.output_directory / "_schemas" / "validation_summary.schema.json",
                                     validation_summary_schema, request.overwrite);
        !written)
        return report_failure(written.error());
    schemas.push_back(validation_summary_schema);
    if (!request.exports && has_sfs_input) {
        if (!report("allocation_summary", allocation_summaries) || !report("allocation_extents", allocation_extents) ||
            !report("allocation_mismatches", allocation_mismatches) || !report("volume_validation", volumes) ||
    ...
    if (auto index = axk::write_report_schema_index(request.output_directory / "_schemas" / "schema_index.json",
        ...
        !index)
        return report_failure(index.error());
    std::cout << "issues=" << issues.size() << " failed=" << (failed ? "True" : "False") << " policy=" << policy
              << '\n';
    std::cout << "reports written to " << axk::text::path_to_utf8(request.output_directory) << '\n';
    return failed ? 1 : 0;
}

axk::Result<axk::ReportSchemaManifest> write_csv_schema(const std::filesystem::path &output, std::string name,
...
int run_corpus_audit_request(const axk::cli::CorpusAuditRequest &request) {
    ...
    for (const auto &source : loaded.loaded) {
        ambiguous += static_cast<std::uint64_t>(std::ranges::count_if(source.graph.relationships, [](const auto &row) {
    ...
    if (auto written = axk::write_report_object(request.output_directory / "corpus_audit_summary.json", summary,
                                                request.overwrite);
        !written)
        return report_failure(written.error());
    if (auto written =
            axk::write_report_json(request.output_directory / "input_manifest.json", manifest, request.overwrite);
        !written)
        return report_failure(written.error());
    std::vector<axk::ReportSchemaManifest> schemas;
    ...
    auto summary_schema = axk::make_report_schema("corpus_audit_summary", summary_rows, base_schema_options);
    if (auto written =
            axk::write_report_schema(request.output_directory / "_schemas" / "corpus_audit_summary.schema.json",
                                     summary_schema, request.overwrite);
        !written)
        return report_failure(written.error());
    ...
    const auto schema_for = [&](std::string name, std::span<const axk::ReportRow> report_rows,
        ...
        axk::Result<axk::ReportSchemaManifest> result = json_already ? [&]() -> axk::Result<axk::ReportSchemaManifest> {
            if (auto written = axk::write_report_csv(request.output_directory / (name + ".csv"), report_rows, {},
                                                     request.overwrite);
                !written)
                return std::unexpected{written.error()};
            axk::ReportSchemaOptions options;
            ...
            if (auto written = axk::write_report_schema(request.output_directory / "_schemas" / (name + ".schema.json"),
                                                        schema, request.overwrite);
                !written)
                return std::unexpected{written.error()};
            return schema;
        }()
            : write_csv_schema(request.output_directory, std::move(name), report_rows, request.overwrite);
        if (!result)
            return false;
        schemas.push_back(*result);
        return true;
    };
    if (!schema_for("input_manifest", manifest, true) || !schema_for("inventory_objects", inventory) ||
        !schema_for("validation_issues", validation_issues) || !schema_for("relationships", relationships) ||
    ...
    std::cout << "containers=" << loaded.loaded.size() << " objects=" << inventory.size()
              << " validation_issues=" << validation_issues.size() << " relationships=" << relationships.size()
