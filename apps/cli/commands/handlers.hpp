...
int run_package_export(const PackageExportRequest &request);
int run_package_inspect(const PackageReadRequest &request, bool verify_only);
...
int run_export(const std::filesystem::path &path, const std::filesystem::path &output_directory,
               bool overwrite, bool write_sfz, bool pretty);
