...
Result<PackageRootKind> parse_root_kind(std::string_view value) {
  ...
  return std::unexpected{
      argument_error("package root kind must be volume, program, sbac, sbnk, smpl, or sequence")};
}

Result<PackageRootSelector> parse_root(const std::string &value,
  ...
  if (*kind == PackageRootKind::volume) {
    ...
    return std::unexpected{
        argument_error("object package roots use KIND=NAME, for example program=001")};
  }
...
Result<PackageRootDestination> parse_destination(const std::string &value) {
  try {
    ...
    static const std::set<std::string, std::less<>> fields{
        "package", "root", "partition", "group", "volume", "raw_group", "raw_volume", "create"};
    if (!has_only_fields(object, fields) || !object.contains("package") ||
        !object.contains("root")) {
      return std::unexpected{argument_error(
          "package destination must be a JSON object with package and root indexes")};
    }
    ...
    return std::unexpected{
        argument_error(std::string{"invalid package destination JSON: "} + error.what())};
  }
}
  ...
  try {
    ...
    for (const auto &item : array) {
      if (!has_only_fields(item, fields) || !item.contains("package") ||
        ...
        return std::unexpected{
            argument_error("each package rename must contain only package, node_id, and name")};
      }
      result.push_back({item.at("package").get<std::size_t>(),
                        item.at("node_id").get<std::string>(), item.at("name").get<std::string>()});
    }
    return result;
  } catch (const nlohmann::json::exception &error) {
    return std::unexpected{
        argument_error(std::string{"invalid package rename map JSON: "} + error.what())};
  }
}

void print_package_summary(const schema::package_v1::PackageOutput &output, bool verify_only) {
  std::cout << output.path_utf8 << '\t' << (output.valid ? "valid" : "invalid")
            << "\tkind=" << output.package_kind << "\tpackage_id=" << output.package_id
            << "\troots=" << output.roots.size() << "\tobjects=" << output.objects.size()
...
int run_package_import(const axk::cli::PackageImportRequest &request) {
  ...
  if (request.apply && plan->valid()) {
    ...
    auto applied =
        apply_package_import(request.target, packages, *plan, *request.output, request.overwrite);
    if (!applied)
      return report_failure(applied.error());
    report = std::move(*applied);
  }
  const auto projected =
      schema::package_v1::project_plan(request.target, request.packages, *plan, report);
  if (request.format == "json") {
    auto serialized = schema::package_v1::serialize(projected, false);
    if (!serialized)
      return report_failure(serialized.error());
