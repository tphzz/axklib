...
parse_root(const std::string &value,
           const axk::cli::PackageExportRequest &request) {
    const auto separator = value.find('=');
    const auto kind_text = value.substr(0U, separator);
    auto kind = parse_root_kind(kind_text);
    if (!kind)
        return std::unexpected{kind.error()};
    const auto object_name = separator == std::string::npos
                                 ? std::string{}
                                 : value.substr(separator + 1U);
    if (*kind == PackageRootKind::volume) {
        if (separator != std::string::npos)
            return std::unexpected{argument_error(
                "volume package roots do not take an object name")};
    } else if (separator == std::string::npos || object_name.empty()) {
        return std::unexpected{argument_error(
            "object package roots use KIND=NAME, for example program=001")};
    }
    if (request.partition_index &&
        *request.partition_index > std::numeric_limits<std::uint8_t>::max()) {
        return std::unexpected{
            argument_error("package source partition index is out of range")};
    }
    PackageRootSelector result;
    result.kind = *kind;
    if (request.partition_index)
        result.partition_index =
            static_cast<std::uint8_t>(*request.partition_index);
...
Result<PackageRootDestination> parse_destination(const std::string &value) {
    try {
        ...
        static const std::set<std::string, std::less<>> fields{
            "package", "root",      "partition",  "group",
            "volume",  "raw_group", "raw_volume", "create"};
        if (!has_only_fields(object, fields) || !object.contains("package") ||
            !object.contains("root")) {
            return std::unexpected{
                argument_error("package destination must be a JSON object with "
        ...
        if (object.contains("partition")) {
            ...
            if (partition > std::numeric_limits<std::uint8_t>::max())
                return std::unexpected{argument_error(
                    "package destination partition is out of range")};
        ...
        return std::unexpected{argument_error(
            std::string{"invalid package destination JSON: "} + error.what())};
    }
}

Result<std::vector<PackageNodeRename>>
load_renames(const std::filesystem::path &path) {
    std::ifstream input{path};
    if (!input)
        return std::unexpected{make_error(ErrorCode::io_open_failed,
                                          ErrorCategory::io,
                                          "could not open package rename map")};
    std::ostringstream buffer;
    buffer << input.rdbuf();
    if (!input && !input.eof())
        return std::unexpected{make_error(ErrorCode::io_read_failed,
                                          ErrorCategory::io,
                                          "could not read package rename map")};
    const auto text_value = buffer.str();
    if (!text::is_valid_utf8(text_value))
        return std::unexpected{
            argument_error("package rename map is not valid UTF-8")};
    try {
        const auto array = Json::parse(text_value);
        if (!array.is_array())
            return std::unexpected{
                argument_error("package rename map must be a JSON array")};
        std::vector<PackageNodeRename> result;
        result.reserve(array.size());
        static const std::set<std::string, std::less<>> fields{
            "package", "node_id", "name"};
        for (const auto &item : array) {
            if (!has_only_fields(item, fields) || !item.contains("package") ||
                !item.contains("node_id") || !item.contains("name")) {
                return std::unexpected{
                    argument_error("each package rename must contain only "
                                   "package, node_id, and name")};
            }
            result.push_back({item.at("package").get<std::size_t>(),
                              item.at("node_id").get<std::string>(),
                              item.at("name").get<std::string>()});
        }
        return result;
    } catch (const nlohmann::json::exception &error) {
        return std::unexpected{argument_error(
            std::string{"invalid package rename map JSON: "} + error.what())};
    }
}

void print_package_summary(const schema::package_v1::PackageOutput &output,
                           bool verify_only) {
    std::cout << output.path_utf8 << '\t'
              << (output.valid ? "valid" : "invalid")
              << "\tkind=" << output.package_kind
              << "\tpackage_id=" << output.package_id
              << "\troots=" << output.roots.size()
              << "\tobjects=" << output.objects.size()
              << "\trelationships=" << output.relationship_count;
    std::cout << "\tverification="
              << (output.payloads_verified ? "full" : "manifest");
    if (!verify_only)
        std::cout << "\textension=" << output.required_extension;
    std::cout << '\n';
    for (const auto &issue : output.issues)
        std::cout << (issue.fatal ? "error" : "warning") << '\t' << issue.code
                  << '\t' << issue.message << '\n';
}

void print_plan_summary(const schema::package_v1::PlanOutput &output) {
    std::cout << output.target_path_utf8 << '\t'
              << (output.valid ? "valid" : "conflicts")
              << "\ttarget=" << output.target_kind
              << "\tplan_id=" << output.plan_id
              << "\tobjects=" << output.objects.size()
              << "\tconflicts=" << output.conflicts.size() << '\n';
...
int run_package_export(const axk::cli::PackageExportRequest &request) {
    ...
    auto published = export_portable_package(*media, roots, request.output,
                                             request.overwrite);
    if (!published)
    ...
    const auto projected =
        schema::package_v1::project_package(published->output_path, *package);
...
int run_package_inspect(const axk::cli::PackageReadRequest &request,
                        bool verify_only) {
    auto package = verify_only ? open_portable_package(request.package)
                               : inspect_portable_package(request.package);
    if (!package)
        return report_failure(package.error());
    if (verify_only) {
    ...
    const auto projected =
        schema::package_v1::project_package(request.package, *package);
    if (request.format == "json") {
        auto serialized = schema::package_v1::serialize(projected, false);
        if (!serialized)
            return report_failure(serialized.error());
    ...
    if (request.apply && plan->valid()) {
        if (!request.output)
            return report_failure(
                argument_error("package import output path is required"));
        auto applied = apply_package_import(request.target, packages, *plan,
                                            *request.output, request.overwrite);
        if (!applied)
            return report_failure(applied.error());
        report = std::move(*applied);
    }
    const auto projected = schema::package_v1::project_plan(
        request.target, request.packages, *plan, report);
    if (request.format == "json") {
        auto serialized = schema::package_v1::serialize(projected, false);
        if (!serialized)
            return report_failure(serialized.error());
