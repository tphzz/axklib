...
int run_create_hds(const std::filesystem::path &manifest_path,
    ...
    const auto written =
        axk::write_hds_image(*manifest, output_path, overwrite);
    if (!written) {
...
int run_create_media(const std::filesystem::path &manifest_path,
    ...
    const auto written =
        axk::write_media_image(*manifest, output_path, overwrite);
    if (!written) {
        std::cerr << axk::render_error(written.error()) << '\n';
        return 2;
    }
...
int run_create_manifest(std::string_view kind,
    ...
    const auto written = axk::write_build_manifest_template(
        *manifest_kind, output_path, overwrite);
    if (!written) {
        std::cerr << axk::render_error(written.error()) << '\n';
        return 2;
    }
    std::cout << "manifest=" << axk::text::path_to_utf8(output_path)
              << " kind=" << kind << '\n';
    return 0;
}
...
int run_alter_manifest(const std::filesystem::path &output_path,
                       bool overwrite) {
    const auto written =
        axk::write_alteration_manifest_template(output_path, overwrite);
    if (!written) {
        std::cerr << axk::render_error(written.error()) << '\n';
        return 2;
    }
