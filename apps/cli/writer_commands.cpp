...
void axk::cli::register_writer_commands(CLI::App &app, WriterCommandState &state) {
    ...
    state.create_manifest_command->add_option("-o,--output", state.create_manifest_output, "output JSON path")
        ->required();
    ...
    state.alter_hds->add_option("manifest", state.alter_manifest, "alteration manifest JSON")->required();
    ...
    state.alter_manifest_command = alter->add_subcommand("manifest", "write a starter alteration manifest");
    state.alter_manifest_command->add_option("-o,--output", state.alter_manifest_output, "output JSON path")
        ->required();
