  ...
  try {
    ...
    for (const auto &bank : volume.sample_banks) {
      ...
      OrderedJson resolved_range = {
          {"low_midi",
           range_low == 255U ? OrderedJson(range_bank.left.root_key) : OrderedJson(range_low)},
          {"high_midi",
           range_high == 128U ? OrderedJson(range_bank.left.root_key) : OrderedJson(range_high)},
          {"low_raw", range_low},
          {"high_raw", range_high},
          ...
          {"basis", range_low == 255U || range_high == 128U ? "sampler-orig-key-limit"
                                                            : "decoded-key-range"},
      };
      sbnk.push_back({
          {"id", bank.object_key},
    ...
    for (const auto &bank : volume.sample_banks) {
      ...
      decisions.push_back({
          ...
          {"decision", bank.stereo_decision->renderable ? "interleaved_stereo_written"
