...
OrderedJson decoded_json(const DecodedObject &object) {
    if (const auto *sample = std::get_if<CurrentSmpl>(&object.payload)) {
        return {{"kind", "SMPL"},
                ...
                {"stored_sample_width_bytes",
                 sample->stored_sample_width_bytes.value},
    ...
    if (const auto *bank = std::get_if<CurrentSbnk>(&object.payload)) {
        ...
        for (const auto &field : bank->numeric_fields)
            fields[field.name] =
                field.value ? OrderedJson(*field.value) : OrderedJson(nullptr);
        auto controls = OrderedJson::array();
        for (const auto &control : bank->control_records)
            controls.push_back({control.device, control.function, control.type,
                                control.range});
        return {
            {"kind", "SBNK"},
            ...
            {"right",
             bank->right ? member_json(*bank->right) : OrderedJson(nullptr)},
    ...
    if (const auto *group = std::get_if<CurrentSbac>(&object.payload)) {
        ...
        for (const auto &slot : group->slots)
            slots.push_back({{"name", slot.name},
                             {"raw_handle", slot.raw_handle},
                             {"offset", slot.offset}});
        return {
            {"kind", "SBAC"},
            {"raw_sample_parameter_block_hex",
             hex(group->raw_sample_parameter_block)},
            {"value_enable_words", group->value_enable_words},
            {"enabled_parameter_numbers", group->enabled_parameter_numbers},
            {"enabled_numbers_outside_table",
             group->enabled_numbers_outside_table},
    ...
    if (const auto *profile = std::get_if<CurrentProfile>(&object.payload))
        return {{"kind", "PRF3"},
                {"raw_payload_hex", hex(profile->raw_payload)}};
    return {{"kind", "generic"}};
}
