...
PackageOutput project_package(const std::filesystem::path &path, const PortablePackage &package) {
  ...
  for (const auto &root : package.roots) {
    result.roots.push_back(
        {std::string{package_root_kind_name(root.kind)}, root.display_name, root.node_ids});
  }
...
PlanOutput project_plan(const std::filesystem::path &target,
  ...
  for (const auto &object : plan.objects) {
    ...
    std::ranges::transform(object.actions, std::back_inserter(projected.actions),
...
Result<std::string> serialize(const PackageOutput &output, bool pretty) {
  try {
    ...
    for (const auto &root : output.roots)
      roots.push_back(
          {{"kind", root.kind}, {"display_name", root.display_name}, {"node_ids", root.node_ids}});
