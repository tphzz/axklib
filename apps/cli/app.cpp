...
int axk::cli::run(int argc, char **argv) {
    ...
    auto *info =
        app.add_subcommand("info", "summarize supported axklib containers");
    info->add_option("paths", info_request.paths, "input files or directories")
        ->required()
        ->expected(1, -1);
    info->add_flag("--strict", info_request.strict,
                   "stop after the first load error");
    info->add_option("--format", info_request.format,
                     "tree, json, summary, or paths")
        ->check(CLI::IsMember({"tree", "json", "summary", "paths"}));
    info->add_option("--max-depth", info_request.max_depth,
                     "maximum rendered tree depth");
    info->add_flag("--show-quality", info_request.show_quality,
                   "show quality labels");
    info->add_flag("--show-unresolved", info_request.show_unresolved,
                   "show unresolved relationship notes");
    info->add_flag("--show-default-programs",
                   info_request.show_default_programs,
                   "show all 128 Program slots");

    axk::cli::ObjectsRequest objects_request;
    auto *objects =
        app.add_subcommand("objects", "decode current sampler objects as JSON");
    objects
        ->add_option("paths", objects_request.paths,
    ...
    auto *relationships = app.add_subcommand(
        "relationships", "resolve sampler object links as JSON");
    relationships
    ...
    relationships->add_flag("--overwrite", relationships_request.overwrite,
                            "allow writing into a non-empty output directory");
    relationships->add_option("--mono-dir",
                              relationships_request.mono_directory,
                              "mono exact-export sidecar directory");

    axk::cli::InventoryRequest inventory_request;
    auto *inventory = app.add_subcommand(
        "inventory", "decode object inventory through the model");
    inventory
        ->add_option("paths", inventory_request.paths,
    ...
    inventory
        ...
        ->required();
    inventory->add_flag("--strict", inventory_request.strict,
                        "stop after the first load error");
    inventory->add_flag("--overwrite", inventory_request.overwrite,
                        "allow writing into a non-empty output directory");

    axk::cli::CoverageRequest coverage_request;
    auto *coverage = app.add_subcommand(
        "coverage", "summarize current relationship coverage");
    coverage
        ->add_option("paths", coverage_request.paths,
                     "input files or directories")
        ->required()
        ->expected(1, -1);
    coverage
    ...
    auto *corpus_audit = corpus->add_subcommand(
        "audit",
        "run inventory, validation, relationship, and waveform smoke checks");
    corpus_audit
        ->add_option("paths", corpus_request.paths,
                     "input files or directories")
        ->required()
        ->expected(1, -1);
    corpus_audit
        ...
        ->required();
    corpus_audit
        ->add_option("--policy", corpus_request.policy, "validation policy")
        ->check(CLI::IsMember({"normal", "strict", "salvage-aware"}));
    corpus_audit->add_option("--wave-smoke-limit",
                             corpus_request.wave_smoke_limit,
    ...
    auto *extract =
        app.add_subcommand("extract", "extract data from supported containers");
    const auto configure_extract = [](CLI::App &command,
                                      axk::cli::ExtractRequest &request) {
        command.add_option("scope", request.scope, "selection scope")
            ->required()
            ->check(
                CLI::IsMember({"file", "volume", "program", "sbac", "sbnk"}));
        command
            .add_option("paths", request.paths, "input files or directories")
            ->required()
            ->expected(1, -1);
        command
            .add_option("-o,--output-dir", request.output_directory,
                        "export output directory")
            ->required();
        command.add_option("--path", request.selector_paths,
                           "selector path from info --format paths");
        command.add_option("--stereo", request.stereo, "stereo export policy")
            ->check(CLI::IsMember({"none", "auto"}));
        command.add_flag("--overwrite", request.overwrite,
                         "replace existing export files");
        command.add_flag("--strict", request.strict,
                         "stop after the first load error");
        command
            .add_option("--progress", request.progress,
                        "progress display policy")
            ->check(CLI::IsMember({"auto", "always", "never"}));
    };
    auto *extract_wav_nested = extract->add_subcommand(
        "wav", "export targeted WAVs to a shared sample pool");
    configure_extract(*extract_wav_nested, extract_wav_request);
    auto *extract_sfz_nested = extract->add_subcommand(
        "sfz", "export targeted WAVs and generate SFZ files");
    ...
    auto *package = app.add_subcommand(
        "package", "move self-contained sampler object graphs");
    auto *package_export =
        package->add_subcommand("export", "export one portable package");
    package_export
        ->add_option("source", package_export_request.source,
    ...
    package_export->add_option("--partition",
                               package_export_request.partition_index,
    ...
    package_export->add_option("--volume", package_export_request.volume_name,
                               "source volume label");
    package_export
        ...
        ->required();
    package_export
        ->add_option("--format", package_export_request.format,
                     "summary or json")
    ...
    const auto configure_package_read =
        [](CLI::App &command, axk::cli::PackageReadRequest &request) {
            command
                .add_option("package", request.package, "portable package file")
                ->required();
    ...
    const auto configure_package_import = [](CLI::App &command,
                                             axk::cli::PackageImportRequest
                                                 &request) {
        command.add_option("target", request.target, "target sampler image")
            ->required();
        command
            .add_option("packages", request.packages, "portable package files")
            ->required()
            ->expected(1, -1);
        command
            .add_option(
    ...
    auto *package_plan =
        package->add_subcommand("plan-import", "plan a package import");
    configure_package_import(*package_plan, package_plan_request);
    auto *package_import =
        package->add_subcommand("import", "atomically import packages");
    configure_package_import(*package_import, package_import_request);
    package_import
        ->add_option("-o,--output", package_import_request.output,
                     "output sampler image")
        ->required();
    ...
    auto *orphans = app.add_subcommand(
        "orphans", "classify physical waveform ownership as JSON");
    ...
    auto *validate = app.add_subcommand(
        "validate", "validate container and sampler organization");
    validate
        ->add_option("paths", validate_request.paths,
                     "input files or directories")
        ->expected(0, -1);
    validate->add_option("--exports", validate_request.exports,
    ...
    validate
        ...
        ->required();
    validate
        ->add_option("--policy", validate_request.policy, "validation policy")
        ->check(CLI::IsMember({"normal", "strict", "salvage-aware"}));
    validate->add_flag("--strict", validate_request.strict,
                       "alias for --policy strict");
    ...
    if (*writer_commands.create_hds) {
        return run_create_hds(
            writer_commands.create_manifest, writer_commands.create_output,
            writer_commands.create_overwrite, writer_commands.create_pretty);
    }
    if (*writer_commands.create_floppy) {
        return run_create_media(writer_commands.create_manifest,
                                writer_commands.create_output, "fat12_floppy",
                                writer_commands.create_overwrite,
                                writer_commands.create_pretty);
    }
    if (*writer_commands.create_iso) {
        return run_create_media(writer_commands.create_manifest,
                                writer_commands.create_output, "iso9660",
                                writer_commands.create_overwrite,
                                writer_commands.create_pretty);
    }
    ...
    if (*writer_commands.alter_hds) {
        const auto output =
            !writer_commands.alter_output.empty()
                ? std::optional<std::filesystem::path>{writer_commands
                                                           .alter_output}
                : std::nullopt;
        return run_alter_hds(writer_commands.alter_source,
                             writer_commands.alter_manifest, output,
