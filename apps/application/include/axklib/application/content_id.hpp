...
using WavContentIdProvider = std::function<axk::Result<ContentId>(const audio_internal::WavSource &)>;
...
class PooledPathAllocator final {
  public:
    ...
    axk::Result<std::filesystem::path> allocate(const std::filesystem::path &selection_root, std::string_view kind,
