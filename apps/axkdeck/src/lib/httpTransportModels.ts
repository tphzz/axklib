...
export function mapObject(item: ApiObjectItem): SamplerObject {
    return {
        ...
        storedFrameCount: item.waveform?.storedFrameCount ?? 0,
        waveStartFrame: item.waveform?.waveStartFrame ?? 0,
        waveLengthFrames: item.waveform?.waveLengthFrames ?? 0,
        storageState: item.waveform?.storageState ?? 'INCOMPLETE',
        sampleWidthBytes: item.waveform?.sampleWidthBytes ?? 0,
        embeddedContainerName: item.waveform?.embeddedContainerName,
