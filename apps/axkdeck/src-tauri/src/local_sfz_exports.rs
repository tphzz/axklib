...
#[derive(Default)]
pub(crate) struct SfzSaveCandidateStore {
    values: HashMap<String, (PathBuf, Instant)>,
}

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct SfzSaveCandidate {
    candidate_id: String,
    directory_name: String,
}

pub(crate) fn normalize_sfz_destination(path: PathBuf) -> Result<PathBuf, String> {
    let parent = path
        .parent()
...
#[tauri::command]
pub(crate) async fn select_local_sfz_destination(
    app: AppHandle,
...
pub(crate) fn extract_sfz_tar(archive: &mut File, destination: &Path) -> Result<(), String> {
...
#[tauri::command]
pub(crate) async fn save_retained_sfz_export(
    candidate_id: String,
    content_path: String,
    expected_size: u64,
